...
| **Light RP** (`/light-rp`) | Story-less 1:1 chats with roster character cards; import/export chara_card_v2 (JSON + PNG-embedded, embedded lorebook, `{{user}}/{{char}}` macros, lorebook directives) | `POST/GET /api/entities/characters/{id}/conversations` (M-31 / Character Versions). Reference: [Ecosystem Interop](./3_loops_and_systems/ecosystem_interop.md) |
| **Image generation** | "Generate portrait" on cards, "Generate scene image" in chat; images stored in MinIO, `avatar_url` holds the object key. The full flow is **generate → preview → approve** (use-as-avatar, primary/supporting reference), with optional **canon-anchoring**: editing an entity-anchored `VisualIdentity` stages a `ProposedChange` that only CanonKeeper commits. | `POST /api/image/portrait`, `POST /api/image/scene`, `GET /api/image/avatar/{id}` (`routers/image_gen.py` + `monitor_data/llm/image_providers.py`); approve/reject, visual-identities, settings, gallery under `POST/GET /api/image/assets[...]`, `/api/image/visual-identities`, `/api/image/settings`. Reference: [Image Generation](./3_loops_and_systems/image_generation.md) and [Visual Identity](./4_ontology/visual_identity.md). |

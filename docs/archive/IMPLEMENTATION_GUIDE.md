# MONITOR Implementation Guide (Historical Reference)

*Archived implementation walkthrough retained for context and planning history.*

---

## Overview

This guide preserves the original step-by-step implementation thinking for MONITOR, but it is **not** the canonical source of truth for the live repo layout or runtime flow.

> **Use these docs first:** `SYSTEM.md` (product vision), `STRUCTURE.md` (repo layout), `ARCHITECTURE.md` (layer rules), `docs/USE_CASES.md` (workflow catalog), and `docs/AI_DOCS.md` (quick implementation reference).
>
> **Historical translation guide:**
>
> | Older term in this file | Read it as in the live repo |
> |---|---|
> | `services/data-layer` | `packages/data-layer` |
> | `services/agents` | `packages/agents` |
> | `services/cli` | `packages/cli` |
> | `Orchestrator` | session bootstrap + LangGraph loop orchestration |
> | `main_loop.py` | web-session routing in `packages/ui/backend/src/monitor_ui/routers/chat.py` |
> | `memory_manager` | split responsibility across MongoDB/Qdrant plus `Indexer` and `ContextAssembly` |
...
MONITOR is built on a **data-first, loop-orchestrated architecture**:

```
User surfaces (Web UI / CLI)
          │
          ▼
LangGraph loops + agents
(SceneLoop, StoryLoop, TurnLoop, ConversationLoop, WorldBuildingLoop)
          │
          ▼
Data-layer API + authority middleware
          │
          ▼
Neo4j | MongoDB | Qdrant | PostgreSQL | MinIO
(OpenSearch remains optional in infra)
```
...
Create the following structure (mapped to the current repo layout):
...
├── packages/
│   ├── data-layer/        # MCP server + DB clients
│   │   ├── src/monitor_data/
│   │   │   ├── server.py
│   │   │   ├── schemas/
│   │   │   ├── db/
...
Create `packages/agents/src/monitor_agents/base.py`:
...
Create `packages/agents/src/monitor_agents/orchestrator.py`:
...
class Orchestrator(BaseAgent):
    ...
    async def start_new_story(self):
        ...
        # 2. Request Story creation through the authorized CanonKeeper/data-layer path
        story = await self.request_story_creation(
            universe_id=universe_id,
            title="New Campaign",
            story_type="campaign",
        )
...
Create `packages/agents/src/monitor_agents/canonkeeper.py`:

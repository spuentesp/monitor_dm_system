# Agents Layer Implementation (Historical Reference)

> Machine-optimized task list for implementing Layer 2.
>
> **Historical planning note:** this file predates the current LangGraph-based orchestration model. Treat `ARCHITECTURE.md`, `docs/architecture/AGENT_ORCHESTRATION.md`, and `packages/agents/src/monitor_agents/loops/` as the live source of truth; there is no monolithic runtime `Orchestrator` class in the current repo.
>
> **Current live reality:** the repo now centers on `SceneLoop`, `StoryLoop`, `TurnLoop`, `ConversationLoop`, and `WorldBuildingLoop`, with agent responsibilities implemented in `packages/agents/src/monitor_agents/`.
...
**Authority:** MongoDB (loop state) only. Any canonical story creation requested by Orchestrator still routes through the CanonKeeper-authorized data-layer path.
...
class Orchestrator(BaseAgent):
    ...
    authority = ["mongodb_scene", "mongodb_story_outline"]
```
...
|-------|-------|---------|--------|-------|
| Orchestrator | - (requests route through CanonKeeper) | scenes, outlines | - | - |

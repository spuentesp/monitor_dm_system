# Data Layer Implementation (Historical Reference)

> Machine-optimized task list for implementing Layer 1.
>
> **Historical planning note:** this task list is broader than the currently wired MCP registry. For the live tool surface, verify `packages/data-layer/src/monitor_data/server.py`, which currently auto-registers `neo4j_*`, `mongodb_*`, `qdrant_*`, and `ingest_*` functions.
>
> **Current live reality:** PostgreSQL is now part of the control plane, MinIO is primarily surfaced through ingest flows, and OpenSearch remains optional/planned at the application layer.
...
| Tool | Authority | Use Case |
|------|-----------|----------|
| `neo4j_create_story` | CanonKeeper | P-1 |

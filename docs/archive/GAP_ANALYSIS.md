...
>
> **Planning-note status:** this is a gap-analysis snapshot, not a canonical implementation-status ledger. Verify current behavior against the codebase and the canonical docs (`ARCHITECTURE.md`, `docs/USE_CASES.md`, `docs/architecture/*`) before treating any gap here as still open.
...
### Cross-cutting audit: tabletop RPG system ingestion

> Detailed design follow-up: `docs/architecture/SYSTEM_LIBRARY_AND_CHARACTER_CREATION_REFACTOR_PLAN.md`

This area needs a **TTRPG-specific audit lens**. For tabletop RPGs, a “game system” is not just one dice formula — it includes edition identity, character creation flow, subsystem rules, NPC stat logic, and supplement layering.

| Gap | Verified current repo state | Why it matters for tabletop RPGs | Priority |
|---|---|---|---|
| **Duplicate generic systems across books** | `mongodb_create_game_system()` upserts only on `name + source_document_id`, so multiple books in the same rules family can each create a separate `game_systems` entry. | A core rulebook, GM guide, bestiary, and supplement should usually resolve to one reusable system identity plus overlays — not many near-duplicates. | **P0** |
| **No canonical world binding for pack-embedded systems** | Worlds only persist `default_game_system_id`, and `_propagate_system_to_universe()` only copies `game_system_id`. | Pack-integrated/internal systems can be ingested, but the world cannot cleanly remember whether its rules came from the generic library or a specific pack snapshot. | **P0** |
| **Chat/play path is still generic-first** | `chat.py` session state uses `system_id`; the pack-embedded path is first-class in entity generation, but not in the main active-play bootstrap. | A self-contained tabletop pack should be playable without first collapsing its mechanics into a generic-library-only flow. | **P1** |
| **Supplement / expansion layering is under-modeled** | A `KnowledgePack` has one `game_system_id` / `game_system_data`, and merged packs keep only the first `game_system_id`. | TTRPGs commonly span core books, splats, optional modules, and house rules; the current model does not preserve that layered structure. | **P1** |
| **Subsystem fidelity is flattened** | Analyzer extraction can find powers/subsystems/NPC rules, but `_save_game_system()` mostly stores them as tagged `rules`. | Ship combat, duels, factions, magic schools, disciplines, and similar TTRPG subsystems need stronger typed structure than a flat rule list. | **P1** |
| **Character creation is still preview-centric** | `GameSystemRuntime.roll_character()` provides quick preview rolls, but it does not fully execute the richer `CharacterCreationProcedure` choices end-to-end. | Many tabletop systems depend on species/class/background/equipment/playbook decisions, not just random stat generation. | **P1** |
| **Edition/family provenance is not carried through strongly enough** | `EmbeddedSourceProfile` tracks `edition` and `family`, but the saved system/library path does not use them as the main identity key. | In TTRPG ingestion, edition drift is a real failure mode: “D&D 5e”, “2024 5e”, OSR variants, and setting hacks must remain distinguishable. | **P2** |
| **Non-rulebook ingest can still create noisy system records** | The default processing checklist asks for `game_system` and `rules` extraction across rulebooks, supplements, adventure modules, session notes, wikis, and custom sources. | TTRPG source types often mention mechanics without being the canonical rules source; this should produce overlays/hints, not always a new full system doc. | **P2** |

### Recommended canon for this area

1. Keep **reusable generic systems** in `game_systems` only.
2. Keep **pack-integrated/internal systems** in `KnowledgePack.game_system_data`.
3. Add an explicit **system binding** record for world/story/session runtime (`source_type`, `source_id`, `version/snapshot`).
4. Treat most TTRPG supplements as **overlays/extensions** to a base system unless the user explicitly publishes them as a standalone generic system.
5. Preserve **edition/family/provenance** as part of the system identity, not just display text.

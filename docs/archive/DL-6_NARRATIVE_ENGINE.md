...
> **Planning-note status:** subsystem design reference, not the primary flow canon. For authoritative runtime flow and orchestration behavior, prefer `docs/USE_CASES.md`, `docs/architecture/AGENT_ORCHESTRATION.md`, and `docs/architecture/CONVERSATIONAL_LOOPS.md`.
>
> Older mentions of an `Orchestrator` in this document refer to **story/scene orchestration via the current loop-based runtime**, not to a live monolithic agent class.

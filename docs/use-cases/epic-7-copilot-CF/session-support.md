...
|----|---------|----------|-------|
| CF-1 | ✅ | ✅ | Web recorder: `components/gm/SessionRecorder.tsx` (gm_assistant chat session); story/scene bootstrap in `packages/agents/src/monitor_agents/loops/session_bootstrap.py:121-139` |
| CF-2 | ✅ | ✅ | `RecapAgent.generate_recap` via `GET /chat/{id}/recap` (`packages/ui/backend/src/monitor_ui/routers/chat.py:499-545`) |
...
The GM Assistant surface is the web `/gm` page — there is no CLI `monitor
copilot` (decision: `docs/architecture/GM_ASSISTANT_MODE_PLAN.md`).

See [`epic-7-copilot-CF/CF-N/CF-N.yml`](../epic-7-copilot-CF/CF-N/) for canonical YAML.

---

*Last verified: 2026-07-23. If a link is broken, the canonical spec lives one directory up at `epic-7-copilot-CF/CF-N/`.*

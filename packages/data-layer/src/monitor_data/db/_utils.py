...
def _run_on_overflow_thread(coro: Coroutine[Any, Any, Any], timeout: float | None) -> Any:
    ...
    t.join(timeout)
    if t.is_alive():
        raise TimeoutError(f"run_sync (nested) timed out after {timeout}s — coroutine still running")
    if "error" in result:
        raise result["error"]

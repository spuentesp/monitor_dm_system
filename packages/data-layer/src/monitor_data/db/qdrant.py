...
class QdrantClient:
    ...
    def __init__(
        ...
        self._url: Optional[str] = url or settings.qdrant_url
        self._api_key: Optional[str] = api_key or settings.qdrant_api_key
        self._path: Optional[str] = path or settings.qdrant_path

        # Fall back to localhost when no config provided
        if not self._url and not self._path:
            self._url = "http://localhost:6333"

        self._client: Optional[QdrantAsyncClientLib] = None
    ...
    async def ensure_collection(self, collection_name: str) -> None:
        ...
        if collection_name not in COLLECTION_CONFIGS:
            raise ValueError(
                f"Unknown collection '{collection_name}'. Valid: {', '.join(COLLECTION_CONFIGS)}"
            )
    ...
    async def _ensure_payload_indexes(self, collection_name: str) -> None:
        ...
        if collection_name == "snippets":
            try:
                ...
                logger.debug(
                    "Qdrant text payload index skipped for %s.text: %s", collection_name, exc
                )

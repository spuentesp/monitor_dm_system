...
from monitor_data.tools import ingest_tools, mongodb_tools, neo4j_tools, qdrant_tools
...
# Auto-initialize tool registry when this module is imported by agents.
# When running as a standalone MCP server, discover_tools() is also called
# explicitly in main() — this is idempotent (duplicate entries are no-ops
# because dict assignment overwrites with the same function).
discover_tools()


def extract_tool_schema(func: Callable[..., Any]) -> dict[str, Any]:
    """
    ...
    try:
        ...
        for param_name, param in sig.parameters.items():
            ...
            if param_type and hasattr(param_type, "model_json_schema"):
                ...
                json_type = type_map.get(param_type, "string") if param_type else "string"
        ...
        try:
            ...
            try:
                if not check_authority(name, agent_type):
                    ...
                    error_msg = (
                        f"Agent '{agent_type}' is not authorized to call '{name}'. Allowed agents: {', '.join(allowed)}"
                    )
                ...
                return [TextContent(type="text", text=f"Authorization error: {e!s}")]
            ...
            # 6. Format response
            # Convert result to string (handle Pydantic models, including
            # models nested inside lists/dicts — default=str would otherwise
            # stringify each model into its repr and corrupt the payload)
            if hasattr(result, "model_dump_json"):
                result_text = result.model_dump_json(indent=2)
            elif hasattr(result, "json") and not isinstance(result, (list, dict, str)):
                ...
                result_text = json.dumps(_jsonable(result), indent=2, default=str)
            ...
            return [TextContent(type="text", text=f"Error executing tool: {e!s}")]

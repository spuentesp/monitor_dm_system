...
from monitor_data.schemas.facts import FactCreate, FactUpdate, FactResponse
...
class FactPreConditions:
    ...
    def create_fact(
        ...
        entity_ids_exist: Optional[List[str]] = None,
        source_ids_exist: Optional[List[str]] = None,
        scene_ids_exist: Optional[List[str]] = None,
    ...
    def update_fact(
        ...
        new_canon_level: Optional[CanonLevel] = None,
        caller_is_canon_keeper: bool = False,
    ) -> bool:
        """
        ...
        if new_canon_level is not None:
            ...
            if new_canon_level not in allowed:
                raise ValueError(
                    f"Invalid canon_level transition from {current_fact.canon_level} "
                    f"to {new_canon_level}"
                )
...
class FactPostConditions:
    ...
    def create_fact(result: FactResponse, params: FactCreate) -> bool:
        ...
        if result.canon_level != params.canon_level:
            raise ValueError(
                f"canon_level mismatch: expected {params.canon_level}, got {result.canon_level}"
            )
    ...
    def update_fact(result: FactResponse, old_fact: FactResponse, params: FactUpdate) -> bool:
        ...
        if params.statement is not None and result.statement != params.statement:
            raise ValueError(
                f"statement mismatch: expected {params.statement}, got {result.statement}"
            )
        if params.canon_level is not None and result.canon_level != params.canon_level:
            raise ValueError(
                f"canon_level mismatch: expected {params.canon_level}, got {result.canon_level}"
            )
        if result.updated_at <= old_fact.created_at:
...
class FactInvariants:
    ...
    def fact_response_invariant(fact: FactResponse) -> bool:
        ...
        if not isinstance(fact.knowledge_scope, KnowledgeScope):
            raise ValueError(
                f"knowledge_scope must be KnowledgeScope, got {type(fact.knowledge_scope)}"
            )
        return True

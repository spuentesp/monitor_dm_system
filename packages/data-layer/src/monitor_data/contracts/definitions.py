...
from monitor_data.schemas.base import Authority, CanonLevel, ProposalStatus, ProposalType
from monitor_data.schemas.facts import FactType, FactStatus, SimulationScope
...
class VectorUpsertContract(BaseModel):
    ...
    def preconditions(self) -> list[ContractResult]:
        ...
        if len(self.embedding) not in (768, 1536, 3072):
            results.append(
                ContractResult.fail(f"Invalid embedding dimension: {len(self.embedding)}")
            )
        return results
    ...
    def verify_writer_authority(
        ...
        if tool_name in self.requires_proposed_change and not has_proposed_change:
            return ContractResult.fail(
                f"Tool {tool_name} requires ProposedChange workflow; "
                f"direct Neo4j write from {agent_type} is forbidden",
        ...
        if agent_type != "CanonKeeper":
            return ContractResult.fail(
                f"Tool {tool_name} requires CanonKeeper authority; "
                f"called by {agent_type} without ProposedChange",
                {"tool": tool_name, "agent": agent_type},
            )
...
def requires_contract(contract_class: type[BaseModel]):
    """

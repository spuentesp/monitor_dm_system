...
class SceneBehavioralTrigger(BaseModel):
    ...
    gm_note: Optional[str] = Field(None, max_length=300, description="Secret GM-only context")
...
class GeneratedNPCProfile(BaseModel):
    ...
    role: str = Field(
        max_length=200, description="Social role: tavern keeper, city guard, merchant"
    )
    ...
    agenda_title: Optional[str] = Field(
        None, max_length=200, description="Which agenda drives them"
    )
...
class ScenePrompt(BaseModel):
    ...
    random_table_rolls: List[dict] = Field(
        default_factory=list,
    ...
    suggested_conflict: Optional[str] = Field(
        None, max_length=500, description="Core dramatic tension in this scene"
    )
    tags: List[str] = Field(default_factory=list)
...
class NPCSceneGeneratorOutput(BaseModel):
    ...
    generated_from_pack_id: Optional[str] = Field(None, max_length=200)

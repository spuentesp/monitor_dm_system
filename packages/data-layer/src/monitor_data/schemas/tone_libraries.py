...
class ToneLibraryCreate(BaseModel):
    ...
    pack_id: Optional[UUID] = Field(
        None,
        description=(
            "If from a Pack, ID of the Pack. None = global library (available to all universes)."
        ),
    )
    universe_id: Optional[UUID] = Field(
        None,
        description=(
            "If set, library is only visible within this universe. "
            "If None and pack_id is None, library is global."
        ),
    )
...
class ToneLibraryUpdate(BaseModel):
    ...
    tone_profile_ids: Optional[List[UUID]] = None

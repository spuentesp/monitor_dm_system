...
class DISWeaponProperty(str, Enum):
...
class DISWeapon(BaseWeapon):
    ...
    range_m: Optional[int] = Field(None, ge=0)
...
class DISShipSheet(BaseEquipmentItem):
    ...
    def hp_lte_max(self) -> "DISShipSheet":
        if self.hull_points > self.max_hull_points:
            raise ValueError(
                f"hull_points ({self.hull_points}) cannot exceed "
                f"max_hull_points ({self.max_hull_points})"
            )
        return self
...
class DISCharacterSheet(BaseCharacterSheet):
    ...
    cosmic_rot: int = Field(
        ...
        default=0,
        description=(
            "Cosmic Rot accumulation (0=clean, 10=fully corrupted). Powers unlocked at 3, 6, and 9."
        ),
    )
    ...
    origin: Optional[DISOrigin] = Field(
        None,
    ...
    def validate_max_hp(self) -> "DISCharacterSheet":
        ...
        if self.max_hp != expected:
            raise ValueError(
                f"max_hp must be {expected} (= max(1, BODY + 6) where BODY={self.body}), "
                f"got {self.max_hp}"
            )
        return self
    ...
    def validate_cosmic_powers_gated_by_rot(self) -> "DISCharacterSheet":
        ...
        if len(self.cosmic_powers) > max_powers:
            raise ValueError(
                f"cosmic_rot={self.cosmic_rot} allows at most {max_powers} power(s), "
                f"but got {len(self.cosmic_powers)}"
            )
        return self

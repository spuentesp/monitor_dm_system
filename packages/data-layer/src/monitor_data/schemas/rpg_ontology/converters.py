...
::
    ...
    # VtM also works
    VTMModel = make_entity_model(registry.get("vtm_5e"), "character")
"""
...
def _condition_to_field(condition: Dict[str, Any]) -> tuple[FieldSpec, Optional[ValidatorSpec]]:
...
def _detect_core_mechanic_type(
    ...
    if not isinstance(core_mechanic, dict):
        core_mechanic = (
            core_mechanic.model_dump()
            if hasattr(core_mechanic, "model_dump")
            else dict(core_mechanic)
        )
...
def _build_character_entity_type(
    ...
    def _mod_formula(attr: dict) -> str:
...
def _build_core_mechanic(
    ...
    return CoreMechanicSpec(
        ...
        success_condition=success_type[:200]
        if success_type
        else f"roll {formula[:50]} vs {success_threshold[:200]}",
        critical_rule=(
...
def builtin_to_game_system_spec(
    entry: Dict[str, Any],
    explicit_system_id: Optional[str] = None,
) -> GameSystemSpec:
    """
    ...
    core_mechanic: Dict[str, Any] = entry.get("core_mechanic")
    if not core_mechanic:
        raise ValueError(
            f"builtin_systems.json entry '{name}' missing required field: 'core_mechanic'"
        )
...
def get_spec_for_system(system_name: str) -> GameSystemSpec | None:
    ...
    try:
        entries: list[dict[str, Any]] = json.load(open(_BUILTIN_SYSTEMS_PATH, encoding="utf-8"))

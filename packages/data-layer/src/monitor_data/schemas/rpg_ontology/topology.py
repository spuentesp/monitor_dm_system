...
class CollectionSpec(BaseModel):
    ...
    name: str
    description: str
    schema_module: Optional[str] = Field(
        None,
...
class DataTopology(BaseModel):
    ...
    def get_node(self, backend: StorageBackend) -> Optional[StorageNode]:
...
MONITOR_TOPOLOGY = DataTopology(
    storage_nodes=[
        ...
        StorageNode(
            ...
            collections=[
                ...
                CollectionSpec(
                    name="Scene",
                    description=(
                        "A scene node in the canon graph (lightweight — "
                        "full scene data lives in MongoDB)"
                    ),
                    indexed_by=IndexedBy.PRIMARY_KEY,
                    key_fields=["id"],
                    has_mcp_tools=True,
            ...
            notes=(
                "Write access: CanonKeeper ONLY. "
                "All other agents submit ProposedChange to MongoDB and await verdict."
            ),
        ),
        # ── MongoDB ──────────────────────────────────────────────────────
        StorageNode(
            ...
            is_active=True,
            description=(
                "Primary narrative store — all mutable, evolving, in-play data. "
                "18 collections; 7 have formal indexes."
            ),
            collections=[
                CollectionSpec(
                    name="scenes",
                ...
                CollectionSpec(
                    ...
                    description=(
                        "CanonKeeper staging area — changes proposed during play "
                        "awaiting canonisation into Neo4j"
                    ),
                    indexed_by=IndexedBy.FORMAL_INDEX,
                ...
                CollectionSpec(
                    ...
                    has_mcp_tools=True,
                    notes=(
                        "GAP: not formally indexed; "
                        "entity_id is a soft reference to PostgreSQL character_sheet_index"
                    ),
                ),
                CollectionSpec(
                    name="documents",
        ...
        StorageNode(
            ...
            is_active=True,
            description=(
                "Object storage for raw binary files. "
                "Supports minio, s3, and local-folder backends."
            ),
            collections=[
                CollectionSpec(
                    ...
                    description=(
                        ...
                        "Path pattern: uploads/{doc_id}/{timestamp}_{filename}"
                    ),
                    indexed_by=IndexedBy.NO_INDEX,
                    has_mcp_tools=True,
                    notes=(
        ...
        StorageNode(
            ...
            is_active=True,
            description=(
                "Relational store for structured app config, routing, and "
                "formally typed game-mechanical data."
            ),
            collections=[
                CollectionSpec(
                    name="system_config",
    ...
    data_edges=[
        ...
        DataEdgeSpec(
            ...
            is_enforced=False,
            notes=(
                "GAP: ephemeral state is not guaranteed to match canonical sheet. "
                "No cleanup on scene end is enforced."
            ),
        ),
        DataEdgeSpec(
        ...
        DataEdgeSpec(
            ...
            description=(
                "MongoDB game_systems record mirrors spec to PostgreSQL game_system_schemas"
            ),
            from_backend=StorageBackend.MONGODB,
            from_collection="game_systems",
    ...
    for node in topology.storage_nodes:
        ...
        for coll in node.collections:
            if not coll.has_mcp_tools:
                gaps.append(
                    f"[NO_MCP_TOOLS] {node.backend.value}/{coll.name}: "
                    f"{coll.description}. {coll.notes or ''}"
                )

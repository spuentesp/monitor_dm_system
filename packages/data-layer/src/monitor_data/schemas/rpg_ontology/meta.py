...
class OntologyMechanicType(StrEnum):
    ...
    def enum_required_for_enum_types(self) -> FieldSpec:
        if self.field_type in (FieldType.ENUM, FieldType.LIST_ENUM):
            if not self.enum_values:
                raise ValueError(f"Field '{self.name}' has type {self.field_type} but no enum_values")
        return self
...
class ValidatorSpec(BaseModel):
    ...
    field: str | None = None  # primary field
    other_field: str | None = None  # secondary (for REQUIRES_IF, COUNT_GATE)
...
class EntityTypeSpec(BaseModel):
    ...
    storage_backends: list[StorageBackend] = Field(
        default_factory=list,
        description=("Backends where instances live. E.g. character → [POSTGRES], scene → [MONGODB, QDRANT]"),
    )
...
class CoreMechanicSpec(BaseModel):
    ...
    dice_notation: str | None = Field(
        None,
        max_length=50,
        description="E.g. 'd20', '2d6', 'pool:d6'",
    )
    roll_vs: str | None = Field(
        None,
        max_length=200,
...
class GameSystemSpec(BaseModel):
    ...
    entity_types: dict[str, EntityTypeSpec] = Field(
        description=("All entity types keyed by type_key. Standard keys: 'character', 'item', 'vehicle', 'creature'."),
    )
    ...
    def validate_relations(self) -> GameSystemSpec:
        ...
        for rel in self.relations:
            if rel.from_type not in self.entity_types:
                raise ValueError(f"Relation '{rel.relation_id}' references unknown from_type '{rel.from_type}'")
            if rel.to_type not in self.entity_types:
                raise ValueError(f"Relation '{rel.relation_id}' references unknown to_type '{rel.to_type}'")
        return self

    def get_entity_type(self, type_key: str) -> EntityTypeSpec | None:
...
class SystemRegistry:
    ...
    _instance: ClassVar[SystemRegistry | None] = None
    ...
    def get(self, system_id: str) -> GameSystemSpec | None:

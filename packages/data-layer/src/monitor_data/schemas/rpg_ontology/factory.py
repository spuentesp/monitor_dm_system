...
def _eval_formula(expr: str, ctx: Dict[str, Any]) -> Any:
    ...
    try:
        return eval(expr, _SANDBOX_GLOBALS, ctx)  # noqa: S307
...
def _field_spec_to_pydantic(spec: FieldSpec) -> Tuple[Any, FieldInfo]:
    ...
    if spec.field_type in (FieldType.INT, FieldType.FLOAT):
        if spec.min_value is not None:
            kwargs["ge"] = (
                int(spec.min_value) if spec.field_type == FieldType.INT else spec.min_value
            )
        if spec.max_value is not None:
            kwargs["le"] = (
                int(spec.max_value) if spec.field_type == FieldType.INT else spec.max_value
            )
...
def _build_validator_fn(
    validators: List[ValidatorSpec], fields: List[FieldSpec]
) -> Callable[[Any], Any]:
    """
    ...
    def _validate(self: Any) -> Any:
        ...
        for field_name, allowed in list_enum_constraints.items():
            ...
            if bad:
                raise ValueError(
                    f"'{field_name}' contains invalid values {bad}; allowed: {allowed}"
                )
            ...
            if vtype == ValidatorType.RANGE:
                if field_val is not None:
                    if spec.min_bound is not None and field_val < spec.min_bound:
                        raise ValueError(
                            spec.error_message.format(
                                field=spec.field, value=field_val, bound=spec.min_bound
                            )
                        )
                    if spec.max_bound is not None and field_val > spec.max_bound:
                        raise ValueError(
                            spec.error_message.format(
                                field=spec.field, value=field_val, bound=spec.max_bound
                            )
                        )
                ...
                if spec.expr and field_val is not None:
                    ...
                    if field_val != expected:
                        raise ValueError(
                            spec.error_message.format(
                                field=spec.field, value=field_val, bound=expected
                            )
                        )
                ...
                if field_val is not None and spec.allowed_values:
                    if field_val not in spec.allowed_values:
                        raise ValueError(
                            f"Field '{spec.field}' must be one of "
                            f"{spec.allowed_values}, got {field_val!r}"
                        )
...
def clear_model_cache(system_id: Optional[str] = None) -> None:
    """

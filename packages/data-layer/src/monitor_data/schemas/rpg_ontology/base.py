...
class ResourceSpec(BaseModel):
    ...
    max_formula: Optional[str] = Field(
        None,
        max_length=200,
        description="Formula for max value, e.g. 'BODY + 6'",
    )
    depleted_effect: Optional[str] = Field(
        None,
        max_length=200,
...
class BaseGameSystem(BaseModel):
    ...
    def attribute_by_abbr(self, abbreviation: str) -> Optional[AttributeSpec]:
...
class GameSystemRegistry:
    ...
    _instance: ClassVar[Optional["GameSystemRegistry"]] = None
    ...
    def get_definition(self, system_id: str) -> Optional[BaseGameSystem]:
        """Get the system definition by ID."""
        entry = self._systems.get(system_id)
        return entry[0] if entry else None

    def get_sheet_class(self, system_id: str) -> Optional[Type[BaseCharacterSheet]]:

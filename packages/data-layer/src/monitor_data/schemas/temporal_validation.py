...
class TemporalViolationType(StrEnum):
...
class FactValidityStatus(StrEnum):
...
class TemporalViolation(BaseModel):
    ...
    scene_id: UUID | None = Field(None, description="Scene where violation occurred")
    fact_id: UUID | None = Field(None, description="Related fact if applicable")
    event_id: UUID | None = Field(None, description="Related event if applicable")
    scene_time_ref: datetime | None = Field(None, description="Scene timestamp")
    canon_time_ref: datetime | None = Field(None, description="Canonical event timestamp")
    # Additional context
...
class TemporalValidationRequest(BaseModel):
    ...
    location_ref: UUID | None = Field(None, description="Scene location")
...
class TemporalValidationResult(BaseModel):
    ...
    def compute_totals(self) -> None:
        ...
        self.warning_count = sum(1 for v in self.violations if v.severity == TemporalSeverity.WARNING)
...
class FactValidity(BaseModel):
    ...
    time_ref: datetime | None = Field(None, description="When fact became true")
    ...
    time_until_expiry: timedelta | None = Field(
        None,
        description="Time remaining until expiration (None if never expires)",
    )
    time_until_start: timedelta | None = Field(
        None,
...
class FactExpirationBatch(BaseModel):
    ...
    def compute_totals(self) -> None:
        ...
        self.expired_facts = [f for f in self.expired_facts if f.status == FactValidityStatus.EXPIRED]
        self.not_yet_started_facts = [
            f for f in self.not_yet_started_facts if f.status == FactValidityStatus.NOT_YET_STARTED
        ]
        self.always_valid_facts = [f for f in self.always_valid_facts if f.status == FactValidityStatus.ALWAYS_VALID]
...
class SceneTemporalContext(BaseModel):
    ...
    scene_id: UUID
    old_time_ref: datetime | None = Field(None, description="Previous scene timestamp")
    ...
    shift_magnitude: timedelta | None = Field(
        None,
        description="How much time shifted (None if no shift)",
    )
    # Validation results
    old_validity: TemporalValidationResult | None = Field(None)
    new_validity: TemporalValidationResult | None = Field(None)
    # Conflicts
...
class FactReplacement(BaseModel):
    ...
    replacement_time: datetime = Field(default_factory=lambda: datetime.now(UTC))
    scene_id: UUID | None = Field(None, description="Scene that caused replacement")

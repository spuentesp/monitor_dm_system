...
class TurnCreate(BaseModel):
    ...
    speaker: Speaker
    entity_id: Optional[UUID] = Field(None, description="Entity ID if speaker is entity")
    text: str = Field(min_length=1, max_length=10000)
    resolution_ref: Optional[UUID] = Field(None, description="Reference to resolution document")

    @field_validator("entity_id")
    @classmethod
    def validate_entity_speaker(cls, v: Optional[UUID], info: ValidationInfo) -> Optional[UUID]:
...
class SceneCreate(BaseModel):
    ...
    story_id: UUID
    universe_id: UUID
    scene_id: Optional[UUID] = Field(None, description="Optional explicit Scene ID")
    title: str = Field(min_length=1, max_length=200)
    purpose: str = Field(default="", max_length=1000, description="Scene purpose or goal")
    narrative_order: Optional[int] = Field(
        None,
        ge=0,

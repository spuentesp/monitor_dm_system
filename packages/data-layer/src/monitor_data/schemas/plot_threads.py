...
class PlotThreadCategory(StrEnum):
...
class PlotThreadUrgency(StrEnum):
...
class PlotThreadDetectionResult(BaseModel):
    ...
    def compute_totals(self) -> None:
        ...
        self.high_urgency_count = sum(
            1 for t in self.threads if t.urgency in (PlotThreadUrgency.HIGH, PlotThreadUrgency.CRITICAL)
        )

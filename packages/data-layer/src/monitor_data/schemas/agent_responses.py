...
class ResolverSuccessLevel(str, Enum):
...
class ResolverOutcome(BaseModel):
    ...
    success_level: ResolverSuccessLevel = Field(
        description="Final outcome level of the mechanical resolution"
    )
    roll_total: Optional[int] = Field(
        default=None,
        description="Total value of the dice roll (None for narrative resolutions)",
    )
    roll_breakdown: Optional[str] = Field(
        default=None,
...
class CanonKeeperDecision(str, Enum):
    ACCEPTED = "accepted"
    REJECTED = "rejected"
    DEFERRED = "deferred"
...
class CanonKeeperVerdict(BaseModel):
    ...
    canon_node_type: Optional[str] = Field(
        default=None,
        description="Neo4j node label to create if accepted (e.g. 'EntityInstance')",
    )
    canon_properties: Optional[Dict[str, Any]] = Field(
        default=None,

...
class ToneProfileCreate(BaseModel):
    ...
    instruction: str = Field(
        ...
        max_length=2000,
        description=(
            "Complete narrator instruction for this tone. This replaces BUILTIN_TONE_PROFILES[key] from the old system."
        ),
    )
    ...
    category: str = Field(
        default="narrative",
        max_length=50,
        description=("Category of the tone: narrative, genre, mood, pacing. Helps with organization and UI grouping."),
    )
    language: str = Field(
        default="en",
        max_length=10,
    ...
    pack_id: UUID | None = Field(
        None,
        description=("If from a Pack, ID of the Pack. None = global built-in or user-created profile."),
    )
...
class ToneProfileUpdate(BaseModel):
    ...
    example_output: str | None = Field(None, max_length=1000)

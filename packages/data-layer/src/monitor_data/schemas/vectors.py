...
class SceneChunkPayload(BaseModel):
    ...
    play_session_id: str | None = Field(None, description="Real-world session that produced this scene")
    temporal_mode: str | None = Field(None, description="present | flashback | flash_forward | dream")
...
class SnippetChunkPayload(BaseModel):
    ...
    ingestion_job_id: str | None = Field(None, description="IngestionJob that produced this snippet")
...
class EntityChunkPayload(BaseModel):
    ...
    is_archetype: bool
    detail_level: str | None = Field(None, description="stub | sketched | detailed | elaborated")
...
class KnowledgeChunkPayload(BaseModel):
    ...
    universe_id: str
    domain: str | None = Field(None, description="physics | magic | society | history | religion | etc.")
...
class VectorPoint(BaseModel):
    ...
    vector: list[float] = Field(description="Embedding vector (typically 1536 dimensions for OpenAI)")
...
class VectorUpsertRequest(BaseModel):
    ...
    collection: str = Field(description="Collection name: scenes | memories | snippets | entities | knowledge")
    id: UUID = Field(description="Unique identifier for the vector point")
...
class VectorFilter(BaseModel):
    ...
    story_id: UUID | None = Field(default=None, description="Filter by story_id (scenes)")
    scene_id: UUID | None = Field(default=None, description="Filter by scene_id (scenes, memories)")
    entity_id: UUID | None = Field(default=None, description="Filter by entity_id (memories, entities)")
    universe_id: UUID | None = Field(default=None, description="Filter by universe_id (all collections)")
    ...
    play_session_id: UUID | None = Field(default=None, description="Filter scenes by real-world play session")
    ...
    is_archetype: bool | None = Field(default=None, description="Filter entities: True=archetypes, False=instances")
    ...
    node_type: str | None = Field(default=None, description="Filter knowledge by node type: axiom | fact | lore_event")
    domain: str | None = Field(
        default=None,
        description="Filter knowledge by domain: physics | magic | society | history | etc.",
    )
    canon_level: str | None = Field(default=None, description="Filter by canon level: proposed | canon | retconned")

    # snippets collection
    doc_id: UUID | None = Field(default=None, description="Filter snippets by source document")
    ingestion_job_id: UUID | None = Field(default=None, description="Filter snippets by ingestion job")

    # Generic type discriminator (used in scenes: 'scene_summary' | 'turn')
    type: str | None = Field(default=None, description="Filter by 'type' field in payload")
...
class VectorSearchRequest(BaseModel):
    ...
    filter: VectorFilter | None = Field(default=None, description="Optional payload filters")
...
class CollectionInfo(BaseModel):
    ...
    indexed_vectors_count: int | None = Field(default=None, description="Number of indexed vectors")

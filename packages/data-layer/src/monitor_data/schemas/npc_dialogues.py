...
class DialogueMessage(BaseModel):
    ...
    npc_emotional_state: Optional[str] = Field(
        None,
        max_length=200,
...
class NPCDialogueCreate(BaseModel):
    ...
    player_entity_id: Optional[UUID] = Field(
        None, description="Player character doing the talking (if applicable)"
    )
    scene_id: Optional[UUID] = Field(
        None, description="Scene this dialogue is part of (if during a scene)"
    )
    story_id: Optional[UUID] = Field(None, description="Story context for this dialogue")
    ...
    # Opening context
    opening_context: Optional[str] = Field(
        None,
        max_length=1000,
...
class AppendDialogueMessage(BaseModel):
    ...
    npc_emotional_state: Optional[str] = Field(None, max_length=200)
...
class NPCDialogueFilter(BaseModel):
    ...
    entity_id: Optional[UUID] = Field(None, description="Filter by NPC")
    player_entity_id: Optional[UUID] = Field(None, description="Filter by player")
    scene_id: Optional[UUID] = None
    story_id: Optional[UUID] = None
    status: Optional[DialogueStatus] = None

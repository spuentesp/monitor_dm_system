...
class CompletenessCategory(BaseModel):
    ...
    category: str = Field(
        description="Category name: axioms, entities, lore, tables, agendas, game_system, topologies"
    )
    ...
    score: float = Field(
        description="present/expected if expected > 0, else 1.0 if present > 0 else 0.0"
    )

...
class DuplicateCategory(StrEnum):
...
class DuplicateItemRef(BaseModel):
    ...
    pack_id: UUID | None = Field(None, description="Pack it belongs to (None = new pack)")
    identity_value: str = Field(max_length=500, description="Value of the identity field")
    confidence: float = Field(ge=0.0, le=1.0, description="Confidence score")
    content_hash: str | None = Field(None, max_length=64, description="MD5 of item content for EXACT detection")
    item_snapshot: dict[str, Any] = Field(
        default_factory=dict,
        description="Full item snapshot for review UI",
    )
    source_ref: str | None = Field(None, max_length=200, description="Page/section cite")
...
class DeduplicationResult(BaseModel):
    ...
    universe_id: UUID | None = Field(None, description="Universe being applied to")

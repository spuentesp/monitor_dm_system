...
BUILTIN_TONE_PROFILES: list[dict] = [
    {
        "name": "Dramatic",
        "description": (
            "Rich, emotive prose with attention to sensory detail and emotional weight"
        ),
        "instruction": (
...
BUILTIN_TAG_DEFINITIONS: list[dict] = [
    {
        "tag": "dramatic",
...
def ensure_tone_builtins() -> None:
    ...
    for tag_def in BUILTIN_TAG_DEFINITIONS:
        ...
        if not mongodb_get_tag_definition(tag):
            params = TagDefinitionCreate(**tag_def, is_builtin=True)
            mongodb_create_tag_definition(params)
    ...
    if not mongodb_get_default_tone_library():
        ...
        result = mongodb_create_tone_library(lib_params)

...
def mongodb_get_ingestion_job(job_id: UUID) -> Optional[IngestionJobResponse]:
    """
...
def mongodb_update_ingestion_job(job_id: UUID, params: IngestionJobUpdate) -> IngestionJobResponse:
    ...
    if "last_error" in fields_set:
        update_fields["last_error"] = params.last_error
...
def mongodb_delete_ingestion_job(job_id: UUID) -> bool:
    ...
    if status not in terminal:
        raise ValueError(
            f"Job {job_id} is in '{status}' state. "
            "Only terminal jobs can be deleted. Cancel the job first."
        )

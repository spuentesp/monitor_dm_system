...
def mongodb_create_game_system(params: GameSystemCreate) -> GameSystemResponse:
    ...
    system_doc: Dict[str, Any] = {
        ...
        "source_document_id": (
            str(params.source_document_id) if params.source_document_id else None
        ),
        "is_builtin": False,
    }
...
def mongodb_get_game_system(system_id: UUID) -> Optional[GameSystemResponse]:
    ...
    return GameSystemResponse(
        ...
        npc_creation_rules=(
            NPCCreationRules(**system_doc["npc_creation_rules"])
            if system_doc.get("npc_creation_rules")
            else None
        ),
        source_document_id=(
            UUID(system_doc["source_document_id"]) if system_doc.get("source_document_id") else None
        ),
...
def mongodb_list_game_systems(
    ...
    systems = [
        GameSystemResponse(
            ...
            character_creation=(
                CharacterCreationProcedure(**doc["character_creation"])
                if doc.get("character_creation")
                else None
            ),
            npc_stat_blocks=[NPCStatBlock(**b) for b in doc.get("npc_stat_blocks", [])],
            npc_creation_rules=(
                NPCCreationRules(**doc["npc_creation_rules"])
                if doc.get("npc_creation_rules")
                else None
            ),
            source_document_id=(
                UUID(doc["source_document_id"]) if doc.get("source_document_id") else None
            ),

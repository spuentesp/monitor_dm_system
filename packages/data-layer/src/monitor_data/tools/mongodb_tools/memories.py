...
def mongodb_create_memory(params: MemoryCreate) -> MemoryResponse:
    ...
    if params.linked_fact_id:
        ...
        RETURN f.id as id
        """
        result = neo4j_client.execute_read(fact_check_query, {"fact_id": str(params.linked_fact_id)})
        if not result:
...
def mongodb_list_memories(params: MemoryFilter) -> MemoryListResponse:
    ...
    cursor = memories_collection.find(filter_dict).sort("importance", -1).skip(params.offset).limit(params.limit)
    ...
    memories = [
        MemoryResponse(
            ...
            linked_fact_id=(UUID(mem_doc["linked_fact_id"]) if mem_doc.get("linked_fact_id") else None),
    ...
    return MemoryListResponse(memories=memories, total=total, limit=params.limit, offset=params.offset)

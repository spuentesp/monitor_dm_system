...
def _convert_turn_dict_to_response(turn_dict: Dict[str, Any]) -> TurnResponse:
    ...
    return TurnResponse(
        ...
        resolution_ref=(
            UUID(turn_dict["resolution_ref"]) if turn_dict.get("resolution_ref") else None
        ),
    )
...
def _convert_scene_doc_to_response(scene_doc: Dict[str, Any]) -> SceneResponse:
    ...
    return SceneResponse(
        ...
        parent_scene_id=(
            UUID(scene_doc["parent_scene_id"]) if scene_doc.get("parent_scene_id") else None
        ),
...
def mongodb_create_scene(params: SceneCreate) -> SceneResponse:
    ...
    if params.participating_entities:
        ...
        verify_nodes_exist(neo4j_client, "Entity", entity_ids_str)
...
def mongodb_list_scenes(params: SceneFilter) -> SceneListResponse:
    ...
    sort_field = (
        params.sort_by
        if params.sort_by in ["created_at", "order", "narrative_order"]
        else "created_at"
    )

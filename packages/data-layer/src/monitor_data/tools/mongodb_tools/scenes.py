...
def _convert_turn_dict_to_response(turn_dict: dict[str, Any]) -> TurnResponse:
    ...
    return TurnResponse(
        ...
        resolution_ref=(UUID(turn_dict["resolution_ref"]) if turn_dict.get("resolution_ref") else None),
    )
...
def _convert_scene_doc_to_response(scene_doc: dict[str, Any]) -> SceneResponse:
    ...
    return SceneResponse(
        ...
        narrative_order=scene_doc.get("narrative_order", scene_doc.get("order")),
        temporal_mode=scene_doc.get("temporal_mode", "present"),
        time_ref=scene_doc.get("time_ref"),
        time_description=scene_doc.get("time_description"),
        parent_scene_id=(UUID(scene_doc["parent_scene_id"]) if scene_doc.get("parent_scene_id") else None),
...
def mongodb_create_scene(params: SceneCreate) -> SceneResponse:
    ...
    if not result:
        raise ValueError(f"Story {params.story_id} or Universe {params.universe_id} not found")

    # Verify participating entities if provided
    if params.participating_entities:
        from monitor_data.tools.neo4j_tools._helpers import verify_nodes_exist

        entity_ids_str = [str(eid) for eid in params.participating_entities]
        verify_nodes_exist(neo4j_client, "Entity", [UUID(e) for e in entity_ids_str])

    # Verify location_ref if provided
    if params.location_ref:
        location_check_query = """
        MATCH (e:Entity {id: $location_id})
        RETURN e.id as id
        """
        result = neo4j_client.execute_read(location_check_query, {"location_id": str(params.location_ref)})
        if not result:
            raise ValueError(f"Location entity {params.location_ref} not found")

    # Create scene in MongoDB
    scene_id = params.scene_id or uuid4()
    ...
    scene_doc = {
        ...
        "narrative_order": params.narrative_order,
        "order": params.narrative_order,
        "temporal_mode": params.temporal_mode.value,
        "time_ref": params.time_ref,
        "time_description": params.time_description,
        "parent_scene_id": str(params.parent_scene_id) if params.parent_scene_id else None,
...
def mongodb_get_scene(scene_id: UUID) -> SceneResponse | None:
    """
...
def mongodb_get_recent_turns(scene_id: UUID, limit: int = 10) -> list[dict[str, Any]]:
    """
    Return the most recent turns for a scene in lightweight dict form.

    This is a hot-path helper for runtime context assembly, so it fetches only
    the trailing window of turns instead of converting the full SceneResponse.
    """
    mongo_client = get_mongodb_client()
    scenes_collection = mongo_client.get_collection("scenes")

    safe_limit = max(1, min(int(limit), 50))
    scene_doc = scenes_collection.find_one(
        {"scene_id": str(scene_id)},
        {"_id": 0, "turns": {"$slice": -safe_limit}},
    )
    if not scene_doc:
        return []

    turns = scene_doc.get("turns", []) or []
    recent = []
        ...
        recent.append(
            {
                ...
                "resolution_ref": turn.get("resolution_ref"),
            }
        )
    return recent


def mongodb_get_scene_summary(scene_id: UUID) -> str:
    """
    Return the best available short summary string for a scene.

    Prefers the persisted ``summary`` field. When no explicit summary has been
    stored yet, falls back to the scene title/purpose so query formulation still
    has a stable contextual anchor.
    """
    mongo_client = get_mongodb_client()
    scenes_collection = mongo_client.get_collection("scenes")
    ...
    scene_doc = scenes_collection.find_one(
        {"scene_id": str(scene_id)},
        {"_id": 0, "title": 1, "purpose": 1, "summary": 1},
    )
    if not scene_doc:
        return ""

    summary = str(scene_doc.get("summary") or "").strip()
    if summary:
        return summary

    title = str(scene_doc.get("title") or "").strip()
    purpose = str(scene_doc.get("purpose") or "").strip()
...
def mongodb_update_scene(scene_id: UUID, params: SceneUpdate) -> SceneResponse:
    ...
    update_doc: dict[str, Any] = {"updated_at": datetime.now(UTC)}
...
def mongodb_list_scenes(params: SceneFilter) -> SceneListResponse:
    ...
    # Build sort
    sort_field = params.sort_by if params.sort_by in ["created_at", "order", "narrative_order"] else "created_at"
    if sort_field == "narrative_order":
        sort_field = "order"
    sort_order = -1 if params.sort_order == "desc" else 1

    # Query with pagination
    cursor = scenes_collection.find(filter_query).sort(sort_field, sort_order).skip(params.offset).limit(params.limit)
...
def mongodb_update_turn_resolution(
    ...
    """
    Link a resolution_ref to a turn and optionally set summary/checkpoint.

    Authority: Narrator and Orchestrator agents
    Use Case: DL-4

    Args:
        scene_id: UUID of the scene
        turn_id: UUID of the turn to link
        resolution_id: UUID of the resolution to link
        summary: Optional scene summary to store
        checkpoint: Optional checkpoint data to push

    Returns:
        True if update succeeded, False otherwise
    """
    from monitor_data.db.mongodb import get_mongodb_client as _get_mongodb_client
    ...
    update_doc: dict[str, Any] = {
        "$set": {
            "turns.$.resolution_ref": str(resolution_id),
    ...
    if checkpoint:
        update_doc["$set"]["checkpoints"] = checkpoint

    result = scenes_collection.update_one(
        {"scene_id": str(scene_id), "turns.turn_id": str(turn_id)},
        update_doc,
    )
...
def mongodb_append_turn(scene_id: UUID, params: TurnCreate) -> TurnResponse:
    ...
    turn_doc = {
        ...
        "text": params.text,
        "timestamp": timestamp,
        "resolution_ref": str(params.resolution_ref) if params.resolution_ref else None,
    }
    ...
    return TurnResponse(
        ...
        text=params.text,
        timestamp=timestamp,
        resolution_ref=params.resolution_ref,
    )

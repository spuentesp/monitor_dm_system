...
def _character_sheet_doc_to_response(doc: Dict[str, Any]) -> CharacterSheetResponse:
    ...
    return CharacterSheetResponse(
        ...
        history_log=[
            SheetHistoryEntry(**entry) if isinstance(entry, dict) else entry
            for entry in doc.get("history_log", [])
        ],
...
def mongodb_create_character_sheet(params: CharacterSheetCreate) -> CharacterSheetResponse:
    ...
    if params.game_system_id is not None:
        ...
        system_doc = mongo_client["game_systems"].find_one(
            {"system_id": str(params.game_system_id)}
        )
        if not system_doc:
...
def mongodb_get_character_sheet(sheet_id: UUID) -> Optional[CharacterSheetResponse]:

...
def mongodb_get_tone_library(library_id: UUID) -> ToneLibraryResponse | None:
    """
...
def mongodb_get_default_tone_library() -> ToneLibraryResponse | None:
    """
    ...
    client = get_mongodb_client()
    try:
        doc = client["tone_libraries"].find({"is_default": True}).sort("priority", -1).limit(1).next()
...
def mongodb_update_tone_library(library_id: UUID, params: ToneLibraryUpdate) -> ToneLibraryResponse | None:
    """

...
def mongodb_get_world_profile(universe_id: UUID) -> Optional[EmbeddedWorldProfile]:
    """
...
def mongodb_get_gm_profile(profile_id: UUID) -> Optional[GMProfileResponse]:
    """
...
def mongodb_update_gm_profile(
    profile_id: UUID, params: GMProfileUpdate
) -> Optional[GMProfileResponse]:
    """
    ...
    if existing.get("is_builtin"):
        ...
        if attempted:
            raise ValueError(
                f"Cannot modify {attempted} on built-in profile "
                f"'{existing['name']}'. Duplicate it to customize."
            )

    # Convert UUID fields to strings for MongoDB
    if "universe_id" in update_fields:
        update_fields["universe_id"] = (
            str(update_fields["universe_id"]) if update_fields["universe_id"] else None
        )
...
def mongodb_delete_gm_profile(profile_id: UUID) -> bool:
    ...
    if existing.get("is_builtin"):
        raise ValueError(
            f"Cannot delete built-in profile '{existing['name']}'. "
            "Set active profile to a custom one instead."
        )

...
def mongodb_get_random_table(table_id: UUID) -> Optional[RandomTableResponse]:
...
def mongodb_list_random_tables(filters: RandomTableFilter) -> RandomTableListResponse:
    ...
    return RandomTableListResponse(
        tables=tables, total=total, limit=filters.limit, offset=filters.offset
    )
...
def mongodb_roll_on_table(table_id: UUID, actor_id: Optional[UUID] = None) -> Dict[str, Any]:
    """

    ...
    for beat_dict in doc.get("beats", []):
        beat = StoryBeat(
            ...
            completed_in_scene_id=(
                UUID(beat_dict["completed_in_scene_id"]) if beat_dict.get("completed_in_scene_id") else None
            ),
        )
        beats.append(beat)
...
def _calculate_pacing_metrics(beats: list[StoryBeat], scenes_since_major_event: int = 0) -> PacingMetrics:
    """
...
def mongodb_create_story_outline(params: StoryOutlineCreate) -> StoryOutlineResponse:
    ...
    doc = {
        ...
        "mystery_structure": (params.mystery_structure.model_dump(mode="json") if params.mystery_structure else None),
...
def mongodb_get_story_outline(story_id: UUID) -> StoryOutlineResponse | None:
    """
...
def mongodb_update_story_outline(story_id: UUID, params: StoryOutlineUpdate) -> StoryOutlineResponse:
    """
    ...
    pacing = _calculate_pacing_metrics(current_beats, doc.get("pacing_metrics", {}).get("scenes_since_major_event", 0))
    ...
    if params.mark_clue_discovered and "mystery_structure" in doc:
        ...
        if mystery is None:
            raise ValueError("Cannot mark clue as discovered: story outline has no mystery structure")

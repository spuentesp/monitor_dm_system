...
def mongodb_get_world_snapshot(snapshot_id: UUID) -> Optional[WorldSnapshot]:
...
def mongodb_list_world_snapshots(filters: WorldSnapshotFilter) -> WorldSnapshotListResponse:
    ...
    return WorldSnapshotListResponse(
        snapshots=snapshots, total=total, limit=filters.limit, offset=filters.offset
    )
...
def mongodb_restore_world_snapshot(snapshot_id: UUID) -> dict:
    """
    ...
    for entity_data in entities:
        try:
            ...
            logger.warning(
                "Failed to create entity proposal for %s: %s", entity_data.get("name"), exc
            )
...
def mongodb_compare_snapshots(snapshot_a_id: UUID, snapshot_b_id: UUID) -> SnapshotCompareResponse:
    ...
    for rel_key in rels_a - rels_b:
        ...
        relationship_diffs.append(diff)

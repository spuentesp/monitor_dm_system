...
def mongodb_get_npc_profile(entity_id: UUID) -> Optional[NPCProfileResponse]:
...
def mongodb_update_npc_profile(entity_id: UUID, params: NPCProfileUpdate) -> NPCProfileResponse:
    ...
    if params.add_preference is not None:
        update_fields["preferences"] = list(existing.get("preferences", [])) + [
            params.add_preference.model_dump()
        ]
    if params.add_trigger is not None:
        update_fields["triggers"] = list(existing.get("triggers", [])) + [
            params.add_trigger.model_dump()
        ]

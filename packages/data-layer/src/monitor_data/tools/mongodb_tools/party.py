...
def mongodb_transfer_item(params: TransferItemRequest) -> Dict[str, str]:
    ...
    if (
        params.from_type == TransferSourceType.CHARACTER
        or params.to_type == TransferTargetType.CHARACTER
    ):
    ...
    if current_quantity < params.quantity:
        raise ValueError(
            f"Insufficient quantity: have {current_quantity}, trying to transfer {params.quantity}"
        )
...
def _transfer_character_item(params: TransferItemRequest) -> Dict[str, str]:
    ...
    return {
        "status": "transferred",
        "message": (
            f"Transferred {params.quantity}x {params.item_name.strip()} "

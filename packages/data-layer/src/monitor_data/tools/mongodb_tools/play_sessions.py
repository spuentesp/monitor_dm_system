...
def _convert_doc_to_response(doc: dict[str, Any]) -> PlaySessionResponse:
    ...
    ended_at: datetime | None = doc.get("ended_at")
...
def mongodb_create_play_session(params: PlaySessionCreate) -> PlaySessionResponse:
    ...
    if coll.find_one(
        ...
        raise ValueError(f"Session #{params.session_number} already exists for story {params.story_id}")
...
def mongodb_get_play_session(session_id: UUID) -> PlaySessionResponse | None:
...
def mongodb_list_play_sessions(filters: PlaySessionFilter) -> PlaySessionListResponse:
    ...
    cursor = coll.find(query).sort("session_number", direction).skip(filters.offset).limit(filters.limit)
...
def mongodb_update_play_session(session_id: UUID, params: PlaySessionUpdate) -> PlaySessionResponse | None:
...
def mongodb_add_scene_to_play_session(session_id: UUID, scene_id: UUID) -> PlaySessionResponse | None:
...
def mongodb_get_latest_play_session(story_id: UUID) -> PlaySessionResponse | None:
...
def mongodb_list_recent_play_sessions(universe_id: UUID, limit: int = 10) -> list[PlaySessionResponse]:

...
def neo4j_create_multiverse(params: MultiverseCreate) -> MultiverseResponse:
    ...
    client.execute_write(
        create_query,
        {
            ...
            "parent_multiverse_id": str(params.parent_multiverse_id) if params.parent_multiverse_id else None,
...
def neo4j_get_multiverse(multiverse_id: UUID) -> MultiverseResponse | None:
    """
    ...
    return MultiverseResponse(
        ...
        parent_multiverse_id=UUID(m["parent_multiverse_id"]) if m.get("parent_multiverse_id") else None,
        created_at=m["created_at"].to_native() if hasattr(m["created_at"], "to_native") else m["created_at"],
    )


def neo4j_list_multiverses(omniverse_id: UUID | None = None) -> list[MultiverseResponse]:
    """
...
def neo4j_update_multiverse(multiverse_id: UUID, params: MultiverseUpdate) -> MultiverseResponse:
    """
    Update mutable fields of a Multiverse.

    Authority: CanonKeeper only
    Use Case: DL-1

    Raises:
        ValueError: If multiverse not found
    """
    client = get_neo4j_client()

    verify_node_exists(client, "Multiverse", multiverse_id)

    set_clauses = []
    update_params: dict[str, Any] = {"id": str(multiverse_id)}

    if params.name is not None:
        set_clauses.append("m.name = $name")
        update_params["name"] = params.name

    if params.system_name is not None:
        set_clauses.append("m.system_name = $system_name")
        update_params["system_name"] = params.system_name

    if params.description is not None:
        set_clauses.append("m.description = $description")
        update_params["description"] = params.description

    if params.is_template is not None:
        set_clauses.append("m.is_template = $is_template")
        update_params["is_template"] = params.is_template

    if params.knowledge_tree_type is not None:
        set_clauses.append("m.knowledge_tree_type = $knowledge_tree_type")
        update_params["knowledge_tree_type"] = params.knowledge_tree_type.value
    ...
    updated = neo4j_get_multiverse(multiverse_id)
    if updated is None:
...
def neo4j_delete_multiverse(multiverse_id: UUID, force: bool = False) -> dict[str, Any]:
    ...
    if child_count > 0 and not force:
        raise ValueError(f"Multiverse has {child_count} universe(s). Use force=True to cascade delete.")
...
def neo4j_create_universe(params: UniverseCreate) -> UniverseResponse:
    ...
    client.execute_write(
        create_query,
        {
            ...
            "parent_universe_id": str(params.parent_universe_id) if params.parent_universe_id else None,
            "alt_world_type": params.alt_world_type.value,
            "is_template": params.is_template,
            "default_game_system_id": str(params.default_game_system_id) if params.default_game_system_id else None,
...
def neo4j_get_universe(universe_id: UUID) -> UniverseResponse | None:
    """
    ...
    return UniverseResponse(
        id=UUID(u["id"]),
        multiverse_id=UUID(u["multiverse_id"]) if u.get("multiverse_id") else None,
        name=u["name"],
        ...
        default_game_system_id=UUID(u["default_game_system_id"]) if u.get("default_game_system_id") else None,
        ...
        created_at=u["created_at"].to_native() if hasattr(u["created_at"], "to_native") else u["created_at"],
    )
...
def neo4j_delete_universe(universe_id: UUID, force: bool = False) -> dict[str, Any]:
    ...
    if not force:
        ...
        if deps["sources"] > 0 or deps["axioms"] > 0 or deps["stories"] > 0 or deps["entities"] > 0:
            raise ValueError(f"Universe {universe_id} has dependent data. Use force=True to cascade delete.")
...
def neo4j_get_source(source_id: UUID) -> SourceResponse | None:
    """
...
def _clone_entity_into_universe(client: Any, new_uid: str, old_data: dict[str, Any], now_iso: str) -> str:
...
def _clone_induced_relationships(
    client: Any, source_uid: str, id_map: dict[str, str], seen: set[Any] | None = None
) -> int:

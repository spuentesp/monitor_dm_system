...
from monitor_data.schemas.base import AxiomAuthority, CanonLevel, KnowledgeScope
...
def _flatten_properties(props: dict[str, Any] | None) -> dict[str, Any] | None:
...
def _serialize_properties(props: dict[str, Any] | None) -> str | None:
    """Serialize a properties dict to a single JSON string property value.

    Neo4j property values must be primitives or arrays of primitives --
    a dict can NEVER be assigned to a single property (``f.properties =
    $properties``), even after ``_flatten_properties`` has flattened its
    OWN nested values, because the outer dict itself is still a Map.
    Storing the whole (flattened) dict as one JSON string is the only way
    to keep a single ``properties`` property while round-tripping an
    arbitrary dict shape.
    """
    import json
...
def _deserialize_properties(value: Any) -> dict[str, Any] | None:
    """Inverse of ``_serialize_properties``. Tolerates None/non-string."""
    import json
...
def neo4j_create_fact(params: FactCreate) -> FactResponse:
    ...
    client.execute_write(
        create_query,
        {
            ...
            "properties": _serialize_properties(params.properties),
        },
    )
...
def neo4j_get_fact(fact_id: UUID) -> FactResponse | None:
    """
    ...
    return FactResponse(
        ...
        properties=_deserialize_properties(f.get("properties")),
...
def neo4j_list_facts(filters: FactFilter | None = None) -> list[FactResponse]:
    """
...
def neo4j_update_fact(fact_id: UUID, params: FactUpdate) -> FactResponse:
    ...
    if params.properties is not None:
        ...
        update_params["properties"] = _serialize_properties(params.properties)
...
def neo4j_get_event(event_id: UUID) -> EventResponse | None:
    """
    ...
    return EventResponse(
        ...
        properties=_deserialize_properties(ev.get("properties")),
        entity_ids=[UUID(eid) for eid in record["entity_ids"] if eid],
        source_ids=[UUID(sid) for sid in record["source_ids"] if sid],
...
def neo4j_list_events(filters: EventFilter | None = None) -> list[EventResponse]:
    """
...
def neo4j_update_event(event_id: UUID, params: EventUpdate) -> EventResponse:
    """
    ...
    Returns:
        EventResponse with updated event data
    ...
    # Verify event exists
    verify_node_exists(client, "Event", event_id)

    # Build SET clause for only provided fields
    set_clauses = []
    update_params: dict[str, Any] = {"id": str(event_id)}

    if params.title is not None:
        set_clauses.append("ev.title = $title")
        update_params["title"] = params.title

    if params.description is not None:
        set_clauses.append("ev.description = $description")
        update_params["description"] = params.description

    if params.start_time is not None:
        set_clauses.append("ev.start_time = datetime($start_time)")
        update_params["start_time"] = params.start_time.isoformat()

    if params.end_time is not None:
        set_clauses.append("ev.end_time = datetime($end_time)")
        update_params["end_time"] = params.end_time.isoformat()

    if params.magnitude is not None:
        set_clauses.append("ev.magnitude = $magnitude")
        update_params["magnitude"] = params.magnitude

    if params.scope is not None:
        set_clauses.append("ev.scope = $scope")
        update_params["scope"] = params.scope.value

    if params.canon_level is not None:
        set_clauses.append("ev.canon_level = $canon_level")
        update_params["canon_level"] = params.canon_level.value

    if params.knowledge_scope is not None:
        set_clauses.append("ev.knowledge_scope = $knowledge_scope")
        update_params["knowledge_scope"] = params.knowledge_scope.value

    if params.confidence is not None:
        set_clauses.append("ev.confidence = $confidence")
        update_params["confidence"] = params.confidence

    if params.properties is not None:
        set_clauses.append("ev.properties = $properties")
        update_params["properties"] = _serialize_properties(params.properties)

    if not set_clauses:
        # No updates, just return current state
        existing_event = neo4j_get_event(event_id)
        if existing_event is None:
            raise ValueError(f"Event {event_id} not found after verification")
    ...
    # Retrieve updated event with relationships
    updated_event = neo4j_get_event(event_id)
    if updated_event is None:
        raise ValueError(f"Event {event_id} not found after update")
    return updated_event


def neo4j_delete_event(event_id: UUID, force: bool = False) -> dict[str, Any]:
    """
    ...
    Args:
        ...
        force: If True, allow deletion of canon events. If False, prevent it.

    Returns:
        Dict with deletion status

    Raises:
        ValueError: If event doesn't exist or is canon (when force=False)
    """
    client = get_neo4j_client()
    ...
    verify_query = """
    MATCH (ev:Event {id: $id})
    RETURN ev.canon_level as canon_level
    """
    result = client.execute_read(verify_query, {"id": str(event_id)})
    if not result:
        raise ValueError(f"Event {event_id} not found")

    canon_level = result[0]["canon_level"]

    # Prevent deletion of canon events unless force=True
    if canon_level == CanonLevel.CANON.value and not force:
        raise ValueError(
            f"Cannot delete canon event {event_id} without force=True. "
            "Canon events must be explicitly retconned before deletion."
        )

    # Delete event and all relationships
    delete_query = """
    MATCH (ev:Event {id: $id})
    DETACH DELETE ev
    """
    client.execute_write(delete_query, {"id": str(event_id)})
...
def neo4j_get_axiom(axiom_id: UUID) -> AxiomResponse | None:
    """
    ...
    return AxiomResponse(
        ...
        properties=_deserialize_properties(a.get("properties")),
...
def neo4j_update_axiom(axiom_id: UUID, params: AxiomUpdate) -> AxiomResponse:
    ...
    # Verify axiom exists
    verify_node_exists(client, "Axiom", axiom_id)
    ...
    if params.domain is not None:
        set_clauses.append("a.domain = $domain")
        update_params["domain"] = params.domain

    if params.magnitude is not None:
        set_clauses.append("a.magnitude = $magnitude")
        update_params["magnitude"] = params.magnitude

    if params.scope is not None:
        set_clauses.append("a.scope = $scope")
        update_params["scope"] = params.scope.value
    ...
    if params.tags is not None:
        set_clauses.append("a.tags = $tags")
        update_params["tags"] = params.tags

    if params.properties is not None:
        set_clauses.append("a.properties = $properties")
        update_params["properties"] = _serialize_properties(params.properties)

    if not set_clauses:
        # No updates, just return current state
        existing_axiom = neo4j_get_axiom(axiom_id)
        if existing_axiom is None:
    ...
    # Retrieve updated axiom with relationships
    updated_axiom = neo4j_get_axiom(axiom_id)
    if updated_axiom is None:
        raise ValueError(f"Axiom {axiom_id} not found after update")
    return updated_axiom


def neo4j_delete_axiom(axiom_id: UUID, force: bool = False) -> dict[str, Any]:
    """
    Delete an Axiom node.

    Authority: CanonKeeper only
    Use Case: DL-B1

    Args:
        axiom_id: UUID of the axiom to delete
        force: If True, allow deletion of canon axioms. If False, prevent it.

    Returns:
        Dict with deletion status

    Raises:
        ValueError: If axiom doesn't exist or is canon (when force=False)
    """
    client = get_neo4j_client()
    ...
    verify_query = """
    MATCH (a:Axiom {id: $id})
    RETURN a.canon_level as canon_level
    """
    ...
    # Prevent deletion of canon axioms unless force=True
    if canon_level == CanonLevel.CANON.value and not force:
        raise ValueError(
            f"Cannot delete canon axiom {axiom_id} without force=True. "
            "Canon axioms must be explicitly retconned before deletion."
        )

    # Delete axiom and all relationships
    delete_query = """
    MATCH (a:Axiom {id: $id})
    DETACH DELETE a
    """
    client.execute_write(delete_query, {"id": str(axiom_id)})

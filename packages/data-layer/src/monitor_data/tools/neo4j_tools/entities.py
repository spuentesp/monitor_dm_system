...
def neo4j_create_entity(params: EntityCreate) -> EntityResponse:
    ...
    if params.archetype_id:
        ...
        RETURN a.id as id
        """
        archetype_result = client.execute_read(archetype_query, {"archetype_id": str(params.archetype_id)})
...
def neo4j_get_entity(entity_id: UUID) -> EntityResponse | None:
    """
...
def neo4j_list_entities(filters: EntityFilter) -> EntityListResponse:
    ...
    return EntityListResponse(entities=entities, total=total, limit=filters.limit, offset=filters.offset)
...
def neo4j_update_entity(entity_id: UUID, params: EntityUpdate) -> EntityResponse:
    ...
    update_params["updated_at"] = datetime.now(UTC).isoformat()
...
def neo4j_delete_entity(entity_id: UUID, force: bool = False) -> dict[str, Any]:
    ...
    if not force:
        ...
        if dependent_count > 0:
            raise ValueError(
                f"Entity {entity_id} has {dependent_count} dependent facts/events. Use force=True to delete anyway."
            )
...
def neo4j_set_state_tags(entity_id: UUID, params: StateTagsUpdate) -> EntityResponse:
    ...
    if params.remove_tags or params.add_tags:
        ...
        if params.add_tags:
            ...
            tag_expr = f"{kept_expr} + [tag IN $add_tags WHERE NOT tag IN {kept_expr}]"
        else:
            tag_expr = kept_expr
...
def neo4j_save_entity_as_template(entity_id: UUID, template_name: str) -> dict[str, Any]:
    """
...
def neo4j_link_to_archetype(entity_id: UUID, archetype_id: UUID) -> dict[str, Any]:
    """
    ...
    """
    result = client.execute_read(verify_query, {"entity_id": str(entity_id), "archetype_id": str(archetype_id)})
    records = [dict(r) for r in result]
    ...
    RETURN r
    """
    existing = client.execute_read(check_query, {"entity_id": str(entity_id), "archetype_id": str(archetype_id)})
    if list(existing):
        return {
            "entity_id": str(entity_id),
    ...
    RETURN e.id AS eid, a.id AS aid
    """
    client.execute_write(link_query, {"entity_id": str(entity_id), "archetype_id": str(archetype_id)})
...
def neo4j_create_character_relationship(
    ...
    """
    result = client.execute_read(verify_query, {"from_id": str(from_id), "to_id": str(to_id)})
    records = [dict(r) for r in result]
    ...
    if rel_type not in safe_types:
        raise ValueError(f"Invalid relationship type: {rel_type}. Must be one of: {', '.join(sorted(safe_types))}")
    ...
    client.execute_write(
        create_query, {"from_id": str(from_id), "to_id": str(to_id), "rel_type": rel_type, "props": props}
    )

...
from monitor_data.schemas.base import PlotThreadStatus, StoryStatus
...
def neo4j_create_story(params: StoryCreate) -> StoryResponse:
    ...
    if params.pc_ids:
        for pc_id in params.pc_ids:
            ...
            MATCH (pc:Entity {id: $pc_id})
            CREATE (pc)-[:PARTICIPATES]->(s)
            """
            client.execute_write(pc_edge_query, {"story_id": str(story_id), "pc_id": str(pc_id)})
    ...
    return StoryResponse(
        ...
        premise=s["premise"],
        status=s["status"],
        start_time_ref=_to_native_datetime(s.get("start_time_ref")),
        end_time_ref=_to_native_datetime(s.get("end_time_ref")),
        created_at=_to_native_datetime(s["created_at"]),
        completed_at=_to_native_datetime(s.get("completed_at")),
        scene_count=0,
        pc_ids=params.pc_ids,
    )


def neo4j_get_story(story_id: UUID) -> StoryResponse | None:
    """
    ...
    OPTIONAL MATCH (pc:Entity)-[:PARTICIPATES]->(s)
    RETURN s,
...
def neo4j_update_story(story_id: UUID, params: StoryUpdate) -> StoryResponse:
    ...
    if params.status is not None:
        ...
        if params.status.value == "completed":
            ...
            update_params["completed_at"] = datetime.now(UTC).isoformat()
...
def neo4j_create_plot_thread(params: PlotThreadCreate) -> PlotThreadResponse:
    ...
    # Verify story exists
    verify_node_exists(client, "Story", params.story_id)
    ...
    query_params = {
        ...
        "deadline": (params.deadline.model_dump(mode="json") if params.deadline else None),
    ...
    if params.foreshadowing_events:
        ...
        client.execute_write(
            ...
            {
                "thread_id": str(thread_id),
                "event_ids": [str(event_id) for event_id in params.foreshadowing_events],
            },
        )
    ...
    if params.revelation_events:
        ...
        client.execute_write(
            ...
            {
                "thread_id": str(thread_id),
                "event_ids": [str(event_id) for event_id in params.revelation_events],
            },
        )
...
def neo4j_get_plot_thread(id: UUID) -> PlotThreadResponse | None:
    """
    ...
    foreshadowing_events = [UUID(fid) for fid in record["foreshadowing_event_ids"] if fid]
...
def neo4j_update_plot_thread(id: UUID, params: PlotThreadUpdate) -> PlotThreadResponse:
    ...
    if params.status is not None:
        ...
        if params.status == PlotThreadStatus.RESOLVED:
            ...
            query_params["resolved_at"] = datetime.now(UTC).isoformat()
    ...
    if params.add_scene_ids:
        for scene_id in params.add_scene_ids:
            ...
            MERGE (t)-[:ADVANCED_BY]->(sc)
            """
            client.execute_write(scene_rel_query, {"thread_id": str(id), "scene_id": str(scene_id)})
    ...
    if params.add_entity_ids:
        for entity_id in params.add_entity_ids:
            ...
            MERGE (t)-[:INVOLVES]->(e)
            """
            client.execute_write(entity_rel_query, {"thread_id": str(id), "entity_id": str(entity_id)})
    ...
    if params.add_foreshadowing_events:
        for event_id in params.add_foreshadowing_events:
            ...
            MERGE (e)-[:FORESHADOWS]->(t)
            """
            client.execute_write(foreshadow_query, {"thread_id": str(id), "event_id": str(event_id)})
    ...
    if params.add_revelation_events:
        for event_id in params.add_revelation_events:
            ...
            MERGE (e)-[:REVEALS]->(t)
            """
            client.execute_write(reveal_query, {"thread_id": str(id), "event_id": str(event_id)})
    ...
    for record in results:
        ...
        threads.append(
            PlotThreadResponse(
                ...
                foreshadowing_events=foreshadowing_events,
                revelation_events=revelation_events,
                created_at=_to_native_datetime(t["created_at"]),
                updated_at=_to_native_datetime(t["updated_at"]),
                resolved_at=_to_native_datetime(t.get("resolved_at")),
            )
        )

    return PlotThreadListResponse(threads=threads, total=total, limit=params.limit, offset=params.offset)

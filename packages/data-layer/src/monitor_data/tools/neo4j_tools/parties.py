...
def neo4j_create_party(params: PartyCreate) -> PartyResponse:
    ...
    if params.initial_member_ids:
        ...
        if len(valid_ids) != len(params.initial_member_ids):
            raise ValueError("All initial_member_ids must be EntityInstance nodes of type CHARACTER")

    # Verify active_pc_id is in initial_member_ids if both are provided
    if params.active_pc_id and params.initial_member_ids and params.active_pc_id not in params.initial_member_ids:
    ...
    if params.initial_member_ids:
        for idx, entity_id in enumerate(params.initial_member_ids):
            ...
            if not member_result:
                raise ValueError(f"Failed to add initial member {entity_id} to party - entity may not exist")
            r = member_result[0]["r"]
            members.append(
                PartyMemberInfo(
                    entity_id=entity_id,
...
def neo4j_get_party(party_id: UUID) -> PartyResponse | None:
    """
...
def neo4j_set_active_pc(params: SetActivePC) -> PartyResponse:
    ...
    if params.entity_id not in member_ids:
        raise ValueError(f"Entity {params.entity_id} is not a member of party {params.party_id}")
...
def neo4j_update_party_location(party_id: UUID, location_id: UUID | None) -> PartyResponse:
    """
...
def neo4j_update_party_formation(party_id: UUID, formation: list[UUID]) -> PartyResponse:
    """

...
def _check_axioms(
    ...
    for idx, axiom in enumerate(new_axioms):
        for _canon_idx, canon in enumerate(canon_axioms):
...
def _check_lore_facts(
    ...
    for idx, fact in enumerate(new_facts):
        ...
        for _canon_idx, canon in enumerate(canon_facts):
            canon_stmt = canon.get("statement", "")
            if not canon_stmt:
                continue
...
def _check_entity_names(
    ...
    for idx, entity in enumerate(new_pack.entity_archetypes):
        ...
        relevant_facts = [f for f in canon_facts if entity_name_lower in f.get("statement", "").lower()]
            ...
            if similarity < _SIMILARITY_THRESHOLD:
                ...
                matches.append(
                    ContradictionMatch(
                        ...
                        explanation=(
                            f"Entity '{entity.name}' description may contradict established canon: '{canon_stmt[:100]}'"
                        ),
...
def _compare_statements(
    ...
    if _has_negation_conflict(new_lower, existing_lower):
        ...
        ctype = ContradictionType.SEMANTIC_OPPOSITE
        explanation = f"Negation conflict: '{new_statement[:100]}' vs canon: '{existing_statement[:100]}'"
    ...
    if opposite_match and similarity >= _SIMILARITY_THRESHOLD and _shares_subject(new_statement, existing_statement):
        severity = _classify_severity(similarity, existing_canon_level)
        explanation = (
            f"Opposing terms ({opposite_match}): '{new_statement[:100]}' vs canon: '{existing_statement[:100]}'"
        )
        return ContradictionMatch(
            contradiction_id=f"contra:{category}:{item_index}:{hash(existing_statement) % 10000}",
...
def _has_opposite_terms(text_a: str, text_b: str) -> str | None:

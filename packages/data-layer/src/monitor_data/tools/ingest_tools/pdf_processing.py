...
_PDF_STREAM_BUDGET_BYTES: int = int(os.environ.get("MONITOR_PDF_STREAM_BUDGET_BYTES", str(64 * 1024 * 1024)))
...
def _open_pdf(pdf_bytes: bytes) -> fitz.Document:
    ...
    try:
        if len(pdf_bytes) > _PDF_STREAM_BUDGET_BYTES:
            tmp = tempfile.NamedTemporaryFile(  # noqa: SIM115 — lifetime tied to doc
        ...
        raise PdfExtractionError(f"This file could not be opened as a PDF (corrupt or truncated): {exc}") from exc
...
def extract_pdf_text(pdf_bytes: bytes) -> list[dict]:
    ...
    if not pages:
        raise PdfExtractionError(
            "No extractable text found — this looks like a scanned/image-only PDF. OCR it first, then re-upload."
        )
    return pages
...
# [G-8](d) In-memory content-hash LRU. ``extract_pdf_structure`` is the
# public wrapper; ``_extract_pdf_structure_uncached`` does the full
# extract. The cache key is ``sha256(pdf_bytes)`` (the same fingerprint
# used for upload-time SHA-256 dedup in ``ingestion_pipeline.py:306-343``
# — cross-process dupes are caught there; this LRU is per-process).
# Cap = 4 entries; a few in-flight re-ingests of recent PDFs without
# disk-cache complexity (disk buys serialization + invalidation
# complexity for marginal benefit).
_PDF_STRUCTURE_CACHE_MAX: int = 4
_PDF_STRUCTURE_CACHE: OrderedDict[str, list[SectionBlock]] = OrderedDict()


def _deep_copy_sections(sections: list[SectionBlock]) -> list[SectionBlock]:
    """Per-field copies of cached ``SectionBlock`` instances.

    Caller-mutated text and ``heading_path`` don't bleed into the
    cache entry. ``text`` is already an immutable ``str`` so the only
    mutable reference that needs copying is the ``heading_path`` list.
    """
    return [
        SectionBlock(
            heading_path=list(sb.heading_path),
            depth=sb.depth,
...
def extract_pdf_structure(pdf_bytes: bytes) -> list[SectionBlock]:
    """Cached, copy-on-return wrapper over ``_extract_pdf_structure_uncached``.

    Same bytes → SHA-256 hit; one copy per call so caller mutations don't
    poison the cache. Test: ``test_extract_pdf_structure_cache_isolation``
    in ``test_db/test_ingest_tools.py`` asserts (a) the second call
    doesn't re-open fitz and (b) mutating the first call's return
    doesn't affect the second.
    """
    cache_key = hashlib.sha256(pdf_bytes).hexdigest()

    if cache_key in _PDF_STRUCTURE_CACHE:
        # Move-to-end = MRU. Pop then re-insert because OrderedDict
        # doesn't expose ``move_to_end`` until 3.8+ but we target >=3.11;
        # the explicit pop+reinsert is robust across all OrderedDicts.
        cached = _PDF_STRUCTURE_CACHE.pop(cache_key)
        _PDF_STRUCTURE_CACHE[cache_key] = cached
        return _deep_copy_sections(cached)

    sections = _extract_pdf_structure_uncached(pdf_bytes)
    _PDF_STRUCTURE_CACHE[cache_key] = sections
    while len(_PDF_STRUCTURE_CACHE) > _PDF_STRUCTURE_CACHE_MAX:
        _PDF_STRUCTURE_CACHE.popitem(last=False)  # FIFO eviction (LRU=4)
    return _deep_copy_sections(sections)


def _extract_pdf_structure_uncached(pdf_bytes: bytes) -> list[SectionBlock]:
    """
    ...
    if toc:
        ...
        for i, entry in enumerate(toc):
            ...
            text_parts = [doc[p].get_text() for p in range(max(0, page_start), min(doc.page_count, page_end + 1))]
    ...
    try:
        for page_num, page in enumerate(doc, start=1):
            ...
            for block in page_dict.get("blocks", []):
                ...
                dom_size: float = 12.0
                if total_chars:
                    dom_size = sum(s.get("size", 12.0) * len(s.get("text", "")) for s in spans) / total_chars

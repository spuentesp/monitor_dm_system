            ...
            if exact_match is not None:
                ...
                if content_hash == old_content_hash:
                    ...
                    strategy, reason = _recommend_strategy(
                        category_type, item, old_item, identity_field
                    )
                    ...
                    is_negation_conflict = _is_likely_negation_conflict(
                        identity_lower, old_identity_value.lower()
                    )
                ...
                negation_conflict = _find_negation_conflict(
                    identity, existing_by_identity, identity_field
                )
                if negation_conflict is not None:
                    ...
                    semantic_match = _find_semantic_match(
                        identity, existing_by_identity, threshold=0.70
                    )
...
def _build_all_identity_maps(
    existing_packs: List[KnowledgePackResponse],
) -> Dict[str, Dict[str, Tuple[Any, Optional[str], str]]]:
    """
    ...
    """
    result: Dict[str, Dict[str, Tuple[Any, Optional[str], str]]] = {}
        ...
        for field_name, identity_field in _ITEM_CATEGORIES:
            ...
            for item in items:
                ...
                if identity_lower not in result[field_name]:
                    result[field_name][identity_lower] = (item, pack.id, content_hash)
...
def _get_item_identity(item: Any, identity_field: str) -> Optional[str]:
...
def _get_source_ref(item: Any) -> Optional[str]:
    try:
...
def _find_exact_match(
    identity_lower: str,
    existing_by_identity: Dict[str, Tuple[Any, Optional[str], str]],
...
def _find_negation_conflict(
...
) -> Optional[Tuple[Any, Optional[str], str, float]]:
    """
    ...
    for key, (item, pack_id, content_hash) in existing_by_identity.items():
...
def _find_semantic_match(
    ...
    """
    new_lower = new_identity.lower()
    best: Optional[Tuple[Any, Optional[str], float]] = None
    best_ratio = 0.0
...
def _recommend_strategy(
    ...
    if category == DuplicateCategory.CONFLICTING:
        return DeduplicationStrategy.REVIEW, (
            "Conflicting content detected. Manual review required."
        )
    ...
    if category == DuplicateCategory.EXACT:
        ...
        return (
            ...
            f"New item has higher confidence ({new_conf:.2f} vs {existing_conf:.2f}). "
            f"Newer source supersedes.",
        )
    ...
    return (
        DeduplicationStrategy.KEEP_NEW,
        f"New item has similar or better confidence ({new_conf:.2f} vs {existing_conf:.2f}). "
        f"Newer source wins.",
    )

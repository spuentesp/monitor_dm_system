...
def compute_ingestion_delta(
    ...
    source_id: UUID,
    pack_id_current: Optional[UUID] = None,
    pack_id_new: Optional[UUID] = None,
) -> IngestionDelta:
    """
    ...
    rels_delta = _diff_relationships(
        current_pack.entity_relationships, new_pack.entity_relationships
    )
...
def filter_delta_for_apply(
    ...
    def _keep(item: DeltaItem) -> bool:
        ...
        if item.delta_category == DeltaCategory.UNCHANGED:
            return False
        if item.delta_category == DeltaCategory.MODIFIED and not request.include_modified:
            return False
        return True
    ...
    filtered = IngestionDelta(
        ...
        total_axioms=len(
            [d for d in delta.axioms_delta if d.delta_category != DeltaCategory.REMOVED]
        ),
        total_entities=len(
            [d for d in delta.entities_delta if d.delta_category != DeltaCategory.REMOVED]
        ),
        total_lore_facts=len(
            [d for d in delta.lore_facts_delta if d.delta_category != DeltaCategory.REMOVED]
        ),
        total_relationships=len(
            [d for d in delta.relationships_delta if d.delta_category != DeltaCategory.REMOVED]
        ),
        total_random_tables=len(
            [d for d in delta.random_tables_delta if d.delta_category != DeltaCategory.REMOVED]
        ),
        total_agendas=len(
            [d for d in delta.agendas_delta if d.delta_category != DeltaCategory.REMOVED]
        ),
        total_topologies=len(
            [d for d in delta.topologies_delta if d.delta_category != DeltaCategory.REMOVED]
        ),
        ...
        net_new_items=sum(
            1 for d in delta.axioms_delta if d.delta_category == DeltaCategory.ADDED and _keep(d)
        )
        + sum(
            1 for d in delta.entities_delta if d.delta_category == DeltaCategory.ADDED and _keep(d)
        )
        + sum(
            1
            for d in delta.lore_facts_delta
            if d.delta_category == DeltaCategory.ADDED and _keep(d)
        )
        + sum(
            1
            for d in delta.relationships_delta
            if d.delta_category == DeltaCategory.ADDED and _keep(d)
        )
        + sum(
            1
            for d in delta.random_tables_delta
            if d.delta_category == DeltaCategory.ADDED and _keep(d)
        )
        + sum(
            1 for d in delta.agendas_delta if d.delta_category == DeltaCategory.ADDED and _keep(d)
        )
        + sum(
            1
            for d in delta.topologies_delta
            if d.delta_category == DeltaCategory.ADDED and _keep(d)
        ),
        modified_items=sum(
            1 for d in delta.axioms_delta if d.delta_category == DeltaCategory.MODIFIED and _keep(d)
        )
        + sum(
            1
            for d in delta.entities_delta
            if d.delta_category == DeltaCategory.MODIFIED and _keep(d)
        )
        + sum(
            1
            for d in delta.lore_facts_delta
...
def _diff_entities(
    ...
    return _diff_by_identity_field(
        current, new, field_name="entity_archetypes", identity_field="name"
    )
...
def _diff_lore_facts(
    ...
    return _diff_by_identity_field(
        current, new, field_name="lore_facts", identity_field="statement"
    )
...
def _diff_agendas(
    current: List[ExtractedAgenda],
    new: List[ExtractedAgenda],
) -> List[DeltaItem]:
...
def _diff_by_identity_field(
    ...
    for idx, item in enumerate(new):
        ...
        if identity_lower in current_by_identity:
            ...
            if diff["changed_fields"]:
                conf_delta = _get_field(item, "confidence", 0.0) - _get_field(
                    old_item, "confidence", 0.0
                )
                results.append(
                    DeltaItem(
                        item_index=idx,
                        ...
                        source_ref=_get_field(item, "source_ref")
                        or _get_field(item, "source_reference"),
                    )
                )
            else:
                results.append(
...
def _find_best_content_match(
    ...
    best_match: Optional[Tuple[int, Any]] = None
    best_ratio = 0.0
...
def _classify_significance(
    ...
    if total <= 2:
        if removed > (added + modified):
            ...
            return "minor"
        elif change_count == 1 and (modified == 1 or added == 1):
            return "moderate"
        elif change_count <= 2:
            return "moderate"
        return "major"
...
def _build_recommendations(
    ...
    if significance in ("minimal", "minor"):
        ...
        recs.append(
            "Major changes detected. Full manual review strongly recommended "
            "before applying to the multiverse."
        )
    ...
    if added > 5:
        recs.append(
            f"Large number of new items ({added}). Consider batching the apply "
            "or applying to a test universe first."
        )

...
def compute_knowledge_pack_completeness(
    ...
    Returns:
        ...
        and recommendations.
    """
    pack_type = KnowledgePackType(pack.pack_type.value if hasattr(pack.pack_type, "value") else pack.pack_type)
    ...
    categories = CompletenessScores(
        ...
        game_system=_build_category("game_system", present["game_system"], thresholds["game_system"]),
    ...
    missing = [
        name for name in cat_fields if thresholds.get(name, 0) > 0 and getattr(categories, name).status == "missing"
    ]
    partial = [
        name for name in cat_fields if thresholds.get(name, 0) > 0 and getattr(categories, name).status == "partial"
    ]

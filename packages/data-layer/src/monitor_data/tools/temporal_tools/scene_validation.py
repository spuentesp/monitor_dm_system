...
def validate_scene_temporal(
    request: TemporalValidationRequest,
    canon_events: Optional[Sequence[Dict[str, Any]]] = None,
    canon_facts: Optional[Sequence[Dict[str, Any]]] = None,
) -> TemporalValidationResult:
    """
...
def _check_anachronisms(
    ...
    violations.extend(
        TemporalViolation(
            ...
            description=(
                f"Possible anachronism: '{keyword}' in {period} setting "
                f"(scene time: {request.scene_time_ref.year})"
            ),
            scene_id=request.scene_id,
            scene_time_ref=request.scene_time_ref,
            context={
            ...
            if validity.status == FactValidityStatus.EXPIRED:
                ...
                violations.append(
                    TemporalViolation(
                        ...
                        context={
                            "fact_statement": fact.get("statement"),
                            "valid_from": validity.time_ref.isoformat()
                            if validity.time_ref
                            else None,
                        },
                    )
                )

...
2. **Novelty detection**: Flag new entities that aren't in the knowledge graph
   so CanonKeeper can prioritize them.

Intent classification used to live here too (word-set intersection against
hardcoded combat/exploration/dialogue/social/investigation/OOC tables). It was
demoted to a semantic classifier and finally removed — the GMAgent LLM emits
``intent_type`` directly now.
    ...
    for entity in entities[1:]:
        ...
        if entity.start < prev.end:
            if entity.confidence > prev.confidence or (
                entity.confidence == prev.confidence and (entity.end - entity.start) > (prev.end - prev.start)
            ):
                result[-1] = entity
        else:
...
class SpacyBackend(PerceptionBackend):
    ...
    def __init__(self, model_name: str = "en_core_web_trf") -> None:
        try:
            ...
            raise ImportError(
                "spaCy not installed.  Install with: pip install spacy && python -m spacy download en_core_web_trf"
            )
...
class PerceptionPipeline:
    ...
    def __init__(
        ...
        self._known_entities: set[str] = {e.lower() for e in (known_entities or [])}
        logger.info(
    ...
    async def detect(self, text: str) -> PerceptionOutput:
        ...
        -------
        PerceptionOutput
            Detected entities, novelty info, timing.
            (Intent is emitted directly by the GMAgent LLM now.)
        """
    ...
    async def detect_with_input(self, inp: PerceptionInput) -> PerceptionOutput:
        ...
        if inp.known_entities is not None:
            ...
            self._known_entities = {e.lower() for e in inp.known_entities}
            try:

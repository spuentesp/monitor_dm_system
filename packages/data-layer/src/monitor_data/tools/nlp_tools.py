...
def _ensure_nlp_enabled() -> None:
    ...
    if not settings.nlp_enabled:
        raise ValueError(
            "NLP is not enabled. Set NLP_ENABLED=true in your environment configuration."
        )
...
_spacy_backend: Optional[PerceptionBackend] = None
...
def nlp_extract_entities(
    ...
    if settings.nlp_backend == "spacy":
        ...
        entities = _call_sync(
            extract_entities(request.text, labels=request.labels, threshold=request.threshold)
        )
...
def nlp_extract_entities_batch(
    ...
    if settings.nlp_backend == "spacy":
        ...
        results = _call_sync(
            extract_entities_batch(
                request.texts, labels=request.labels, threshold=request.threshold
            )
        )
...
def nlp_get_supported_entity_types() -> dict:
    """

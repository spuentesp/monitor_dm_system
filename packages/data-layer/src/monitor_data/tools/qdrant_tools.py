...
def _upsert_points(client: Any, collection: str, points: list) -> None:
    ...
    try:
        ...
        if "Event loop is closed" not in str(exc):
            raise
        logger.warning(
            "qdrant upsert hit a closed event loop; rebuilding client and retrying once "
...
def qdrant_search(params: VectorSearchRequest) -> VectorSearchResponse:
    ...
    if not query_vector:
        ...
        query_vector = _call_sync(embed_text(query_text))
...
def qdrant_embed_memory(params: MemoryEmbedRequest) -> MemoryEmbedResponse:
    ...
    embedding = _call_sync(embed_text(params.text))
...
def qdrant_search_memories(params: MemorySearchRequest) -> MemorySearchResponse:
    ...
    query_vector = _call_sync(embed_text(params.query_text))

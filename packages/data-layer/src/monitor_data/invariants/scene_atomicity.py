...
class SceneAtomicity:
    ...
    def assert_scene_atomic(scene_boundary: SceneBoundary) -> None:
        ...
        raise ValueError(
            f"Scene {scene_boundary.scene_id} is in invalid state {scene_boundary.status}"
        )
...
def get_atomic_boundary(
    ...
    Returns:
        SceneBoundary object
    """
    return SceneAtomicity.get_atomic_boundary_info(
        scene_id, status, turn_count, proposed_change_count
    )

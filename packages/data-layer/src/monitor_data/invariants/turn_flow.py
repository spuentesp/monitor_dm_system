...
class TurnFlow:
    ...
    @staticmethod
    def validate_turn_sequence(turns: list[Any]) -> tuple[bool, str | None]:
        """
        ...
        for i, turn in enumerate(turns):
            ...
            # Check transitions
            if i > 0:
                prev_phase = turns[i - 1].phase if hasattr(turns[i - 1], "phase") else turns[i - 1].get("phase")
                if not TurnFlow.is_valid_transition(prev_phase, phase):
                    return False, (f"Invalid transition from {prev_phase.name} to {phase.name} at turn {i}")

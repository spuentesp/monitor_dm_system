...
class LayerDirection:
    ...
    def is_valid_import(source_module: str, target_module: str) -> bool:
        ...
        if source_layer < target_layer:
            return False  # Upward import forbidden

        return True

    @staticmethod
    def check_import_legality(source_module: str, target_module: str) -> tuple[bool, Optional[str]]:
        """
        ...
        for forbidden_source, forbidden_target in FORBIDDEN_PATTERNS:
            if source_module.startswith(forbidden_source) and target_module.startswith(
                forbidden_target
            ):
                return False, (

...
CANONKEEPER_REQUIRED_TRANSITIONS: FrozenSet[tuple[ProposedChangeStatus, ProposedChangeStatus]] = (
    frozenset(
        {
...
class ProposedChangeWorkflow:
    ...
    def is_valid_transition(
        from_status: ProposedChangeStatus, to_status: ProposedChangeStatus
    ) -> bool:
        """
    ...
    @staticmethod
    def requires_canon_keeper_action(
        from_status: ProposedChangeStatus, to_status: ProposedChangeStatus
    ) -> bool:
        """
    ...
    @staticmethod
    def validate_workflow_sequence(changes: list) -> tuple[bool, Optional[str]]:
        """
        ...
        for i, change in enumerate(changes):
            ...
            if i > 0:
                prev_status = (
                    changes[i - 1].status
                    if hasattr(changes[i - 1], "status")
                    else changes[i - 1].get("status")
                )

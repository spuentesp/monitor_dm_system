...
class TestComputeIngestionDeltaSignificance:
    ...
    def test_minor_single_addition(self):
        current = _make_pack(axioms=[])
        new = _make_pack(
            axioms=[ExtractedAxiom(statement="One axiom.", domain="test", confidence=0.9)]
        )
        delta = compute_ingestion_delta(current, new, source_id=uuid4())
    ...
    def test_recommendations_include_review(self):
        ...
        new = _make_pack(
            axioms=[
                ExtractedAxiom(statement=f"Axiom {i}", domain="test", confidence=0.9)
                for i in range(10)
            ]
        )
        delta = compute_ingestion_delta(current, new, source_id=uuid4())
...
class TestFilterDeltaForApply:
    def test_removes_unchanged_items(self):
        current = _make_pack(
            entity_archetypes=[
                ExtractedEntityArchetype(name="Goblin", entity_type="character", confidence=0.8)
            ]
        )
        new = _make_pack(
            entity_archetypes=[
    ...
    def test_keeps_added_items(self):
        ...
        new = _make_pack(
            entity_archetypes=[
                ExtractedEntityArchetype(name="Orc", entity_type="character", confidence=0.8)
            ]
        )
        delta = compute_ingestion_delta(current, new, source_id=uuid4())
        request = IngestionDeltaApplyRequest(
...
class TestIngestionDeltaProperties:
    def test_has_changes_true_when_added(self):
        current = _make_pack(axioms=[])
        new = _make_pack(
            axioms=[ExtractedAxiom(statement="New fact.", domain="test", confidence=0.9)]
        )
        delta = compute_ingestion_delta(current, new, source_id=uuid4())
    ...
    def test_has_changes_false_when_identical(self):
        current = _make_pack(
            axioms=[ExtractedAxiom(statement="Fact.", domain="test", confidence=0.9)]
        )

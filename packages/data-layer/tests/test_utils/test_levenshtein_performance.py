...
class TestLevenshteinPerformance:
    ...
    def test_speedup_similar_strings(self, long_similar_strings: tuple[str, str]) -> None:
        ...
        print(f"    rapidfuzz: {rapidfuzz_time:.4f}s ({rapidfuzz_time / BENCHMARK_ITERATIONS * 1000:.3f}ms/call)")
        print(f"    pure-Python: {pure_time:.4f}s ({pure_time / BENCHMARK_ITERATIONS * 1000:.3f}ms/call)")

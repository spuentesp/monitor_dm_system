...
class TestZeroFalseNegatives:
    ...
    def test_candidate_reduction(self) -> None:
        ...
        canon = [_make_canon_axiom(f"{subjects[i % 10]} fact {i} about Middle Earth") for i in range(100)]
        ...
        assert reduction <= 0.20, f"{len(candidates)}/{len(canon)} ({reduction:.1%}) candidates, expected <=20%"

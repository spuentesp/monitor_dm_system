...
# =============================================================================
# F2-4(a) — Ingestion job visibility contract (I-7: Source Library job history)
# =============================================================================


def test_update_ingestion_job_total_attempts_and_failed_sections_round_trip(monkeypatch):
    """total_attempts and failed_sections must persist through
    mongodb_update_ingestion_job and the doc-to-response converter.

    Regression (F2-4(a)): both fields existed on the schema but the update
    tool silently dropped them, so the converter always returned 0/[]."""
    fake_mongo = _FakeMongo()
    monkeypatch.setattr(
        "monitor_data.tools.mongodb_tools.ingestion_jobs.get_mongodb_client",
        lambda: fake_mongo,
    ...
    created_job = mongodb_create_ingestion_job(
        IngestionJobCreate(
            universe_id=uuid4(),
            source_id=uuid4(),
            doc_id=uuid4(),
    ...
    assert created_job.total_attempts == 0
    assert created_job.failed_sections == []

    sections = [
        {"section_path": "Chapter 3 > Spells", "stage": "analyze", "reason": "LLM timeout"},
        {"section_path": "Chapter 4 > Feats", "stage": "analyze", "reason": "provider blocked"},
    ]
    updated_job = mongodb_update_ingestion_job(
        created_job.job_id,
        IngestionJobUpdate(total_attempts=42, failed_sections=sections),
    )

    assert updated_job.total_attempts == 42
    assert updated_job.failed_sections == sections

    # Read-back through the converter must return the same values.
    fetched_job = mongodb_get_ingestion_job(created_job.job_id)
    assert fetched_job is not None
    assert fetched_job.total_attempts == 42
    assert fetched_job.failed_sections == sections


def test_update_ingestion_job_failed_sections_full_replacement(monkeypatch):
    """failed_sections is a full-replacement $set: the analyzer re-sends the
    complete capped list on every update, so last write wins (no append
    duplication)."""
    fake_mongo = _FakeMongo()
    monkeypatch.setattr(
        "monitor_data.tools.mongodb_tools.ingestion_jobs.get_mongodb_client",
    ...
    mongodb_update_ingestion_job(
        created_job.job_id,
        IngestionJobUpdate(failed_sections=[{"section_path": "A", "stage": "analyze", "reason": "boom"}]),
    )
    updated_job = mongodb_update_ingestion_job(
        created_job.job_id,
        IngestionJobUpdate(
            failed_sections=[
                {"section_path": "A", "stage": "analyze", "reason": "boom"},
                {"section_path": "B", "stage": "analyze", "reason": "timeout"},
            ]
        ),
    )

    assert len(updated_job.failed_sections) == 2
    assert updated_job.failed_sections[1]["section_path"] == "B"


def test_update_ingestion_job_warnings_append_remains_capped(monkeypatch):
    """Warning appends stay capped at the most recent 100 entries ($slice: -100).

    Guard for F2-4(a): the visibility fix must not change the existing
    warnings cap behavior."""
    fake_mongo = _FakeMongo()
    monkeypatch.setattr(
        "monitor_data.tools.mongodb_tools.ingestion_jobs.get_mongodb_client",
        lambda: fake_mongo,
    ...
    updated_job = mongodb_update_ingestion_job(
        created_job.job_id,
        IngestionJobUpdate(warnings=[f"warning {i}" for i in range(150)]),
    )

    assert len(updated_job.warnings) == 100
    # $slice: -100 keeps the most recent entries, dropping the oldest.
    assert updated_job.warnings[0] == "warning 50"
    assert updated_job.warnings[-1] == "warning 149"

    # A second append keeps the cap across updates.
    updated_job = mongodb_update_ingestion_job(
        created_job.job_id,
        IngestionJobUpdate(warnings=["warning 150"]),
    )
    assert len(updated_job.warnings) == 100
    assert updated_job.warnings[-1] == "warning 150"

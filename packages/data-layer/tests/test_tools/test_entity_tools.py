...
@pytest.fixture
def entity_instance_data(universe_data: dict[str, Any], entity_archetype_data: dict[str, Any]) -> dict[str, Any]:
...
def test_update_entity(
    ...
    # Mock entity exists check (verify query returns e + archetype_id)
    mock_neo4j_client.execute_read.return_value = [
        {"e": entity_instance_data, "archetype_id": entity_instance_data.get("archetype_id")}
    ]
    ...
    mock_neo4j_client.execute_write.return_value = [{"e": updated_data, "archetype_id": updated_data["archetype_id"]}]
...
@patch("monitor_data.tools.neo4j_tools.entities.get_neo4j_client")
def test_set_state_tags_entity_not_found(mock_get_client: Mock, mock_neo4j_client: Mock):
...
@patch("monitor_data.tools.neo4j_tools.entities.get_neo4j_client")
def test_update_entity_with_properties(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
    entity_instance_data: dict[str, Any],
):
    """Test updating entity with new properties."""
    mock_get_client.return_value = mock_neo4j_client

    # Mock entity exists
    mock_neo4j_client.execute_read.return_value = [{"e": entity_instance_data, "a": None}]

    # Mock update
    updated_data = entity_instance_data.copy()
    updated_data["properties"]["role"] = "Protagonist"
    updated_data["updated_at"] = "2024-01-02T00:00:00"
    mock_neo4j_client.execute_write.return_value = [{"e": updated_data}]

    params = EntityUpdate(properties={"role": "Protagonist", "archetype": "wizard"})

    result = neo4j_update_entity(UUID(entity_instance_data["id"]), params)

    assert result.properties["role"] == "Protagonist"


@patch("monitor_data.tools.neo4j_tools.entities.get_neo4j_client")
def test_update_entity_invalid_params(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
    entity_instance_data: dict[str, Any],
):
    """Test that updating entity with unsupported fields uses neo4j_set_state_tags for state tags."""
    mock_get_client.return_value = mock_neo4j_client

    # Mock entity exists
    mock_neo4j_client.execute_read.return_value = [{"e": entity_instance_data, "archetype_id": None}]
    ...
    # EntityUpdate doesn't support state_tags field - it's validated by schema
    # This test verifies that attempting to update fields works correctly
    params = EntityUpdate(description="Updated description")
...
@patch("monitor_data.tools.neo4j_tools.entities.get_neo4j_client")
def test_get_entity_with_archetype(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
    entity_instance_data: dict[str, Any],
    entity_archetype_data: dict[str, Any],
):
    """Test getting entity with archetype relationship."""
    mock_get_client.return_value = mock_neo4j_client

    # Mock entity with archetype - query returns a.id as archetype_id
    mock_neo4j_client.execute_read.return_value = [
        {
            "e": entity_instance_data,
            "archetype_id": entity_archetype_data["id"],
    ...
    result = neo4j_get_entity(UUID(entity_instance_data["id"]))
...
@patch("monitor_data.tools.neo4j_tools.entities.get_neo4j_client")
def test_list_entities_with_combined_filters(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
    entity_instance_data: dict[str, Any],
):
    """Test listing entities with multiple filters."""
    mock_get_client.return_value = mock_neo4j_client

    # Mock count query returns total, list query returns entities
    mock_neo4j_client.execute_read.side_effect = [
        [{"total": 1}],  # Count query
        [{"e": entity_instance_data, "archetype_id": None}],  # List query
    ]

    filters = EntityFilter(
        universe_id=UUID(entity_instance_data["universe_id"]),
        entity_type=EntityType.CHARACTER,
        state_tags=["alive", "traveling"],
    )
    ...
    assert len(result.entities) == 1
    assert result.entities[0].name == "Gandalf"


@patch("monitor_data.tools.neo4j_tools.entities.get_neo4j_client")
def test_list_entities_by_canon_level(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
    universe_data: dict[str, Any],
):
    """Test listing entities filtered by canon_level."""
    mock_get_client.return_value = mock_neo4j_client
    ...
    mock_neo4j_client.execute_read.side_effect = [
        [{"total": 1}],  # Count query
        [{"e": entity_data, "archetype_id": None}],  # List query
    ]
    ...
    assert len(result.entities) == 1
    assert result.entities[0].canon_level == CanonLevel.PROPOSED


@patch("monitor_data.tools.neo4j_tools.entities.get_neo4j_client")
def test_delete_entity_with_force_archetype(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
    entity_archetype_data: dict[str, Any],
):
    """Test force deleting an archetype ignores check."""
    mock_get_client.return_value = mock_neo4j_client

    # Mock verify_node_exists (dependency check skipped when force=True)
    mock_neo4j_client.execute_read.return_value = [{"id": str(entity_archetype_data["id"])}]

    # Mock delete succeeds (DETACH DELETE returns nothing)
    mock_neo4j_client.execute_write.return_value = None

    result = neo4j_delete_entity(UUID(entity_archetype_data["id"]), force=True)
...
@patch("monitor_data.tools.neo4j_tools.entities.get_neo4j_client")
def test_set_state_tags_clear_all(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
    entity_instance_data: dict[str, Any],
):
    """Test clearing all state tags."""
    mock_get_client.return_value = mock_neo4j_client

    # Mock entity exists and is not an archetype (verify query)
    mock_neo4j_client.execute_read.return_value = [{"is_archetype": False}]

    # Mock update clears tags (update query)
    updated_data = entity_instance_data.copy()
    updated_data["state_tags"] = []
    updated_data["updated_at"] = "2024-01-02T00:00:00"
    mock_neo4j_client.execute_write.return_value = [{"e": updated_data, "archetype_id": None}]

    params = StateTagsUpdate(add_tags=[], remove_tags=["alive", "traveling", "resting"])
...
@patch("monitor_data.tools.neo4j_tools.entities.get_neo4j_client")
def test_set_state_tags_nonexistent_entity(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
):
    """Test setting state tags on non-existent entity."""
    mock_get_client.return_value = mock_neo4j_client
    mock_neo4j_client.execute_read.return_value = []

    params = StateTagsUpdate(add=["alive"], remove=[])
...
def test_create_entity_with_all_fields(
    ...
    """Test creating entity with all optional fields."""
    mock_get_client.return_value = mock_neo4j_client

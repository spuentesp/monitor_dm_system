...
@pytest.fixture
def story_outline_data(story_data: dict[str, Any], story_beat_data: dict[str, Any]) -> dict[str, Any]:
...
@patch("monitor_data.tools.mongodb_tools.stories.get_neo4j_client")
@patch("monitor_data.tools.mongodb_tools.stories.get_mongodb_client")
...
def test_update_story_outline_mark_clue_discovered(
    ...
    updated_data["mystery_structure"]["core_clues"][0]["visibility"] = ClueVisibility.DISCOVERED.value
...
@patch("monitor_data.tools.mongodb_tools.stories.get_neo4j_client")
@patch("monitor_data.tools.mongodb_tools.stories.get_mongodb_client")
def test_create_story_outline_with_branching_points(
    mock_get_mongo: Mock,
    mock_get_neo4j: Mock,
    mock_neo4j_client: Mock,
    story_data: dict[str, Any],
    ...
    """Test creating story outline with branching points."""
    # Setup mocks
    mock_get_neo4j.return_value = mock_neo4j_client
    mock_neo4j_client.execute_read.return_value = [{"id": story_data["id"]}]
    ...
    from monitor_data.schemas.story_outlines import BranchingPoint
    ...
    branching_point = BranchingPoint(
        ...
        decision="Players must choose between saving the village or pursuing the villain",
        branches=[
            {
                "name": "Save Village",
                "condition": "Players choose to protect civilians",
                "next_beat_order": 1,
            },
            {
                "name": "Pursue Villain",
                "condition": "Players choose to chase the antagonist",
                "next_beat_order": 1,
            },
        ],
    )
    ...
    params = StoryOutlineCreate(
        story_id=UUID(story_data["id"]),
        theme="Moral Choices",
        premise="Heroes face difficult decisions",
        beats=[beat],
        branching_points=[branching_point],
    )

    result = mongodb_create_story_outline(params)

    assert len(result.branching_points) == 1
    assert (
        result.branching_points[0].decision == "Players must choose between saving the village or pursuing the villain"
    )
    assert len(result.branching_points[0].branches) == 2


@patch("monitor_data.tools.mongodb_tools.stories.get_neo4j_client")
@patch("monitor_data.tools.mongodb_tools.stories.get_mongodb_client")
def test_create_story_outline_with_constraints(
    mock_get_mongo: Mock,
    mock_get_neo4j: Mock,
    mock_neo4j_client: Mock,
    story_data: dict[str, Any],
):
    """Test creating story outline with constraints."""
    # Setup mocks
    mock_get_neo4j.return_value = mock_neo4j_client
    mock_neo4j_client.execute_read.return_value = [{"id": story_data["id"]}]
    ...
    params = StoryOutlineCreate(
        story_id=UUID(story_data["id"]),
        theme="Constrained Adventure",
        premise="Heroes must work within limitations",
        constraints=["No magic", "No resurrection", "Time limit: 3 days"],
    )

    result = mongodb_create_story_outline(params)

    assert len(result.constraints) == 3
    assert "No magic" in result.constraints
    assert "No resurrection" in result.constraints
    assert "Time limit: 3 days" in result.constraints


@patch("monitor_data.tools.mongodb_tools.stories.get_mongodb_client")
def test_update_story_outline_add_branching_points(
    mock_get_mongo: Mock,
    story_data: dict[str, Any],
    story_outline_data: dict[str, Any],
    story_beat_data: dict[str, Any],
):
    """Test adding branching points to existing story outline."""
    from monitor_data.schemas.story_outlines import BranchingPoint
    ...
    # Get beat ID from existing beats
    beat_id = UUID(story_beat_data["beat_id"])

    # Add branching points to outline
    updated_data = story_outline_data.copy()
    new_branching_point = {
        "beat_id": str(beat_id),
        "decision": "Players must make a pivotal decision",
        "branches": [
            {
                "name": "Path A",
                "condition": "Choose carefully",
                "next_beat_order": 1,
            },
            {
                "name": "Path B",
                "condition": "Alternative choice",
                "next_beat_order": 1,
            },
        ],
    }
    updated_data["branching_points"] = [new_branching_point]
    ...
    params = StoryOutlineUpdate(
        add_branching_points=[branching_point],
    )

    result = mongodb_update_story_outline(UUID(story_data["id"]), params)

    assert len(result.branching_points) == 1
    assert result.branching_points[0].decision == "Players must make a pivotal decision"
    assert len(result.branching_points[0].branches) == 2


@patch("monitor_data.tools.mongodb_tools.stories.get_mongodb_client")
def test_update_story_outline_update_mystery_structure(
    mock_get_mongo: Mock,
    story_data: dict[str, Any],
    story_outline_data: dict[str, Any],
    mystery_clue_data: dict[str, Any],
):
    """Test updating mystery structure in story outline."""
    mock_mongo_client = MagicMock()
    mock_collection = MagicMock()
    mock_get_mongo.return_value = mock_mongo_client
    mock_mongo_client.get_collection.return_value = mock_collection

    # Create new mystery structure
    new_mystery = MysteryStructure(
        truth="The captain was bribed by the enemy",
        question="Who betrayed the kingdom?",
        core_clues=[
            MysteryClue(
                content="Suspicious bank deposits",
                discovery_methods=["investigation"],
            ),
            MysteryClue(
                content="Secret correspondence",
                discovery_methods=["search"],
            ),
        ],
    )

    # Updated data with new mystery
    updated_data = story_outline_data.copy()
    updated_data["mystery_structure"] = new_mystery.model_dump(mode="json")

    mock_collection.find_one.side_effect = [story_outline_data, updated_data]
    mock_collection.update_one.return_value = Mock()

    params = StoryOutlineUpdate(
        update_mystery_structure=new_mystery,
    )

    result = mongodb_update_story_outline(UUID(story_data["id"]), params)

    assert result.mystery_structure is not None
    assert result.mystery_structure.truth == "The captain was bribed by the enemy"
    assert len(result.mystery_structure.core_clues) == 2


@patch("monitor_data.tools.mongodb_tools.stories.get_mongodb_client")
def test_update_story_outline_update_constraints(
    mock_get_mongo: Mock,
    story_data: dict[str, Any],
    story_outline_data: dict[str, Any],
):
    """Test updating constraints in story outline."""
    mock_mongo_client = MagicMock()
    mock_collection = MagicMock()
    mock_get_mongo.return_value = mock_mongo_client
    mock_mongo_client.get_collection.return_value = mock_collection

    # Updated data with new constraints
    updated_data = story_outline_data.copy()
    updated_data["constraints"] = ["No resurrection", "Time limit: 1 week", "Must keep evidence"]

    mock_collection.find_one.side_effect = [story_outline_data, updated_data]
    mock_collection.update_one.return_value = Mock()

    params = StoryOutlineUpdate(
        constraints=["No resurrection", "Time limit: 1 week", "Must keep evidence"],
    )

    result = mongodb_update_story_outline(UUID(story_data["id"]), params)

    assert len(result.constraints) == 3
    assert "Must keep evidence" in result.constraints
    assert result.constraints == ["No resurrection", "Time limit: 1 week", "Must keep evidence"]


@patch("monitor_data.tools.mongodb_tools.stories.get_neo4j_client")
@patch("monitor_data.tools.mongodb_tools.stories.get_mongodb_client")
def test_create_story_outline_with_minimal_fields(
    mock_get_mongo: Mock,
    mock_get_neo4j: Mock,
    mock_neo4j_client: Mock,
    story_data: dict[str, Any],
):
    """Test creating story outline with only required story_id field."""
    # Setup mocks
    mock_get_neo4j.return_value = mock_neo4j_client
    mock_neo4j_client.execute_read.return_value = [{"id": story_data["id"]}]
    ...
    assert result.beats == []
    assert result.structure_type == StoryStructureType.LINEAR
    assert result.template == ArcTemplate.CUSTOM
    assert result.branching_points == []
    assert result.mystery_structure is None


@patch("monitor_data.tools.mongodb_tools.stories.get_mongodb_client")
def test_update_story_outline_structure_type_only(
    mock_get_mongo: Mock,
    story_data: dict[str, Any],
    story_outline_data: dict[str, Any],
):
    """Test updating only the structure_type field of story outline."""
    mock_mongo_client = MagicMock()
    mock_collection = MagicMock()
    mock_get_mongo.return_value = mock_mongo_client
    mock_mongo_client.get_collection.return_value = mock_collection

    # Updated data with new structure type
    updated_data = story_outline_data.copy()
    updated_data["structure_type"] = StoryStructureType.BRANCHING.value

    mock_collection.find_one.side_effect = [story_outline_data, updated_data]
    mock_collection.update_one.return_value = Mock()

    params = StoryOutlineUpdate(
        structure_type=StoryStructureType.BRANCHING,
    )

    result = mongodb_update_story_outline(UUID(story_data["id"]), params)

    assert result.structure_type == StoryStructureType.BRANCHING


@patch("monitor_data.tools.mongodb_tools.stories.get_mongodb_client")
def test_update_story_outline_template_only(
    mock_get_mongo: Mock,
    story_data: dict[str, Any],
    story_outline_data: dict[str, Any],
):
    """Test updating only the template field of story outline."""
    mock_mongo_client = MagicMock()
    mock_collection = MagicMock()
    mock_get_mongo.return_value = mock_mongo_client
    mock_mongo_client.get_collection.return_value = mock_collection

    # Updated data with new template
    updated_data = story_outline_data.copy()
    updated_data["template"] = ArcTemplate.FIVE_ACT.value

    mock_collection.find_one.side_effect = [story_outline_data, updated_data]
    mock_collection.update_one.return_value = Mock()

    params = StoryOutlineUpdate(
        template=ArcTemplate.FIVE_ACT,
    )

    result = mongodb_update_story_outline(UUID(story_data["id"]), params)

    assert result.template == ArcTemplate.FIVE_ACT


@patch("monitor_data.tools.mongodb_tools.stories.get_mongodb_client")
def test_update_story_outline_multiple_fields(
    mock_get_mongo: Mock,
    story_data: dict[str, Any],
    story_outline_data: dict[str, Any],
):
    """Test updating multiple fields at once in story outline."""
    mock_mongo_client = MagicMock()
    mock_collection = MagicMock()
    mock_get_mongo.return_value = mock_mongo_client
    mock_mongo_client.get_collection.return_value = mock_collection

    # Updated data with multiple fields changed
    updated_data = story_outline_data.copy()
    updated_data["theme"] = "Updated theme"
    updated_data["premise"] = "Updated premise"
    updated_data["structure_type"] = StoryStructureType.BRANCHING.value
    updated_data["template"] = ArcTemplate.HEROES_JOURNEY.value
    ...
    params = StoryOutlineUpdate(
        ...
        premise="Updated premise",
        structure_type=StoryStructureType.BRANCHING,
        template=ArcTemplate.HEROES_JOURNEY,
    )
    ...
    assert result.premise == "Updated premise"
    assert result.structure_type == StoryStructureType.BRANCHING
    assert result.template == ArcTemplate.HEROES_JOURNEY


@patch("monitor_data.tools.mongodb_tools.stories.get_mongodb_client")
def test_update_story_outline_mark_clue_discovered_bonus_clue(
    mock_get_mongo: Mock,
    story_data: dict[str, Any],
    story_outline_data: dict[str, Any],
    mystery_clue_data: dict[str, Any],
):
    """Test marking a bonus clue (not core clue) as discovered in mystery structure."""
    mock_mongo_client = MagicMock()
    mock_collection = MagicMock()
    mock_get_mongo.return_value = mock_mongo_client
    mock_mongo_client.get_collection.return_value = mock_collection

    # Add mystery structure with bonus clue to outline
    outline_with_mystery = story_outline_data.copy()
    bonus_clue = mystery_clue_data.copy()
    bonus_clue["clue_id"] = str(uuid4())  # Different ID
    outline_with_mystery["mystery_structure"] = {
        "truth": "The butler did it",
        "question": "Who killed Lord Blackwood?",
        "core_clues": [],
        "bonus_clues": [bonus_clue],
        "red_herrings": [],
        "suspects": [],
        "current_player_theories": [],
    }

    # Updated with discovered bonus clue
    updated_data = outline_with_mystery.copy()
    updated_data["mystery_structure"]["bonus_clues"][0]["is_discovered"] = True
    updated_data["mystery_structure"]["bonus_clues"][0]["visibility"] = ClueVisibility.DISCOVERED.value

    mock_collection.find_one.side_effect = [outline_with_mystery, updated_data]
    mock_collection.update_one.return_value = Mock()

    clue_id = UUID(bonus_clue["clue_id"])
    params = StoryOutlineUpdate(
        mark_clue_discovered=clue_id,
    )
    ...
    assert result.mystery_structure is not None
    assert len(result.mystery_structure.bonus_clues) == 1
    assert result.mystery_structure.bonus_clues[0].is_discovered is True
    assert result.mystery_structure.bonus_clues[0].visibility == ClueVisibility.DISCOVERED

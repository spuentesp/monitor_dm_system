...
def test_get_recent_conversation_turns_flattens_and_orders(mock_mongo: Mock) -> None:
    ...
    newer = datetime(2026, 1, 1, 10, 5, 0, tzinfo=UTC)
...
def test_list_conversations_filter_by_status(mock_mongo: Mock) -> None:
    ...
    completed_doc = _make_conv_doc(uuid4(), universe_id, [npc_id], status=ConversationStatus.COMPLETED)
    ...
    filter_params = ConversationSessionFilter(universe_id=universe_id, status=ConversationStatus.COMPLETED)
                ...
                "turns": [
                    {
                        ...
                        "text": f"Turn {i}",
                        "emotional_state": None,
                        "timestamp": datetime(2026, 1, 1, 10, i, 0, tzinfo=UTC),
                    }
                ],
            }
        )

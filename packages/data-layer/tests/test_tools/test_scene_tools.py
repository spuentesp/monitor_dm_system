...
@patch("monitor_data.tools.mongodb_tools.scenes.get_neo4j_client")
@patch("monitor_data.tools.mongodb_tools.scenes.get_mongodb_client")
def test_create_scene_success(
    ...
    mock_neo4j_client.execute_read.return_value = [{"story_id": story_data["id"], "universe_id": universe_data["id"]}]
...
# =============================================================================
# TESTS: mongodb_get_scene_summary / mongodb_get_recent_turns
# =============================================================================


@patch("monitor_data.tools.mongodb_tools.scenes.get_mongodb_client")
def test_get_scene_summary_prefers_summary(
    mock_get_mongo: Mock,
    mock_mongodb_client: Mock,
    scene_data: dict[str, Any],
):
    """Test that persisted scene summaries are returned directly."""
    mock_get_mongo.return_value = mock_mongodb_client

    scene_data["summary"] = "The party breaches the keep and alarms the guard."
    collection = mock_mongodb_client.get_collection.return_value
    collection.find_one.return_value = scene_data

    result = mongodb_get_scene_summary(UUID(scene_data["scene_id"]))

    assert result == "The party breaches the keep and alarms the guard."
    collection.find_one.assert_called_once()


@patch("monitor_data.tools.mongodb_tools.scenes.get_mongodb_client")
def test_get_scene_summary_falls_back_to_title_and_purpose(
    mock_get_mongo: Mock,
    mock_mongodb_client: Mock,
    scene_data: dict[str, Any],
):
    """Test that title/purpose provide fallback scene context when summary is blank."""
    mock_get_mongo.return_value = mock_mongodb_client
    ...
    assert result == "Opening Scene. Introduce the characters"


@patch("monitor_data.tools.mongodb_tools.scenes.get_mongodb_client")
def test_get_recent_turns_returns_tail_window(
    mock_get_mongo: Mock,
    mock_mongodb_client: Mock,
    scene_data: dict[str, Any],
):
    """Test recent-turn helper returns only the most recent requested turns."""
    mock_get_mongo.return_value = mock_mongodb_client
    ...
    second_turn = {
        ...
        "speaker": Speaker.GM.value,
        "entity_id": None,
        "text": "A chill wind moves through the hall.",
        "timestamp": datetime.now(UTC),
        "resolution_ref": None,
    }
    third_turn = {
        ...
        "speaker": Speaker.USER.value,
        "entity_id": None,
        "text": "I raise the lantern higher.",
        "timestamp": datetime.now(UTC),
        "resolution_ref": None,
    }
    scene_data["turns"] = [first_turn, second_turn, third_turn]

    collection = mock_mongodb_client.get_collection.return_value
    collection.find_one.return_value = scene_data

    result = mongodb_get_recent_turns(UUID(scene_data["scene_id"]), limit=2)

    assert [turn["text"] for turn in result] == [
        "A chill wind moves through the hall.",
        "I raise the lantern higher.",
    ]
...
def test_scene_status_transition_active_to_completed(
    ...
    updated_data["completed_at"] = datetime.now(UTC)

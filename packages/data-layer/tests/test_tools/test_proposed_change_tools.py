...
from monitor_data.schemas.base import Authority, PromotionIntent, ProposalStatus, ProposalType
...
@pytest.fixture
def scene_doc_data(story_data: dict[str, Any], universe_data: dict[str, Any]) -> dict[str, Any]:
...
@patch("monitor_data.tools.mongodb_tools.proposals.get_neo4j_client")
@patch("monitor_data.tools.mongodb_tools.proposals.get_mongodb_client")
...
def test_create_proposed_change_entity(
    ...
    proposed_changes_collection.insert_one.assert_called_once()
    # Entity-promotion defaults: no intent tagged, first mention, not
    # mechanically bound — see docs/2_architecture/data_model_workflow.md.
    assert result.promotion_intent is None
    assert result.interaction_count == 1
    assert result.is_mechanically_bound is False


@patch("monitor_data.tools.mongodb_tools.proposals.get_neo4j_client")
@patch("monitor_data.tools.mongodb_tools.proposals.get_mongodb_client")
def test_create_proposed_change_entity_with_promotion_fields(
    mock_get_mongo: Mock,
    mock_get_neo4j: Mock,
    mock_mongodb_client: Mock,
    mock_neo4j_client: Mock,
    ...
    """An anchor-tagged entity proposal persists its promotion fields and
    round-trips through the doc-to-response converter correctly."""
    mock_get_mongo.return_value = mock_mongodb_client
    mock_get_neo4j.return_value = mock_neo4j_client
    ...
    assert result.promotion_intent == PromotionIntent.ANCHOR
    assert result.interaction_count == 3
    assert result.is_mechanically_bound is True

    # Confirm the actual persisted doc (what insert_one received) carries
    # the plain string value, not the enum object — Mongo can't store enums.
    persisted_doc = proposed_changes_collection.insert_one.call_args[0][0]
    assert persisted_doc["promotion_intent"] == "anchor"
    assert persisted_doc["interaction_count"] == 3
    assert persisted_doc["is_mechanically_bound"] is True
...
def test_create_proposed_change_no_scene_or_story():
    ...
    with pytest.raises(ValueError, match="Either scene_id or story_id must be provided"):
        ProposedChangeCreate(
            change_type=ProposalType.FACT,
            content={"statement": "Test"},
            proposer="TestAgent",
...
def test_list_proposed_changes_by_scene(
    ...
    collection.find.return_value.sort.return_value.skip.return_value.limit.return_value = [proposed_change_doc]
...
def test_list_proposed_changes_by_story(
    ...
    collection = mock_mongodb_client.get_collection.return_value
    collection.count_documents.return_value = 1
    collection.find.return_value.sort.return_value.skip.return_value.limit.return_value = [story_proposal_doc]
...
def test_update_proposed_change_accept(
    ...
    params = ProposedChangeUpdate(status=ProposalStatus.ACCEPTED, decision_metadata=decision)
...
def test_update_proposed_change_reject(
    ...
    params = ProposedChangeUpdate(status=ProposalStatus.REJECTED, decision_metadata=decision)
...
def test_update_proposed_change_invalid_status(
    ...
    params = ProposedChangeUpdate(status=ProposalStatus.PENDING, decision_metadata=decision)

    proposal_id = UUID(proposed_change_doc["proposal_id"])

    with pytest.raises(ValueError, match="Can only transition from pending to accepted or rejected"):
        mongodb_update_proposed_change(proposal_id, params)
...
@patch("monitor_data.tools.mongodb_tools.proposals.get_mongodb_client")
def test_update_proposed_change_invalid_transition_from_rejected(
    mock_get_mongo: Mock,
    mock_mongodb_client: Mock,
    proposed_change_doc: dict[str, Any],
):
    """Test that updating a rejected proposal to accepted raises an error."""
    mock_get_mongo.return_value = mock_mongodb_client

    collection = mock_mongodb_client.get_collection.return_value

    # Create a rejected proposal doc
    rejected_doc = proposed_change_doc.copy()
    rejected_doc["status"] = ProposalStatus.REJECTED.value
    rejected_doc["decision_metadata"] = {
        "decided_by": "CanonKeeper",
    ...
    collection.find_one.return_value = rejected_doc
    ...
    with pytest.raises(
        ValueError,
        match="Cannot update proposal with status rejected. Only pending proposals can be accepted or rejected.",
    ):
        mongodb_update_proposed_change(UUID(proposed_change_doc["proposal_id"]), params)


@patch("monitor_data.tools.mongodb_tools.proposals.get_mongodb_client")
def test_update_proposed_change_invalid_transition_from_accepted(
    mock_get_mongo: Mock,
    mock_mongodb_client: Mock,
    proposed_change_doc: dict[str, Any],
):
    """Test that updating an accepted proposal to rejected raises an error."""
    mock_get_mongo.return_value = mock_mongodb_client

    collection = mock_mongodb_client.get_collection.return_value

    # Create an accepted proposal doc
    accepted_doc = proposed_change_doc.copy()
    accepted_doc["status"] = ProposalStatus.ACCEPTED.value
    accepted_doc["decision_metadata"] = {
        "decided_by": "CanonKeeper",
    ...
    with pytest.raises(
        ValueError,
        match="Cannot update proposal with status accepted. Only pending proposals can be accepted or rejected.",
    ):
        mongodb_update_proposed_change(UUID(proposed_change_doc["proposal_id"]), params)


@patch("monitor_data.tools.mongodb_tools.proposals.get_mongodb_client")
def test_list_proposed_changes_by_source(
    mock_get_mongo: Mock,
    mock_mongodb_client: Mock,
    proposed_change_doc: dict[str, Any],
):
    """Test listing proposed changes filtered by source tag."""
    mock_get_mongo.return_value = mock_mongodb_client
    ...
    params = ProposedChangeFilter(source="knowledge_pack:test-uuid")
    result = mongodb_list_proposed_changes(params)

    assert result.total == 1
    assert len(result.proposed_changes) == 1
    call_args = collection.count_documents.call_args[0][0]
    assert call_args["source"] == "knowledge_pack:test-uuid"


@patch("monitor_data.tools.mongodb_tools.proposals.get_mongodb_client")
def test_list_proposed_changes_sort_by_confidence(
    mock_get_mongo: Mock,
    mock_mongodb_client: Mock,
    proposed_change_doc: dict[str, Any],
):
    """Test listing proposed changes sorted by confidence."""
    mock_get_mongo.return_value = mock_mongodb_client
    ...
    params = ProposedChangeFilter(sort_by="confidence")
    result = mongodb_list_proposed_changes(params)

    assert result.total == 2
    assert len(result.proposed_changes) == 2
    # Verify sort is called with confidence field
    collection.find.return_value.sort.assert_called_once_with("confidence", -1)


@patch("monitor_data.tools.mongodb_tools.proposals.get_mongodb_client")
def test_list_proposed_changes_sort_order_asc(
    mock_get_mongo: Mock,
    mock_mongodb_client: Mock,
    proposed_change_doc: dict[str, Any],
):
    """Test listing proposed changes with ascending sort order."""
    mock_get_mongo.return_value = mock_mongodb_client
    ...
    params = ProposedChangeFilter(sort_order="asc")
    result = mongodb_list_proposed_changes(params)

    assert result.total == 1
    assert len(result.proposed_changes) == 1
    # Verify sort is called with ascending order (1)

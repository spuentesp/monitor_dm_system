...
class TestRegexEntityDetection:
    ...
    def test_skips_common_words(self) -> None:
        ...
        # "I" and "The" should not be detected as entities
        names = [e.text for e in entities]
    ...
    def test_detects_entity_with_location_hint(self) -> None:
        ...
        # Should detect "Shadowfell Forest" as a location
    ...
    def test_detects_entity_with_object_hint(self) -> None:
        ...
        # Should detect something with "Sword" in it
        assert (
            any("Sword" in e.text or "Dragonslayer" in e.text for e in objects) or len(objects) >= 1
        )


# ============================================================================
# REGEX BACKEND — INTENT CLASSIFICATION
# ============================================================================


class TestRegexIntentClassification:
    """Test RegexPerceptionBackend.classify_intent()."""

    def setup_method(self) -> None:
        self.backend = RegexPerceptionBackend()

    def test_combat_intent(self) -> None:
        intent, conf = self.backend.classify_intent("I attack the goblin with my sword")
        assert intent == IntentType.COMBAT
        assert conf > 0.5

    def test_exploration_intent(self) -> None:
        intent, conf = self.backend.classify_intent("I examine the room and search for traps")
        assert intent == IntentType.EXPLORATION
        assert conf > 0.5

    def test_dialogue_intent(self) -> None:
        intent, conf = self.backend.classify_intent("I tell the guard that we are travelers")
        assert intent == IntentType.DIALOGUE
        assert conf > 0.5

    def test_ooc_intent(self) -> None:
        intent, conf = self.backend.classify_intent("(( wait, actually I meant to go left ))")
        assert intent == IntentType.OOC
        assert conf > 0.6

    def test_ooc_keyword_correction(self) -> None:
        intent, conf = self.backend.classify_intent("sorry, correction: I actually attack")
        assert intent == IntentType.OOC

    def test_unknown_intent(self) -> None:
        intent, conf = self.backend.classify_intent("hello")
        # "hello" has no strong keywords — could be unknown
        assert isinstance(intent, IntentType)

    def test_combat_intent_high_confidence(self) -> None:
        intent, conf = self.backend.classify_intent(
            "I attack the dragon, roll initiative, and deal critical damage"
        )
        assert intent == IntentType.COMBAT
        assert conf > 0.7  # many combat keywords = high confidence
...
class TestDeduplication:
    ...
    def test_no_overlaps(self) -> None:
        entities = [
            DetectedEntity(
                text="A", entity_type=DetectedEntityType.CHARACTER, start=0, end=1, confidence=0.8
            ),
            DetectedEntity(
                text="B", entity_type=DetectedEntityType.LOCATION, start=5, end=6, confidence=0.8
            ),
        ]
...
class TestPerceptionPipeline:
    """Test PerceptionPipeline full async flow."""
    ...
    @pytest.mark.asyncio
    async def test_pipeline_intent(self) -> None:
        pipe = PerceptionPipeline()
        result = await pipe.detect("I attack the Dragon with my Flame Sword")
        assert result.intent == IntentType.COMBAT
    ...
    async def test_pipeline_novelty_detection(self) -> None:
        ...
        # "Dragon" should NOT be novel (it's in known_entities)
        assert not any("Dragon" in n for n in novel_names) or len(result.novel_entities) < len(
            result.entities
        )

    @pytest.mark.asyncio
    async def test_pipeline_no_known_entities(self) -> None:
        pipe = PerceptionPipeline()  # no known_entities
        result = await pipe.detect("I attack the Dragon")

        # Without known_entities, ALL entities are "novel"
...
class TestRegexBackendAsync:
    ...
    @pytest.mark.asyncio
    async def test_async_intent(self) -> None:
        backend = RegexBackend()
        intent, conf = await backend.classify_intent("I attack the Dragon")
        assert intent == IntentType.COMBAT
...
class TestPerformanceBudget:
    ...
    async def test_regex_under_50ms(self) -> None:
        ...
        # Regex backend should be well under 50ms
        assert result.processing_time_ms < 50, (
            f"Regex backend took {result.processing_time_ms:.1f}ms (budget: 50ms)"
        )

    @pytest.mark.asyncio
    async def test_batch_100_inputs_under_500ms(self) -> None:
        """100 inputs should complete in < 500ms total."""
        import time

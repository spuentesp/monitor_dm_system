...
@patch("monitor_data.tools.neo4j_tools.core.get_neo4j_client")
...
@patch("monitor_data.tools.neo4j_tools.core.get_neo4j_client")
def test_create_universe_invalid_multiverse(mock_get_client: Mock, mock_neo4j_client: Mock):
...
@patch("monitor_data.tools.neo4j_tools.core.get_neo4j_client")
@patch("monitor_data.tools.neo4j_tools.core.neo4j_get_universe")
...
@patch("monitor_data.tools.neo4j_tools.core.get_neo4j_client")
def test_create_multiverse_invalid_omniverse(mock_get_client: Mock, mock_neo4j_client: Mock):
...
@patch("monitor_data.tools.neo4j_tools.core.get_neo4j_client")
def test_ensure_omniverse_already_exists(mock_get_client: Mock, mock_neo4j_client: Mock):
...
# =============================================================================
# TESTS: neo4j_get_universe_state
# =============================================================================


@patch("monitor_data.tools.neo4j_tools.core.get_neo4j_client")
def test_get_universe_state(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
    universe_data: dict[str, Any],
):
    """Test getting universe state with entity and relationship counts."""
    mock_get_client.return_value = mock_neo4j_client

    # Mock universe state queries (4 separate queries)
    entity_data = {"id": str(uuid4()), "name": "Test Entity"}
    mock_neo4j_client.execute_read.side_effect = [
        [{"e": entity_data, "labels": ["EntityInstance"]}],  # entities
        [],  # facts (empty)
        [],  # axioms (empty)
        [],  # relationships (empty)
    ]

    result = neo4j_get_universe_state(UUID(universe_data["id"]))
...
# =============================================================================
# TESTS: neo4j_list_universes with additional filters
# =============================================================================


@patch("monitor_data.tools.neo4j_tools.core.get_neo4j_client")
def test_list_universes_with_genre_filter(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
    universe_data: dict[str, Any],
):
    """Test listing universes filtered by genre."""
    mock_get_client.return_value = mock_neo4j_client

    universe_with_genre = {**universe_data, "genre": "fantasy"}
    mock_neo4j_client.execute_read.return_value = [{"u": universe_with_genre}]

    filters = UniverseFilter(genre="fantasy")
    result = neo4j_list_universes(filters)
...
@patch("monitor_data.tools.neo4j_tools.core.get_neo4j_client")
def test_list_universes_with_is_template_filter(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
    universe_data: dict[str, Any],
):
    """Test listing universes filtered by is_template."""
    mock_get_client.return_value = mock_neo4j_client

    template_universe = {**universe_data, "is_template": True}
    mock_neo4j_client.execute_read.return_value = [{"u": template_universe}]

    filters = UniverseFilter(is_template=True)
    result = neo4j_list_universes(filters)
...
@patch("monitor_data.tools.neo4j_tools.core.get_neo4j_client")
def test_list_universes_with_canon_level_filter(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
    universe_data: dict[str, Any],
):
    """Test listing universes filtered by canon_level."""
    mock_get_client.return_value = mock_neo4j_client

    canon_universe = {**universe_data, "canon_level": "canon"}
    mock_neo4j_client.execute_read.return_value = [{"u": canon_universe}]

    filters = UniverseFilter(canon_level=CanonLevel.CANON)
    result = neo4j_list_universes(filters)

    assert len(result) == 1
    assert result[0].canon_level == CanonLevel.CANON


@patch("monitor_data.tools.neo4j_tools.core.get_neo4j_client")
def test_list_universes_with_alt_world_type_filter(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
    universe_data: dict[str, Any],
):
    """Test listing universes filtered by alt_world_type."""
    mock_get_client.return_value = mock_neo4j_client

    alt_world = {**universe_data, "alt_world_type": "fork"}
    mock_neo4j_client.execute_read.return_value = [{"u": alt_world}]

    filters = UniverseFilter(alt_world_type=AltWorldType.FORK)
    result = neo4j_list_universes(filters)

    assert len(result) == 1
    assert result[0].alt_world_type == AltWorldType.FORK

...
from monitor_data.schemas.base import Authority, AxiomAuthority, CanonLevel
...
@patch("monitor_data.tools.neo4j_tools.facts.get_neo4j_client")
@patch("monitor_data.tools.neo4j_tools.facts.neo4j_get_fact")
def test_create_fact_with_nested_properties_serializes_to_json_string(
    mock_get_fact: Mock,
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
    universe_data: dict[str, Any],
):
    """Regression: CanonKeeper.end_scene's scene_end marker fact passes a
    nested dict as ``properties`` (``{kind, scene_id, story_id}``). Neo4j
    property values can only be primitives or arrays of primitives -- a
    dict can NEVER be bound directly to a single node property, so the
    CREATE query must receive a JSON *string*, not a dict, for
    ``properties`` or the write raises CypherTypeError (confirmed live
    2026-07-21: every scene_end write failed silently, caught only by
    the caller's best-effort try/except)."""
    from monitor_data.schemas.facts import FactResponse

    mock_get_client.return_value = mock_neo4j_client
    mock_neo4j_client.execute_read.return_value = [{"id": universe_data["id"]}]
    mock_neo4j_client.execute_write.return_value = [{"f": {}}]

    params = FactCreate(
        universe_id=UUID(universe_data["id"]),
        statement="Scene ended",
        fact_type=FactType.STATE,
        properties={
            "kind": "scene_end",
            "scene_id": str(uuid4()),
    ...
    mock_get_fact.return_value = FactResponse(
        id=uuid4(),
        universe_id=UUID(universe_data["id"]),
        statement="Scene ended",
        fact_type=FactType.STATE,
    ...
    create_call = mock_neo4j_client.execute_write.call_args_list[0]
    bound_properties = create_call[0][1]["properties"]
    assert isinstance(bound_properties, str), (
        "properties must be JSON-serialized to a string before being bound "
        "to the Cypher query -- a dict/Map value is rejected by Neo4j for "
        f"a single node property. Got: {type(bound_properties)}"
    )
    import json

    assert json.loads(bound_properties) == {
        "kind": "scene_end",
        "scene_id": params.properties["scene_id"],
        "story_id": params.properties["story_id"],
    }


@patch("monitor_data.tools.neo4j_tools.facts.get_neo4j_client")
def test_get_fact_deserializes_json_string_properties_back_to_dict(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
    fact_data: dict[str, Any],
):
    """Round-trip: a fact stored with JSON-string properties (the only
    representation Neo4j accepts) must come back out as a dict."""
    import json

    mock_get_client.return_value = mock_neo4j_client
    stored = dict(fact_data)
    stored["properties"] = json.dumps({"kind": "scene_end", "scene_id": "abc"})
    mock_neo4j_client.execute_read.return_value = [{"f": stored, "entity_ids": [], "source_ids": [], "scene_ids": []}]

    result = neo4j_get_fact(UUID(fact_data["id"]))

    assert result is not None
    assert result.properties == {"kind": "scene_end", "scene_id": "abc"}
...
@patch("monitor_data.tools.neo4j_tools.facts.get_neo4j_client")
def test_delete_confirmed_fact_without_force(mock_get_client: Mock, mock_neo4j_client: Mock, fact_data: dict[str, Any]):
...
@patch("monitor_data.tools.neo4j_tools.facts.get_neo4j_client")
def test_delete_confirmed_fact_with_force(mock_get_client: Mock, mock_neo4j_client: Mock, fact_data: dict[str, Any]):
...
@patch("monitor_data.tools.neo4j_tools.facts.get_neo4j_client")
def test_delete_proposed_fact(mock_get_client: Mock, mock_neo4j_client: Mock, fact_data: dict[str, Any]):
...
def test_list_events_by_time_range(
    ...
    start_after = datetime(2024, 1, 1, 0, 0, 0, tzinfo=UTC)
    start_before = datetime(2024, 1, 2, 0, 0, 0, tzinfo=UTC)

...
@patch("monitor_data.tools.neo4j_tools.parties.get_neo4j_client")
...
@patch("monitor_data.tools.neo4j_tools.parties.neo4j_get_party")
@patch("monitor_data.tools.neo4j_tools.parties.get_neo4j_client")
...
def test_set_active_pc_success(
    ...
    from monitor_data.schemas.parties import PartyMemberInfo, PartyResponse
...
@patch("monitor_data.tools.neo4j_tools.parties.get_neo4j_client")
def test_list_parties_by_status(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
):
    """Test listing parties filtered by status."""
    mock_get_client.return_value = mock_neo4j_client
    ...
    filters = PartyFilter(status=PartyStatus.COMBAT)
    result = neo4j_list_parties(filters)

    assert len(result) == 1
    assert result[0].status == PartyStatus.COMBAT


@patch("monitor_data.tools.neo4j_tools.parties.neo4j_get_party")
@patch("monitor_data.tools.neo4j_tools.parties.get_neo4j_client")
def test_add_party_member_with_role_and_position(
    mock_get_client: Mock,
    mock_get_party: Mock,
    mock_neo4j_client: Mock,
):
    """Test adding member with specific role and position."""
    mock_get_client.return_value = mock_neo4j_client
...
@patch("monitor_data.tools.neo4j_tools.parties.get_neo4j_client")
def test_list_parties_by_multiple_filters(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
    story_data: dict[str, Any],
):
    """Test listing parties with multiple filters (story_id and status)."""
    mock_get_client.return_value = mock_neo4j_client
    ...
    filters = PartyFilter(story_id=UUID(story_data["id"]), status=PartyStatus.COMBAT)
    result = neo4j_list_parties(filters)
...
@patch("monitor_data.tools.neo4j_tools.parties.neo4j_get_party")
@patch("monitor_data.tools.neo4j_tools.parties.get_neo4j_client")
def test_remove_active_pc_from_party(
    mock_get_client: Mock,
    mock_get_party: Mock,
    mock_neo4j_client: Mock,
):
    """Test removing the active PC clears the active_pc_id."""
    mock_get_client.return_value = mock_neo4j_client

    party_id = uuid4()
    entity_id = uuid4()

    # Mock party exists with this member as active PC
    from monitor_data.schemas.parties import PartyResponse
...
@patch("monitor_data.tools.neo4j_tools.parties.get_neo4j_client")
def test_list_parties_with_pagination(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
):
    """Test listing parties with pagination parameters."""
    mock_get_client.return_value = mock_neo4j_client
    ...
    filters = PartyFilter(limit=1, offset=5)
    result = neo4j_list_parties(filters)

    assert len(result) == 1
    assert result[0].name == "Paginated Party"

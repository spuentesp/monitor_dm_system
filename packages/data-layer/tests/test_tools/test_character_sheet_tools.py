    ...
    def count_documents(self, query):
        ...
        for sheet in self.sheets.values():
            ...
            if "entity_id" in query and sheet.get("entity_id") != query["entity_id"]:
                match = False
            if "game_system_id" in query and sheet.get("game_system_id") != query.get("game_system_id"):
                match = False
            if "system_source_type" in query and sheet.get("system_source_type") != query.get("system_source_type"):
                match = False
            if "system_source_id" in query and sheet.get("system_source_id") != query.get("system_source_id"):
                match = False
...
def test_create_character_sheet_persists_source_persona_id(monkeypatch):
    """source_persona_id (character-template bridge, plan Q2) must round-trip
    through creation and retrieval -- it's the back-reference letting a
    returning player re-instantiate the same persona in a different world."""
    entity_id = uuid4()
    system_id = uuid4()
    ...
    result = mongodb_create_character_sheet(
        CharacterSheetCreate(
            ...
            system_source_id=str(system_id),
            system_name="Test System",
            source_persona_id="persona-42",
        )
    )

    assert result.source_persona_id == "persona-42"

    retrieved = mongodb_get_character_sheet(result.sheet_id)
    assert retrieved is not None
    assert retrieved.source_persona_id == "persona-42"


def test_create_character_sheet_without_source_persona_id_is_none(monkeypatch):
    entity_id = uuid4()
    system_id = uuid4()
    ...
    assert result.source_persona_id is None
...
def test_list_character_sheets_by_system_source_type(monkeypatch):
    ...
    result = mongodb_list_character_sheets(CharacterSheetFilter(system_source_type="generic_library"))
...
def test_list_character_sheets_sort_by_updated_at(monkeypatch):
    ...
    sheet_2 = {
        ...
        "history_log": [],
        "created_at": now,
        "updated_at": datetime(2024, 1, 3, 12, 0, 0, tzinfo=UTC),
    }
    sheet_3 = {

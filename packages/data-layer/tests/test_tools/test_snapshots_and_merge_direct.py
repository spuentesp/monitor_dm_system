...
class TestSnapshotsDirect:
    def test_create_captures_counts(self, mongo, world_state):
        universe_id = uuid4()
        resp = snap.mongodb_create_world_snapshot(
            WorldSnapshotCreate(universe_id=universe_id, name="before-raid")
        )
    ...
    def test_get_round_trip_and_missing(self, mongo, world_state):
        resp = snap.mongodb_create_world_snapshot(
            WorldSnapshotCreate(universe_id=uuid4(), name="s1")
        )
    ...
    def test_delete(self, mongo, world_state):
        resp = snap.mongodb_create_world_snapshot(
            WorldSnapshotCreate(universe_id=uuid4(), name="gone")
        )
    ...
    def test_restore_creates_proposals(self, mongo, world_state, monkeypatch):
        ...
        resp = snap.mongodb_create_world_snapshot(
            WorldSnapshotCreate(universe_id=uuid4(), name="restore-me")
        )
    ...
    def test_compare_diffs_added_removed_modified(self, mongo, monkeypatch):
        ...
        a = snap.mongodb_create_world_snapshot(
            WorldSnapshotCreate(universe_id=universe_id, name="a")
        )
        b = snap.mongodb_create_world_snapshot(
            WorldSnapshotCreate(universe_id=universe_id, name="b")
        )
...
class TestMergeCandidatesDirect:
    ...
    def test_mark_reviewed(self, mongo):
        ...
        doc = mongo.get_collection("entity_merge_candidates").find_one(

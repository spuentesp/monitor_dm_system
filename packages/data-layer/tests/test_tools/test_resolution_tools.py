...
@patch("monitor_data.tools.mongodb_tools.resolutions.get_neo4j_client")
@patch("monitor_data.tools.mongodb_tools.resolutions.get_mongodb_client")
def test_create_resolution_success(
    ...
    mock_mongodb.get_collection.side_effect = lambda name: mock_resolutions if name == "resolutions" else mock_scenes
    ...
    with patch("monitor_data.tools.mongodb_tools.resolutions.uuid4", return_value=resolution_id):
...
def test_create_resolution_turn_not_found(
    ...
    with pytest.raises(ValueError, match=f"Turn {turn_id} not found in scene {scene_id}"):
        mongodb_create_resolution(params)
...
def test_create_resolution_contested(
    ...
    mechanics = Mechanics(
        ...
        contested=ContestedRoll(
            ...
            opponent_roll=RollResult(raw_rolls=[12], kept_rolls=[12], total=14, natural=12),
            opponent_modifiers=[Modifier(source="Athletics", value=2, reason="Athletics skill")],
            margin_of_victory=4,
        ),
    )
...
def test_get_resolution_success(mock_get_mongodb: Mock):
    ...
    assert result.success_level == SuccessLevel.SUCCESS
    mock_resolutions.find_one.assert_called_once_with({"resolution_id": str(resolution_id)})
...
def test_list_resolutions_by_scene(mock_get_mongodb: Mock):
    ...
    mock_resolutions.count_documents.assert_called_once_with({"scene_id": str(scene_id)})
...
def test_list_resolutions_by_actor(mock_get_mongodb: Mock):
    ...
    mock_resolutions.count_documents.assert_called_once_with({"actor_id": str(actor_id)})
...
def test_list_resolutions_by_success_level(mock_get_mongodb: Mock):
    ...
    mock_resolutions.count_documents.assert_called_once_with({"success_level": "critical_success"})
...
def test_delete_resolution_success(mock_get_mongodb: Mock):
    ...
    mock_resolutions.delete_one.assert_called_once_with({"resolution_id": str(resolution_id)})
...
@patch("monitor_data.tools.mongodb_tools.resolutions.get_neo4j_client")
@patch("monitor_data.tools.mongodb_tools.resolutions.get_mongodb_client")
def test_create_resolution_with_margin(
    mock_get_mongodb: Mock,
    mock_get_neo4j: Mock,
):
    """Test creating a resolution with margin (success/failure by how much)."""
    turn_id = uuid4()
    scene_id = uuid4()
    story_id = uuid4()
    actor_id = uuid4()
    ...
    mechanics = Mechanics(
        roll=RollResult(raw_rolls=[18], kept_rolls=[18], total=22, natural=18),
        target=12,
        formula="1d20 + 4 vs DC 12",
    )
    ...
    assert result.success_level == SuccessLevel.SUCCESS
    assert len(result.effects) == 1
    assert result.effects[0].effect_type == EffectType.DAMAGE
    assert result.effects[0].magnitude == 8
    mock_resolutions.insert_one.assert_called_once()


@patch("monitor_data.tools.mongodb_tools.resolutions.get_mongodb_client")
def test_update_resolution_description_only(mock_get_mongodb: Mock):
    ...
    mock_resolutions.update_one.return_value = MagicMock(matched_count=1)
    mock_resolutions.find_one.return_value = updated_doc

    params = ResolutionUpdate(description="Updated description")
    result = mongodb_update_resolution(resolution_id, params)

    assert result is not None
    assert result.description == "Updated description"
    mock_resolutions.update_one.assert_called_once()
    update_call = mock_resolutions.update_one.call_args[0][1]
    assert "$set" in update_call
    assert "description" in update_call["$set"]
    assert "updated_at" in update_call["$set"]


@patch("monitor_data.tools.mongodb_tools.resolutions.get_mongodb_client")
def test_list_resolutions_pagination(mock_get_mongodb: Mock):
    """Test pagination in list_resolutions."""
    scene_id = uuid4()
    ...
    mock_resolutions.count_documents.return_value = 100

    # Create mock cursor with pagination - chain the mocks correctly
    mock_cursor = MagicMock()
    mock_resolutions.find.return_value = mock_cursor
    mock_cursor.sort.return_value = mock_cursor
    mock_cursor.skip.return_value = mock_cursor
    ...
    params = ResolutionFilter(scene_id=scene_id, limit=10, offset=20)
    result = mongodb_list_resolutions(params)
...
@patch("monitor_data.tools.mongodb_tools.resolutions.get_mongodb_client")
def test_list_resolutions_multiple_filters(mock_get_mongodb: Mock):
    """Test listing resolutions with multiple filters combined."""
    scene_id = uuid4()
    actor_id = uuid4()
    ...
    # Create mock cursor that returns a valid document
    mock_cursor = MagicMock()
    mock_resolutions.find.return_value = mock_cursor
    mock_cursor.sort.return_value = mock_cursor
    mock_cursor.skip.return_value = mock_cursor
    ...
    assert len(result.resolutions) == 1
    mock_resolutions.find.assert_called_once()


@patch("monitor_data.tools.mongodb_tools.resolutions.get_neo4j_client")
@patch("monitor_data.tools.mongodb_tools.resolutions.get_mongodb_client")
def test_create_resolution_narrative(
    mock_get_mongodb: Mock,
    mock_get_neo4j: Mock,
):
    """Test creating a narrative resolution (no dice)."""
    turn_id = uuid4()
    scene_id = uuid4()
    story_id = uuid4()
    actor_id = uuid4()
    ...
    params = ResolutionCreate(
        ...
        story_id=story_id,
        actor_id=actor_id,
        action="Character sneaks past guards",
        action_type=ActionType.EXPLORATION,
        resolution_type=ResolutionType.NARRATIVE,
        mechanics=mechanics,
        success_level=SuccessLevel.SUCCESS,
        description="Quiet movement through shadows allows passage",
    )

    result = mongodb_create_resolution(params)

    assert result.resolution_type == ResolutionType.NARRATIVE
    assert result.success_level == SuccessLevel.SUCCESS
    assert result.description == "Quiet movement through shadows allows passage"
    mock_resolutions.insert_one.assert_called_once()

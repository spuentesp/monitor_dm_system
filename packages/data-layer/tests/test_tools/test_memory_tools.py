...
def _stub_embed_text():
    """Provide a real-shaped embedding vector for the embed/search paths.

    ``qdrant_tools`` now routes embeddings through the RetrievalService's
    ``_embed_query_via_service`` helper (the single embed owner). These tests
    mock Qdrant itself, so they only need a deterministic vector to exercise
    the upsert/search plumbing — patch the service-routed embed helper.
    """
    with patch(
        "monitor_data.tools.qdrant_tools._embed_query_via_service",
        new=lambda text: [0.1] * 768,
    ):
        yield
...
@patch("monitor_data.tools.mongodb_tools.memories.get_mongodb_client")
def test_list_memories_with_min_importance_filter(mock_mongo_client: Mock, entity_data: dict[str, Any]):
...
@patch("monitor_data.tools.mongodb_tools.memories.get_mongodb_client")
def test_list_memories_with_max_importance_filter(mock_mongo_client: Mock, entity_data: dict[str, Any]):

...
def test_detect_contradictions_with_negation():
    pack = _pack(
        axioms=[
            ExtractedAxiom(
                statement="Elves are not immortal", source_ref="new-book", confidence=0.9
            )
        ]
    )
    canon = [
...
def test_detect_contradictions_ignores_identical():
    ...
    pack = _pack(
        axioms=[
            ExtractedAxiom(statement="Elves are immortal", source_ref="new-book", confidence=0.9)
        ]
    )
    canon = [
        {
...
def test_shares_subject_fallback_tokens():
    ...
    assert (
        _shares_subject("the fortress stands in a kingdom", "the fortress fell in a kingdom")
        is True
    )
...
def test_full_pipeline_multiple_contradictions():
    pack = _pack(
        ...
        lore_facts=[
            ExtractedLoreFact(
                statement="Dragons are not extinct", source_ref="new", confidence=0.9
            ),
        ],
    )
    canon_axioms = [
...
def test_detect_contradictions_unrelated_statements():
    ...
    pack = _pack(
        axioms=[
            ExtractedAxiom(statement="The sky is blue today", source_ref="book1", confidence=0.9)
        ]
    )
    canon = [
        {"statement": "The ocean is deep and cold", "canon_level": "canon", "source_ref": "core"}
    ]
    result = detect_contradictions(pack, canon, [])
    assert result.total_contradictions == 0
...
def test_detect_contradictions_opposite_pair_across_subjects():
    ...
    pack = _pack(
        axioms=[ExtractedAxiom(statement="Dragons are alive", source_ref="book1", confidence=0.9)]
    )

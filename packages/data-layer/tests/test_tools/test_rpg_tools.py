...
class TestRpgSheetEquip:
    ...
    async def test_equip_returns_row_dict(self):
        ...
        client = _make_client(fetchrow=row)
        result = await rpg_sheet_equip(
            client, _SHEET_ID, _ITEM_ID, quantity=1, is_equipped=True, slot="holster"
        )
...
class TestRpgSchemaInspect:
    ...
    def _ensure_python_spec(self):
        ...
        match expectations.
        """
        from monitor_data.schemas.rpg_ontology import SystemRegistry, DEATH_IN_SPACE

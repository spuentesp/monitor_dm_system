    ...
    def count_documents(self, query):
        ...
        for pack in self.packs.values():
            ...
            if "tags" in query and query["tags"] not in pack.get("tags", []):
                match = False
            if "parent_pack_ids" in query and query["parent_pack_ids"] not in pack.get(
                "parent_pack_ids", []
            ):
                match = False
            if match:
...
def test_list_knowledge_packs_with_multiple_filters(monkeypatch):
    ...
    result = mongodb_list_knowledge_packs(
        KnowledgePackFilter(system_name="Dungeons", tag="fantasy")
    )

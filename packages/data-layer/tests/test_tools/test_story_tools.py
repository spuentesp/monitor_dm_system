...
@patch("monitor_data.tools.neo4j_tools.stories.get_neo4j_client")
def test_update_story_theme(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
    story_data: dict[str, Any],
):
    """Test updating story theme."""
    mock_get_client.return_value = mock_neo4j_client

    updated_data = story_data.copy()
    updated_data["theme"] = "New Theme: Redemption"
    ...
    params = StoryUpdate(theme="New Theme: Redemption")
    result = neo4j_update_story(UUID(story_data["id"]), params)

    assert result.theme == "New Theme: Redemption"


@patch("monitor_data.tools.neo4j_tools.stories.get_neo4j_client")
def test_update_story_premise(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
    story_data: dict[str, Any],
):
    """Test updating story premise."""
    mock_get_client.return_value = mock_neo4j_client

    updated_data = story_data.copy()
    updated_data["premise"] = "Updated premise: The artifact is now cursed"
    ...
    params = StoryUpdate(premise="Updated premise: The artifact is now cursed")
    result = neo4j_update_story(UUID(story_data["id"]), params)

    assert result.premise == "Updated premise: The artifact is now cursed"


@patch("monitor_data.tools.neo4j_tools.stories.get_neo4j_client")
def test_update_story_multiple_fields(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
    story_data: dict[str, Any],
):
...
@patch("monitor_data.tools.neo4j_tools.stories.get_neo4j_client")
def test_list_stories_filtered_by_type(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
    story_data: dict[str, Any],
):
    """Test listing stories filtered by story type."""
    mock_get_client.return_value = mock_neo4j_client
    ...
    params = StoryFilter(story_type=StoryType.CAMPAIGN)
    result = neo4j_list_stories(params)
...
@patch("monitor_data.tools.neo4j_tools.stories.get_neo4j_client")
def test_update_story_start_time_ref(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
    story_data: dict[str, Any],
):
    """Test updating story start time reference."""
    mock_get_client.return_value = mock_neo4j_client

    new_time_ref = datetime(2024, 1, 1, 0, 0, 0)
    updated_data = story_data.copy()
    updated_data["start_time_ref"] = new_time_ref
    ...
    params = StoryUpdate(start_time_ref=new_time_ref)
    result = neo4j_update_story(UUID(story_data["id"]), params)

    assert result.start_time_ref == new_time_ref


# =============================================================================
# TESTS: neo4j_create_story with additional parameters
# =============================================================================


@patch("monitor_data.tools.neo4j_tools.stories.get_neo4j_client")
def test_create_story_with_start_time_ref(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
    universe_data: dict[str, Any],
    story_data: dict[str, Any],
):
    """Test creating a story with start time reference."""
    mock_get_client.return_value = mock_neo4j_client

    # Mock universe exists check
    mock_neo4j_client.execute_read.return_value = [{"id": universe_data["id"]}]

    # Create a modified story_data with start_time_ref
    story_with_time = story_data.copy()
    story_with_time["start_time_ref"] = "2024-01-01T00:00:00"

    # Mock story creation
    mock_neo4j_client.execute_write.return_value = [{"s": story_with_time}]
    ...
    assert result.start_time_ref == start_time
    # Verify that start_time_ref was passed in the Cypher query
    call_args = mock_neo4j_client.execute_write.call_args[0][0]
    assert "$start_time_ref" in call_args


@patch("monitor_data.tools.neo4j_tools.stories.get_neo4j_client")
def test_list_stories_offset_pagination(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
    story_data: dict[str, Any],
):
    """Test listing stories with offset for pagination."""
    mock_get_client.return_value = mock_neo4j_client

    # Mock count and list queries
    mock_neo4j_client.execute_read.side_effect = [
        [{"total": 5}],  # count (total of 5 stories)
        [{"s": story_data, "scene_count": 0, "pc_ids": []}],  # list (1 result)
    ]

    params = StoryFilter(offset=2, limit=1)
    result = neo4j_list_stories(params)
...
# =============================================================================
# TESTS: neo4j_list_stories with sorting
# =============================================================================


@patch("monitor_data.tools.neo4j_tools.stories.get_neo4j_client")
def test_list_stories_sort_by_title(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
    story_data: dict[str, Any],
):
    """Test listing stories sorted by title."""
    mock_get_client.return_value = mock_neo4j_client
    ...
    mock_neo4j_client.execute_read.side_effect = [
        [{"total": 2}],  # count
        [
            {"s": story_data, "scene_count": 0, "pc_ids": []},
            {"s": story_data, "scene_count": 0, "pc_ids": []},
        ],  # list (2 results)
    ]

    params = StoryFilter(sort_by="title")
    result = neo4j_list_stories(params)

    assert result.total == 2
    assert len(result.stories) == 2
    # Verify sort parameter was passed
    call_args = mock_neo4j_client.execute_read.call_args[0][0]
    assert "ORDER BY s.title" in call_args


@patch("monitor_data.tools.neo4j_tools.stories.get_neo4j_client")
def test_list_stories_sort_order_asc(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
    story_data: dict[str, Any],
):
    """Test listing stories with ascending sort order."""
    mock_get_client.return_value = mock_neo4j_client
    ...
    params = StoryFilter(sort_order="asc")
    result = neo4j_list_stories(params)
    ...
    call_args = mock_neo4j_client.execute_read.call_args[0][0]
    assert "ORDER BY" in call_args and "ASC" in call_args.upper()


# =============================================================================
# TESTS: neo4j_list_stories with multiple filters
# =============================================================================


@patch("monitor_data.tools.neo4j_tools.stories.get_neo4j_client")
def test_list_stories_multiple_filters(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
    universe_data: dict[str, Any],
    story_data: dict[str, Any],
):
    """Test listing stories with multiple filters combined."""
    mock_get_client.return_value = mock_neo4j_client
    ...
    call_args = mock_neo4j_client.execute_read.call_args[0][0]
    assert "$universe_id" in call_args
    assert "$story_type" in call_args
    assert "$status" in call_args
    assert "ORDER BY s.title ASC" in call_args

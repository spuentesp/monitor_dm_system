...
@patch("monitor_data.tools.qdrant_tools.embed_text", new_callable=AsyncMock)
@patch("monitor_data.tools.qdrant_tools.get_qdrant_client")
def test_search_accepts_query_text_and_query_points(
    mock_get_client: Mock, mock_embed_text: AsyncMock
):
    """Text-based search should embed the query and work with query_points-only clients."""
    result_id = uuid4()
    mock_embed_text.return_value = [0.25] * 1536

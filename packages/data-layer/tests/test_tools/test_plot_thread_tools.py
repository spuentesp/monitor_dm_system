...
@patch("monitor_data.tools.neo4j_tools.stories.get_neo4j_client")
...
@patch("monitor_data.tools.neo4j_tools.stories.get_neo4j_client")
def test_list_plot_threads_filter_by_priority(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
    plot_thread_data: dict[str, Any],
):
    """Test listing plot threads filtered by priority."""
    mock_get_client.return_value = mock_neo4j_client

    # Update thread data to have MAIN priority
    plot_thread_data["priority"] = ThreadPriority.MAIN.value
    ...
    params = PlotThreadFilter(priority=ThreadPriority.MAIN)
    result = neo4j_list_plot_threads(params)

    assert result.total == 1
    assert len(result.threads) == 1
    assert result.threads[0].priority == ThreadPriority.MAIN


@patch("monitor_data.tools.neo4j_tools.stories.get_neo4j_client")
def test_update_plot_thread_priority(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
    plot_thread_data: dict[str, Any],
):
    """Test updating plot thread priority."""
    mock_get_client.return_value = mock_neo4j_client

    # Prepare existing and updated thread data
    existing_thread = plot_thread_data.copy()
    existing_thread["priority"] = ThreadPriority.MINOR.value

    updated_thread = plot_thread_data.copy()
    updated_thread["priority"] = ThreadPriority.MAIN.value
    ...
    params = PlotThreadUpdate(priority=ThreadPriority.MAIN)
    result = neo4j_update_plot_thread(UUID(plot_thread_data["id"]), params)
...
@patch("monitor_data.tools.neo4j_tools.stories.get_neo4j_client")
def test_update_plot_thread_urgency(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
    plot_thread_data: dict[str, Any],
):
    """Test updating plot thread urgency."""
    mock_get_client.return_value = mock_neo4j_client

    # Prepare existing and updated thread data
    existing_thread = plot_thread_data.copy()
    existing_thread["urgency"] = ThreadUrgency.LOW.value

    updated_thread = plot_thread_data.copy()
    updated_thread["urgency"] = ThreadUrgency.CRITICAL.value
    ...
    params = PlotThreadUpdate(urgency=ThreadUrgency.CRITICAL)
    result = neo4j_update_plot_thread(UUID(plot_thread_data["id"]), params)

    assert result.urgency == ThreadUrgency.CRITICAL
    # Verify update was called
    mock_neo4j_client.execute_write.assert_called_once()


@patch("monitor_data.tools.neo4j_tools.stories.get_neo4j_client")
def test_update_plot_thread_deadline(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
    plot_thread_data: dict[str, Any],
):
    """Test updating plot thread deadline."""
    mock_get_client.return_value = mock_neo4j_client
    ...
    updated_thread = plot_thread_data.copy()
    # Deadline is stored as a dictionary with world_time and description
    updated_thread["deadline"] = {
        "world_time": datetime(2025, 6, 15, 12, 0, 0),
        "description": "Must resolve before the final battle",
    }
    ...
    params = PlotThreadUpdate(deadline=deadline)
    result = neo4j_update_plot_thread(UUID(plot_thread_data["id"]), params)

    assert result.deadline is not None
    assert result.deadline.world_time == datetime(2025, 6, 15, 12, 0, 0)
    assert result.deadline.description == "Must resolve before the final battle"
    # Verify update was called
    mock_neo4j_client.execute_write.assert_called_once()


@patch("monitor_data.tools.neo4j_tools.stories.get_neo4j_client")
def test_update_plot_thread_payoff_status(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
    plot_thread_data: dict[str, Any],
):
    """Test updating plot thread payoff status."""
    mock_get_client.return_value = mock_neo4j_client

    # Prepare existing and updated thread data
    existing_thread = plot_thread_data.copy()
    existing_thread["payoff_status"] = PayoffStatus.SETUP_ONLY.value

    updated_thread = plot_thread_data.copy()
    updated_thread["payoff_status"] = PayoffStatus.PAYOFF_DELIVERED.value
    ...
    params = PlotThreadUpdate(payoff_status=PayoffStatus.PAYOFF_DELIVERED)
    result = neo4j_update_plot_thread(UUID(plot_thread_data["id"]), params)

    assert result.payoff_status == PayoffStatus.PAYOFF_DELIVERED
    # Verify update was called
    mock_neo4j_client.execute_write.assert_called_once()


# =============================================================================
# TESTS: neo4j_list_plot_threads — datetime coercion regression (T-094)
# =============================================================================


@patch("monitor_data.tools.neo4j_tools.stories.get_neo4j_client")
def test_list_plot_threads_coerces_neo4j_datetime(
    mock_get_client: Mock,
    mock_neo4j_client: Mock,
    plot_thread_data: dict[str, Any],
):
    """Regression: neo4j_list_plot_threads previously passed raw neo4j DateTime
    objects to PlotThreadResponse, which raised a pydantic datetime ValidationError
    and made the /threads endpoint silently return 0. They must be coerced to
    native datetimes via _to_native_datetime (T-094)."""
    from neo4j.time import DateTime as Neo4jDateTime

    from monitor_data.schemas.story_outlines import PlotThreadFilter
    from monitor_data.tools.neo4j_tools.stories import neo4j_list_plot_threads

    mock_get_client.return_value = mock_neo4j_client

    node = dict(plot_thread_data)
    # The values that caused the bug: neo4j temporal types, not Python datetimes.
    node["created_at"] = Neo4jDateTime(2026, 6, 14, 12, 0, 0, 0)
    node["updated_at"] = Neo4jDateTime(2026, 6, 14, 12, 5, 0, 0)

    mock_neo4j_client.execute_read.side_effect = [
        [{"total": 1}],  # count query runs first
        [
            {
                "t": node,
                "scene_ids": [],
    ...
    result = neo4j_list_plot_threads(PlotThreadFilter(story_id=UUID(plot_thread_data["story_id"]), limit=50))

    assert len(result.threads) == 1
    t = result.threads[0]
    assert t.title == "The Missing Artifact"
    # Coerced to native python datetimes (the fix).

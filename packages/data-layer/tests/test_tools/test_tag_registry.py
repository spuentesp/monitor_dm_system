...
def test_update_tag_definition_success(mock_mongo: Mock) -> None:
    ...
    result = mongodb_update_tag_definition(
        "grim",
        TagDefinitionUpdate(synonyms=["dark", "bleak", "gritty"], description="Updated description"),
    )
...
def test_suggest_tags_case_insensitive(mock_mongo: Mock) -> None:
    ...
    result = mongodb_suggest_tags(TagSuggestionRequest(partial="GRIM", category=TagCategory.TONE, limit=10))
...
def test_suggest_tags_with_category_filter(mock_mongo: Mock) -> None:
    ...
    result = mongodb_suggest_tags(TagSuggestionRequest(partial="dark", category=TagCategory.THEME, limit=10))

...
class TestComputeDeduplicationNoDuplicates:
    def test_new_pack_with_no_existing_packs(self):
        new = _make_pack(axioms=[ExtractedAxiom(statement="Magic is real.", domain="magic", confidence=0.9)])
    ...
    def test_new_item_not_in_existing(self):
        new = _make_pack(axioms=[ExtractedAxiom(statement="Magic is real.", domain="magic", confidence=0.9)])
        existing = _make_pack(
            axioms=[ExtractedAxiom(statement="Dragons breathe fire.", domain="creatures", confidence=0.85)]
        )
    ...
    def test_extended_narrative_outputs_are_checked(self):
        new = _make_pack(
            ...
            generation_templates=[ExtractedGenerationTemplate(name="Harpy Envoy", archetype_name="Harpy")],
        )
        result = compute_deduplication(new, [], multiverse_id=str(uuid4()))
...
class TestComputeDeduplicationExact:
    def test_exact_duplicate_same_content(self):
        new = _make_pack(
            axioms=[ExtractedAxiom(statement="Magic is real.", domain="magic", confidence=0.9, source_ref="p1")]
        )
        existing = _make_pack(
            axioms=[ExtractedAxiom(statement="Magic is real.", domain="magic", confidence=0.85, source_ref="p2")]
        )
        result = compute_deduplication(new, [existing], multiverse_id=str(uuid4()))
    ...
    def test_exact_duplicate_existing_higher_confidence(self):
        new = _make_pack(axioms=[ExtractedAxiom(statement="Magic is real.", domain="magic", confidence=0.7)])
        existing = _make_pack(axioms=[ExtractedAxiom(statement="Magic is real.", domain="magic", confidence=0.95)])
        result = compute_deduplication(new, [existing], multiverse_id=str(uuid4()))
        assert result.has_duplicates is True
        group = result.duplicate_groups[0]
        assert group.category == DuplicateCategory.EXACT
    ...
    def test_conflicting_different_content_same_identity(self):
        new = _make_pack(axioms=[ExtractedAxiom(statement="Magic is real.", domain="magic", confidence=0.9)])
        existing = _make_pack(axioms=[ExtractedAxiom(statement="Magic is NOT real.", domain="magic", confidence=0.85)])
        result = compute_deduplication(new, [existing], multiverse_id=str(uuid4()))
        assert result.has_duplicates is True
        group = result.duplicate_groups[0]
...
class TestComputeDeduplicationSemantic:
    def test_semantic_similar_but_not_identical(self):
        new = _make_pack(
            axioms=[ExtractedAxiom(statement="Magic is woven into the world.", domain="magic", confidence=0.9)]
        )
        existing = _make_pack(
            axioms=[ExtractedAxiom(statement="Magic is woven into reality.", domain="magic", confidence=0.85)]
        )
        result = compute_deduplication(new, [existing], multiverse_id=str(uuid4()))
        assert result.has_duplicates is True
        group = result.duplicate_groups[0]
...
class TestComputeDeduplicationEntityArchetypes:
    def test_same_entity_name_different_type(self):
        ...
        new = _make_pack(
            entity_archetypes=[ExtractedEntityArchetype(name="Dragon", entity_type="character", confidence=0.9)]
        )
        existing = _make_pack(
            entity_archetypes=[ExtractedEntityArchetype(name="Dragon", entity_type="creature", confidence=0.85)]
        )
        result = compute_deduplication(new, [existing], multiverse_id=str(uuid4()))
...
class TestDeduplicationProperties:
    def test_all_auto_resolvable_true(self):
        new = _make_pack(axioms=[ExtractedAxiom(statement="Magic is real.", domain="magic", confidence=0.9)])
        existing = _make_pack(axioms=[ExtractedAxiom(statement="Magic is real.", domain="magic", confidence=0.5)])
        result = compute_deduplication(new, [existing], multiverse_id=str(uuid4()))
        assert result.has_duplicates is True

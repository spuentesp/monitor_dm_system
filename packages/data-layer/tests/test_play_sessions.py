...
def test_list_paginates_and_sorts():
    ...
    res = mongodb_list_play_sessions(PlaySessionFilter(story_id=story_id, limit=2, offset=0, sort_order="asc"))
    assert res.total == 5
    assert res.limit == 2
    assert [s.session_number for s in res.sessions] == [1, 2]

    res_desc = mongodb_list_play_sessions(PlaySessionFilter(story_id=story_id, limit=2, offset=0, sort_order="desc"))
...
def test_update_with_no_fields_returns_current():
    ...
    created = mongodb_create_play_session(_make_create())
    out = mongodb_update_play_session(created.session_id, PlaySessionUpdate(status=None, gm_notes=None))

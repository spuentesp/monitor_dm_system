...
def _make_pack(
    ...
    return KnowledgePackResponse(
        ...
        entity_archetypes=[
            ExtractedEntityArchetype(name=f"Entity {i}", entity_type="character") for i in range(entities)
        ],
        ...
        topologies=[ExtractedTopology(from_location=f"L{i}", to_location=f"L{i + 1}") for i in range(topologies)],
...
class TestRecommendations:
    ...
    def test_no_recommendations_when_complete(self):
        ...
        # that have 0
        missing_req = [c for c in result.missing_categories if result.thresholds_applied.get(c, 0) > 0]

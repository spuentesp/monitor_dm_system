...
def test_redis_client_round_trips_json(monkeypatch):
    ...
    client = RedisClient(url="redis://example", enabled=True, key_prefix="monitor-test", default_ttl=10)
...
def test_redis_client_pubsub_round_trips_json(monkeypatch):
    ...
    assert client.publish_json("chat_events:session-1", {"event_type": "ws_fanout", "payload": {"type": "done"}}) == 1

...
from monitor_agents.loops.conversation_loop import ConversationMode
...
def start(
    ...
    play_mode: str = typer.Option(
        ...
        "--mode",
        "-m",
        help="Play mode (narrative/dice_standard/dice_game_system)",
    ),
    universe_id: str | None = typer.Option(
        None, "--universe", "-u", help="Universe UUID to play in"
    ),
    ...
    scene_id: str | None = typer.Option(
        None, "--scene", "-s", help="Existing scene UUID to resume"
    ),
...
async def _run_play(
    *,
    ...
    current_scene_id = resolved_scene or story_state.get("current_scene_id")
    ...
    while True:
        ...
        console.print(
            "[dim]Type your actions or dialogue. Use [bold]/quit[/bold] to end the session.[/dim]"
        )
        console.print(
            "[dim]Use [bold]/ooc[/bold] before a message for out-of-character questions.[/dim]\n"
        )
        ...
        while turn_number < max_turns:
            ...
            if user_input.lower() == "/status":
                _print_status(
                    current_scene_id, resolved_story, tone, play_mode, turn_number, max_turns
                )
                continue
            ...
            if user_input.lower().startswith("/flashback "):
                ...
                story_state = await story_loop.create_flashback(
                    purpose=flashback_desc, time_description="A past event"
                )

                current_scene_id = story_state.get("current_scene_id")
            ...
            if user_input.lower() == "/party":
                ...
                new_actor_id = await _select_character_interactive(
                    resolved_universe, resolved_story
                )
            ...
            if user_input.lower() == "/godmode":
                # Toggle between current play_mode and 'narrative'
                if play_mode == "narrative":
                    play_mode = "dice_game_system"  # Fallback to default
                    console.print(
                        "[yellow]God Mode disabled. Rules and dice are back in play.[/yellow]"
                    )
                else:
                    play_mode = "narrative"
                    console.print(
                        "[bold green]Narrative God Mode enabled. Your words are law.[/bold green]"
                    )
                continue
            ...
            if user_input.lower() == "/inventory":
                ...
                await _show_inventory(
                    resolved_story
                )  # In a real app, we'd use party_id linked to story
                continue
            ...
            try:
                ...
                if narrative:
                    if is_ooc:
                        console.print(Panel(narrative, title="OOC", border_style="yellow"))
                    else:
                        console.print(Panel(Markdown(narrative), title="GM", border_style="blue"))
        ...
        if scene_completed_naturally:
            ...
            story_state = await story_loop.complete_current_scene(
                scene_id=current_scene_id, outcome=outcome
            )
...
async def _select_universe_interactive() -> UUID | None:
    ...
    try:
        ...
        universes_data = (
            json.loads(raw_universes) if isinstance(raw_universes, str) else raw_universes
        )
    ...
    for i, u in enumerate(universes):
        console.print(
            f"  [cyan]{i + 1}[/cyan]) {u.get('name')} [dim]({u.get('genre', 'unknown')})[/dim]"
        )
...
async def _select_story_interactive(universe_id: UUID) -> UUID | None:
    ...
    for i, s in enumerate(stories):
        console.print(
            f"  [cyan]{i + 1}[/cyan]) {s.get('title')} [dim]({s.get('status', 'active')})[/dim]"
        )
...
async def _create_story_interactive(universe_id: UUID) -> UUID | None:
    ...
    premise = typer.prompt("Story Premise", default="A hero embarks on a journey.")
...
async def _select_character_interactive(universe_id: UUID, story_id: UUID) -> UUID | None:
    ...
    for i, c in enumerate(characters):
        console.print(
            f"  [cyan]{i + 1}[/cyan]) {c.get('name')} [dim]({c.get('sub_type', 'character')})[/dim]"
        )
...
async def _run_npc_studio(universe_id: UUID, scene_id: UUID, story_id: UUID) -> None:
    ...
    try:
        raw_entities = await agent.call_tool(
            "neo4j_get_entities_by_scene", {"scene_id": str(scene_id)}
        )
    ...
    while True:
        ...
        if user_input.lower() == "/mode":
            # Toggle mode
            new_mode = (
                ConversationMode.ACTOR
                if mode == ConversationMode.DIRECT
                else ConversationMode.DIRECT
            )
...
def _print_status(
    scene_id: UUID, story_id: UUID, tone: str, play_mode: str, turn: int, max_turns: int
) -> None:
...
async def _create_new_character(
    ...
    try:
        ...
        all_systems_raw = await agent.call_tool(
            "mongodb_list_game_systems", {"include_builtin": True, "limit": 50}
        )
        all_systems = (
            json.loads(all_systems_raw) if isinstance(all_systems_raw, str) else all_systems_raw
        )

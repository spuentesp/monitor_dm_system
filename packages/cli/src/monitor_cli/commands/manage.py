...
def create_entity(
    ...
    entity_type: str = typer.Option(
        ..., "--type", "-t", help="Entity type (NPC, LOCATION, ITEM, etc.)"
    ),
...
def create_template(
    ...
    asyncio.run(
        _run_create_template(universe_id, name, entity_type, description, detail_level, parent)
    )
...
def create_table(
    universe_id: str = typer.Option(
        None, "--universe", "-u", help="Universe UUID (optional for global tables)"
    ),
    ...
    dice_formula: str = typer.Option(
        "1d20", "--dice", "-d", help="Dice formula (e.g., 1d20, 2d6, 1d100)"
    ),
...
async def _run_roll_on_table(table_id: str) -> None:
    ...
    try:
        ...
        for sub in result.get("sub_results", []):
            console.print(
                f"  └─ [bold]Subtable:[/bold] {sub.get('table_name')} -> {sub.get('value')}"
            )
...
async def _run_list_snapshots(universe_id: str) -> None:
    ...
    try:
        ...
        for s in snapshots:
            counts = f"{s.get('entity_count')}/{s.get('fact_count')}/{s.get('relationship_count')}/{s.get('axiom_count')}"
            table.add_row(
...
async def _run_create_entity(
    universe_id: str, entity_type: str, name: str, description: str
) -> None:
...
async def _run_update_entity(entity_id: str, name: str | None, description: str | None) -> None:
    ...
    try:
        ...
        if not name and not description:
            console.print(
                "[yellow]Warning:[/yellow] No changes specified. Use --name or --description."
            )
            return
...
async def _run_show_template(template_id: str) -> None:
    ...
    try:
        ...
        naming_str = (
            naming.get("pattern", "LLM-generated") if isinstance(naming, dict) else "LLM-generated"
        )

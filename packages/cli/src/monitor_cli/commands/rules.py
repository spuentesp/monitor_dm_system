...
def import_system(
    ...
    try:
        ...
        args = {"params": system_data}

        console.print(f"Importing system: {system_data.get('name')}...")
        result_json = run_async(agent.call_tool("mongodb_create_game_system", args))
...
def list_systems() -> None:
    ...
    try:
        result_json = run_async(agent.call_tool("mongodb_list_game_systems", args))
        if not result_json:

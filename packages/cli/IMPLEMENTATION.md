# CLI Layer Implementation (Historical Reference)

> Machine-optimized task list for implementing Layer 3.
>
> **Historical planning note:** this file describes the original target CLI rollout. For the live repo surface, check `packages/cli/src/monitor_cli/main.py`, `ARCHITECTURE.md`, and `docs/USE_CASES.md`. The currently wired commands are smaller than the full planned `play/manage/query/...` set.
>
> **Current live reality:** the primary interactive play surface is the web UI chat flow, while the CLI exposes `state`, `rules`, `mechanics`, `ingest`, and `playtest` today.

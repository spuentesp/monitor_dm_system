...
def extract_canon_signal_terms(
    ...
    *,
    ...
    terms.extend(
        raw
        for section in sections
        for raw in re.findall(r"\b[A-Z][A-Za-z'-]{2,}\b", section.section_path or "")
    )
    terms.extend(
        raw.title()
        for section in sections
...
def build_source_profile_seed(
    ...
    *,
    ...
    return EmbeddedSourceProfile(
        ...
        coverage_summary=(
            "; ".join(coverage_bits) or f"Profile seed created from {source_name} headings."
        )[:2000],
...
def merge_source_profiles(
    ...
    synthesized: Optional[EmbeddedSourceProfile],
) -> EmbeddedSourceProfile:
    ...
    seen_refs: set[tuple[str, Optional[str]]] = set()
...
def format_source_profile_context(
    profile: Optional[EmbeddedSourceProfile],
    *,
    ...
    if include("term_lexicon", profile.term_lexicon):
        compact_pairs = [
            ...
            for term, values in list(profile.term_lexicon.items())[:6]
            if values
        ]
        if compact_pairs:
...
def summarize_source_profile(profile: Optional[EmbeddedSourceProfile]) -> str:

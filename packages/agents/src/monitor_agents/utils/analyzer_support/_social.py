...
def annotate_entity_roles(entity: ExtractedEntityArchetype) -> ExtractedEntityArchetype:
    ...
    roles: set[str] = {
        ...
        for role in getattr(entity, "entity_roles", [])
        if str(role).strip()
    }
    for existing_roles in raw_role_values:
        if isinstance(existing_roles, str):
            roles.update(
                part.strip().lower()
                for part in re.split(r"\s*[,/;+]\s*", existing_roles)
                if part.strip()
            )
    ...
    if entity.entity_type in {"organization", "faction"} or any(
        token in role_blob for token in INSTITUTION_ROLE_HINTS
    ):
    ...
    if "taxonomy_member" in roles and any(
        role in roles for role in {"organization", "faction", "institution"}
    ):
...
def materialize_container_entities(
    ...
    existing_by_name = {
        entity.name.lower().strip(): entity
    ...
    for entity in entities:
        ...
        if parent and parent.parent_entity_name:
            ...
            if grandparent_key == key:
                children_of_entity = sum(
                    1 for e in entities if (e.parent_entity_name or "").lower().strip() == key
                )
                children_of_parent = sum(
                    1
                    for e in entities
                    if (e.parent_entity_name or "").lower().strip() == parent_key
                )
    ...
    for entity in normalized_entities:
        for key, raw_value in (entity.properties or {}).items():
            ...
            for target_name in split_semantic_values(raw_value):
                ...
                if (
                    not target_name
                    or target_key == entity.name.lower().strip()
                    or target_key in GENERIC_CATEGORY_NAMES
                ):
                    continue

...
def normalize_source_profile(profile: Any) -> dict[str, Any]:
    ...
    for dict_field in ("term_lexicon", "aliases"):
        ...
        if not isinstance(raw_mapping, Mapping):
            ...
            continue
        normalized[dict_field] = {
            str(key).strip(): _as_list(values) for key, values in raw_mapping.items() if str(key).strip()
        }

    return normalized


def _collect_profile_terms(query: str, player_action: str, profile: Mapping[str, Any], *, limit: int = 12) -> list[str]:
...
def expand_query_with_profile(query: str, player_action: str, profile: Any, *, limit: int = 12) -> str:
...
def build_narrative_profile_context(profile: Any, *, min_confidence: float = 0.65) -> str:
    ...
    if include("institution_model", normalized.get("institution_model")):
        lines.append(f"Institution model: {', '.join(normalized['institution_model'][:6])}")
    if include("canon_signal_terms", normalized.get("canon_signal_terms")):
        lines.append(f"Forms of address / signal terms: {', '.join(normalized['canon_signal_terms'][:10])}")
    if include("term_lexicon", normalized.get("term_lexicon")):
        pairs = [
            f"{term}=>{', '.join(values[:3])}"
            for term, values in list(normalized["term_lexicon"].items())[:6]
...
def build_tone_hints_from_profile(profile: Any, *, min_confidence: float = 0.65) -> str:
    ...
    if include("narrative_frame", normalized.get("narrative_frame")):
        parts.append(f"Frame scenes through {', '.join(normalized['narrative_frame'][:4])} pressures.")
    if include("institution_model", normalized.get("institution_model")):
...
def build_npc_profile_context(
    ...
    *,
    ...
    if include("canon_signal_terms", normalized.get("canon_signal_terms")):
        lines.append(f"Forms of address / rank terms: {', '.join(normalized['canon_signal_terms'][:8])}")

    named_sets = _as_list(normalized.get("important_named_sets"))
    if include("important_named_sets", named_sets):
        relevant_sets = [name for name in named_sets if name.lower() in haystack][:4] or named_sets[:4]
        if relevant_sets:
...
def rank_snippets_with_profile(
    ...
    *,
    ...
    relevant_terms = [
        term.lower() for term in _collect_profile_terms(player_action, player_action, normalized, limit=16)
    ]
    ...
    for index, snippet in enumerate(snippets):
        ...
        text_blob = " ".join(
            str(snippet.get(field, "")) for field in ("text", "document", "content", "snippet", "summary")
        ).lower()

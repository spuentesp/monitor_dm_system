...
async def _async_fallback_to_builtin_profiles(
    ...
    if hasattr(mcp_tools, "mongodb_get_default_tone_library"):
        ...
        if default_lib:
            lib_data = (
                default_lib.model_dump() if hasattr(default_lib, "model_dump") else default_lib
            )
                    ...
                    if matched_instructions:
                        ...
                        if tone_tags_lower:
                            unknown = [
                                ...
                                for t in tone_tags
                                if t.lower().strip()
                                not in [tt.lower() for tt in trigger_tags_lower]
                            ]
                            if unknown:
...
class ToneResolver:
    ...
    def __init__(self, mcp_tools: Any):
        ...
        self._default_library_cache: Optional[Dict[str, Any]] = None
        self._profile_cache: Dict[UUID, Dict[str, Any]] = {}

    async def resolve_from_profile(
        self,
        gm_profile: Optional[Dict[str, Any]],
        ...
        if tone_instructions:
            ...
            if matched_profiles:
                ...
                base = await _async_fallback_to_builtin_profiles(
                    self.mcp_tools, normalized_tags, fallback_tone
                )
    ...
    async def _get_library(self, library_id: UUID) -> Optional[Dict[str, Any]]:
        ...
        if hasattr(self.mcp_tools, "mongodb_get_tone_library"):
            ...
            if lib:
                self._library_cache[library_id] = (
                    lib.model_dump() if hasattr(lib, "model_dump") else lib
                )
                return self._library_cache[library_id]
        return None

    async def _get_default_library(self) -> Optional[Dict[str, Any]]:
...
def get_profile_name(gm_profile: Optional[Dict[str, Any]]) -> str:
...
def profile_has_custom_instructions(gm_profile: Optional[Dict[str, Any]]) -> bool:

...
from monitor_data.schemas.knowledge_packs import EmbeddedWorldProfile, ProfileEvidenceRef  # type: ignore[attr-defined]
...
def _match_labels(text: str, mapping: Sequence[tuple[str, tuple[str, ...]]], *, limit: int = 6) -> list[str]:
    lowered = (text or "").lower()
    labels = [label for label, keywords in mapping if any(keyword in lowered for keyword in keywords)]
...
def _iter_entity_names(summary: Mapping[str, Any]) -> list[str]:
    ...
    if not isinstance(entities, list):
        return []
    return _dedupe([item.get("name", "") for item in entities if isinstance(item, Mapping)], limit=8)
...
def _build_term_lexicon(names: Sequence[str]) -> dict[str, list[str]]:
    ...
    for name in names:
        ...
        if len(words) < 2:
            continue
        aliases = [words[-1]]
        if words[0].lower() in {"house", "clan", "order", "guild", "court", "city"} and len(words) >= 2:
            aliases.append(" ".join(words[1:]))
        lexicon[name] = _dedupe(aliases, limit=3)
    return {key: value for key, value in lexicon.items() if value}


def summarize_world_coverage(summary: Mapping[str, Any], *, profile: EmbeddedWorldProfile | None = None) -> str:
...
def derive_known_open_questions(summary: Mapping[str, Any], profile: EmbeddedWorldProfile) -> list[str]:
    entities = summary.get("entities") or []
    entity_types = {str(item.get("type", "")).lower() for item in entities if isinstance(item, Mapping)}
...
def build_priority_gaps(summary: Mapping[str, Any], profile: EmbeddedWorldProfile) -> list[dict[str, Any]]:
...
def build_world_profile(
    ...
    *,
    ...
    history_text = " ".join(
        str(msg.get("content", "")) for msg in (conversation_history or []) if isinstance(msg, Mapping)
    )
    source_text = " ".join(
        [
            user_message,
...
def render_world_profile_context(profile: EmbeddedWorldProfile) -> str:
    ...
    if profile.relationship_patterns:
        lines.append(f"Likely relationship patterns: {', '.join(profile.relationship_patterns[:6])}")

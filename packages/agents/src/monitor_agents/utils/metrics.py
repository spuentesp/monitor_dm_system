...
class Metrics:
    ...
    def get_tagged_counter(self, name: str, tags: dict[str, str] | None = None) -> float:
        if not tags:
    ...
    def snapshot(self) -> dict[str, Any]:
        with self._lock:
            ...
            tagged = {self._tagged_key_to_name(key): value for key, value in self._tagged_counters.items()}
        return {
            "counters": counters,
            "gauges": gauges,
...
class Timer:
    ...
    __slots__ = ("_elapsed_ms", "_name", "_start")

...
class CircuitBreaker:
    ...
    def __init__(
        ...
        *,
        ...
        expected_exceptions: tuple[Any, ...] = (Exception,),
        name: str = "default",
    ) -> None:
    ...
    def _maybe_transition_to_half_open(self) -> None:
        if self._state == CircuitState.OPEN and (time.monotonic() - self._last_failure_time) >= self.reset_timeout:
            logger.info(
...
class LLMFallbackChain:
    ...
    def __init__(self, fallbacks: list[Any]) -> None:
        if not fallbacks:
...
from monitor_agents.utils.resilience.registry import (
    configure_breaker,
    get_breaker,
    list_registered,
    reset_all,

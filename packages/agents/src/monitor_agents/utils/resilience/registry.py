...
def get_breaker(name: str) -> CircuitBreaker:
    ...
    with _lock:
        if name not in _breakers:
            ...
            _breakers[name] = CircuitBreaker(name=name, **defaults)  # type: ignore[arg-type]
            logger.debug(
...
def configure_breaker(name: str, **kwargs) -> CircuitBreaker:  # type: ignore[no-untyped-def]
...
def unregister(name: str) -> CircuitBreaker | None:

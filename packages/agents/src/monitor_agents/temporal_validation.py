...
async def validate_scene_revision(
    ...
    if not temporal_result.is_valid:
        ...
        for violation in temporal_result.violations:
            result["violations"].append(  # type: ignore[attr-defined]
                {
                    "type": "temporal",
    ...
    if _has_proposals_needing_contradiction_check(proposals):
        ...
        mock_pack = KnowledgePackResponse(  # type: ignore[call-arg]
            pack_id=UUID(),
    ...
    result["recommendations"] = _generate_recommendations(
        temporal_result,
        result.get("contradiction_result"),  # type: ignore[arg-type]
    )
...
def _generate_recommendations(
    temporal_result: TemporalValidationResult | None,
    ...
    if temporal_result and not temporal_result.can_proceed:
        recommendations.append(
            "Scene timeline has critical violations. Adjust scene_time_ref or remove references to future events."
        )

    if temporal_result and temporal_result.warning_count > 0:
        recommendations.append(
            f"Scene has {temporal_result.warning_count} temporal warnings. Review anachronisms and fact validity."
        )
    ...
    if contradiction_result:
        ...
        if total > 0:
            ...
            if critical_count > 0:
                recommendations.append(f"{critical_count} critical contradiction(s) found. Resolve before committing.")
            elif high_count > 0:
                recommendations.append(
                    f"{high_count} high-severity contradiction(s) found. Strongly recommend review before committing."
                )
            else:

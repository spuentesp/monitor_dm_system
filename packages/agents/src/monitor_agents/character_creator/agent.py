...
class CharacterCreator(BaseAgent):
        ...
        if stats is not None:
            ...
            for stat_name, stat_value in stats.items():
                if not isinstance(stat_value, int):
                    raise ValidationError(f"Stat {stat_name} must be an integer, got {type(stat_value)}")
                if stat_value < 1 or stat_value > 30:
                    raise ValidationError(f"Stat {stat_name} must be between 1 and 30, got {stat_value}")
    ...
    def prompt_character_stats(
        self,
        game_system: dict[str, Any] | None = None,
    ) -> dict[str, Any]:
        """

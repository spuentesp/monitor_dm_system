...
class CardDraftSignature(dspy.Signature):  # type: ignore[misc]
    """
    ...
    description: str = dspy.OutputField(desc="2-4 sentence description: who they are, appearance, situation")
    personality: str = dspy.OutputField(desc="2-4 sentence personality: temperament, manner, how they treat strangers")
...
class CardDrafter(dspy.Module):  # type: ignore[misc]
    ...
    def forward(
        ...
        return {
            "name": name[:200],
            "description": _clean(getattr(pred, "description", ""), 2000) or existing_description.strip(),
            "personality": _clean(getattr(pred, "personality", ""), 2000) or existing_personality.strip(),

...
def _normalise_state_tag_update(raw_props: Dict[str, Any]) -> Dict[str, Any]:
    ...
    add_tags = list(
        props.get("add_tags") or props.get("condition_tags") or props.get("conditions") or []
    )
...
def _derive_detail_level(
    ...
    random_table_refs: Optional[List[str]] = None,
) -> str:
    ...
    """
    has_props = bool(
        base_properties and any(v not in (None, [], {}, "") for v in base_properties.values())
    )
...
class CanonKeeper(CommitDispatcherMixin, BaseAgent):
    ...
    async def bulk_commit_proposals(
        ...
        *,
        ...
        """
        Fast bulk-commit path for already-analyzed proposals (ingestion auto-canonize).
    ...
    async def _batch_contradiction_check(
        ...
        from monitor_agents.prompts.verification import BatchContradictionModule
        ...
        for start in range(0, len(candidates), chunk_size):
            ...
            flagged = await asyncio.to_thread(
                module.forward, context=context, new_facts=numbered
            )
    ...
    async def apply_pack_to_universe(
        ...
        already_committed = proposals_coll.count_documents(
            {"source": source_key, "status": "committed"}
        )
        ...
        if pack_doc:
            for doc_id_str in pack_doc.get("source_document_ids", []):
                mongo_doc = mongodb.get_collection("documents").find_one(
    ...
    async def replace_fact(
        ...
        try:
            ...
            logger.info(
                f"Replaced fact {old_fact_id} with {new_fact_id} "
                f"(scene: {scene_id}, reason: {reason})"
            )
    ...
    async def list_multiverses(self, omniverse_id: Optional[UUID] = None) -> List[Dict[str, Any]]:
        """
    ...
    async def get_universe(self, universe_id: UUID) -> Optional[Dict[str, Any]]:
        """
    ...
    async def _commit_spatial_topology(
        ...
        if not from_name or not to_name:
            logger.warning(
                "Skipping spatial_topology proposal: missing from_location or to_location"
            )
            return
    ...
    async def _commit_relationship(
        ...
        if payload.get("from_entity_id") and payload.get("to_entity_id"):
            ...
            rel_category = payload.get("category") or self._REL_CATEGORY_MAP.get(
                rel_type, "generic"
            )
    ...
    async def _commit_entity_relationship(
        ...
        if not from_name or not to_name:
            logger.warning(
                "Skipping entity_relationship proposal: missing from_entity or to_entity"
            )
            return
    ...
    async def _commit_state_change(
        ...
        raw_state_props = (
            verdict.canon_properties or proposal.get("payload", {}) or proposal.get("content", {})
        )
        state_params = _normalise_state_tag_update(raw_state_props)

        if state_params.get("entity_id") and (
            state_params.get("add_tags") or state_params.get("remove_tags")
        ):
            await self.call_tool(
    ...
    async def _resolve_name_to_uuid(self, name: str, universe_id: str) -> Optional[str]:
    ...
    def _extract_pack_id_from_source(self, proposal: Dict[str, Any]) -> Optional[str]:
    ...
    async def _commit_tone_profile(
        ...
        trigger_tags = [
            str(tag).strip() for tag in payload.get("trigger_tags", []) or [] if str(tag).strip()
        ]
        result_text = await self.call_tool(
    ...
    async def _commit_fact_branch(
        ...
        verdict_canon_properties: Optional[Dict[str, Any]] = None,
    ) -> None:
        ...
        _FACT_TYPE_ALIASES = {
            "lore": "state", "rule": "state", "description": "state", "fact": "state",
            "concept": "state", "trait": "attribute", "property": "attribute",
            "stat": "attribute", "event": "occurrence", "action": "occurrence",
            "relation": "relationship", "connection": "relationship",
        }
        raw_fact_type = str(payload.get("fact_type", "state") or "state").strip().lower()
        fact_type = (
            raw_fact_type
            if raw_fact_type in _VALID_FACT_TYPES
            else _FACT_TYPE_ALIASES.get(raw_fact_type, "state")
        )
        result_text = await self.call_tool(
            "neo4j_create_fact",
            {
                "params": {
                    ...
                    or (verdict_canon_properties or {}).get("statement", ""),
                    "fact_type": fact_type,
                    "canon_level": payload.get("canon_level", "proposed"),
    ...
    async def _fetch_related_canon(self, proposal: Dict[str, Any]) -> str:
        ...
        entity_names = proposal.get("entity_names", []) or proposal.get("payload", {}).get(
            "entity_names", []
        )
        universe_id = proposal.get("universe_id", "")
        if not entity_names:
    ...
    async def _check_contradiction(
        ...
        if not universe_id_str:
            logger.warning(
                f"Proposal {proposal.get('proposal_id')} has no universe_id, skipping contradiction check"
            )
            return None
        ...
        try:
            ...
            # Wrap sync DSPy call
            result = await asyncio.to_thread(
                module.forward, context=context, new_fact=new_fact_text
            )
            ...
            if result.get("has_contradiction"):
                ...
                match = ContradictionMatch(
                    ...
                    new_fact=ContradictionFact(
                        statement=new_fact_text, fact_type=proposal.get("change_type", "fact")
                    ),
                    existing_fact=ContradictionFact(
                        statement="Established canon", fact_type="canon"
                    ),
            ...
            logger.error(
                f"Error checking contradictions for proposal {proposal_id}: {e}\n"

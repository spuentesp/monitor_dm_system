...
from ._char_generation import ability_method_description, _skill_description_for_attr
...
def compact_for_narrator(sd: SystemData) -> Dict[str, Any]:
    ...
    attrs = [
        {
            ...
            "name": a.get("name"),
            "use": a.get("description")
            or _skill_description_for_attr(sd, (a.get("abbreviation") or "").upper()),
        }
    ...
    core_mechanic_text = (
        ...
        + (
            f"Critical success: {core['critical_success']}. "
            if core.get("critical_success")
            else ""
        )
        ...
        tracks_summary.append(
            {
                ...
                "depleted": (track.get("depleted_effect") or "")[:100]
                if track.get("depleted_effect")
                else None,
                "maxed": (track.get("maxed_effect") or "")[:100]
                if track.get("maxed_effect")
                else None,
            }
        )

...
class ConversationState(BaseModel):
    ...
    current_player_input: Optional[str] = None
...
async def load_npc_context(state: ConversationState) -> Dict[str, Any]:
    ...
    contexts = {npc_id: npc_data for npc_id, npc_data in loaded_contexts}
...
async def close_session(state: ConversationState) -> Dict[str, Any]:
    ...
    for proposal in state.pending_proposals:
        try:
            params: Dict[str, Any] = {
                "change_type": _normalize_conversation_change_type(
                    proposal.get("change_type", "fact")
                ),

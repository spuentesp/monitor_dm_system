...
class StoryState(BaseModel):
    ...
    in_game_time: datetime = Field(
        default_factory=lambda: datetime(1000, 1, 1, 12, 0, tzinfo=timezone.utc),
        description="The current absolute time within the fictional world.",
    )
    world_ticks: int = Field(
        default=0, description="Total number of regional/global simulation steps."
    )
    last_scene_duration_minutes: int = Field(
        default=0, description="Minutes passed in the last scene."
    )
...
async def evaluate_arc(state: StoryState) -> Dict[str, Any]:
    ...
    Arc phases:
    - rising_action (scenes 1-2): tension building, threads opening
    - rising_action (scenes 3+): complications mounting
    - climax: tension >= 0.7, many active threads
    - falling_action: threads resolving, tension dropping
    - resolution: few or no active threads remain
    - new_thread: a fresh complication emerges after resolution
    """
    ...
    plan = await _plan_next_scene(state)
...
async def transition_scene(state: StoryState) -> Dict[str, Any]:
    ...
    purpose = _arc_label_to_purpose(state.arc_label)
...
def _arc_label_to_purpose(arc_label: str) -> str:
...
async def simulate_world_advance(state: StoryState) -> Dict[str, Any]:
    ...
    """
    from monitor_agents.simulacrum import SimulacrumAgent
    from monitor_agents.utils.db_readers import (
        run_sync_read,
        neo4j_list_facts,
    )
    ...
    high_impact = await run_sync_read(
...
async def _create_story_outline(story_id: UUID, universe_id: UUID) -> Dict[str, Any]:
    ...
    created = await run_sync_read(mongodb_create_story_outline, params)
...
async def _create_scene(
    ...
    result = await run_sync_read(mongodb_create_scene, scene_params)
...
class StoryLoop:
    ...
    async def complete_current_scene(
        ...
        with MongoDBSaver.from_conn_string(settings.mongodb_uri) as checkpointer:
            ...
            if (
                arc_update.get("arc_label") == "resolution"
                or (outcome or {}).get("scene_purpose") == "rest"
            ):
                ...
                if pc_id:
                    prog_loop = ProgressionLoop(
                        entity_id=UUID(str(pc_id)), universe_id=self.universe_id
                    )
    ...
    async def create_flashback(
        ...
        with MongoDBSaver.from_conn_string(settings.mongodb_uri) as checkpointer:
            ...
            new_state = await compiled.aget_state(config)
    ...
    async def complete_story(
        ...
        if not transition_story_status(current, StoryStatus.COMPLETED):
            raise ValueError(
                f"Cannot transition story from {current.value!r} to {StoryStatus.COMPLETED.value!r}"
            )
        ...
        if include_recap:
            try:
                result["recap"] = await build_story_recap(
                    story_id=self.story_id, universe_id=self.universe_id
                )
            except Exception as exc:
    ...
    async def archive_story(self, current_status: str = "completed") -> Dict[str, Any]:
        ...
        if not transition_story_status(current, StoryStatus.ARCHIVED):
            raise ValueError(
                f"Cannot transition story from {current.value!r} to {StoryStatus.ARCHIVED.value!r}"
            )
    ...
    async def abandon_story(self, current_status: str = "active") -> Dict[str, Any]:
        ...
        if not transition_story_status(current, StoryStatus.ABANDONED):
            raise ValueError(
                f"Cannot transition story from {current.value!r} to {StoryStatus.ABANDONED.value!r}"
            )
...
async def build_story_recap(story_id: UUID, universe_id: UUID) -> Dict[str, Any]:
    ...
    try:
        ...
        scenes_response = await run_sync_read(mongodb_list_scenes, filter_obj)
    except Exception as exc:
            ...
            flat_beats.append(
                {
                    ...
                    "label": f"{beat.get('title', f'Beat {len(flat_beats)}')} - "
                    f"{arc_phases[arc_index]}",
...
def schedule_event(
    *,
    ...
    trigger_after: Optional[timedelta] = None,
    ...
    """
    base = in_game_time or datetime.now(timezone.utc)
    if trigger_after is not None:
        fire_at = base + trigger_after
    else:
        fire_at = base

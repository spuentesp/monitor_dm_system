...
class CombatState(BaseModel):
    ...
    current_index: int = 0
    current_combatant_id: UUID | None = None
    round_number: int = 1
    ...
    victory_side: str | None = None  # "pc" | "enemy" | None
...
def _calculate_modifier_from_attributes(
    ...
    if gsr:
        try:
            return gsr.calc_modifier(stat_name, value)  # type: ignore[no-any-return]
        except Exception:
            pass
    ...
    if target is not None:
        ...
        if gsr:
            try:
                ...
                inferred_action_type, inferred_stat, _ = await gsr.infer_action_stat(state.user_input)
                if inferred_stat:
        ...
        if c.entity_id == entity_id:
            if gsr:
                try:
                    ...
                    track_name = gsr.get_damage_model().get("damage_track", "HP") if gsr.get_damage_model() else "HP"
...
def _format_combat_context(state: CombatState) -> str:
    ...
    for entry in state.combat_log[-5:]:
        ...
        if ev == "initiative":
            ...
            lines.append(f"  {entry['attacker']} hits {entry['target']} for {entry['damage']} damage")
        elif ev == "miss":
...
async def check_victory(state: CombatState) -> dict[str, Any]:
    ...
    next_eid = state.initiative_order[next_index] if next_index < len(state.initiative_order) else None
        ...
        for key, value in updates.items():
            if hasattr(self.state, key):
                if key == "combatants" and isinstance(value, list):
                    self.state.combatants = [CombatantState(**c) if isinstance(c, dict) else c for c in value]

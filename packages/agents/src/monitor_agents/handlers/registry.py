...
CommitHandler = Callable[
    [Any, str, dict[str, Any], list[str] | None, BaseModel, Any],  # agent is BaseAgent
    Awaitable[None],
]
...
class CommitHandlerRegistry:
    ...
    def get(self, type_key: str) -> CommitHandler | None:
        """Get handler for a type key, returns None if not found."""
        return self._handlers.get(type_key)

    def get_handler_name(self, type_key: str) -> str | None:
        """
...
_registry: CommitHandlerRegistry | None = None
...
def _register_builtin_handlers() -> None:
    ...
    for _type_key, _method_name in _BUILTIN_HANDLERS.items():
...
class CommitHandlerAdapter:
    ...
    async def execute(
        ...
        if handler is None:
            ...
            await self._fallback_dispatch(type_key, proposals_coll, proposal_id, proposal, source_id_strs, verdict)
            return

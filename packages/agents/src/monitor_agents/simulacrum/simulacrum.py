...
class OpportunistSimulacrumSignature(dspy.Signature):  # type: ignore[misc]
    """
    ...
    high_impact_events: str = dspy.InputField(desc="Recent high-magnitude facts/events from the world.")
...
class RealistSimulacrumSignature(dspy.Signature):  # type: ignore[misc]
    """
...
class CouncilReconcilerSignature(dspy.Signature):  # type: ignore[misc]
    """
    ...
    clock_tick: int = dspy.OutputField(desc="Amount to advance/retreat the clock (-2 to +2). Usually 0 or 1.")

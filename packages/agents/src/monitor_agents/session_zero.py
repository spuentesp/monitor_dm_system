...
class SessionZeroQuestion(BaseModel):
    ...
    category: QuestionCategory = Field(
        ...
        description=(
            "What aspect of the character this question explores. "
            "Used to ensure variety across the interview."
        ),
    )
    ...
    reasoning: str = Field(
        default="",
        description=(
            "Free-form reasoning: why this question, given the tone and "
            "prior answers. For debugging and audit."
        ),
    )
...
class SessionZeroSummary(BaseModel):
    ...
    character_name: Optional[str] = Field(
        None,
        description="The character's name, if mentioned in the answers."
    )
    ...
    key_bonds: List[str] = Field(
        ...
        description="Key relationships or bonds mentioned in the answers."
    )

    key_fears: List[str] = Field(
        default_factory=list,
        description="Fears or vulnerabilities mentioned in the answers."
    )
...
if _DSPY_AVAILABLE:
    ...
    class SessionZeroSignature(dspy.Signature):
        """
        ...
        - After 5-7 substantive answers, set is_final=True to signal the
        ...
        max_questions: int = dspy.InputField(
            desc="Maximum questions before summarizing. Typically 7.",
        )
        ...
        is_final: bool = dspy.OutputField(
            desc="True if this is the last question before summarizing (usually after 5-7 substantive answers). False otherwise.",
        )
        reasoning: str = dspy.OutputField(
            desc="Why this question, given the tone and prior answers.",
        )

    class SessionZeroModule(dspy.Module):
        """
    ...
    class SessionZeroSummarySignature(dspy.Signature):
        """
    ...
    class SessionZeroSummaryModule(dspy.Module):
...
_FALLBACK_QUESTIONS: Dict[str, List[Dict[str, Any]]] = {
    ...
    "heroic": [
        {"category": "origin", "text": "Where did your story begin — and what set you on this path?"},
        ...
        {"category": "skill", "text": "What are you known for — what can you do that others can't?"},

...
class PlotHook(BaseModel):
    ...
    thread_id: Optional[str] = Field(None, description="Related plot thread ID, if any")
...
class SessionPrep(BaseModel):
    ...
    npc_reminders: List[str] = Field(
        default_factory=list, description="NPCs the GM should remember"
    )
    world_state_changes: List[str] = Field(
        default_factory=list, description="Recent changes to the world state"
    )
...
class Handout(BaseModel):
    ...
    related_entities: List[str] = Field(
        default_factory=list, description="Entity names referenced in the handout"
    )
    tone: str = Field(
        default="mysterious",
...
class PlotHookAgent(BaseAgent):
    ...
    async def suggest_hooks(
        ...
        for s in scenes[:5]:
            scene_summaries.append(
                f"- {s.get('title', 'Untitled scene')}: {s.get('summary', 'No summary')}"
            )
    ...
    async def detect_contradictions(
        ...
        for f in facts[:30]:
            fact_list.append(
                f"- [{f.get('canon_level', 'unknown')}] "
                f"{f.get('content', f.get('text', 'No content'))}"
            )
    ...
    async def generate_session_prep(
        ...
        world_changes = [
            ...
            for f in facts[:5]
            if f.get("canon_level") in ("canon", "established")
        ]
    ...
    async def generate_handout(
        ...
        threads = self._parse_result(threads_result, "threads", [])
        thread_summaries = [
            f"- {t.get('title', 'Untitled')}: {t.get('description', '')}" for t in threads[:5]
        ]
        ...
        return self._template_handout(
            handout_type, tone, spoiler_level, entity_summaries, fact_list
        )
    ...
    def _heuristic_contradictions(self, facts: List[Dict[str, Any]]) -> List[Contradiction]:
        ...
        for entity_id, entity_fact_list in entity_facts.items():
                        ...
                        if word_a in content_a and word_b in content_b:
                            contradictions.append(
                                Contradiction(
                                    ...
                                    severity="high",
                                    explanation=(
                                        f"Status contradiction: '{word_a}' vs '{word_b}'."
                                    ),
                                    suggestion=(
                                        f"Resolve the {word_a}/{word_b} conflict for this entity."
                                    ),
                                )
                            )
                            break
    ...
    def _template_handout(
        ...
        if fact_list:
            content_parts.append(
                fact_list[0].lstrip("- ") if fact_list else "Whispers of something yet to come."
            )

        content = (
            "\n\n".join(content_parts)
            if content_parts
            else "The world holds many secrets yet to be discovered."
        )
...
def extract_canon_entity_names(entities: List[dict]) -> List[str]:

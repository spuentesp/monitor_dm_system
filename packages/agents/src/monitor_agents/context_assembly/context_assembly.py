...
class QueryFormulationSignature(dspy.Signature):  # type: ignore[misc]
    """
    ...
    scene_summary: str = dspy.InputField(desc="Brief summary of the current scene (location, NPCs, active threats)")
    ...
    memory_query: str = dspy.OutputField(
        desc=(
            "Semantic query for the Qdrant 'memories' collection. Focus on character memories relevant to this action."
        )
    )
...
#
# class TurnIntentSignature(dspy.Signature):  # type: ignore[misc]
#     ...
# class TurnIntentModule(dspy.Module):  # type: ignore[misc]
#     ...
...
class QueryFormulationModule(dspy.Module):  # type: ignore[misc]
    """

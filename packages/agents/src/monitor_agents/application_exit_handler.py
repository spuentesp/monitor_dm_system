...
class ApplicationExitHandler(BaseAgent):
    ...
    async def check_active_scene(self, user_id: str | None = None) -> dict[str, Any] | None:
        """
    ...
    async def exit_application(self, user_id: str | None = None) -> None:
        """
        ...
        if active_scene:
            ...
            if ConfigLoader.get_auto_save_enabled():
                ...
                should_save = await self.prompt_save_scene(active_scene)
                if should_save:
                    await self.save_scene(active_scene)

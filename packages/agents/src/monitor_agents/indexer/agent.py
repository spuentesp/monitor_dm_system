...
from monitor_agents.analyzer.analyzer import SectionCategorizationModule
...
# Section categorisation concurrency. The lightweight HEAVY LLM call
# per section blocks on the provider; running 500 of them sequentially
# on a 178 MB PDF took ~30 min of the 55 min ingest budget. Eight-way
# parallel cut the categorisation stage from ~30 min to ~5 min in live
# tests. Higher values risk provider backpressure; 8 matches the
# ANALYZER-side _EXTRACTION_CONCURRENCY budget.
_SECTION_CATEGORIZATION_CONCURRENCY = 8
...
class Indexer(BaseAgent):
    ...
    async def index_pdf_from_minio(
        ...
        chunks = await self._chunk_pdf_with_structure(pdf_bytes, source_name, metadata=metadata or {})
        if not chunks:
            return 0
    ...
    async def index_file(
        ...
        *,
        ...
        if fmt == "pdf":
            chunks = await self._chunk_pdf_with_structure(file_bytes, sname, metadata=extra_meta)
            if not chunks:
                return 0
            await self._embed_and_upsert(chunks, collection=collection)
            return len(chunks)
    ...
    async def _download_from_minio(self, object_key: str) -> bytes | None:
    ...
    async def _chunk_pdf_with_structure(
        self,
        pdf_bytes: bytes,
        source_name: str,
        metadata: dict[str, Any] | None = None,
        ...
        2. For each block, runs a TagPool pre-filter on the heading path.
           ...
           - Otherwise, ``SectionCategorizationModule`` is called to classify the
             section via a lightweight LLM. The LLM calls run up to
           :data:`_SECTION_CATEGORIZATION_CONCURRENCY` in parallel via a
           semaphore + gather (each ``categorizer(...)`` invocation is a sync
           DSPy predict, so we wrap it in ``asyncio.to_thread`` to avoid
        ...
        The categoriser is degraded to "general" on first failure (per the
        same protection as the sequential version) — once any single call
        fails we stop issuing LLM categoriser calls for the rest of the
        document. ``chunk_text`` runs strictly serially after classification,
        so we never overlap the chunking pass itself.
        """
        ...
        # Mutable from inner closures below. ``nonlocal`` in the gather
        # worker binds this outer name.
        categorizer_broken = False

        # Build the (block, tag_category) tuples up front so the parallel
        # worker has no shared state race with the chunking loop below.
        blocks_with_tags: list[tuple[Any, str]] = []
        for block in section_blocks:
            heading_text = " > ".join(block.heading_path)
            tag_category = TagPool.classify_text(heading_text) or "general"
            blocks_with_tags.append((block, tag_category))

        sem = asyncio.Semaphore(_SECTION_CATEGORIZATION_CONCURRENCY)

        async def _classify(block: Any, tag_category: str) -> str:
            """Return the category for one block. Falls back to ``general``
            when the LLM is unavailable (one failure flips categorizer_broken
            and short-circuits the rest, identical to the pre-refactor
            sequential behaviour)."""
            nonlocal categorizer_broken
            if tag_category != "general":
                return tag_category
            if categorizer_broken:
                return "general"
            ...
            async with sem:
                try:
                    # ``SectionCategorizationModule`` is a sync dspy.Module
                    # whose ``forward`` blocks on the LLM provider thread;
                    # offload it so the gather actually overlaps.
                    result = await asyncio.to_thread(
                        categorizer,
                        heading_path=heading_text,
                        section_excerpt=block.text[:200],
                    )
                    return result.category  # type: ignore[no-any-return]
                except Exception as exc:
                    if not categorizer_broken:
        ...
        # Categorise all sections in parallel up to the semaphore cap.
        # ``gather(return_exceptions=True)`` is intentional — a single
        # _classify that somehow bypasses the except above would otherwise
        # cancel the whole gather. We post-check the results.
        classify_results = await asyncio.gather(
            *[_classify(b, t) for b, t in blocks_with_tags],
            return_exceptions=True,
        )
        categories: list[str] = []
        for _idx, res in enumerate(classify_results):
            if isinstance(res, BaseException):
                # Should be unreachable because _classify swallows, but
                # guard the contract.
                if not categorizer_broken:
                    categorizer_broken = True
                    logger.warning(
                        "Section categorizer gather raised (%s); remaining "
                        "sections of '%s' will be tagged as 'general'.",
                        res,
                        source_name,
                    )
        ...
        for (block, _), category in zip(blocks_with_tags, categories, strict=False):
    ...
    async def _embed_and_upsert(
        ...
        Embeds via RetrievalService.embed_docs (the single embed owner, pinned
        model) and upserts via the Qdrant client.
        """
        ...
        try:
            ...
            for batch_start in range(0, len(chunks), _UPSERT_BATCH_SIZE):
                ...
                # Embed via the RetrievalService (single embed owner — pinned model).
                texts = [c.text for c in batch]
                from monitor_data.retrieval import default_retrieval_service

                vectors = await default_retrieval_service().embed_docs(texts)

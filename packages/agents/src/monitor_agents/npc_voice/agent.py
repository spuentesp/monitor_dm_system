...
from monitor_agents.npc_voice.npc_voice import NPCActorModule, NPCDirectVoiceModule
...
class NPCVoice(BaseAgent):
    ...
    async def respond_direct(
        ...
        *,
        ...
        relationship_snapshot_before = self._relationship_snapshot(profile, player_entity_id, universe_id=universe_id)
        ...
        social_read = {
            ...
            "stance_after": relationship_snapshot_after.get("stance") if relationship_snapshot_after else None,
            "deltas": relationship_snapshot_after.get("last_delta", {}) if relationship_snapshot_after else {},
            "reason": relationship_snapshot_after.get("last_shift_reason") if relationship_snapshot_after else None,
            "confidence": max(
                0.55,
                min(
                    0.95,
                    0.65 + abs((self._parse_relationship_delta(relationship_delta) or ("none", 0.0))[1]),
                ),
            ),
        }
    ...
    async def _get_story_context(self, npc_id: UUID) -> str:
        ...
        facts_result = await self.call_tool("neo4j_list_facts", {"entity_id": str(npc_id), "limit": 15})
        facts = facts_result if isinstance(facts_result, list) else []
    ...
    def _evaluate_triggers(self, triggers: list[dict[str, Any]], player_said: str) -> list[dict[str, Any]]:
    ...
    def _relationship_snapshot(
        ...
        *,
        ...
        if not existing:
            legacy = (profile.get("relationship_states") or {}).get(str(player_entity_id))
    ...
    def _build_relationship_snapshot(
        ...
        *,
        ...
        snapshot = dict(self._relationship_snapshot(profile, player_entity_id, universe_id=universe_id))
        ...
        if parsed:
            ...
            if axis in {"trust", "confidence", "respect"}:
                ...
                snapshot["affinity"] = self._clamp_social_value(snapshot.get("affinity", 0.0) - (magnitude * 0.5))
                snapshot["fear"] = self._clamp_social_value(snapshot.get("fear", 0.0) + (magnitude * 0.25))
                deltas["trust"] = round(-magnitude, 3)
            elif axis in {"affinity", "friendship", "warmth", "admiration", "loyalty"}:
                snapshot["affinity"] = self._clamp_social_value(snapshot.get("affinity", 0.0) + delta)
                ...
                snapshot["leverage"] = self._clamp_social_value(snapshot.get("leverage", 0.0) + delta)
                deltas["leverage"] = round(delta, 3)
            else:
                snapshot["trust"] = self._clamp_social_value(snapshot.get("trust", 0.0) + (delta * 0.5))
    ...
    def _emotion_to_state_tags(self, emotional_state_after: str) -> tuple[list[str], list[str]]:
        ...
        if any(token in lowered for token in ("warm", "friendly", "respectful", "grateful", "kind")):
    ...
    def _build_proposals(
        ...
        current_emotion_for_diff: str | None = None
        if universe_id is not None:
            by_universe = profile.get("current_emotional_state_by_universe") or {}
        ...
        if emotional_state_after and emotional_state_after != current_emotion_for_diff:
            ...
            if universe_id is not None:
                ...
                state_content["profile_updates"]["current_emotional_state_by_universe"] = {
        ...
        if parsed and player_entity_id is not None and player_entity_id != npc_id:
            ...
            if axis in {"hostility", "anger", "resentment", "suspicion", "distrust"} or trust_after <= -0.2:
                rel_type = "HOSTILE_TO"
            ...
            relationship_content: dict[str, Any] = {
                ...
                "properties": {
                    "sentiment": axis,
                    "delta": delta,
                    "new_value": relationship_snapshot.get(axis if axis in relationship_snapshot else "trust"),
                ...
                "profile_updates": {
                    "current_emotional_state": emotional_state_after,
                    "relationship_states": {
                        str(player_entity_id): {k: v for k, v in relationship_snapshot.items() if k != "last_delta"}
                    },
                },
            }
            if scene_id is not None:
            ...
            if universe_id is not None:
                ...
                relationship_content["profile_updates"]["relationship_states_by_universe"] = {
                    str(universe_id): {
                        str(player_entity_id): {k: v for k, v in relationship_snapshot.items() if k != "last_delta"}
                    }
                }
    ...
    async def _update_working_social_state(
        ...
        *,
        ...
        if player_entity_id is not None and relationship_snapshot:
            snapshot_without_delta = {key: value for key, value in relationship_snapshot.items() if key != "last_delta"}
    ...
    async def _write_npc_memory(
        ...
        resolved_universe_id: str | None = str(universe_id) if universe_id is not None else None
        if resolved_universe_id is None:
            try:
                ent = await self.call_tool("neo4j_get_entity", {"entity_id": str(npc_id)})
                if isinstance(ent, dict):
                    resolved_universe_id = ent.get("universe_id") or ent.get("properties", {}).get("universe_id")
            except Exception:

...
from monitor_data.schemas.coverage import CoverageSnapshot, CoverageThresholds, WorldCoverage

from monitor_agents.base import BaseAgent
from monitor_agents.world_architect.world_architect import (
...
class WorldArchitect(BaseAgent):
    ...
    def __init__(self, agent_id: str = "world-architect-1") -> None:
        ...
        self.multiverse_id: UUID | None = None
    ...
    async def suggest_next(self, universe_id: UUID | None) -> str:
        """
        ...
        def _run_gap_sync() -> dspy.Prediction:
            return self._gap_module(
                ...
                world_profile_context=render_world_profile_context(world_profile),
                coverage_summary=world_profile.coverage_summary,
                known_open_questions=json.dumps(world_profile.known_open_questions[:6], default=str),
            )
    ...
    async def seed_universe(
        ...
        feature_tables = [t for t in tables if t.table_type == RandomTableType.FEATURE]  # type: ignore[attr-defined]
        name_tables = [t for t in tables if t.table_type == RandomTableType.NAME]

        def _roll_table(table_list: list[Any], default: str = "Unknown") -> dict[str, Any]:
    ...
    async def populate_scene_procedurally(
        ...
        if not relevant_tables:
            ...
            relevant_tables = [
                ...
                if t.table_type in (RandomTableType.ENCOUNTER, RandomTableType.FEATURE)  # type: ignore[attr-defined]
            ]
        ...
        for table in relevant_tables[:3]:
            try:
                roll_res = mongodb_roll_on_table(UUID(table.table_id))  # type: ignore[arg-type]
        ...
        for seed in seeds:
            ...
            entity_type = "feature"
            if any(kw in val.lower() for kw in ["guard", "orc", "goblin", "merchant", "beast", "spirit"]):
                entity_type = "npc"
    ...
    async def _load_world_state(self, universe_id: UUID | None) -> dict[str, Any]:
        ...
        try:
            ...
            # 2. Axioms
            axioms = await anyio.to_thread.run_sync(lambda: neo4j_list_axioms(AxiomFilter(universe_id=universe_id)))
            summary["axiom_count"] = len(axioms)
            summary["axioms"] = [{"statement": a.statement, "domain": a.domain} for a in axioms[:15]]

            # 3. Facts
            facts = await anyio.to_thread.run_sync(lambda: neo4j_list_facts(FactFilter(universe_id=universe_id)))
            ...
            if universe:
                ...
                sources = await library.list_by_system(universe.system_name or "generic")  # type: ignore[attr-defined]
    ...
    # ------------------------------------------------------------------
    # F2-1: formal coverage computation
    # ------------------------------------------------------------------
    ...
    async def compute_coverage(
        ...
        """Compute the structured coverage report for a universe (F2-1 wave 1).

        Unlike ``_load_world_state`` (which feeds the LLM prompt and is
        deliberately small), this loads the full primitive set — entities,
        axioms, facts, relationships, the linked game system, and random
        tables — and maps it onto the 8 formal coverage dimensions from
        FORGE_EXPANSION.md §2. Read-only; no Neo4j writes.
        """
        from monitor_agents.utils.world_coverage import build_world_coverage

        snapshot = await self._load_coverage_snapshot(universe_id)
        return build_world_coverage(snapshot, thresholds or CoverageThresholds())

    async def _load_coverage_snapshot(self, universe_id: UUID) -> CoverageSnapshot:
        """Load everything coverage dimensions B-H need, as a typed snapshot."""
        import anyio

        return await anyio.to_thread.run_sync(self._load_coverage_snapshot_sync, universe_id)

    def _load_coverage_snapshot_sync(self, universe_id: UUID) -> CoverageSnapshot:
        """Synchronous snapshot gather (runs in a worker thread).

        Best-effort: a failing source degrades that dimension to empty rather
        than failing the whole report, mirroring ``_load_world_state``.
        """
        from monitor_data.schemas.coverage import CoverageEdge
        from monitor_data.schemas.entities import EntityFilter
        from monitor_data.schemas.facts import AxiomFilter, FactFilter
        from monitor_data.schemas.random_tables import RandomTableFilter
        from monitor_data.tools.mongodb_tools import (
        ...
        try:
            ...
            entity_result = neo4j_list_entities(EntityFilter(universe_id=universe_id, limit=1000))
            snapshot.entities = entity_result.entities if entity_result else []

            snapshot.axioms = neo4j_list_axioms(AxiomFilter(universe_id=universe_id, limit=200))
            snapshot.facts = neo4j_list_facts(FactFilter(universe_id=universe_id, limit=100))

            # Relationships: neo4j_list_relationships has no universe filter, so
            # reuse the snapshot query which scopes edges to universe entities.
            state = neo4j_get_universe_state(universe_id)
            snapshot.edges = [
                CoverageEdge(
                    from_entity_id=str(rel.get("from_id", "")),
                    to_entity_id=str(rel.get("to_id", "")),
            ...
            if universe.default_game_system_id:
                snapshot.game_system = mongodb_get_game_system(universe.default_game_system_id)

            seen_tables: set[UUID] = set()
            filters = [RandomTableFilter(universe_id=universe_id, limit=200)]
            if universe.default_game_system_id:
                filters.append(RandomTableFilter(game_system_id=universe.default_game_system_id, limit=200))
            for table_filter in filters:
                try:
                    table_list = mongodb_list_random_tables(table_filter)
                except Exception as exc:
                    logger.debug("coverage: random table listing failed: %s", exc)
                    continue
                for table in table_list.tables:
                    if table.table_id not in seen_tables:
                        seen_tables.add(table.table_id)
                        snapshot.random_tables.append(table)

            try:
                profile = mongodb_get_world_profile(universe_id)
                if profile and profile.narrative_frame:
                    snapshot.narrative_frame_terms = list(profile.narrative_frame)
            except Exception as exc:
                logger.debug("coverage: world profile lookup failed: %s", exc)

        except Exception as exc:
            logger.debug("_load_coverage_snapshot failed: %s", exc)

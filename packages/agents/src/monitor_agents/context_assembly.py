...
from monitor_agents.prompts.context_assembly import QueryFormulationModule
...
class ContextAssembly(BaseAgent):
    ...
    def _cache_set_json(self, key: str, value: Any, *, ttl: Optional[int] = None) -> None:
        try:
            from monitor_data.db.redis import get_redis_client
    ...
    async def assemble(
        ...
        memory_query = expand_query_with_profile(
            player_action or "", player_action or "", source_profile
        )
        ...
        if player_action and actor_id_str:
            try:
                ...
                if isinstance(lore_result, list):
                    ...
                    matched_lore = lore_result.get("entries", lore_result.get("results", []))
                if matched_lore:
    ...
    async def retrieve_turn_context(
        ...
        if detail_level == "memory_only":
            ...
            if dialogue_intent:
                ...
                windows = build_dialogue_turn_windows(
                    conversation_turns, window_size=3, max_windows=8
                )
        ...
        entities, memories = await asyncio.gather(
            ...
            self._search_memories(
                expanded_memory_query, story_id=story_id, universe_id=universe_id
            ),
        )
        ...
        if dialogue_intent:
            ...
            ranked_dialogue_windows = rank_dialogue_windows(
                ...
                player_action=player_action,
                preferred_speakers=[
                    str(entity.get("name")) for entity in entities if entity.get("name")
                ],
                limit=4,
            )
            memories = [
    ...
    async def _search_memories(
        ...
        *,
        ...
        cache_key = self._cache_key(
            "memory_search", story_id or "global", universe_id or "all", query
        )
        cached = self._cache_get_json(cache_key)
        if isinstance(cached, list):
            return cast(List[Dict[str, Any]], cached)
        ...
        try:
            ...
            logger.warning("context_assembly: JSON parse failed for memories (qdrant), returning []", exc_info=True)
            return []
        self._cache_set_json(cache_key, memories, ttl=self._ttl(short=True))
        return cast(List[Dict[str, Any]], memories)
        ...
        *,
        ...
        for raw in keyword_responses:
            ...
            try:
                ...
                logger.warning("context_assembly: JSON parse failed for keyword search result, skipping", exc_info=True)
                continue
            for item in results:
        ...
        plain = await self._search_snippets(
            query, query_vector=query_vector, universe_id=universe_id
        )
        for item in plain:
            cid = item.get("chunk_id") or item.get("id") or str(item)
            if cid not in seen_ids:
    ...
    async def _collect_traversal_candidates(
        ...
        *,
        ...
        except Exception:  # noqa: BLE001 — fallback to empty result
        ...
        if ranked_paths_raw is not None:
            try:
                payload = (
                    json.loads(ranked_paths_raw)
                    if isinstance(ranked_paths_raw, str)
                    else ranked_paths_raw
                )
            except (json.JSONDecodeError, TypeError):
                payload = []
        ...
        for path in ranked_paths:
            ...
            rel_chain = [
                str(rel).upper() for rel in (path.get("rel_chain") or []) if str(rel).strip()
            ]
        ...
        *,
        ...
        try:
            ...
            def _load_game_system() -> Dict[str, Any]:
                ...
                if (
                    not resolved_system_id
                    and resolved_source_type == "generic_library"
    ...
    async def _fetch_runtime_profile(self, system_name: Optional[str] = None) -> Dict[str, Any]:
                ...
                if not candidates:
                    ...
                    if system_name:
                        narrowed = [
                            ...
                            if (pack.system_name or "") == system_name
                            or system_name in (pack.tags or [])
                        ]
                        if narrowed:
                            candidates = narrowed
                return candidates
                        ...
                        normalized = normalize_source_profile(
                            {
                                ...
                                "taxonomy_containers": list(
                                    mindscape.get("taxonomy_hints", []) or []
                                ),
                                "canon_signal_terms": list(
                                    mindscape.get("taxonomy_hints", []) or []
                                ),
    ...
    async def _summarise_context(
        ...
        if plot_threads:
            for thread in plot_threads:
                ...
                score += 1.5
                scored_items.append(
                    (score, f"[Plot Thread] {title} (Status: {thread.get('status', 'active')})")
                )

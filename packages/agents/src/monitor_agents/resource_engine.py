...
class SpendAttempt:
    ...
    can_afford: bool
    spend_effect: str | None = None  # e.g. "advantage_2d20kh1"
...
class ResourceEngine:
    ...
    def _match_resource(self, intent: str) -> str | None:
    ...
    def _resolve_spec(self, spend: SpendAttempt) -> tuple[str, str]:
        ...
        if mechanic == "d20":
            if "meet_or_beat" in str(self.game_context.get("core_mechanic", {}).get("success_type", "")):
    ...
    def apply_action_economy(
        ...
        is_harm = bool(resolution.get("has_harm")) or success_level in (
            "failure",
            "critical_failure",
        )
    ...
    def check_thresholds(self) -> list[ThresholdEvent]:
        ...
        for res_key, track_def in self._tracks.items():
            ...
            actual_key: str | None = self._ws_key_map.get(res_key.lower())
            if actual_key is None:
            ...
            if actual_key is None:
                ...
                for _res_key_idx, res_def in self._resources.items():
                    res_name_norm = res_def.get("name", "").lower().replace(" ", "").replace("_", "")
                    if res_name_norm and track_norm == res_name_norm:
                        abbr_norm = res_def.get("abbreviation", "").lower().replace(" ", "").replace("_", "")
    ...
    def _parse_restore_spec(self, spec: str) -> ResourceDelta | None:
        ...
        if res_key is None:
            for res_key_schema, res_def in self._resources.items():
                ...
                if norm_res in (abbr, name) or norm_res in name:
                    # Found in schema — map to actual ws key
                    for ws_key in self.working_state:
                        if (
                            ws_key.lower() == res_key_schema.lower()
                            or ws_key.lower().replace(" ", "").replace("_", "") == res_key_schema.lower()
                        ):
                            res_key = ws_key
                            break
                    break
        ...
        elif amount_str == "half_max" or amount_str.startswith("hit_dice"):

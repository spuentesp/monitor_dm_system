...
class NPCDirectVoiceSignature(dspy.Signature):
    """
    ...
    personality_summary: str = dspy.InputField(
        desc=(
            "Personality profile: traits, values, speech style, fears, desires. "
            "Governs voice and behavior."
        )
    )
    ...
    relevant_memories: str = dspy.InputField(
        desc="JSON array of NPC's memories relevant to this conversation"
    )
    known_facts: str = dspy.InputField(
        desc="JSON array of canonical facts this NPC knows (their knowledge boundary)"
    )
    ...
    conversation_history: str = dspy.InputField(
        desc="Recent conversation turns (last N exchanges) for continuity"
    )
    ...
    player_said: str = dspy.InputField(
        desc="What the player character just said or did towards this NPC"
    )
...
class NPCActorSignature(dspy.Signature):
    """
...
class NPCDirectVoiceModule(dspy.Module):
    """
...
class NPCActorModule(dspy.Module):
    """

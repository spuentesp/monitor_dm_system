...
class CanonKeeperReasoningSignature(dspy.Signature):
    """
    ...
    # Inputs
    proposal_summary: str = dspy.InputField(
        desc="Human-readable description of the proposed world-state change"
    )
    proposal_content: str = dspy.InputField(
        desc="JSON details of the proposed change (entity type, properties, relationships)"
    )
    existing_canon: str = dspy.InputField(
        desc="JSON excerpt of relevant Neo4j entities and facts already in canon"
    )
    story_arcs: str = dspy.InputField(
        desc="Active story arcs and their current states that may be affected"
    )
...
class PolicyCheckSignature(dspy.Signature):
    ...
    world_rules: str = dspy.InputField(
        desc="Bullet list of configured world rules (tone, content flags, etc.)"
    )

    violation_found: str = dspy.OutputField(
        desc="'YES' or 'NO' — whether a policy violation was found"
    )
    violation_detail: str = dspy.OutputField(
        desc="Brief description of the violation, or 'none' if no violation"
    )
...
class CanonKeeperReasoningModule(dspy.Module):
    """
...
class PolicyCheckModule(dspy.Module):
    """

...
class QuickWorldSignature(dspy.Signature):
    """
    ...
    tone: str = dspy.InputField(
        desc="Narrative tone, e.g. 'grim', 'heroic', 'mysterious'. May be empty."
    )

    world_name: str = dspy.OutputField(desc="A short, evocative name for the world/setting.")
    world_description: str = dspy.OutputField(
        desc="2-4 sentences establishing the setting's premise and feel."
    )
...
class QuickWorldModule(dspy.Module):

...
class AxiomExtractionSignature(dspy.Signature):
    """
...
class SourceProfileSynthesisSignature(dspy.Signature):
    """
...
class LoreFactExtractionSignature(dspy.Signature):
    """
...
class GameSystemDetectionSignature(dspy.Signature):
    """
    ...
    source_chunks: str = dspy.InputField(
        desc=(
            "Representative text passages from the source document. "
...
class GameRuleExtractionSignature(dspy.Signature):
    ...
    rules: list[GameRule] = dspy.OutputField(
        desc="Structured game rules extracted from this section"
    )


class PlotThreadExtractionSignature(dspy.Signature):
    """
...
class ToneExtractionSignature(dspy.Signature):
    """
    ...
    tone_profiles: list[ExtractedToneProfile] = dspy.OutputField(
        desc="Extracted narrative tone profiles"
    )


class CharacterProfileExtractionSignature(dspy.Signature):
    """
    ...
    character_profiles: list[ExtractedCharacterProfile] = dspy.OutputField(
        desc="Extracted psychological profiles"
    )


class GenerationTemplateExtractionSignature(dspy.Signature):
    """
    ...
    known_entities: str = dspy.InputField(
        desc="Names of archetypes, stat blocks, and profiles already extracted."
    )

    template_reasoning: str = dspy.OutputField(desc="Reasoning about how to spawn these NPCs")
    generation_templates: list[ExtractedGenerationTemplate] = dspy.OutputField(
        desc="Extracted generation recipes"
    )


class CreationLogicSynthesizerSignature(dspy.Signature):
    """
    ...
    logic_reasoning: str = dspy.OutputField(
        desc="Step-by-step reasoning about how to make each step actionable"
    )
...
def _make_analyzer_module(name: str, sig_name: str) -> type:
    ...
    if name == "BatchedExtractionModule":
        cls.forward = getattr(globals()["_AnalyzerModule"], "forward")  # type: ignore[method-assign, union-attr]
    return cls
...
class _AnalyzerModule(dspy.Module):
    """
...
class GameSystemDetectionModule(_AnalyzerModule):
    ...
    _role = (
        ModelRole.HEAVY
    )  # Raised from LIGHT — benefits from strong reasoning with 48-chunk sample
...
class CharacterSheetExtractionSignature(dspy.Signature):
    """
    ...
    *** SUCCESS TYPES ***
    ...
    system_name: str = dspy.InputField(
        desc="Confirmed game system name (e.g. 'Death in Space', 'D&D 5e')"
    )
    source_profile_context: str = dspy.InputField(
    ...
    skills: list[SkillDefinition] = dspy.OutputField(
        desc="Character skills. Ensure evidence_refs are included."
    )
    resources: list[ResourceDefinition] = dspy.OutputField(
        desc="Tracked resources. Ensure evidence_refs are included."
    )
    ...
    core_mechanic: Optional[CoreMechanic] = dspy.OutputField(
...
class CreationProcedureExtractionSignature(dspy.Signature):
    """
...
class NPCExtractionSignature(dspy.Signature):
    ...
    text_chunks: str = dspy.InputField(
        desc="Concatenated text from bestiary, antagonist, and NPC sections"
    )
    source_profile_context: str = dspy.InputField(
    ...
    npc_reasoning: str = dspy.OutputField(
        desc="Brief reasoning about which NPCs and NPC rules were found"
    )
    stat_blocks: list[NPCStatBlock] = dspy.OutputField(
        desc="Extracted NPC/creature stat blocks. Ensure evidence_refs are included."
    )
    creation_rules: Optional[NPCCreationRules] = dspy.OutputField(
...
class RandomTableExtractionSignature(dspy.Signature):
    """
...
class AgendaExtractionSignature(dspy.Signature):
    """
    ...
    agendas: list[ExtractedAgenda] = dspy.OutputField(
        desc="Structured agendas. Ensure evidence_refs are included."
    )
...
class TopologyExtractionSignature(dspy.Signature):
    """
...
class RelationshipInferenceSignature(dspy.Signature):
    """
...
class SectionClassifierSignature(dspy.Signature):
    """
...
class SectionClassifierModule(_AnalyzerModule):
    ...
    _role = ModelRole.LIGHT  # Ollama — structured classification, small input per batch

...
_NODE_DEFAULT_ROLES: dict[str, ModelRole] = {
    ...
    # 2026-07-22: analyzer defaults to HEAVY. Every analyzer module is part of
    # the ingestion pipeline (extraction / classification / summarisation), not
    # chat-time work — those payloads are large and quality-critical. Defaulting
    # to STANDARD would route to a MiniMax-M3 standard-tier provider, which is
    # still remote, but HEAVY is the right complexity tier for the analyzer.
    # Individual modules may override (the chat-time SceneLoop calls also live
    # under analyzer-style nodes and pick HEAVY here too).
    "analyzer": ModelRole.HEAVY,
    ...
    "character_creation": ModelRole.LIGHT,
    "text_matching": ModelRole.LIGHT,
}


def _load_node_roles_from_env() -> dict[str, ModelRole]:
    """Return the per-node role map, optionally overridden via env.

    Reads ``MONITOR_NODE_ROLES`` as a JSON object mapping node name
    (canonical or any-case) to a ``ModelRole`` value
    (``light`` / ``standard`` / ``heavy``). Invalid JSON, invalid
    role values, and unknown node names fall back to the canonical
    ``_NODE_DEFAULT_ROLES`` map. Used by tests and operator overrides.

    2026-07-22: previously the role map was read directly off the
    module-level ``_NODE_DEFAULT_ROLES`` constant. Operators had no
    way to override (e.g. for a one-off cheaper routing experiment
    or a long-form benchmark) without editing source. This is the
    escape hatch — env-driven, no restart needed beyond the next
    import / LLM call.
    """
    out: dict[str, ModelRole] = dict(_NODE_DEFAULT_ROLES)
    ...
    if not isinstance(overrides, dict):
        return out
    for node_name, role_value in overrides.items():
        if not isinstance(node_name, str):
            continue
        canonical = normalize_node_name(node_name)
        try:
            role = ModelRole(str(role_value).strip().lower())
        except ValueError:
            # Unknown role name — keep the default.
            continue
        out[canonical] = role
    return out
...
def default_role_for_node(node_name: str) -> ModelRole:
    """Return the role for ``node_name``.

    Honors ``MONITOR_NODE_ROLES`` env overrides via
    :func:`_load_node_roles_from_env`. Falls back to ``STANDARD``
    for unknown nodes.
    """
    normalized = normalize_node_name(node_name)
    return _load_node_roles_from_env().get(normalized, ModelRole.STANDARD)
...
def _load_keywords(env_var: str, defaults: set[str]) -> set[str]:
    """Read a comma-separated keyword set from env, falling back to defaults.

    Empty / missing env returns the defaults. Whitespace is stripped and
    values are lowercased so ``"ATTACK,KILL,Death"`` matches the
    lowercased keyword scan in :func:`resolve_dynamic_role`.
    """
...
def _get_high_intensity_keywords() -> set[str]:
    """Return the high-intensity keyword set, honoring MONITOR_HIGH_INTENSITY_KEYWORDS."""
    return _load_keywords("MONITOR_HIGH_INTENSITY_KEYWORDS", _HIGH_INTENSITY_KEYWORDS)


def _get_moderate_intensity_keywords() -> set[str]:
    """Return the moderate-intensity keyword set, honoring MONITOR_MODERATE_INTENSITY_KEYWORDS."""
    return _load_keywords("MONITOR_MODERATE_INTENSITY_KEYWORDS", _MODERATE_INTENSITY_KEYWORDS)
...
def resolve_dynamic_role(
    ...
    *,
    ...
    if resolution:
        resolution_text = str(resolution.get("success_level", "")) + " " + str(resolution.get("description", ""))
...
_bg_postgres: PostgresClient | None = None
_bg_postgres_lock = threading.RLock()
_bg_registry: LLMRegistry | None = None
...
async def _resolve_client(node_name: str, role: ModelRole | None = None) -> LLMClient:
...
def describe_dspy_target(
    ...
    normalized = normalize_node_name(node_name)
    # Roles cross config boundaries as raw strings; coerce before the `.value`
    # reads below — the except branch reads it too, and an AttributeError
    # raised from an error handler takes down the whole calling pipeline.
    resolved_role = ModelRole.coerce(role) or default_role_for_node(normalized)
    try:
...
def _model_string_for_dspy(client: LLMClient) -> str:
    if client.provider in (LLMProviderType.ANTHROPIC, LLMProviderType.MINIMAX):
        ...
        return client.model if client.model.startswith("anthropic/") else f"anthropic/{client.model}"
    if "/" in client.model:
        return client.model
...
def get_dspy_lm(
    ...
    *,
    ...
    elif client.provider is LLMProviderType.MINIMAX:
        # MiniMax serves an Anthropic-compatible endpoint via a custom base_url,
        # and its DSPy model string is prefixed "anthropic/" for litellm routing
        # (see _model_string_for_dspy below). Without an explicit api_key here,
        # litellm's anthropic handler silently falls back to whatever
        # ANTHROPIC_API_KEY happens to be in the process environment — i.e. a
        # "MiniMax" call quietly bills the real Anthropic account instead.
        # (Suspected cause of the 2026-07-22 Settlers-ingest rate-limit kill:
        # job died on an AnthropicException even though HEAVY routes to
        # MiniMax, not Anthropic, in llm_providers.) Fail loudly instead.
        raise RuntimeError(
            f"MiniMax provider '{client.model}' for node '{node_name}' has no "
            "resolved API key (set llm_providers.api_key or MINIMAX_API_KEY) — "
            "refusing to let litellm fall through to the ambient ANTHROPIC_API_KEY."
        )
    elif client.provider is not LLMProviderType.ANTHROPIC:
        kwargs.setdefault("api_key", "not-needed")
    if client.base_url:
        kwargs["api_base"] = client.base_url
    # Per-call timeout: prevents a single hung LLM request from stalling the
    # pipeline. Configurable via MONITOR_LLM_TIMEOUT env var (seconds);
    # default 120s (2 min). MUST be the "timeout" kwarg -- litellm.completion()
    # has no "request_timeout" parameter (that name is only read as a
    # MODULE-LEVEL global, litellm.request_timeout, as a fallback default,
    # never as a per-call kwarg). dspy.LM stores **kwargs verbatim and splats
    # them straight into litellm.completion(**self.kwargs), so the wrong key
    # here is silently absorbed and never reaches the actual HTTP client.
    # Live bug 2026-07-22: with the wrong key, this timeout never fired, and
    # a single stuck LLM call hung an ingestion job for 50+ minutes with zero
    # CPU and zero retry activity -- the outer asyncio.wait_for(...) wrapper
    # in llm_execution.py can stop *waiting* on a blocked thread but cannot
    # kill it, so the only real protection is litellm's own client-side
    # (httpx) timeout actually being armed.
    ...
    class CachedDSPyLM(dspy.LM):
        ...
        def __call__(self, prompt=None, messages=None, **call_kwargs):
            ...
            if cb and messages and self.node_name == "narrator":
                ...
                try:
                    response = litellm.completion(model=self.model, messages=messages, stream=True, **merged_kwargs)
                    ...
                    def _emit(kind: str, data):
                        ...
                        try:
                            ...
                            return
                        try:
                            if (kind in ("narrative_token",) and data) or (kind == "thinking_chunk" and data):

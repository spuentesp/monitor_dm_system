...
from monitor_data.schemas.knowledge_packs import ExtractedEntityArchetype, ExtractedRandomTable
...
class NPCSceneGeneratorSignature(dspy.Signature):  # type: ignore[misc]
    """
    ...
    agendas: list[ExtractedAgenda] = dspy.InputField(desc="Extracted agendas from the source (may be empty)")
    ...
    npc_scene_output: NPCSceneGeneratorOutput = dspy.OutputField(desc="Generated NPC profiles and scene prompts")


class ScenePromptSignature(dspy.Signature):  # type: ignore[misc]
    """
    ...
    entry_value: str = dspy.InputField(desc="The specific table entry value (e.g. '2d4 RatCatchers')")
...
class _GeneratorModule(dspy.Module):  # type: ignore[misc]

...
class ContradictionSignature(dspy.Signature):  # type: ignore[misc]
    ...
    has_contradiction = dspy.OutputField(desc="Boolean: True if the new fact contradicts the context, False otherwise.")
    explanation = dspy.OutputField(
        desc="If has_contradiction is True, explain why. If False, explain why it's consistent."
    )


class ContradictionModule(dspy.Module):  # type: ignore[misc]
    """
    ...
    def forward(self, context: str, new_fact: str) -> dict[str, Any]:
...
class BatchContradictionSignature(dspy.Signature):  # type: ignore[misc]
    ...
    new_facts = dspy.InputField(desc="A numbered list of candidate new facts, one per line, e.g. '1. ...\\n2. ...'.")
...
class BatchContradictionModule(dspy.Module):  # type: ignore[misc]
    """
    ...
    def forward(self, context: str, new_facts: str) -> dict[int, str]:

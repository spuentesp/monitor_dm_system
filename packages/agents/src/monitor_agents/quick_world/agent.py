...
class QuickWorldBuilder:
    ...
    async def build(
        ...
        *,
        ...
        lore_facts = [str(x).strip() for x in _coerce_json_list(blueprint.get("lore_json")) if str(x).strip()][:6]

        mv_id, universe_id = await anyio.to_thread.run_sync(self._create_world, world_name, world_description, genre)
    ...
    async def _generate(self, *, seed: str, genre: str, tone: str) -> dict[str, Any]:
        from monitor_agents.quick_world.quick_world import QuickWorldModule
    ...
    def _create_world(world_name: str, description: str, genre: str) -> tuple[UUID, UUID]:
        ...
        for sys in getattr(systems_res, "systems", systems_res):
            if sys.name == "Mistlands Core":  # type: ignore[union-attr]
                mistlands_id = sys.id  # type: ignore[union-attr]
                break
    ...
    async def _canonize(
        ...
        *,
        ...
        lore = [ExtractedLoreFact(statement=stmt[:1000], source_ref="quick-world") for stmt in lore_facts]

...
__all__ = ["AnalysisRunResult", "Analyzer"]
...
class _SectionBundle:
    ...
    def __init__(
        ...
        *,
        ...
        table_sections: list[_SectionDigest] = None,  # type: ignore[assignment]
        tone_sections: list[_SectionDigest] = None,  # type: ignore[assignment]
        character_profile_sections: list[_SectionDigest] = None,  # type: ignore[assignment]
        generation_template_sections: list[_SectionDigest] = None,  # type: ignore[assignment]
        plot_thread_sections: list[_SectionDigest] = None,  # type: ignore[assignment]
    ) -> None:
...
class Analyzer(BaseAgent):
    ...
    def __init__(self, agent_id: str = "analyzer-1") -> None:
        ...
        self._llm_runner = LLMTaskRunner(agent_name=self.agent_type, timeout_s=self._LLM_CALL_TIMEOUT)
        self._active_job_id: UUID | None = None
        self._active_execution_summary: LLMExecutionSummary | None = None
        ...
        # [G-2](b) Module-intro extractor — only invoked for ADVENTURE_MODULE
        # packs (gate enforced in ``_extract_module_intro`` below).
        self._module_intro_extractor = ModuleIntroExtractionModule()
    ...
    async def _call_module(
        ...
        *,
        ...
        summary: LLMExecutionSummary | None = None,
        **kwargs: Any,
    ) -> Any:
        ...
        module_role = ModelRole.coerce(getattr(module, "_role", None)) or self._default_model_role()
        # 2026-07-22: ingestion-stage analysis must always use HEAVY. Any
        # module called through the analyzer pipeline gets escalated from
        # LIGHT/STANDARD → HEAVY so local-ollama fallback never blocks a real
        # ingest on a slow qwen2.5 model. This is the defense-in-depth layer
        # on top of (a) the explicit `_role = HEAVY` on individual modules and
        # (b) the analyzer node-level default in `_NODE_DEFAULT_ROLES`.
        if module_role != ModelRole.HEAVY:
            module_role = ModelRole.HEAVY
        ...
        async def _run() -> Any:
            return await self._llm_runner.run_blocking(
                ...
                on_attempt=lambda attempt: self._record_llm_attempt(active_job_id, active_summary, attempt),
            )
    ...
    def _record_llm_attempt(
        self,
        job_id: UUID | None,
        summary: LLMExecutionSummary | None,
        ...
        if attempt.status == LLMAttemptStatus.BACKING_OFF:
            ...
            if attempt.backoff_ms:
                next_retry_at = datetime.now(UTC) + timedelta(milliseconds=attempt.backoff_ms)
    ...
    def _format_attempt_message(attempt: LLMAttemptRecord) -> str:
        ...
        if attempt.status == LLMAttemptStatus.FAILED_NON_RETRYABLE:
            return f"Provider blocked during {attempt.stage}{batch_label}: {error_label} ({provider_label})"
        if attempt.status == LLMAttemptStatus.CANCELLED:
            return f"LLM work cancelled during {attempt.stage}{batch_label} ({provider_label})"
        return f"LLM batch failed during {attempt.stage}{batch_label}: {error_label} ({provider_label})"
    ...
    async def analyze_source(
        ...
        # [G-2](b) Detect the verbatim authored intro for ADVENTURE_MODULE
        # packs. Runs AFTER every other extraction pass (we don't want the
        # LLM to depend on partial state). Returns ``None`` when there's
        # no authored intro; empty string when the pack isn't an adventure
        # module; ``>40 char str`` otherwise.
        intro_text = await self._extract_module_intro(
            pack_type=pack_type,
            source_name=source_name,
            sections=sections,
        )
        ...
        *,
        ...
        if include_content:
            ...
            generation_template_sections = [
                s
                for s in content_sections
                if s.semantic_category in _CATEGORIES_GENERATION_TEMPLATES or "generation" in s.section_path.lower()
            ]
            ...
            plot_thread_sections = [
                s
                for s in content_sections
                if any(kw in s.text.lower() or kw in s.section_path.lower() for kw in _plot_keywords)
            ]
    ...
    async def _classify_and_build_profile(
        ...
        include_lore: bool,
    ) -> tuple[list[_SectionDigest], EmbeddedSourceProfile | None, str]:
        ...
        source_profile: EmbeddedSourceProfile | None = None
        source_profile_context = ""

        if include_content and sections.content_sections:
            filtered_sections = await self._classify_and_filter_sections(sections.content_sections, source_name)
            self._update_job(
                job_id,
                progress=0.2,
                activity_log=[
        ...
        profile_sections = (
            filtered_sections or sections.content_sections or sections.rule_sections or sections.charsheet_sections
        )
    ...
    async def _detect_game_system_phase(
        ...
        game_chunks: list[str],
        include_game_system: bool,
        source_profile: EmbeddedSourceProfile | None,
        ...
        if include_game_system:
            ...
            if detected_system_name and source_profile is not None and not source_profile.system_name:
    ...
    async def _extract_and_enrich_entities(
        ...
        if synthesized_count:
            self._update_job(
                job_id,
                progress=0.59,
                activity_log=[f"Container synthesis complete: {synthesized_count} named container entities added"],
            )

        entities, heuristic_relationships, social_synth_count = self._materialize_social_entities_and_relationships(
            entities,
            source_name,
        )
        ...
        hierarchy_relationships = derive_hierarchy_relationships(entities) if include_entities else []
        ...
        self._update_job(
            job_id,
            progress=0.65,
            activity_log=[f"Relationship inference complete: {len(relationship_facts)} relationship(s) inferred"],
        )
    ...
    async def _extract_game_subsystems(
        ...
        sections: _SectionBundle,
        is_game_system: bool,
        detected_system_name: str | None,
        include_rules: bool,
        source_profile_context: str,
        known_entities: list[ExtractedEntityArchetype] | None = None,
        ...
        self._update_job(
            job_id,
            progress=0.88,
            activity_log=[f"Character profile pass complete: {len(character_profiles)} profiles extracted"],
        )
        ...
        if include_rules and n_steps > 0:
            ...
            entities_summary = (
                ...
                f"Attributes: {', '.join([a.name for a in charsheet_data.get('attributes', []) or []])}"
            )
            ...
            self._update_job(
                job_id,
                progress=0.885,
                activity_log=[f"Creation logic synthesis complete: {len(creation_logic)} actionable steps generated"],
            )
        ...
        self._update_job(
            job_id,
            activity_log=[f"NPC pass complete: {n_blocks} stat blocks, creation_rules={'yes' if has_rules else 'no'}"],
        )
        ...
        if include_generation_templates:
            ...
            statblock_list = [s.name for s in npc_data.get("stat_blocks", []) or []]
        ...
        self._update_job(
            job_id,
            progress=0.89,
            activity_log=[f"Generation template pass complete: {len(generation_templates)} templates extracted"],
        )
    ...
    async def _extract_module_intro(
        self,
        pack_type: KnowledgePackType,
        source_name: str,
        sections: _SectionBundle,
    ) -> str | None:
        """[G-2](b) Detect the verbatim read-aloud intro of an adventure module.

        Returns the intro string (>40 chars), an empty string when the source
        is not an ``ADVENTURE_MODULE`` pack, or ``None`` when extraction
        fails (warned-and-continues per the plan). The 40-char floor
        prevents a one-line quote from short-circuiting the system's cold
        open; the empty-when-not-adventure gate keeps the HEAVY LLM call
        off rulebooks/settings.
        """
        if pack_type != KnowledgePackType.ADVENTURE_MODULE:
            return ""

        sections_ctx = self._format_first_n_sections(sections, n=6)
        if not sections_ctx:
            return None
        ...
        try:
            ...
            logger.warning(
                "analyzer._extract_module_intro dspy_runtime import failed for '%s': %s",
                source_name,
                exc,
            )
            return None
        ...
        try:
            with dspy_context_for("analyzer", ModelRole.HEAVY):
                result = self._module_intro_extractor(
                    sections_context=sections_ctx,
                    source_name=source_name,
                )
        except Exception as exc:
            logger.warning(
                "analyzer._extract_module_intro LLM call failed for '%s': %s",
                source_name,
                exc,
            )
            return None

        intro = (getattr(result, "intro_text", "") or "").strip()
        if len(intro) <= 40:
            return None
        return intro

    @staticmethod
    def _format_first_n_sections(sections: _SectionBundle, n: int = 6) -> str:
        """Format the first ``n`` page-ordered sections as LLM input.

        Helper for ``_extract_module_intro``. ``_SectionBundle`` already
        orders by ``(min_page, min_chunk_index)`` so ``content_sections[:n]``
        is the first page-ordered window. Each section's ``section_path``
        is its " > "-joined heading; ``text`` is the combined chunk content
        (per ``format_section_context`` in ``_sections.py``).
        """
        content = getattr(sections, "content_sections", None) or []
        if not content:
            return ""
        rendered: list[str] = []
        for s in content[:n]:
            heading = s.section_path or "General"
    ...
    async def _assemble_and_finalize(
        ...
        mongodb_update_knowledge_pack(
            ...
            KnowledgePackUpdate(
                ...
                # [G-2](b) Authored module intro — set in ``analyze_source``
                # via ``_extract_module_intro``. ``None`` here is the
                # no-intro case (rulebooks, sources w/o authored intro).
                intro_text=intro_text,
            ),
        )
        ...
        if auto_apply and multiverse_id is not None:
            try:
                ...
                self._update_job(
                    ...
                    activity_log=[f"Auto-apply complete: {proposals} proposals generated for CanonKeeper"],
                )
            except Exception as apply_exc:
                logger.warning("Auto-apply failed for pack %s (pack is still usable): %s", pack_id, apply_exc)
                self._update_job(
                    job_id,
                    progress=0.95,
        ...
        result = AnalysisRunResult(
            ...
            total_attempts=execution_summary.total_attempts,
            ...
            failed_sections=list(execution_summary.failed_sections),
        )
        ...
        try:
            cat_filter = Filter(
                ...
                must_not=[
                    FieldCondition(
                        key="artifact_type",
                        match=MatchAny(any=["chunk_summary", "section_summary", "source_mindscape"]),
                    )
                ],
            )
            offset = None
        ...
        if not sections and fallback_queries:
            ...
            for query in fallback_queries:
                try:
                    ...
                    vector = await default_retrieval_service().embed_query(query)
                    src_filter = Filter(
    ...
    async def _classify_and_filter_sections(
        ...
        for batch_num, batch_start in enumerate(range(0, len(sections), _CLASSIFIER_BATCH_SIZE), 1):
            ...
            try:
                ...
                batch_classifications = parse_section_classifications(result.classification_reasoning)
    ...
    async def _batched_extract_all(
        ...
        async def _run_batch(batch_idx: int, batch: list[_SectionDigest]) -> Any | None:
        ...
        batch_results = list(await asyncio.gather(*[_run_batch(i, b) for i, b in enumerate(batches)]))
    ...
    async def _synthesize_source_profile(
        ...
        representative_sections = "\n\n---SECTION---\n\n".join(
            self._format_section_context(section) for section in (sections[:6] or reference_sections[:6])
        )
        heading_paths = (
            "\n".join(
        ...
        reference_signals = (
            "\n\n---REFERENCE---\n\n".join(self._format_section_context(section) for section in reference_sections[:8])
            or "None"
        )

        synthesized: EmbeddedSourceProfile | None = None
        try:
            result = await self._call_module(
        ...
        async def _extract_one(section: _SectionDigest) -> list[dict[str, Any]]:
            ...
            async with sem:
                try:
                    ...
                    if self._active_execution_summary is not None:
                        self._active_execution_summary.record_section_failure(
    ...
    async def _extract_character_sheet(
        ...
        if not sections:
            logger.warning(
                "Character sheet extraction for '%s' got zero candidate sections — "
                "returning an all-empty result. This is a total miss, not a partial "
                "one; save_game_system will flag the resulting document needs_review.",
                system_name,
            )
            return {
                "attributes": [],
        ...
        parsed_results: list[dict[str, Any]] = [heuristic_extract_character_sheet_sections(focus_sections, system_name)]
        parsed_results.extend(await asyncio.gather(*[_extract_one(s) for s in focus_sections]))

        if len(focus_sections) > 1:
            combined_context = "\n\n---SECTION---\n\n".join(
                self._format_section_context(section) for section in focus_sections[:_SYSTEM_SCHEMA_SECTION_LIMIT]
            )
            try:
    ...
    async def _extract_creation_logic(
        ...
        # Convert steps to a readable prompt string. Production callers pass
        # DSPy CreationStep Pydantic models (attribute access); tests and
        # some legacy ingest paths pass plain dicts. Accept either shape.
        def _get(s: Any, key: str, default: str = "") -> str:
            if isinstance(s, dict):
                return s.get(key, default)  # type: ignore[no-any-return]
            return getattr(s, key, default)

        steps_str = "\n".join(
            f"STEP {_get(s, 'step_number', '?')}: {_get(s, 'title', '')} - {_get(s, 'instructions', '')}" for s in steps
        )
    ...
    async def _extract_npc_data(
        ...
        sections: list[_SectionDigest],
        system_name: str,
        core_mechanic_hint: dict[str, Any] | None = None,
        source_profile_context: str = "",
    ) -> dict[str, Any]:
        ...
        # CoreMechanic is a Pydantic model returned by DSPy, not a dict.
        cm_type = getattr(cm_hint, "type", None) or "unknown"
        cm_success = getattr(cm_hint, "success_type", None) or "unknown"
        system_context = f"Core mechanic type: {cm_type}, success_type: {cm_success}"
    ...
    async def _extract_random_tables(
        ...
        async def _run_batch(batch: list[_SectionDigest]) -> list[ExtractedRandomTable]:
            combined_context = "\n\n---SECTION---\n\n".join(self._format_section_context(section) for section in batch)
            async with sem:
                try:
                    result = await self._call_module(
    ...
    async def _infer_relationships(
        ...
        parent_names_in_use: set[str] = {e.parent_entity_name.lower().strip() for e in entities if e.parent_entity_name}
        ...
        for container in real_containers:
            ...
            batches.extend(
                family[start : start + self._REL_BATCH_SIZE] for start in range(0, len(family), self._REL_BATCH_SIZE)
            )
        ...
        cross_family = [
            e for e in real_containers + orphans if e.entity_type in ("faction", "location", "organization", "concept")
        ]
        batches.extend(
        ...
        async def _run_batch(batch: list[ExtractedEntityArchetype]) -> list[ExtractedRelationship]:
            ...
            async with sem:
                try:
                    ...
                    logger.warning("Relationship inference batch failed for '%s': %s", source_name, exc)
                    return []
    ...
    def _update_job(self, job_id: UUID, **kwargs: Any) -> None:
        ...
        try:
            ...
            if "status" in kwargs:
                logger.warning("Failed to update job %s status to %s: %s", job_id, kwargs["status"], exc)
            else:

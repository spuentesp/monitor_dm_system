...
async def test_progression_loop_finalize():
    ...
    with patch("monitor_agents.canonkeeper.CanonKeeper.create_fact") as mock_create_fact:
        ...
        state = ProgressionState(
            entity_id=e_id, universe_id=u_id, selected_upgrades=[{"id": "str_up", "stat": "STR"}]
        )
...
class TestProgressionLoopHelpers:
    ...
    async def test_load_progression_options_with_xp(self):
        ...
        state = ProgressionState(
            entity_id=uuid4(), universe_id=uuid4(), available_xp=50, selected_upgrades=[]
        )
    ...
    async def test_load_progression_options_returns_upgrades(self):
        ...
        state = ProgressionState(
            entity_id=uuid4(), universe_id=uuid4(), available_xp=100, selected_upgrades=[]
        )
        result = await load_progression_options(state)
        assert "available_upgrades" in result

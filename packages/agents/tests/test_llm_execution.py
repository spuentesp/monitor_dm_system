...
async def test_task_runner_tolerates_attempt_ledger_failures(monkeypatch):
    monkeypatch.setattr("monitor_agents.llm_execution.get_postgres_client", lambda: _FailingPostgres())
    monkeypatch.setattr(
        "monitor_agents.llm_execution.describe_dspy_target",
        lambda *_args, **_kwargs: {"provider": "github-models-default", "model": "gpt-4.1-mini"},
    )
...
def test_compute_backoff_increases_with_attempt_no():
    ...
    runner = LLMTaskRunner(agent_name="Analyzer", max_attempts=3, base_delay_s=1.0, max_delay_s=60.0, timeout_s=10)

    backoff_1 = runner._compute_backoff(
        attempt_no=1,
        error_info=LLMErrorInfo(error_class=LLMErrorClass.UNKNOWN, retryable=True, message="error1"),
    )
    backoff_2 = runner._compute_backoff(
        attempt_no=2,
        error_info=LLMErrorInfo(error_class=LLMErrorClass.UNKNOWN, retryable=True, message="error2"),
    )
    backoff_3 = runner._compute_backoff(
        attempt_no=3,
        error_info=LLMErrorInfo(error_class=LLMErrorClass.UNKNOWN, retryable=True, message="error3"),
    )
...
def test_compute_backoff_respects_max_delay():
    ...
    runner = LLMTaskRunner(agent_name="Analyzer", max_attempts=3, base_delay_s=10.0, max_delay_s=30.0, timeout_s=10)

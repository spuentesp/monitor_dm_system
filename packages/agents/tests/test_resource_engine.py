...
class TestDetectSpend:
    ...
    def test_roll_mode_not_spend(self, system_name):
        ...
        assert "advantage" not in spend_intents, (
            f"attack shouldn't trigger advantage: {system_name}"
        )
...
class TestApplyEarnVampireV5:
    def test_hunger_on_failure(self):
        ...
        hunger_deltas = [
            d
            for d in deltas
            if "hngr" in d.resource_key.lower() or "hunger" in d.resource_name.lower()
        ]
...
class TestResourceEngineFull:
    ...
    def test_vampire_hunger_escalation(self):
        ...
        assert len(frenzy_events) >= 1, (
            f"Expected >= 1 frenzy event, got {len(frenzy_events)}: {events}"
        )
        # at_or_above 5 is the highest threshold crossed
        max_event = max(frenzy_events, key=lambda e: e.threshold_value)
        assert max_event.threshold_value == 5, (
            f"Expected threshold_value=5, got {max_event.threshold_value}"
        )

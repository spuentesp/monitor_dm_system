...
class TestSession0CharacterCreationFlow:
    ...
    def test_character_creation_provides_stat_rolling_options(self):
        ...
        stats_step = next(
            (
                s
                for s in steps
                if "roll" in (s.get("step_type") or "").lower() or "stat" in (s.get("title") or "").lower()
            ),
            None,
        )
    ...
    def test_character_creation_includes_backstory_question(self):
        ...
        assert free_text_steps, "Character creation must include at least one open-ended backstory/motivation question."
...
class TestCausalityCheckStructure:
    ...
    def test_causality_verdict_has_required_fields(self):
        ...
        from monitor_agents.causality_check import CausalityAction, CausalityVerdict
...
class TestCausalityCheckBehavior:
    ...
    async def test_narrative_mode_always_accepts(self):
        ...
        from monitor_agents.causality_check import CausalityAction, check_causality
    ...
    async def test_dice_mode_calls_llm(self):
        ...
        from monitor_agents.causality_check import (
            CausalityAction,
            CausalityVerdict,
            check_causality,
        )

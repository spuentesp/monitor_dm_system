...
class TestDescriptionFilling:
    def test_short_description_is_replaced_by_evidence_snippet(self):
        ...
        result = enrich_entities_from_evidence([e], {"Dragon": ["An ancient serpent of immense power and cunning."]})
        assert "ancient serpent" in result[0].description

    def test_empty_description_is_filled(self):
        e = _entity("Wizard", "")
        result = enrich_entities_from_evidence([e], {"Wizard": ["A master of arcane arts who bends reality."]})
    ...
    def test_exactly_39_chars_is_replaced(self):
        e = _entity("Elf", "X" * 39)
        result = enrich_entities_from_evidence([e], {"Elf": ["Graceful immortal beings of the forest realm."]})
        assert "Graceful" in result[0].description

    def test_exactly_40_chars_is_not_replaced(self):
        desc = "Y" * 40
        e = _entity("Dwarf", desc)
        result = enrich_entities_from_evidence([e], {"Dwarf": ["Stout mountain folk with great endurance."]})
        assert result[0].description == desc

    def test_seed_uses_only_first_snippet(self):
        e = _entity("Rogue", "short")
        result = enrich_entities_from_evidence([e], {"Rogue": ["First snippet content.", "Second snippet ignored."]})
...
class TestPropertyExtraction:
    def test_key_value_colon_pattern_extracted(self):
        e = _entity("NPC")
        result = enrich_entities_from_evidence([e], {"NPC": ["Habitat: deep forest"]})
    ...
    def test_key_value_em_dash_pattern_extracted(self):
        e = _entity("NPC")
        result = enrich_entities_from_evidence([e], {"NPC": ["Alignment — chaotic neutral"]})
        assert "alignment" in result[0].properties

    def test_key_spaces_normalized_to_underscores(self):
        e = _entity("NPC")
        result = enrich_entities_from_evidence([e], {"NPC": ["Hit Points: 42"]})
        assert "hit_points" in result[0].properties

    def test_key_lowercased(self):
        e = _entity("NPC")
        result = enrich_entities_from_evidence([e], {"NPC": ["Strength: 18"]})
    ...
    def test_value_truncated_to_120_chars(self):
        e = _entity("NPC")
        long_val = "x" * 200
        result = enrich_entities_from_evidence([e], {"NPC": [f"Description: {long_val}"]})
    ...
    def test_at_8_properties_no_more_added(self):
        ...
        result = enrich_entities_from_evidence([e], {"NPC": ["Extra: should not be added"]})
        assert len(result[0].properties) == 8
...
class TestModelCopyBehavior:
    def test_no_change_returns_same_object(self):
        ...
        result = enrich_entities_from_evidence([e], {"NPC": ["Habitat: mountains"]})
        # description unchanged (>=40), existing prop not overwritten → no copy
        assert result[0] is e

    def test_description_change_creates_new_object(self):
        e = _entity("NPC", "short")
        result = enrich_entities_from_evidence([e], {"NPC": ["A much longer description for this NPC entity."]})
        assert result[0] is not e

    def test_new_property_creates_new_object(self):
        long_desc = "A" * 50
        e = ExtractedEntityArchetype(name="NPC", entity_type="character", description=long_desc, properties={})

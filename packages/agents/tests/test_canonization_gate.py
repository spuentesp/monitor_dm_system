...
def _make_proposal(
    ...
    Defaults to change_type="fact" (not "entity") -- entity proposals are
    now fully decided by CanonKeeper's deterministic promotion gate
    (canonkeeper_support.gate_entity_proposals) before the LLM pipeline
    ever runs, so tests exercising the LLM policy/reasoning path need a
    non-ENTITY proposal to actually reach it. Tests that specifically
    target the promotion gate pass change_type="entity" explicitly.
    """
    return {
...
class TestCanonizationGate:
    ...
    async def test_accept_valid_proposal(self, mock_call_tool, mock_call_llm, passing_dspy):
        """Valid non-entity proposal: passes policy, gets reasoning, is accepted + committed."""
        from monitor_agents.canonkeeper.agent import CanonKeeper

        proposal = _make_proposal(change_type="fact")
        mock_call_llm.return_value = _make_verdict(
            proposal["proposal_id"], CanonKeeperDecision.ACCEPTED, canon_node_type="fact"
        )
        ...
        assert "neo4j_create_fact" in committed

    @pytest.mark.asyncio
    async def test_anchor_tagged_entity_auto_accepted_by_gate(self, mock_call_tool, mock_call_llm):
        """Anchor-tagged entity: auto-accepted by the promotion gate, committed
        to Neo4j, WITHOUT ever calling the LLM pipeline."""
        from monitor_agents.canonkeeper.agent import CanonKeeper

        proposal = _make_proposal(change_type="entity", promotion_intent="anchor")
    ...
    @pytest.mark.asyncio
    async def test_untagged_entity_below_threshold_gc_rejected_by_gate(self, mock_call_tool, mock_call_llm):
        """An entity proposal with no promotion signal and only 1 mention is
        garbage-collected by the gate -- never reaches Neo4j or the LLM."""
        from monitor_agents.canonkeeper.agent import CanonKeeper

        proposal = _make_proposal(change_type="entity", interaction_count=1)
    ...
    async def test_policy_gate_blocks_hard_violation(self, mock_call_tool, mock_call_llm):
        ...
        Uses a non-ENTITY proposal: entity proposals no longer reach the
        policy-check/reasoning pipeline at all (see
        test_untagged_entity_below_threshold_gc_rejected_by_gate).
        """
        violation = MagicMock()
        ...
        with patch(
            ...
            proposal = _make_proposal(change_type="fact", summary="Delete the Omniversal library")
    ...
    async def test_llm_reject_skips_neo4j_commit(self, mock_call_tool, mock_call_llm, passing_dspy):
        ...
        proposal = _make_proposal(change_type="fact")
        mock_call_llm.return_value = _make_verdict(
            proposal["proposal_id"], CanonKeeperDecision.REJECTED, canon_node_type="fact"
        )
    ...
    @pytest.mark.asyncio
    async def test_audit_recorded_for_every_verdict(self, mock_call_tool, mock_call_llm, passing_dspy):
        """Both accepted and rejected verdicts are always written to MongoDB."""
        from monitor_agents.canonkeeper.agent import CanonKeeper

        p1 = _make_proposal(change_type="fact", summary="New NPC")
        p2 = _make_proposal(change_type="fact", summary="Contradictory lore")
        mock_call_llm.side_effect = [
            _make_verdict(p1["proposal_id"], CanonKeeperDecision.ACCEPTED, canon_node_type="fact"),
            _make_verdict(p2["proposal_id"], CanonKeeperDecision.REJECTED, canon_node_type="fact"),
        ]

        await CanonKeeper().evaluate_proposals(uuid4(), [p1, p2])

        # mongodb_record_verdict must appear twice (one per proposal)
        audit_calls = [c for c in mock_call_tool.call_args_list if c.args[0] == "mongodb_record_verdict"]
        assert len(audit_calls) == 2

    @pytest.mark.asyncio
    async def test_batch_mixed_verdicts_correct_commits(self, mock_call_tool, mock_call_llm, passing_dspy):
        """Batch: gate-decided entity proposals + an LLM-evaluated fact
        proposal are all committed/skipped correctly in the same batch."""
        from monitor_agents.canonkeeper.agent import CanonKeeper

        p_entity = _make_proposal(change_type="entity", summary="New character", promotion_intent="anchor")
        p_reject = _make_proposal(change_type="entity", summary="Contradicts lore", interaction_count=1)
        p_fact = _make_proposal(change_type="fact", summary="New relationship")

        mock_call_llm.return_value = _make_verdict(
            p_fact["proposal_id"], CanonKeeperDecision.ACCEPTED, canon_node_type="fact"
        )
...
class TestCanonKeeperVerdictSchema:
    ...
    def test_accept_verdict_round_trips(self):
        ...
        assert CanonKeeperVerdict.model_validate(v.model_dump()).decision == CanonKeeperDecision.ACCEPTED

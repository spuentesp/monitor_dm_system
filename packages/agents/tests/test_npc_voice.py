...
def test_evaluate_triggers_caps_at_three() -> None:
    ...
    triggers2 = [
        {"condition": f"trigger{i}", "reaction": f"r{i}", "intensity": 0.5} for i in range(6)
    ]
...
def test_build_proposals_emotional_state_change(
    npc_id: UUID, sample_profile: Dict[str, Any]
) -> None:
...
def test_build_proposals_relationship_delta_positive(
    npc_id: UUID, sample_profile: Dict[str, Any]
) -> None:
...
def test_build_proposals_malformed_delta_ignored(
    npc_id: UUID, sample_profile: Dict[str, Any]
) -> None:
...
def test_build_insight_proposals_update_profile(npc_id: UUID) -> None:
    agent = NPCVoice()
    proposals = agent._build_insight_proposals(
        npc_id, "update_profile:speech_style=slow and deliberate"
    )
...
def test_build_insight_proposals_new_fact(npc_id: UUID) -> None:
    agent = NPCVoice()
    proposals = agent._build_insight_proposals(
        npc_id, "new_fact:Aldric secretly admires the PC's courage"
    )
    assert len(proposals) == 1
...
async def test_respond_actor_full_flow(
    ...
    mock_prediction = Mock()
    mock_prediction.actor_response = (
        "As Aldric, I would feel deeply betrayed. My loyalty is my identity."
    )

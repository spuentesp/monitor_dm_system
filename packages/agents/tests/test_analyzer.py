...
from monitor_agents.prompts.analyzer import BatchedExtractionModule
    ...
    parsed = heuristic_extract_character_sheet_sections(
        [
            ...
            SectionDigest(
                ...
                text=(
                    "ENEMIES IN COMBAT Stats-wise, an enemy has a defense rating (DR), "
                    "hit points (HP), morale (ML)."
                ),
                min_page=52,
                min_chunk_index=1,
            ),
...
def test_batched_extraction_module_raises_max_tokens_for_large_outputs(monkeypatch):
    ...
    monkeypatch.setattr(
        "monitor_agents.prompts.analyzer.dspy_context_for",
        lambda node_name, role, max_tokens=None: (
            captured.update({"node_name": node_name, "role": role, "max_tokens": max_tokens})
            or _DummyContext()
        ),
    )
...
async def test_analyzer_persists_source_profile_data_when_synthesis_succeeds(monkeypatch):
    ...
    monkeypatch.setattr(
        "monitor_agents.analyzer._core.mongodb_update_knowledge_pack", _capture_update
    )
    ...
    async def fake_call_module(_module, **kwargs):
        ...
        if stage == "section_summary":
            return SimpleNamespace(
                summary="Summarizes the Iron Ring and its scavenger factions.", themes=["scarcity"]
            )
...
async def test_project_mindscape_to_qdrant_updates_payloads_and_upserts_summary_vectors(
    ...
    monkeypatch.setattr(
        "monitor_agents.analyzer._mindscape_projection.get_qdrant_client", lambda: fake_client
    )
    monkeypatch.setattr(
        "monitor_agents.analyzer._mindscape_projection.embed_batch",
...
def test_materialize_social_entities_adds_membership_and_multi_role_support():
    ...
    assert {"organization", "taxonomy_member", "multi_role"}.issubset(
        set(by_name["House Tremere"].entity_roles)
    )
    assert any(
        rel.from_entity == "House Tremere"
        and rel.rel_type == "member_of"
        and rel.to_entity == "Camarilla"
...
async def test_analyzer_reads_ingested_chunks_from_snippets(monkeypatch):
    ...
    async def fake_embed_text(_query):
        return [0.1] * 1536
    ...
    monkeypatch.setattr("monitor_agents.analyzer._core.embed_text", fake_embed_text)

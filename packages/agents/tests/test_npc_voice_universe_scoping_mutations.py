...
def _patch_mongo_character_lookup(npc_id: UUID, universe_id: UUID):
    ...
    """
    coll = MagicMock()
    coll.find_one.return_value = {"versions": [{"entity_id": str(npc_id), "universe_id": str(universe_id)}]}
    client = MagicMock()
...
def npc_data(universe_id, player_id) -> dict[str, Any]:
    return {
        ...
        "profile": {
            "triggers": [],
            "relationship_states": {},
            "relationship_states_by_universe": {str(universe_id): {str(player_id): {"trust": 0.1, "stance": "warm"}}},
...
class TestFormatEmotionalContext:
    def test_per_universe_read_wins_over_legacy(self, agent, npc_data, universe_id):
        ...
        result = agent._format_emotional_context(npc_data["profile"], universe_id=universe_id)
...
class TestBuildProposalsUniverseStamping:
    def test_state_change_stamps_universe_and_partition(self, agent, npc_data, universe_id, npc_id, player_id):
        ...
        assert state["content"]["profile_updates"]["current_emotional_state_by_universe"] == {
            str(universe_id): "warmer"
        }

    def test_state_change_no_universe_stamps_neither(self, agent, npc_data, npc_id, player_id):
        ...
        if state:
            ...
            assert "current_emotional_state_by_universe" not in state["content"]["profile_updates"]

    def test_prior_emotion_reads_per_universe_first(self, agent, npc_data, universe_id, npc_id, player_id):
    ...
    def test_player_id_equals_npc_id_skips_relationship_proposal(self, agent, npc_data, npc_id):
...
class TestBuildRelationshipSnapshot:
    def test_forwards_universe_id_to_internal_read(self, agent, npc_data, universe_id, player_id):
...
class TestUpdateWorkingSocialState:
    ...
    def test_universe_partition_written_when_provided(self, agent, npc_id, player_id, universe_id):
        ...
        params = _captured["mongodb_update_npc_profile"][-1]
        assert params["relationship_states_by_universe"] == {str(universe_id): {str(player_id): snap}}
        assert params["current_emotional_state_by_universe"] == {str(universe_id): "guarded"}
...
class TestRespondDirectThreadingMutations:
    def test_cross_incarnation_without_universe_threads_flag(self, agent, npc_data, npc_id, player_id, conv_id):
    ...
    def test_legacy_memory_write_when_no_universe(self, agent, npc_data, npc_id, player_id, conv_id):
        ...
        assert _captured["mongodb_create_memory"][-1]["universe_id"] == str(conversatory_universe)

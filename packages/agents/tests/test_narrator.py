...
class TestFormatResolution:
    ...
    def test_with_zero_roll_and_stat(self):
        result = self.narrator._format_resolution(
            {"success_level": "failure", "roll_total": 0, "stat": "DEX"}
        )
    ...
    def test_parts_joined_by_period_space(self):
        ...
        result = self.narrator._format_resolution(
            {"success_level": "success", "roll_total": 10, "stat": "CHA"}
        )
        assert ". " in result
    ...
    def test_propose_roll_no_dc(self):
        result = self.narrator._format_resolution(
            {"resolution_type": "propose_roll", "stat": "STR"}
        )
        assert result == "propose_roll:STR"

    def test_narrative_resolution_type(self):
        result = self.narrator._format_resolution(
            {"resolution_type": "narrative", "success_level": "success"}
        )
        assert result == "narrative"
...
class TestNarrateTurn:
    ...
    async def test_narrate_turn_injects_profile_context_and_tone_hints(self):
        ...
        pred = self._make_prediction(
            "The prince regards you over the rim of a crystal goblet.", "[]"
        )
        narrator = self._make_narrator(pred)
        ...
        assert (
            "Institution model: sect allegiance, prince-led domains"
            in call_kwargs["profile_context"]
        )
    ...
    async def test_narrate_turn_with_gm_profile(self):
        ...
        narrator._tone_resolver.resolve_from_profile.assert_called_once_with(
            gm_profile, fallback_tone="grim"
        )
    ...
    async def test_narrate_turn_injects_story_state(self):
        ...
        assert (
            "- Active Threads: Finding the lost sword, The traitor among us"
            in call_kwargs["profile_context"]
        )
...
class TestParseProposedChanges:
    ...
    def test_non_dict_items_filtered_out(self):
        ...
        raw = json.dumps(
            ["string_item", 42, {"change_type": "valid", "summary": "ok", "content": {}}]
        )
        result = Narrator._parse_proposed_changes(raw)
        assert len(result) == 1
    ...
    def test_multiple_proposals_preserved(self):
        proposals = [
            {"change_type": "entity_state_change", "summary": f"Change {i}", "content": {}}
            for i in range(5)
        ]

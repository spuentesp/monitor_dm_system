...
from monitor_agents.character_creator import card_draft as cd
...
class TestCardDrafterContract:
    ...
    def test_falls_back_to_safe_default_when_name_blank(self):
        out = _run(SimpleNamespace(name="  ", description="x", personality="x", first_message="x", gm_notes="x"))
        assert out["name"] == "New Character"

    def test_falls_back_to_user_provided_name_when_llm_leaves_blank(self):
        out = _run(
            SimpleNamespace(name="", description="x", personality="x", first_message="x", gm_notes="x"),
            given_name="Halvar",
        )
    ...
    def test_preserves_existing_description_when_llm_returns_empty(self):
        ...
        def fake(**_kw):
            return SimpleNamespace(name="Lin", description="", personality="", first_message="", gm_notes="")

...
from monitor_agents.indexer.agent import _UPSERT_BATCH_SIZE, Indexer
...
def _stub_retriever(embed_docs_mock):
    """A RetrievalService stub whose embed_docs is the given AsyncMock.

    The indexer now embeds via ``default_retrieval_service().embed_docs`` —
    the single embed owner — instead of calling embed_batch directly.
    """
    stub = MagicMock()
    stub.embed_docs = embed_docs_mock
    return stub
...
class TestIndexText:
    ...
    async def test_returns_chunk_count(self):
        ...
        with patch("monitor_agents.indexer.agent.ingest_text", return_value=chunks) as mock_ingest:
    ...
    async def test_custom_collection_forwarded(self):
        ...
        with patch("monitor_agents.indexer.agent.ingest_text", return_value=chunks):
    ...
    async def test_returns_zero_when_no_chunks(self):
        ...
        with patch("monitor_agents.indexer.agent.ingest_text", return_value=[]):
...
class TestIndexMarkdown:
    ...
    async def test_returns_chunk_count(self):
        ...
        with patch("monitor_agents.indexer.agent.ingest_markdown", return_value=chunks):
    ...
    async def test_returns_zero_on_empty_chunks(self):
        ...
        with patch("monitor_agents.indexer.agent.ingest_markdown", return_value=[]):
...
class TestIndexFile:
    ...
    async def test_image_format_routes_to_index_image(self):
        ...
        with patch("monitor_agents.indexer.agent.detect_format", return_value="image"):
    ...
    async def test_pdf_format_routes_to_structure_aware_path(self):
        ...
        agent._embed_and_upsert = AsyncMock()
        # 2026-07-22: _chunk_pdf_with_structure is async (parallel categoriser
        # gather) — must mock with AsyncMock.
        agent._chunk_pdf_with_structure = AsyncMock(return_value=chunks)

        with patch("monitor_agents.indexer.agent.detect_format", return_value="pdf"):
            result = await agent.index_file(b"%PDF", "rulebook.pdf")

        assert result == 2
        agent._chunk_pdf_with_structure.assert_awaited_once()
        agent._embed_and_upsert.assert_awaited_once_with(chunks, collection="snippets")
    ...
    async def test_returns_zero_when_ingest_file_empty(self):
        ...
        with (
            patch("monitor_agents.indexer.agent.detect_format", return_value="text"),
            patch("monitor_agents.indexer.agent.ingest_file", return_value=[]),
        ):
    ...
    async def test_source_name_defaults_to_filename_stem(self):
        ...
        with (
            patch("monitor_agents.indexer.agent.detect_format", return_value="text"),
            patch("monitor_agents.indexer.agent.ingest_file", return_value=chunks) as mock_ingest,
        ):
...
class TestEmbedAndUpsert:
    ...
    async def test_single_batch_calls_upsert_once(self):
        ...
        with (
            patch("monitor_agents.indexer.agent.get_qdrant_client", return_value=mock_wrapper),
            patch(
                "monitor_data.retrieval.default_retrieval_service",
                return_value=_stub_retriever(AsyncMock(return_value=fake_vectors)),
            ),
        ):
    ...
    async def test_multiple_batches_when_chunks_exceed_batch_size(self):
        ...
        with (
            ...
            patch(
                "monitor_data.retrieval.default_retrieval_service",
                return_value=_stub_retriever(AsyncMock(side_effect=embed_side_effects)),
            ),
        ):
            await agent._embed_and_upsert(chunks, collection="snippets")
    ...
    async def test_empty_chunks_skips_upsert(self):
        ...
        with (
            ...
            patch(
                "monitor_data.retrieval.default_retrieval_service",
                return_value=_stub_retriever(AsyncMock(return_value=[])),
            ),
        ):
...
class TestChunkPdfWithStructure:
    ...
    async def test_heading_path_appears_in_chunk_metadata(self):
        ...
        with (
            patch("monitor_agents.indexer.agent.extract_pdf_structure", return_value=[block]),
            patch("monitor_agents.indexer.agent.TagPool") as mock_tag_pool,
            patch("monitor_agents.indexer.agent.SectionCategorizationModule"),
            patch("monitor_agents.indexer.agent.chunk_text", return_value=[fake_chunk]),
        ):
            mock_tag_pool.classify_text.return_value = "combat_rules"

            chunks = await agent._chunk_pdf_with_structure(b"%PDF", "rulebook")
    ...
    async def test_section_categorization_module_not_called_when_tagpool_matches(self):
        ...
        with (
            ...
            patch("monitor_agents.indexer.agent.TagPool") as mock_tag_pool,
            patch(
                "monitor_agents.indexer.agent.SectionCategorizationModule",
            ...
            await agent._chunk_pdf_with_structure(b"%PDF", "grimoire")
    ...
    async def test_section_categorization_module_called_when_tagpool_returns_none(self):
        ...
        with (
            ...
            await agent._chunk_pdf_with_structure(b"%PDF", "sourcebook")
    ...
    async def test_section_depth_forwarded_to_chunk_metadata(self):
        ...
        with (
            ...
            patch("monitor_agents.indexer.agent.TagPool") as mock_tag_pool,
            patch("monitor_agents.indexer.agent.SectionCategorizationModule"),
            patch("monitor_agents.indexer.agent.chunk_text", return_value=[fake_chunk]) as mock_chunk_text,
        ):
            mock_tag_pool.classify_text.return_value = "game_mechanics"

            await agent._chunk_pdf_with_structure(b"%PDF", "rulebook")
    ...
    async def test_falls_back_to_ingest_pdf_when_no_sections(self):
        ...
        with (
            patch("monitor_agents.indexer.agent.extract_pdf_structure", return_value=[]),
            patch("monitor_agents.indexer.agent.ingest_pdf", return_value=fallback_chunks) as mock_ingest_pdf,
        ):
            result = await agent._chunk_pdf_with_structure(b"%PDF", "doc")

        assert result == fallback_chunks
        mock_ingest_pdf.assert_called_once_with(b"%PDF", "doc", metadata={})

    # ---------------------------------------------------------------------------
    # Parallel section categorisation (Gap 1 of ingest hardening plan, 2026-07-22)
    # ---------------------------------------------------------------------------

    @pytest.mark.asyncio
    async def test_categories_computed_in_parallel_for_many_sections(self):
        """200 sections with a per-block tagged mock should all be
        classified via the LLM-fallback path. This verifies the
        gather wiring covers every block (no off-by-one or early-exit
        regression) without flaking on real wall-clock."""
        agent = _make_indexer()
        blocks = [
            _make_section_block(
                heading_path=[f"Chapter {i}", "Body"],
                depth=1,
                text=f"Section {i} body.",
            )
            for i in range(200)
        ]
        # Track how many times the LLM fallback actually fires.
        call_count_box: list[int] = [0]
        mock_pred = MagicMock()
        mock_pred.category = "general"

        def counting_categorizer(**_):
            call_count_box[0] += 1
            return mock_pred

        mock_categorizer_instance = MagicMock(side_effect=counting_categorizer)

        fake_chunks = [_chunk("body")]
        with (
            patch(
                "monitor_agents.indexer.agent.extract_pdf_structure",
                return_value=blocks,
            ),
            patch("monitor_agents.indexer.agent.TagPool") as mock_tag_pool,
            patch(
            ...
            patch("monitor_agents.indexer.agent.chunk_text", return_value=fake_chunks),
        ):
            mock_tag_pool.classify_text.return_value = "general"

            chunks = await agent._chunk_pdf_with_structure(b"%PDF", "rulebook")

        # All 200 blocks forced through the LLM fallback (TagPool misses).
        assert call_count_box[0] == 200, f"expected 200 categoriser calls, got {call_count_box[0]}"
        # Each fake_chunks per block → 200 * 1 = 200 chunks.
        assert len(chunks) == 200

    @pytest.mark.asyncio
    async def test_categorizer_failure_short_circuits_to_general(self):
        """Once any single LLM call fails, remaining blocks skip the
        LLM and get the 'general' category. Failure semantics from the
        pre-refactor sequential version are preserved."""
        agent = _make_indexer()
        blocks = [_make_section_block(heading_path=[f"§{i}"], depth=0, text=f"body {i}") for i in range(5)]

        def explode(**_):
            raise RuntimeError("ollama down")

        mock_categorizer = MagicMock(side_effect=explode)
        ...
        with (
            ...
            patch("monitor_agents.indexer.agent.chunk_text", return_value=fake_chunks) as mock_chunk_text,
        ):
            mock_tag_pool.classify_text.return_value = "general"

            await agent._chunk_pdf_with_structure(b"%PDF", "broken")

        # All 5 blocks produce chunks — categorisation degrades to general.
        assert mock_chunk_text.call_count == 5
        for call in mock_chunk_text.call_args_list:
            metadata = call.kwargs["metadata"]
            assert metadata["semantic_category"] == "general"

...
class TestTrivialRouting:
    ...
    async def test_simple_speech_is_trivial(self):
        with _patch_gm(trivial_dialogue()):
            resolver = Resolver()
            result, _ = await resolver.resolve_turn("s1", 'I say "Hello"', _make_context())
        assert result["resolution_type"] == "trivial"
...
class TestForcedNarrative:
    ...
    async def test_high_stakes_declaration_pushback(self):
        ...
        with _patch_gm(forced_narrative_pushback(stat="Strength", dc=15)):
            resolver = Resolver()
            result, _ = await resolver.resolve_turn("s1", "I kill the boss", _make_context())
    ...
    async def test_deus_ex_machina_clarification(self):
        ...
        with _patch_gm(deus_ex_machina()):
            resolver = Resolver()
            result, _ = await resolver.resolve_turn("s1", "I find the key in my pocket", _make_context())
...
class TestOracleRoute:
    ...
    async def test_world_truth_question_routes_to_oracle(self):
        ...
        with _patch_gm(query_intent()):
            resolver = Resolver()
            result, _ = await resolver.resolve_turn("s1", "Is the door locked?", _make_context())
    ...
    async def test_query_picks_up_ooc_likelihood_marker(self):
        ...
        with _patch_gm(query_intent()):
            resolver = Resolver()
            result, _ = await resolver.resolve_turn("s1", "Is the guard still there? ((Likely))", _make_context())
        assert result["resolution_type"] == "oracle"
...
class TestLLMFallback:
    ...
    async def test_llm_failure_returns_safe_propose_roll(self):
        """If the LLM raises, the embeddings classifier decides instead.

        Here it returns propose_roll — the safe default where the player
        gets to choose. (With no LLM *and* no embeddings the turn would raise
        by design; this test stubs the classifier to model "embeddings still
        available".)
        """
        ...
        with (
            ...
            force_roll_necessity("propose_roll"),
        ):
            result, _ = await resolver.resolve_turn("s1", "I try to open the door", _make_context())
        # Should not crash; should return a structured verdict
        assert "resolution_type" in result
        # Classifier proposes a roll — never roll automatically
    ...
    async def test_llm_unavailable_does_not_crash_narrative_mode(self):
        ...
        resolver = Resolver()
        result, _ = await resolver.resolve_turn("s1", "anything", _make_context(), play_mode="narrative")
        assert result["resolution_type"] == "narrative"
...
class TestPendingRollInterception:
    @pytest.mark.asyncio
    async def test_pending_roll_auto_resolves_when_player_narrates_past(self):
        """If the previous turn left a pending roll, the next turn resolves it
        server-side using the proposed spec — no keyword gate, no pushback nag.
        The player narrating past the roll never blocks the fiction."""
        with _patch_gm(trivial_attempt()):
            resolver = Resolver()
            ctx = _make_context()
            ctx["pending_roll"] = {
            ...
            with patch("monitor_data.utils.dice.random.randint", return_value=10):
                result, _ = await resolver.resolve_turn("s1", "I push the door open.", ctx)
        assert result["resolution_type"] == "dice"
        assert result["resolution_type"] != "forced_narrative_pushback"
    ...
    async def test_pending_roll_passes_through_when_player_rolls(self):
        ...
        with _patch_gm(trivial_attempt()):
            ...
            with patch("monitor_data.utils.dice.random.randint", return_value=10):
                result, _ = await resolver.resolve_turn("s1", "I roll Strength (DC 12)", ctx)
...
# ===========================================================================
# All non-narrative / non-auto turns go through the LLM
# ===========================================================================


class TestAllTurnsCallLLM:
    """With the keyword narration pre-filter removed, every dice-mode turn
    that isn't the narrative/auto fast-path is decided by the LLM verdict."""

    @pytest.mark.asyncio
    async def test_action_calls_llm(self):
        from monitor_agents.gm_awareness import check_gm_awareness

        controlled = GMAwareness(
            intent_type=IntentType.ACTION,
            action_type=ActionType.COMBAT,
            roll_necessity=RollNecessity.CONTESTED,
            declares_outcome=False,
        ...
        assert verdict.roll_necessity == RollNecessity.CONTESTED
        assert verdict.suggested_stat == "STR"

    @pytest.mark.asyncio
    async def test_declarative_narration_also_calls_llm(self):
        """Character intros / atmosphere used to be short-circuited by the
        keyword heuristic; now the LLM handles them like any other turn."""
        from monitor_agents.gm_awareness import check_gm_awareness
        ...
        with patch(
            ...
            verdict = await check_gm_awareness(
                agent=None,
                user_input="I am Kael Draven, a void-born salvage engineer.",
                scene_context={"entities": [], "turns": []},
                play_mode="dice_game_system",
                established_facts=[],
                roll_mode="normal",

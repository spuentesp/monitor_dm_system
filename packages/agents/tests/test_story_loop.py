...
class TestInitStoryNode:
    ...
    async def test_init_creates_first_scene_when_none_exists(self):
        ...
        with (
            patch("monitor_agents.canonkeeper.CanonKeeper") as mock_ck_class,
            patch(
...
class TestFinalizeStoryNode:
    ...
    async def test_finalize_calls_canonkeeper(self):
        ...
        with patch("monitor_agents.canonkeeper.CanonKeeper", return_value=mock_ck):
...
class TestBootstrapResiliency:
    ...
    async def test_create_story_outline_inserts_to_mongodb(self):
        ...
        story_id = uuid4()
        universe_id = uuid4()
        fake_created_outline = MagicMock(
            model_dump=lambda mode="json": {"story_id": str(story_id), "premise": "x"}
        )
...
def _import_arc_eval():
    ...
    return StoryState, evaluate_arc, _arc_label_to_purpose


class TestArcLabelToPurpose:
    def test_all_labels(self):
        (_, _, mapper) = _import_arc_eval()
        assert mapper("rising_action") == "Tension Building"
        assert mapper("climax") == "Climactic Confrontation"
        assert mapper("falling_action") == "Aftermath"
        assert mapper("resolution") == "Resolution"
        assert mapper("new_thread") == "New Development"
        assert mapper("unknown") == "Story Continues"


class TestEvaluateArc:
    @pytest.mark.asyncio
    async def test_early_scenes_rising_action(self):
        (StoryState, evaluate_arc, _) = _import_arc_eval()
        state = StoryState(
            story_id=uuid4(),
            universe_id=uuid4(),
            scenes_completed=1,
            tension_score=0.2,
            active_threads=["central_conflict"],
        )
        with patch("monitor_agents.prompts.story.StoryPlannerModule.forward") as mock_forward:
            mock_forward.return_value = {"next_scene_type": "exploration", "plot_hook": "hook"}
            result = await evaluate_arc(state)
        assert result["arc_label"] == "rising_action"
        assert result["next_scene_type"] == "exploration"

    @pytest.mark.asyncio
    async def test_high_tension_climax(self):
        (StoryState, evaluate_arc, _) = _import_arc_eval()
        state = StoryState(
            story_id=uuid4(),
            universe_id=uuid4(),
            scenes_completed=4,
            tension_score=0.75,
            active_threads=["major_battle", "betrayal"],
            completed_threads=["minor_setup"],
        )
        with patch("monitor_agents.prompts.story.StoryPlannerModule.forward") as mock_forward:
            mock_forward.return_value = {"next_scene_type": "combat", "plot_hook": "hook"}
            result = await evaluate_arc(state)
        assert result["arc_label"] == "climax"
        assert result["next_scene_type"] == "combat"

    @pytest.mark.asyncio
    async def test_falling_action(self):
        (StoryState, evaluate_arc, _) = _import_arc_eval()
        state = StoryState(
            story_id=uuid4(),
            universe_id=uuid4(),
            scenes_completed=6,
            tension_score=0.6,
            active_threads=["epilogue"],
            completed_threads=["major_battle", "betrayal", "setup"],
        )
        with patch("monitor_agents.prompts.story.StoryPlannerModule.forward") as mock_forward:
            mock_forward.return_value = {"next_scene_type": "dialogue", "plot_hook": "hook"}
            result = await evaluate_arc(state)
        assert result["arc_label"] == "falling_action"

    @pytest.mark.asyncio
    async def test_resolution_completes_story(self):
        (StoryState, evaluate_arc, _) = _import_arc_eval()
        state = StoryState(
            story_id=uuid4(),
            universe_id=uuid4(),
            ...
            tension_score=0.4,
            active_threads=[],
            completed_threads=["everything", "major_battle"],
        )
        with patch("monitor_agents.prompts.story.StoryPlannerModule.forward") as mock_forward:
            mock_forward.return_value = {"next_scene_type": "rest", "plot_hook": "hook"}
            result = await evaluate_arc(state)
        assert result["arc_label"] == "resolution"
        assert result["story_complete"] is True

    @pytest.mark.asyncio
    async def test_new_thread_after_empty(self):
        (StoryState, evaluate_arc, _) = _import_arc_eval()
        state = StoryState(
            story_id=uuid4(),
            universe_id=uuid4(),
        ...
        with patch("monitor_agents.prompts.story.StoryPlannerModule.forward") as mock_forward:
            mock_forward.return_value = {"next_scene_type": "revelation", "plot_hook": "hook"}
            result = await evaluate_arc(state)
        assert result["arc_label"] == "new_thread"

    @pytest.mark.asyncio
    async def test_tension_clamped(self):
        (StoryState, evaluate_arc, _) = _import_arc_eval()
        state = StoryState(
            story_id=uuid4(),
            universe_id=uuid4(),
    ...
    async def test_history_appended(self):
        ...
        state = StoryState(
            ...
            scenes_completed=3,
            tension_score=0.3,
            active_threads=["conflict"],
            scenes_history=[{"scene_index": 2, "arc_label": "rising_action"}],
        )
        with patch("monitor_agents.prompts.story.StoryPlannerModule.forward") as mock_forward:
            mock_forward.return_value = {"next_scene_type": "exploration", "plot_hook": "hook"}
            result = await evaluate_arc(state)
        assert len(result["scenes_history"]) == 2
...
class TestStoryLoopPlanning:
    ...
    async def test_story_loop_generates_next_scene(self):
        ...
        with patch("monitor_agents.prompts.story.StoryPlannerModule.forward") as mock_forward:
            ...
            state = StoryState(
                story_id=story_id, universe_id=universe_id, arc_label="Rising Action"
            )

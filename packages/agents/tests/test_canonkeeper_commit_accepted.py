...
async def test_commit_accepted_success(monkeypatch: pytest.MonkeyPatch):
    ...
    # Mock proposals
    proposal = {"proposal_id": pid, "proposal_type": "create_lore_fact"}

    pack_doc = {"pack_id": str(pack_id), "source_document_ids": [str(doc_id)]}

    doc_doc = {"doc_id": str(doc_id), "source_id": source_id}
    ...
    proposals_coll.update_one.assert_called_with({"proposal_id": pid}, {"$set": {"status": "committed"}})
    packs_coll.update_one.assert_called_with({"pack_id": str(pack_id)}, {"$set": {"status": KPS.APPLIED.value}})
            ...
            "resolution_mechanics": [
                {
                    "dice_formula": "1d20",
                    "mechanic_type": "d20",
                    "difficulty_model": "target_number",

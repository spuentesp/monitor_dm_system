    ...
    "tracks": [
        {
            ...
            "threshold_effects": [{"value": 5, "direction": "at_or_below", "effect": "Wounded"}],
        }
    ]
}
...
def test_wounded_derived_from_system_threshold():
    ...
    tags = canonical_state_tags([], {"Health": -6}, {"Health": {"current": 10, "max": 10}}, MISTLANDS_LIKE)
            ...
            "tracks": [
                {
                    ...
                    "min_value": 0,
                    "max_value": 5,
                    "threshold_effects": [{"value": 5, "direction": "at_or_above", "effect": "Frenzy"}],
                }
            ]
        }
    )
...
def test_tags_pass_through_without_alias_remap():
    ...
    out = _normalise_state_tag_update({"entity_id": "e1", "add_tags": ["Injured", "off_balance"]})
...
def test_no_hp_based_tag_inference():
    ...
    out = _normalise_state_tag_update({"entity_id": "e1", "resources": {"HP": {"current": 0}}})
    assert out["add_tags"] == []

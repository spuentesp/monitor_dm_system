...
from monitor_agents.character_creator import npc_profile_gen as gen
from monitor_agents.character_creator.npc_profile_gen import (
    NPCProfileGenerator,
    _coerce_profile,
    _neutral_profile,
)
...
class TestForwardParsing:
    def test_parses_clean_json(self):
        out = _run_forward_with_output(
            '{"traits": {"wariness": 0.8}, "values": ["self-preservation"], "current_emotional_state": "guarded"}'
        )
    ...
    def test_parses_json_with_surrounding_prose(self):
        out = _run_forward_with_output('Here is the profile you asked for:\n{"fears": ["betrayal"]}\nHope that helps!')

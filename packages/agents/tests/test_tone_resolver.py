...
class TestToneResolver:
    ...
    @pytest.mark.asyncio
    async def test_resolve_with_matching_profiles(
        self, mock_mcp_tools, sample_tone_profile, sample_library
    ):
        ...
        mock_mcp_tools.mongodb_get_tone_library.return_value = MagicMock(
            model_dump=lambda: sample_library
        )
    ...
    @pytest.mark.asyncio
    async def test_resolve_merges_with_default_library(
        self, mock_mcp_tools, sample_tone_profile, sample_library
    ):
        ...
        mock_mcp_tools.mongodb_get_default_tone_library.return_value = MagicMock(
            model_dump=lambda: default_library
        )
    ...
    async def test_resolve_fallback_to_builtin_if_no_matches(self, mock_mcp_tools):
        ...
        mock_mcp_tools.mongodb_get_tone_library.return_value = MagicMock(
            model_dump=lambda: empty_library
        )
    ...
    async def test_merge_with_default_library_false(self, mock_mcp_tools, sample_tone_profile):
        ...
        mock_mcp_tools.mongodb_get_tone_library.return_value = MagicMock(
            model_dump=lambda: specific_library
        )
        mock_mcp_tools.mongodb_get_default_tone_library.return_value = MagicMock(
            model_dump=lambda: default_library
        )
    ...
    @pytest.mark.asyncio
    async def test_empty_tone_instructions_uses_normal_flow(
        self, mock_mcp_tools, sample_tone_profile
    ):

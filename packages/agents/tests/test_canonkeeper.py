...
async def test_canonkeeper_flags_contradiction():
    ...
    with patch.object(ck, "_policy_check") as mock_policy:
        ...
        with patch("monitor_agents.canonkeeper.verification.ContradictionModule.forward") as mock_verify:
            ...
            verdict = await ck._evaluate_single(proposal=proposal, world_rules="- No violence", protected_entities="[]")
...
class TestApplyPackMechanicNodes:
    ...
    async def test_writes_mechanic_nodes_when_game_system_present(self):
        ...
        with (
            ...
            patch("monitor_agents.canonkeeper.agent.neo4j_create_ability_system") as mock_ab,
            patch("monitor_agents.canonkeeper.agent.neo4j_create_track") as mock_tr,
            patch("monitor_agents.canonkeeper.agent.neo4j_create_condition") as mock_cn,
        ):

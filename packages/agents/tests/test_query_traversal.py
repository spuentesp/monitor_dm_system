...
def test_score_candidate_relationship_boosts_preferred_type() -> None:
    ...
    low = score_candidate_relationship(
        {"rel_type": "RELATED_TO", "properties": {}}, preferred_rel_types=preferred
    )
...
def test_collect_traversal_query_terms_respects_limit() -> None:
    candidates = [
        {"anchor_name": f"Entity{i}", "counterpart_name": f"Other{i}", "rel_type": "REL"}
        for i in range(20)
    ]

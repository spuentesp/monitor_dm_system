...
class TestExpandedPriorTurns:
    ...
    async def test_narrator_receives_at_least_8_prior_turns(self):
        ...
        eight_turns = [
            {
                "turn_id": f"t{i}",
                "text": f"Turn {i} narration",
                "speaker": "gm" if i % 2 else "player",
            }
            for i in range(8)
        ]
    ...
    async def test_narrator_receives_all_available_turns_when_fewer_than_8(self):
        ...
        assert len(prior_turns) == 2, "When fewer than 8 turns exist, narrator should receive all available turns."
...
class TestSettingAnchor:
    ...
    async def test_setting_anchor_includes_character_identity(self):
        ...
        assert "kael" in anchor.lower(), f"setting_anchor must include the character's name, got: {anchor}"


# ===========================================================================
# Story premise conditioning (character-template plan Q3): a player-stated
# pitch must be a hard content constraint on the opening, not just folded
# into mood -- carried the same way genre/setting already are, via
# setting_anchor, so it can't be silently dropped in the middle of a larger
# profile_context block.
# ===========================================================================


class TestStoryPremise:
    @pytest.mark.asyncio
    async def test_story_premise_appears_in_setting_anchor(self):
        Narrator = _import_narrator()
        ...
        fake_prediction = MagicMock()
        fake_prediction.narrative_text = "The vault door hisses open."
        fake_prediction.proposed_changes = "[]"
        fake_prediction.narrative_time_elapsed = "5"
        narrator._narrator_module.return_value = fake_prediction
        ...
        with (
            ...
            await narrator.generate_opening(
                user_input="(session opening)",
                context={"entities": [], "memories": [], "turns": []},
                story_premise="heist against a rival Prince, no combat",
            )

        anchor = captured_kwargs.get("setting_anchor", "")
        assert "heist against a rival prince" in anchor.lower(), (
            f"setting_anchor must carry the player's stated premise, got: {anchor}"
        )
        assert "premise" in anchor.lower()

    @pytest.mark.asyncio
    async def test_no_story_premise_leaves_setting_anchor_unchanged(self):
        """Omitting story_premise must behave exactly as before -- no
        PREMISE segment, no extra CRITICAL clause."""
        Narrator = _import_narrator()
        ...
        anchor = captured_kwargs.get("setting_anchor", "")
        assert "premise" not in anchor.lower()

    @pytest.mark.asyncio
    async def test_story_premise_precedes_genre_in_setting_anchor(self):
        """The premise is the most specific steering signal available --
        it must lead, not trail behind genre/setting."""
        Narrator = _import_narrator()
        ...
        with (
            ...
            await narrator.generate_opening(
                ...
                context={
                    ...
                    "memories": [],
                    "turns": [],
                    "source_profile": {"genre": "noir"},
                },
                story_premise="heist against a rival Prince",
            )

        anchor = captured_kwargs.get("setting_anchor", "")
        assert anchor.lower().index("premise") < anchor.lower().index("genre")

    @pytest.mark.asyncio
    async def test_ongoing_turn_carries_premise_via_story_state(self):
        """The opening-only explicit story_premise kwarg doesn't reach
        ongoing scene turns (narrate_turn never passes it) -- StoryState is
        the channel for those, so the premise must keep steering content
        turn after turn, not just at the cold open."""
        ...
        story_state = StoryState(
            story_id=uuid4(),
            universe_id=uuid4(),
            story_premise="a heist against a rival Prince, no combat",
        )
        ...
        with (
            ...
            await narrator.narrate_turn(
                scene_id=uuid4(),
                user_input="I case the tower from across the street",
                resolution=None,
                context={"entities": [], "memories": [], "turns": []},
    ...
    @pytest.mark.asyncio
    async def test_story_state_without_premise_omits_it(self):
        from monitor_agents.loops.story_loop import StoryState
        ...
        story_state = StoryState(story_id=uuid4(), universe_id=uuid4())
...
class TestCharacterSheetPassthrough:
    ...
    async def test_narrator_includes_character_stats_in_profile_context(self):
        ...
        assert "STR" in profile_context or "14" in profile_context, (
            "Character stats must be injected into profile_context so the narrator can reference them in prose."
        )
...
class TestTurnContextObject:
    ...
    async def test_scene_state_has_turn_context_field(self):
        ...
        assert hasattr(state, "turn_context"), (
            "SceneState must have a 'turn_context' field to carry the TurnContext object through the scene loop."
        )
...
class TestConsistencyGuard:
    ...
    async def test_check_consistency_no_violations_returns_empty(self):
        ...
        assert len(violations) == 0, (
            "When narration is consistent with established facts, check_consistency should return no violations."
        )
...
class TestPendingRollState:
    ...
    async def test_scene_state_has_pending_roll_field(self):
        ...
        assert hasattr(state, "pending_roll"), (
            "SceneState must have a 'pending_roll' field to carry roll state between turns."
        )
    ...
    async def test_pending_roll_cleared_on_successful_roll(self):
        ...
        assert result.get("pending_roll") is None, "When a roll is resolved, pending_roll must be cleared from state."
...
class TestGraphFlowWithTurnContext:
    ...
    def test_graph_includes_new_coherence_nodes(self):
        ...
        assert not missing, f"Graph must include new coherence nodes: {missing}. Current nodes: {nodes}"
    ...
    def test_extract_facts_runs_after_narrate(self):
        ...
        assert ("narrate", "extract_facts") in edges or any(
            edge[0] == "narrate" and edge[1] == "extract_facts" for edge in edges
    ...
    def test_check_consistency_runs_before_persist(self):
        ...
        graph = build_scene_graph()
        edges = graph.edges
        # check_consistency and check_events run in parallel and both connect to persist_turn_artifacts
...
class TestResolverPendingRollInterception:
    """The resolver must read pending_roll from context and auto-resolve it
    server-side (rolling for the player) rather than gating the narrative behind
    a magic keyword. The player is never blocked for narrating past a roll."""

    @pytest.mark.asyncio
    async def test_resolver_auto_resolves_pending_roll_when_player_narrates_past(self):
        """When pending_roll is set and the player narrates instead of typing
        'roll', the resolver must roll server-side using the proposed spec and
        return a graded dice outcome (never a pushback nag)."""
        ...
        # Auto-rolled server-side and resolved — no keyword gate, no pushback.
        assert result["resolution_type"] == "dice", (
            "A pending roll the player narrates past must be auto-resolved server-side, not pushed back."
        )
        assert result["resolution_type"] != "forced_narrative_pushback"
        assert result["success_level"] in _GRADED_LEVELS
        assert result["success_level"] != "pending"
        # The proposed spec must drive the roll.
        assert result["stat"] == "Strength"
        assert result["difficulty_class"] == 12
        assert result["modifier"] == -5
        assert result["roll_detail"]["total"] is not None

    @pytest.mark.asyncio
    async def test_resolver_resolves_pending_roll_on_explicit_roll(self):
        """When pending_roll is set and the player explicitly rolls, the resolver
        resolves the same way (graded dice outcome, not a pushback)."""
        from monitor_agents.resolver import Resolver
        ...
        assert result["resolution_type"] == "dice", (
            "An explicit roll on a pending roll must resolve to a graded outcome."
        )
        assert result["resolution_type"] != "forced_narrative_pushback"
        assert result["success_level"] in _GRADED_LEVELS
    ...
    async def test_resolver_no_pending_roll_allows_trivial(self):
        ...
        with force_roll_necessity("trivial"):
            result, _ = await resolver.resolve_turn(
                scene_id=str(uuid4()),
...
class TestExtractFactsFunctional:
    ...
    async def test_extract_facts_finds_named_entities(self):
        ...
        assert "Ostensible" in all_facts_text, f"extract_facts should find 'Ostensible' in narration. Facts: {facts}"
        assert "Kael" in all_facts_text, f"extract_facts should find 'Kael' in narration. Facts: {facts}"
    ...
    async def test_extract_facts_caps_at_50(self):
        ...
        assert len(facts) <= 50, f"established_facts must be capped at 50, got {len(facts)}"
...
class TestCheckConsistencyFunctional:
    ...
    async def test_check_consistency_detects_genre_drift(self):
        ...
        state = SceneState(
            ...
            story_id=uuid4(),
            narrative_text=(
                "The tavern reeks of stale ale. A corkboard hangs by the hearth. The innkeeper polishes a tankard."
            ),
    ...
    async def test_check_consistency_no_violations_for_consistent_narration(self):
        ...
        state = SceneState(
            ...
            story_id=uuid4(),
            narrative_text=(
                "The airlock hisses open. Kael steps into the corridor, his lamp cutting through the darkness."
            ),
            source_profile={"genre": "sci-fi"},
        )
        ...
        violations = result.get("consistency_violations", [])
        assert len(violations) == 0, (
            f"check_consistency should not flag consistent sci-fi narration. Violations: {violations}"
        )
    ...
    async def test_check_consistency_detects_name_drift(self):
        ...
        assert len(name_drifts) > 0, (
            "check_consistency must detect name drift when 'Ostensible' contradicts established name 'Rust Nail'."
        )
        assert name_drifts[0]["expected"] == "Rust Nail"
        assert name_drifts[0]["found"] == "Ostensible"

    @pytest.mark.asyncio
    async def test_check_consistency_ignores_unrelated_called_named_phrases(self):
        """Regression: bare 'called X' / 'named X' must NOT trigger name_drift.

        Live e2e run (2026-07-21, vtm_primogen) produced 10 false-positive
        high-severity name_drift violations from "a mortal development
        company... is called Marrow & Vance" -- the old ship_name_pattern
        treated ANY "called X"/"named X" phrase as a vessel-naming
        declaration and cross-checked it against every unrelated
        established name in scope (a company name, an NPC's name, a
        faction's name), none of which it was ever claiming to rename.
        """
        (SceneState, *_) = _import_scene_loop()
        from monitor_agents.loops.scene_loop import check_consistency
        ...
        state = SceneState(
            ...
            story_id=uuid4(),
            narrative_text=(
                "Three blocks north, a mortal development company is preparing "
                "to break ground. They are called Marrow & Vance."
            ),
            established_facts=[
                "Named entity mentioned: The Order",
                "Named entity mentioned: Brother Matthias",
                "Named entity mentioned: General",
            ],
        )
        ...
        name_drifts = [v for v in violations if v["type"] == "name_drift"]
        assert name_drifts == [], (
            "a company being 'called Marrow & Vance' is not a vessel-renaming "
            f"event and must not be cross-checked against unrelated established "
            f"names. Got: {name_drifts}"
        )

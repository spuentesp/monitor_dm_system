...
def test_golden_routing(golden_input: dict[str, Any], fake_lm_factory):
    ...
    expected_response = {
        ...
        "action": (
            "PUSH_BACK" if golden_input["expected_violates"] and golden_input["expected_declares_outcome"] else "ACCEPT"
        ),
        "suggested_stat": (
            "Strength"
            ...
            if golden_input["expected_action"] == "stealth" or golden_input["expected_action"] == "combat"
            else "Wisdom"
        ...
        "suggested_dc": (
            15
            if golden_input["expected_violates"]
            else 12
            if golden_input["expected_roll_necessity"] == "propose_roll"
            else 10
            if golden_input["expected_roll_necessity"] == "contested"
            else 0
        ),
    ...
    assert verdict.intent_type.value == golden_input["expected_intent"], (
        f"intent_type mismatch: got {verdict.intent_type.value}, expected {golden_input['expected_intent']}"
    )
    assert verdict.action_type.value == golden_input["expected_action"], (
        f"action_type mismatch: got {verdict.action_type.value}, expected {golden_input['expected_action']}"
    )
    ...
    assert verdict.violates_causality == golden_input["expected_violates"], (
        f"violates_causality mismatch: got {verdict.violates_causality}, expected {golden_input['expected_violates']}"
    )
...
class TestSignatureStructure:
    ...
    def test_signature_input_fields_match_pydantic(self):
        ...
        from monitor_agents.gm_awareness import GMAwareness, GMAwarenessSignature

...
class TestHeuristicContradictionCheck:
    ...
    def test_negation_detected(self):
        ...
        assert (
            "king" in result["explanation"].lower() or "contradict" in result["explanation"].lower()
        )
...
class TestVerifyFact:
    ...
    async def test_verify_fact_with_llm(self):
        ...
        with patch("monitor_agents.prompts.verification.ContradictionModule") as mock_module_cls:
            ...
            assert result["has_contradiction"] is True
            assert (
                "king" in result["explanation"].lower()
                or "contradict" in result["explanation"].lower()
            )

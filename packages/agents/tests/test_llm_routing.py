...
@pytest.mark.asyncio
async def test_call_llm_structured_uses_heavy_role_for_analyzer(monkeypatch):
    """2026-07-22: analyzer node defaults to HEAVY (was STANDARD) for ingestion robustness."""
    calls = []
    ...
    assert result.ok is True
    assert ("for_node_or_role", "analyzer", ModelRole.HEAVY) in calls

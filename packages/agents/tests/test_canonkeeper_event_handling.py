...
async def test_commit_fact_handles_temporal_event_create(mock_call_tool):
    ...
    with patch.object(agent, "_store_neo4j_id_on_proposal") as mock_store:
        await agent._commit_fact(
            proposals_coll=MockColl(), proposal_id=prop_id, proposal=proposal, source_id_strs=[], verdict=verdict
        )

        mock_call_tool.assert_called_once_with("neo4j_create_event", {"params": ANY})
        mock_store.assert_called_once_with(ANY, prop_id, "neo4j_uuid")
...
async def test_commit_fact_handles_temporal_event_update(mock_call_tool):
    ...
    proposal = {
        "change_type": "event",
        "content": {"operation": "update", "event_id": str(uuid4()), "description": "Updated desc"},
    }
    prop_id = str(uuid4())
    verdict = CanonKeeperVerdict(
        proposal_id=prop_id,
...
async def test_commit_fact_handles_temporal_event_delete(mock_call_tool):
    ...
    proposal = {"change_type": "event", "content": {"operation": "delete", "event_id": str(uuid4()), "force": True}}
    prop_id = str(uuid4())
    verdict = CanonKeeperVerdict(
        proposal_id=prop_id,
        decision=CanonKeeperDecision.ACCEPTED,
    ...
    with patch.object(agent, "_store_neo4j_id_on_proposal"):
        ...
        mock_call_tool.assert_called_once_with(
            "neo4j_delete_event", {"event_id": proposal["content"]["event_id"], "force": True}
        )

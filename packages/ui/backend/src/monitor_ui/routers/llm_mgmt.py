...
async def _maybe_seed_from_env() -> None:
    ...
    google_key = os.getenv("GOOGLE_API_KEY", "").strip() or existing_rows.get("google-ai-studio-default", {}).get(
        "api_key", ""
    )
    anthropic_key = os.getenv("ANTHROPIC_API_KEY", "").strip() or existing_rows.get("anthropic-default", {}).get(
        "api_key", ""
    )
    openai_key = os.getenv("OPENAI_API_KEY", "").strip() or existing_rows.get("openai-default", {}).get("api_key", "")

    has_existing_default = any(bool(row.get("is_default")) for row in existing_rows.values())
    ...
    builtin_profiles: list[dict[str, Any]] = [
        {
            ...
            "name": existing_rows.get("github-models-default", {}).get("name", "GitHub Models"),
            ...
            "role": existing_rows.get("github-models-default", {}).get("role", "standard"),
            "status": existing_rows.get("github-models-default", {}).get("status", "unconfigured"),
            "latency_ms": existing_rows.get("github-models-default", {}).get("latency_ms"),
        ...
        },
        {
            ...
            "name": existing_rows.get("google-ai-studio-default", {}).get("name", "Google AI Studio"),
            "provider": "google_ai_studio",
            "model": _google_model,
            "api_key": google_key,
            "base_url": None,
            "model_params": {"temperature": 0.7, "max_tokens": 8192},
            "role": existing_rows.get("google-ai-studio-default", {}).get("role", "heavy"),
            "status": existing_rows.get("google-ai-studio-default", {}).get("status", "unconfigured"),
            "latency_ms": existing_rows.get("google-ai-studio-default", {}).get("latency_ms"),
            "is_default": existing_rows.get("google-ai-studio-default", {}).get("is_default", False),
        },
        {
            "id": "ollama-local",
            ...
            "status": existing_rows.get("ollama-local", {}).get("status", "unconfigured"),
            "latency_ms": existing_rows.get("ollama-local", {}).get("latency_ms"),
            "is_default": existing_rows.get("ollama-local", {}).get(
                "is_default",
                not gh_token and not anthropic_key and not openai_key and not has_existing_default,
            ),
        },
    ]
        ...
        builtin_profiles.append(
            {
                ...
                "name": existing_rows.get("anthropic-default", {}).get("name", "Anthropic (Claude)"),
                "provider": "anthropic",
                ...
                "status": existing_rows.get("anthropic-default", {}).get("status", "unconfigured"),
                "latency_ms": existing_rows.get("anthropic-default", {}).get("latency_ms"),
                "is_default": existing_rows.get("anthropic-default", {}).get("is_default", False),
            }
        )
        ...
        builtin_profiles.append(
            {
                ...
                "status": existing_rows.get("openai-default", {}).get("status", "unconfigured"),
                "latency_ms": existing_rows.get("openai-default", {}).get("latency_ms"),
                "is_default": existing_rows.get("openai-default", {}).get("is_default", False),
            }
        )
    ...
    if minimax_key or any(k in existing_rows for k in ("minimax-light", "minimax-her", "minimax-m27")):
        # Light — fast, cheap
        builtin_profiles.append(
            {
                "id": "minimax-light",
                "name": existing_rows.get("minimax-light", {}).get("name", "MiniMax M2-her (Light)"),
                "provider": "minimax",
                "model": _resolve_model(
                    "MINIMAX_LIGHT_MODEL",
                ...
                "status": existing_rows.get("minimax-light", {}).get("status", "unconfigured"),
                "latency_ms": existing_rows.get("minimax-light", {}).get("latency_ms"),
                "is_default": existing_rows.get("minimax-light", {}).get("is_default", False),
            }
        )
        ...
        builtin_profiles.append(
            {
                "id": "minimax-her",
                "name": existing_rows.get("minimax-her", {}).get("name", "MiniMax M2.7 (Standard)"),
                "provider": "minimax",
                "model": _resolve_model(
                    "MINIMAX_HER_MODEL",
                ...
                "status": existing_rows.get("minimax-her", {}).get("status", "unconfigured"),
                "latency_ms": existing_rows.get("minimax-her", {}).get("latency_ms"),
                "is_default": existing_rows.get("minimax-her", {}).get("is_default", False),
            }
        )
        ...
        builtin_profiles.append(
            {
                "id": "minimax-m27",
                "name": existing_rows.get("minimax-m27", {}).get("name", "MiniMax M2.7 (Heavy)"),
                "provider": "minimax",
                "model": _resolve_model(
                    "MINIMAX_M27_MODEL",
                ...
                "status": existing_rows.get("minimax-m27", {}).get("status", "unconfigured"),
                "latency_ms": existing_rows.get("minimax-m27", {}).get("latency_ms"),
                "is_default": existing_rows.get("minimax-m27", {}).get("is_default", False),
            }
        )
    ...
    if zai_key or any(k in existing_rows for k in ("zai-light", "zai-standard", "zai-heavy")):
        # Light — flash model, fastest
        builtin_profiles.append(
            {
                "id": "zai-light",
                "name": existing_rows.get("zai-light", {}).get("name", "Z.AI glm-z1-flash (Light)"),
                "provider": "z_ai",
                "model": _resolve_model(
                    "Z_AI_LIGHT_MODEL",
                ...
                "status": existing_rows.get("zai-light", {}).get("status", "unconfigured"),
                "latency_ms": existing_rows.get("zai-light", {}).get("latency_ms"),
                "is_default": existing_rows.get("zai-light", {}).get("is_default", False),
            }
        )
        # Standard — balanced
        builtin_profiles.append(
            {
                "id": "zai-standard",
                "name": existing_rows.get("zai-standard", {}).get("name", "Z.AI glm-4-flash (Standard)"),
                "provider": "z_ai",
                "model": _resolve_model(
                    "Z_AI_STANDARD_MODEL",
                ...
                "status": existing_rows.get("zai-standard", {}).get("status", "unconfigured"),
                "latency_ms": existing_rows.get("zai-standard", {}).get("latency_ms"),
                "is_default": existing_rows.get("zai-standard", {}).get("is_default", False),
            }
        )
        # Heavy — most capable
        builtin_profiles.append(
            {
                "id": "zai-heavy",
                "name": existing_rows.get("zai-heavy", {}).get("name", "Z.AI glm-4-plus (Heavy)"),
                "provider": "z_ai",
                "model": _resolve_model(
                    "Z_AI_HEAVY_MODEL",
                ...
                "status": existing_rows.get("zai-heavy", {}).get("status", "unconfigured"),
                "latency_ms": existing_rows.get("zai-heavy", {}).get("latency_ms"),
                "is_default": existing_rows.get("zai-heavy", {}).get("is_default", False),
            }
        )
    ...
    with suppress(Exception):
        await postgres.config_set("llm_provider", "github_models" if gh_token else "ollama")
        await postgres.config_set("llm_provider_order", "google_ai_studio,github_models,ollama")
...
@router.post("/providers/{provider_id}/duplicate", response_model=LLMProvider, status_code=201)
async def duplicate_provider(provider_id: str, body: DuplicateProviderBody | None = None) -> LLMProvider:
...
async def upsert_node_assignment(node_name: str, body: NodeAssignmentUpdate) -> NodeAssignment:
    postgres = get_postgres_client()
    ...
    try:
        if provider == "anthropic":
            async with httpx.AsyncClient(timeout=15) as client:
                ...
                await postgres.provider_upsert({**p, "status": "connected", "latency_ms": int(latency)})
                return TestResult(
                    ok=True,
                    latency_ms=latency,
                    model_response=resp.json().get("content", [{}])[0].get("text", "ok"),
                )
            ...
            async with httpx.AsyncClient(timeout=10) as client:
                ...
                return TestResult(
                    ok=True,
                    latency_ms=latency,
                    model_response=f"model={resolved_model}" + (" (auto-corrected)" if resolved_model != model else ""),
                )

        elif provider == "minimax":
            # MiniMax uses Anthropic SDK with OpenAI-compatible endpoint
            minimax_base = base_url or os.getenv("MINIMAX_BASE_URL", "").strip() or "https://api.minimax.chat/v1"
            async with httpx.AsyncClient(timeout=15) as client:
                resp = await client.post(
            ...
            if not base_url:
                return TestResult(ok=False, error="base_url is required for custom provider")
            async with httpx.AsyncClient(timeout=15) as client:
                resp = await client.post(
        ...
        if provider == "anthropic" and api_key:
            ...
            async with httpx.AsyncClient(timeout=10) as client:
                ...
                if resp.status_code == 200:
                    data = resp.json().get("data", [])
                    chat_models = sorted(
                        [m["id"] for m in data if any(k in m["id"] for k in ("gpt-4", "gpt-3.5", "o1", "o3"))],
                        reverse=True,
                    )
            ...
            async with httpx.AsyncClient(timeout=10) as client:
                ...
                if resp.status_code == 200:
                    ...
                    models = sorted({str(m.get("id", "")).strip() for m in items if m.get("id")})
            ...
            async with httpx.AsyncClient(timeout=5) as client:
                ...
                if resp.status_code == 200:
                    ...
                    return [m["name"] for m in models_data] or AVAILABLE_MODELS.get("ollama", [])
            ...
            async with httpx.AsyncClient(timeout=10) as client:
                ...
                if resp.status_code == 200:
                    data = resp.json().get("data", [])
                    return sorted(
                        [m["id"] for m in data if any(k in m["id"] for k in ("llama", "mixtral", "gemma"))],
                        reverse=True,
            ...
            async with httpx.AsyncClient(timeout=10) as client:
                ...
                if resp.status_code == 200:
                    data = resp.json().get("data", [])
                    return sorted([m["id"] for m in data if m.get("id")]) or AVAILABLE_MODELS.get("openrouter", [])

...
async def get_play_session(session_id: str) -> PlaySessionResponse:
    ...
    if not result:
        raise HTTPException(
            status_code=404, detail=f"PlaySession {session_id} not found"
        )
    return result
...
async def update_play_session(
    session_id: str, body: PlaySessionUpdate
) -> PlaySessionResponse:
...
@router.post(
    "/play-sessions/{session_id}/scenes/{scene_id}", response_model=PlaySessionResponse
)
...
@router.get(
    ...
    response_model=Optional[PlaySessionResponse],
)
async def latest_session(story_id: str) -> Optional[PlaySessionResponse]:

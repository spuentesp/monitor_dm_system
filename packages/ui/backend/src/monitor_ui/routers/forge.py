...
_DEMO_PC_DESC = (
    "A wandering investigator who took the last room at the Millhaven Inn — lantern in one hand, knife in the other."
)
...
def _demo_pc_properties(system_id: UUID | None) -> dict:  # type: ignore
    ...
    try:
        ...
        if derived:
            props["resources"] = {name: {"current": val, "max": val} for name, val in derived.items()}
...
def _ensure_demo_pc(universe_id: UUID, system_id: UUID | None = None) -> str | None:
    ...
    try:
        existing = neo4j_list_entities(EntityFilter(universe_id=universe_id, name=_DEMO_PC_NAME, limit=10))
...
async def demo_world(start_playing: bool = True) -> DemoWorldResponse:
    ...
    """
    import anyio
    from monitor_agents.quick_world.agent import QuickWorldResult
    ...
    def _ensure_demo() -> tuple[QuickWorldResult, bool]:
        ...
        existing = [u for u in neo4j_list_universes(UniverseFilter(limit=500)) if u.name == _DEMO_NAME]
        entities_payload = [
            {"name": n, "type": t, "description": d, "want": w, "tags": []} for (n, t, d, w) in _DEMO_ENTITIES
        ]
        if existing:
            u = existing[0]
            return (
    ...
    if not reused:
        from monitor_agents.quick_world.agent import QuickWorldBuilder

        builder = QuickWorldBuilder()
        committed, errors = await builder._canonize(
    ...
    if start_playing:
        try:
            ...
            pc_id: str | None = None
            try:
                pc_id = await anyio.to_thread.run_sync(_ensure_demo_pc, result.universe_id, mistlands_id)
            except Exception as exc:
                payload.setdefault("errors", []).append(f"Demo PC bootstrap failed: {exc}")
    ...
    if body.start_playing:
        try:
            ...
            try:
                ...
                payload.setdefault("errors", []).append(f"Quick-world PC bootstrap failed: {exc}")

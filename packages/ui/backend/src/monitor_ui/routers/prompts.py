...
try:
    ...
    from monitor_agents.prompts.context_assembly import QueryFormulationSignature
...
async def _load_override(module_id: str) -> Optional[dict[str, Any]]:
...
async def update_module_instructions(
    module_id: str, body: PromptOverrideUpdate
) -> PromptModuleDetail:
...
async def test_module(module_id: str, body: PromptTestRequest) -> PromptTestResult:
    ...
    pred_cls = (
        dspy.ChainOfThought
        if entry["predictor_type"] == "ChainOfThought"
        else dspy.Predict
    )
    ...
    try:
        ...
        role = (
            ModelRole.HEAVY
            if role_str == "heavy"
            else (ModelRole.LIGHT if role_str == "light" else ModelRole.STANDARD)
        )

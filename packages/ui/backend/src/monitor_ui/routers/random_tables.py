...
async def list_random_tables(
    ...
    table_type: RandomTableType | None = Query(
        default=None, description="Filter by table type category"
    ),
    game_system_id: str | None = Query(
        default=None, description="Filter by game system"
    ),
...
async def create_random_table(body: RandomTableCreateRequest) -> RandomTableResponse:
    ...
    try:
        params = RandomTableCreate(
            ...
            entries=[e.to_entry() for e in body.entries],
...
async def update_random_table(
    table_id: str, body: RandomTableUpdateRequest
) -> RandomTableResponse:

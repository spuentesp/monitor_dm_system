...
_CHAT_CACHE_TTL_SECONDS = max(
    int(getattr(settings, "redis_cache_ttl_seconds", 30) or 30) * 20, 300
)
...
def db_save_session(session: dict[str, Any]) -> None:
    ...
    try:
        ...
        _publish_chat_event(
            session["id"], "session_saved", {"updated_at": session.get("updated_at")}
        )
    except Exception as exc:  # noqa: BLE001
        ...
        if session_id:
            ...
            if isinstance(cached_messages, list):
                existing_index = next(
                    (
                        i
                        for i, item in enumerate(cached_messages)
...
def db_load_messages(session_id: str) -> list[dict[str, Any]]:
    ...
    try:
        ...
        col = mdb.get_collection("chat_messages")
        rows = list(
            col.find({"session_id": session_id}, {"_id": 0}, sort=[("timestamp", 1)])
        )
        _redis_set_json(cache_key, rows[-_CHAT_MESSAGES_LIMIT:])
        return rows
    except Exception as exc:  # noqa: BLE001

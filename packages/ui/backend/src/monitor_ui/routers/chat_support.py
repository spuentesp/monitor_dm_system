...
VALID_TONES = frozenset(
    {"dramatic", "grim", "horror", "heroic", "mystery", "adventure"}
)
...
_OOC_BLOCK_RE = _re.compile(
    r"^\s*\(\((?P<content>.*?)\)\)\s*$", _re.IGNORECASE | _re.DOTALL
)
...
def build_session_state_payload(
    ...
    latest_gm = next(
        (message for message in reversed(messages) if message.get("role") == "gm"), None
    )
    ...
    recent_success_levels = [
        ...
        and message.get("metadata", {}).get("success_level")
    ][-10:]
    recent_npc_stances = [
        str(
    ...
    return {
        ...
        "latest_working_state": latest_metadata.get(
            "working_state", session.get("latest_working_state", {})
        )
        or {},
        "latest_scene_checkpoint": latest_metadata.get(
            "scene_checkpoint", session.get("latest_scene_checkpoint", {})
        )
        or {},
        "latest_social_read": latest_metadata.get(
            "social_read", session.get("latest_social_read", {})
        )
        or {},
...
def match_consequence_choice(user_text: str, options: list[str]) -> str | None:
    """Resolve a player's follow-up into one of the offered consequence options."""
    lowered = (user_text or "").strip().lower()
    if not lowered or not options:
        return None

    number_match = _re.search(r"\b([1-9])\b", lowered)
    if number_match:
        idx = int(number_match.group(1)) - 1
        if 0 <= idx < len(options):
            return options[idx]

    user_words = {word for word in _re.findall(r"[a-z']+", lowered) if len(word) >= 3}
    best_option = None
    best_score = 0
    for option in options:
        option_words = {
            word for word in _re.findall(r"[a-z']+", option.lower()) if len(word) >= 3
        }
        score = len(user_words & option_words)
        if score > best_score:
            best_option = option
            best_score = score
...
def resolve_pending_consequence_choice(
    ...
    *,
    ...
    chosen = match_consequence_choice(user_content, options)
    if chosen is None:
...
def handle_tone_command(
    ...
    *,
    ...
    return (
        f"Unknown tone '{requested}'. Available tones: "

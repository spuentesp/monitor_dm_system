...
def parse_character_card(
    ...
    return CharacterCreate(
        name=name[:200],
        description=str(
            data.get("description") or data.get("char_persona") or ""
        ).strip(),
        personality=str(data.get("personality") or "").strip(),
        first_message=str(
            data.get("first_mes") or data.get("char_greeting") or ""
        ).strip(),
        gm_notes=gm_notes,
        avatar_url=None,

...
async def list_story_threads(story_id: UUID, only_open: bool = True) -> list[ThreadOut]:
    ...
    except Exception as exc:  # noqa: BLE001 — graph threads are optional
    ...
    if not threads:
        ...
        if state:
            threads = [
                ThreadOut(title=label, status="open", source="checkpoint")
                for label in state.active_threads
            ]
            if not only_open:
                threads += [
                    ThreadOut(title=label, status="resolved", source="checkpoint")
                    for label in state.completed_threads
                ]
    return threads
...
async def get_story(story_id: UUID) -> StoryResponse:
    ...
    if not state:
        ...
        except Exception:  # noqa: BLE001 — graph down → treat as missing
            record = None
        if record is None:
        ...
        return StoryResponse(
            ...
            in_game_time=datetime.now(timezone.utc).isoformat(),
            world_ticks=0,
            scenes_completed=0,
        )
...
async def patch_story(story_id: UUID, body: StoryPatch) -> StoryResponse:
    ...
    with MongoDBSaver.from_conn_string(settings.mongodb_uri) as checkpointer:
        ...
        if not current or not current.values:
            raise HTTPException(
                status_code=500, detail="Failed to reload story state after update"
            )

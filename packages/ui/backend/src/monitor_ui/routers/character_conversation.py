...
async def ensure_character_backed(
    character_id: str, universe_id: Optional[str] = None
) -> dict[str, Any]:
    ...
    if universe_id is None:
        universe_id = (
            char.get("default_universe_id")
            or char.get("source_universe_id")
            or await ensure_conversatory_universe()
        )
    ...
    entity_id = await asyncio.to_thread(
        _provision_entity_and_profile, universe_id, char, fields
    )
...
async def delete_incarnation(character_id: str, universe_id: str) -> bool:
    ...
    try:
        ...
        def _drop_profile_doc() -> None:
            get_mongodb_client().get_collection("npc_profiles").delete_one(
                {"entity_id": entity_id}
            )
...
async def send_message(
    ...
    if include_cross_incarnation and getattr(loop, "state", None) is not None:
        loop.state.include_cross_incarnation = False  # type: ignore[attr-defined]

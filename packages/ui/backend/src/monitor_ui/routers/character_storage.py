...
def _version_summary(version: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "last_chatted_at": last_chat.isoformat()
        if hasattr(last_chat, "isoformat")
        else last_chat,
    }


def get_version(character_id: str, universe_id: str) -> Optional[dict[str, Any]]:
...
def get_version_summary(
    character_id: str, universe_id: str
...
def add_version(
    ...
    universe_id: str,
    entity_id: str,
    npc_profile_id: Optional[str] = None,
) -> dict[str, Any]:
...
def delete_version(
    ...
    if char.get("default_universe_id") == universe_id:
        ...
        update_set["default_universe_id"] = (
            new_default.get("universe_id") if new_default else None
        )
        update_set["entity_id"] = (
            new_default.get("entity_id") if new_default else None
        )
        update_set["source_universe_id"] = (
            new_default.get("universe_id") if new_default else None
        )
    _coll().update_one(
        {"id": character_id},
...
def set_default_universe(
    character_id: str, universe_id: Optional[str]
) -> Optional[dict[str, Any]]:
    ...
    if universe_id is not None:
        ...
        if match is None:
            raise ValueError(
                f"No version for universe {universe_id!r} on character {character_id!r}"
            )

...
Upload flow:
  ...
  3. POST /packs/{id}/canonize — apply a pack to a new or existing world via CanonKeeper
"""
...
from .ingest_shared import _pack_to_dict  # noqa: F401 — re-exported for ingest router tests
...
_ingest_executor: Optional[concurrent.futures.ThreadPoolExecutor] = (
    _build_ingest_executor()
)
...
_ingest_active_title: Optional[str] = None
_ingest_active_job_id: Optional[str] = None  # tracks the current job for cancellation
...
def _interrupt_ingest_runtime(reason: str, *, close_executor: bool) -> dict[str, Any]:
    ...
    try:
        ...
        logger.warning(
            "Could not mark stale ingestion jobs failed during runtime cleanup: %s", exc
        )
...
def _reserve_ingest_slot(
    source_title: Optional[str], *, source_id: Optional[str] = None
) -> str:
...
def _mark_ingest_started(token: str, title: str) -> None:
    ...
    with _ingest_lock:
        entry = next(
            (item for item in _ingest_pending_requests if item["token"] == token), None
        )
...
def _attach_job_id_to_request(token: str, job_id: Optional[str]) -> None:
...
def _clear_ingest_slot(
    *, queue_token: Optional[str] = None, expected_title: Optional[str] = None
) -> None:
    """Remove a queued/active ingest request when setup fails or work finishes."""
    removed_title: Optional[str] = None
    with _ingest_lock:
...
def _run_ingest_in_thread(
    *,
    ...
    _captured_job_id: list[Optional[str]] = [None]
    ...
    async def _run() -> None:
        ...
        async def _ingest_with_capture() -> None:
            ...
            import monitor_agents.ingestion_pipeline as _pipeline_module  # noqa: PLC0415

            _orig_create_job = _pipeline_module.mongodb_create_ingestion_job
            ...
            _pipeline_module.mongodb_create_ingestion_job = _patched_create_job
            try:
                    ...
                    try:
                        ...
                        if universe:
                            await library.add_source(
                                multiverse_id=universe.multiverse_id,
                                source_type="file",
                                uri=filename,
                                system_name=universe.system_name or "generic",
                                metadata={
                                    "title": source_title,
                ...
                _pipeline_module.mongodb_create_ingestion_job = _orig_create_job
        ...
        try:
            ...
            if _captured_job_id[0]:
                try:
                    ...
                    from monitor_data.schemas.ingestion_jobs import IngestionJobUpdate  # noqa: PLC0415
                    from monitor_data.schemas.base import IngestionStatus  # noqa: PLC0415
                    ...
                    mongodb_update_ingestion_job(
                        ...
                        IngestionJobUpdate(
                            ...
                            errors=[
                                f"Job timed out after {_JOB_TIMEOUT_SECS // 60} minutes"
                            ],
                            completed_at=datetime.now(timezone.utc),
                        ),
                    )
            ...
            logger.exception(
                "Background ingestion failed for '%s': %s", source_title, exc
            )
...
def _check_ingest_busy() -> None:
    ...
    with _ingest_lock:
        if _ingest_active_requests or _ingest_pending_requests:
            ...
            raise HTTPException(
                409,
                f"Ingestion busy: {active_count} running / {queued_count} queued. "
                f"Current head item: '{active_title}'.",
            )
...
def _build_pending_job_placeholder(
    ...
    *,
    ...
    now = queued_at or datetime.now(timezone.utc).isoformat()
...
def _build_source_document_job_placeholder(source: dict[str, Any]) -> dict[str, Any]:
    ...
    errors = (
        ...
        if is_failed and source.get("error_message")
        else []
    )
    ...
    return {
        ...
        "axioms_extracted": 0,
        "proposals_generated": 0,
        "started_at": source.get("uploaded_at")
...
def _normalize_selected_layers(
    raw_layers: Any, pack_type_enum: KnowledgePackType
) -> list[str]:
    """Accept FastAPI defaults or raw iterables and return a clean layer list."""
    candidates = (
        raw_layers
        if isinstance(raw_layers, list)
        else _default_processing_checklist(pack_type_enum)
    )
    return [
...
async def force_unlock_ingest() -> dict:
...
async def list_sources() -> list[dict]:
    ...
    seen_file_keys: set[tuple[str, str]] = {
        (s["filename"], str(s["metadata"].get("universe_id", ""))) for s in sources
    }
    for doc in all_docs:
...
async def upload_source(
    ...
    analysis_layers: Optional[list[str]] = Form(default=None),
    ingest_now: bool = Form(default=True),
    # New setting (creates Multiverse + Universe)
    new_setting_name: Optional[str] = Form(default=None),
    new_setting_system: Optional[str] = Form(default=None),
    # Existing setting (finds or creates Universe under it)
    multiverse_id: Optional[str] = Form(default=None),
    ...
    requested_setting_name = (
        ...
        if isinstance(new_setting_name, str) and new_setting_name.strip()
        else None
    )
    requested_setting_system = (
        new_setting_system.strip()
        if isinstance(new_setting_system, str) and new_setting_system.strip()
        else None
    )
    requested_multiverse_id = (
        multiverse_id.strip()
        if isinstance(multiverse_id, str) and multiverse_id.strip()
        else None
    )
    ...
    queue_position: Optional[int] = None
    saved_source: Optional[dict[str, UUID | str]] = None
    ...
    if should_ingest:
        ...
        with _ingest_lock:
            queue_position = next(
                (
                    index + 1
                    for index, entry in enumerate(_ingest_pending_requests)
                    if entry["token"] == queue_token
                ),
                None,
            )
    ...
    try:
        ...
        _MAX_UPLOAD_BYTES = int(
            _os.environ.get("MONITOR_MAX_INGEST_FILE_BYTES", str(200 * 1024 * 1024))
        )
        ...
        try:
            if requested_setting_name:
                ...
                universe_uid = _ensure_universe_for_multiverse(
                    UUID(requested_multiverse_id)
                )
            else:
        ...
        if should_ingest:
            try:
                ...
                raise HTTPException(
                    500, f"Could not queue ingestion pipeline: {exc}"
                ) from exc
        else:
    ...
    slug = (
        "".join(ch.lower() if ch.isalnum() else "-" for ch in source_title).strip("-")
        or "ingest"
    )
...
async def delete_source(source_id: str) -> None:
    ...
    if source_busy:
        raise HTTPException(
            409, "This source is currently queued or running and cannot be deleted yet."
        )
    ...
    try:
        from monitor_data.db.qdrant import get_qdrant_client  # noqa: PLC0415
        ...
        await qdrant.delete(
            ...
            points_selector=Filter(
                must=[FieldCondition(key="source_id", match=MatchValue(value=str(uid)))]
            ),
        )
...
async def rescan_source(
    ...
    analysis_layers: Optional[list[str]] = Query(default=None),
) -> dict:
    ...
    try:
        ...
        with db_op("Database unavailable"):
            jobs_result = mongodb_list_ingestion_jobs(
                IngestionJobFilter(source_id=src_uid, limit=1, sort_order="desc")
            )
        ...
        if not doc:
            ...
            with db_op("Could not retrieve document"):
                ...
                doc = next(
                    (d for d in all_docs if str(d.source_id) == str(src_uid)), None
                )
        if not doc:
        ...
        if universe_uid is not None:
            ...
            if existing is None:
                ...
                try:
                    ...
                    raise HTTPException(
                        500, f"Could not re-create setting: {exc}"
                    ) from exc
        else:
        ...
        try:
            mongodb = get_mongodb_client()
            mongodb.get_collection("ingestion_jobs").delete_many(
                {"source_id": str(src_uid)}
            )
        except Exception as exc:  # noqa: BLE001
            logger.warning(
...
async def cancel_job(job_id: str) -> dict:
    ...
    with db_op("Database unavailable"):
        ...
        if result.matched_count == 0:
            ...
            raise HTTPException(
                409, f"Job is not running (status={existing['status']})"
            )
...
async def purge_failed_jobs() -> dict:
...
async def clear_llm_cache() -> dict:
    ...
    from monitor_agents.dspy_runtime import clear_bg_registry_cache  # noqa: PLC0415
    from monitor_data.db.redis import clear_runtime_cache  # noqa: PLC0415
...
async def stream_job(job_id: str):
    ...
    async def _event_generator():
        ...
        job = None
        for attempt in range(_MAX_STARTUP_RETRIES):
            try:
                job = await asyncio.to_thread(mongodb_get_ingestion_job, uid)
                if job:
                    break
...
async def upload_asset(
    ...
    universe_id: Optional[str] = Form(default=None),
    ...
    _MAX_ASSET_BYTES = int(
        os.environ.get("MONITOR_MAX_ASSET_BYTES", str(500 * 1024 * 1024))
    )
    ...
    await db_op(lambda: mongo.get_collection("binary_assets").insert_one(asset_doc))
...
async def list_assets(
    ...
    cursor = (
        mongo.get_collection("binary_assets")
        .find(query)
        .sort("created_at", -1)
        .skip(offset)
        .limit(limit)
    ...
    total = await db_op(
        lambda: mongo.get_collection("binary_assets").count_documents(query)
    )
    items = []
...
async def get_asset(asset_id: str) -> dict:
    ...
    mongo = get_mongodb_client()
    doc = await db_op(
        lambda: mongo.get_collection("binary_assets").find_one(
            {"_id": asset_id, "deleted": False}
        )
    )
    if not doc:
...
async def delete_asset(asset_id: str) -> dict:
    ...
    await db_op(
        lambda: mongo.get_collection("binary_assets").update_one(
            {"_id": asset_id},
            {
                "$set": {
                    "deleted": True,

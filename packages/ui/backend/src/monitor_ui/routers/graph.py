...
async def get_world_graph(
    multiverse_id: Optional[UUID] = Query(
        None, description="Scope to a single setting"
    ),
    universe_id: Optional[UUID] = Query(None, description="Scope to a single universe"),
    entity_types: Optional[str] = Query(
        None, description="Comma-separated entity types"
    ),
    related_to: Optional[UUID] = Query(
        None, description="Show only entities related to this entity"
    ),
) -> dict:
    """
    ...
    try:
        ...
        if entity_types:
            allowed_entity_types = {
                t.strip().lower() for t in entity_types.split(",") if t.strip()
            }
    ...
    for i, mv in enumerate(_tier_layout(multiverses, y=0, x_spread=MV_SPREAD)):
        mv_obj, x, y = mv
        mv_id = str(mv_obj.id)
        ...
        nodes.append(
            {
                ...
                "data": {
                    "label": mv_obj.name,
                    "kind": "multiverse",
                    "subtitle": mv_obj.system_name or None,
                },
            }
        )
    ...
    for mv_id, mv_univs in mv_children.items():
        ...
        for u_obj, x, y in _tier_layout(
            mv_univs, y=220, parent_x=parent_x, x_spread=240
        ):
            u_id = str(u_obj.id)
            u_positions[u_id] = x
            nodes.append(
                {
                    ...
                    "data": {
                        "label": u_obj.name,
                        "kind": "universe",
                        "subtitle": u_obj.genre or None,
                        "tags": [u_obj.canon_level.value] if u_obj.canon_level else [],
                    },
                }
            )
            edges.append(
    ...
    return {
        ...
        "rel_types": sorted(
            {rel.rel_type for rel in rels_r.relationships if rel.rel_type}
        ),
    }
...
async def get_universe_graph(
    ...
    rel_types: Optional[str] = Query(
        default=None, description="Comma-separated relationship types to include"
    ),
    limit_per_depth: int = Query(
        default=50, ge=1, le=200, description="Max entities per depth level"
    ),
) -> dict:
    """
    ...
    try:
        ...
        if rel_types:
            allowed_rel_types = {
                t.strip().lower() for t in rel_types.split(",") if t.strip()
            }
        ...
        if allowed_entity_types:
            filtered_entities = [
                ...
                if e.entity_type.lower() in allowed_entity_types
            ]
        ...
        if allowed_rel_types:
            filtered_rels = [
                ...
                if r.rel_type and r.rel_type.lower() in allowed_rel_types
            ]
...
async def get_entity_ego_graph(
    ...
    try:
        ...
        rels_r = neo4j_list_relationships(
            RelationshipFilter(universe_id=universe_id, limit=2000)
        )
    ...
    for nid in neighbor_ids:
        ...
        except Exception:  # noqa: BLE001 — skip unfetchable neighbours
            e = None
        if e is not None:
    ...
    return {
        ...
        "entity_types": sorted(
            {
                entity_map.get(
                    eid, type("E", (), {"entity_type": "unknown"})()
                ).entity_type
        ...
        "rel_types": sorted(
            {
                ...
                for rel in rels_r.relationships
                if rel.rel_type
                and str(rel.from_entity_id) in entity_ids
                and str(rel.to_entity_id) in entity_ids
            }
        ),
    }

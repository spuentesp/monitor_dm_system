...
_STATUS_MAP: dict[str, str] = {
    ...
    # Default is "indexed" so any unknown status doesn't render as "failed".
    "pending": "saved",
...
def _default_processing_checklist(pack_type: KnowledgePackType) -> list[str]:
    ...
    return mapping.get(pack_type, ["axioms", "entities", "lore", "game_system", "rules"])


def _create_setting(name: str, system_name: str, game_system_id: UUID | None = None) -> tuple[UUID, UUID]:
...
def _ensure_universe_for_multiverse(multiverse_id: UUID) -> UUID:
    ...
    universes = neo4j_list_universes(UniverseFilter(multiverse_id=multiverse_id, limit=1))
    if universes:
...
def _source_from_job(job: Any) -> dict:  # type: ignore
    ...
    title = (source_node.title if source_node else None) or (doc.title if doc else "Unknown")
    source_type = doc.file_type if doc else (source_node.source_type.value if source_node else "txt")
    ...
    return {
        ...
        "error_message": (job.last_error.message if job.last_error else (job.errors[0] if job.errors else None)),
...
def _job_to_dict(job: Any) -> dict:  # type: ignore
    ...
    # last_error is a structured LastError model — serialize to a JSON-safe
    # dict (never the raw Pydantic object, or the SSE json.dumps crashes) and
    # expose a separate display-safe `error` string from its message.
    last_error = job.last_error.model_dump(mode="json") if job.last_error else None
    return {
        "id": str(job.job_id),
        "universe_id": str(job.universe_id),
        ...
        "failed_sections": [dict(s) for s in job.failed_sections],
        ...
        "error": (job.last_error.message if job.last_error else (job.errors[0] if job.errors else None)),
    }
...
def _game_system_summary(pack: Any) -> dict | None:  # type: ignore
    ...
    if gsd is not None:
        return {
            "name": gsd.name,
            "core_mechanic": gsd.core_mechanic.model_dump(mode="json") if gsd.core_mechanic else None,
...
def _pack_to_dict(pack: Any) -> dict:  # type: ignore
    ...
    source_profile_data_dict = source_profile.model_dump(mode="json") if source_profile else None
    source_mindscape_dict = source_mindscape.model_dump(mode="json") if source_mindscape else None
    chunk_summaries = [
        ...
        "axioms": [
            {
                ...
                "tags": a.tags,
                "evidence_refs": [
                    er.model_dump(mode="json") if hasattr(er, "model_dump") else er for er in (a.evidence_refs or [])
                ],
            }
        ...
        "entity_archetypes": [
            {
                ...
                "evidence_refs": [
                    er.model_dump(mode="json") if hasattr(er, "model_dump") else er for er in (e.evidence_refs or [])
                ],
            }
        ...
        "lore_facts": [
            {
                ...
                "evidence_refs": [
                    er.model_dump(mode="json") if hasattr(er, "model_dump") else er for er in (f.evidence_refs or [])
                ],
            }

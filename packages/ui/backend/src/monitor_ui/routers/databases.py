...
def _make_offline(name: str, db_type: str, url: str, error: str) -> DatabaseStatus:
    ...
    return DatabaseStatus(
        name=name, db_type=db_type, url=url, status="offline", error=error
    )
...
async def _probe_mongodb() -> DatabaseStatus:
    ...
    client: AsyncIOMotorClient = AsyncIOMotorClient(
        _settings.mongodb_uri, serverSelectionTimeoutMS=3000
    )
    await client.admin.command("ping")
    db = client[_settings.mongodb_database]
    collections = await db.list_collection_names()
    stats: dict = {"collections": len(collections)}
...
_PROBES: dict[
    str, tuple[str, str, Callable[[], Coroutine[Any, Any, DatabaseStatus]]]
] = {

...
class QueryPatternMetrics(BaseModel):
    ...
    p95_time_ms: Optional[float] = Field(
        None, description="95th percentile execution time (milliseconds)"
    )
    p99_time_ms: Optional[float] = Field(
        None, description="99th percentile execution time (milliseconds)"
    )
...
class PerformanceOverview(BaseModel):
    ...
    avg_time_ms: float = Field(
        ..., description="Average query execution time (milliseconds)"
    )
    slow_queries: int = Field(..., description="Number of slow queries (>150ms)")
    slow_query_rate: float = Field(
        ..., description="Percentage of queries that are slow (0-100)"
    )
    unique_patterns: int = Field(..., description="Number of unique query patterns")
    uptime_seconds: Optional[float] = Field(
        None, description="Time since tracker started (seconds)"
    )
...
class SlowQueryInfo(BaseModel):
    ...
    sample_query: Optional[str] = Field(
        None, description="Sample query (first 200 chars)"
    )
...
class PerformanceReport(BaseModel):
    ...
    top_patterns: List[QueryPatternMetrics] = Field(
        ..., description="Top query patterns by frequency"
    )
    slowest_patterns: List[QueryPatternMetrics] = Field(
        ..., description="Query patterns with highest average time"
    )
    recent_slow_queries: List[SlowQueryInfo] = Field(
        ..., description="Recent slow query executions"
    )
...
async def get_query_patterns(
    ...
    min_count: int = Query(
        default=1, ge=1, description="Minimum query count to include"
    ),
...
async def get_slow_queries(
    ...
    min_time_ms: float = Query(
        default=150.0, ge=0.0, description="Minimum execution time to include"
    ),
) -> List[SlowQueryInfo]:
...
@router.get(
    "/performance/report", response_model=PerformanceReport, tags=["performance"]
)
async def get_performance_report(
    pattern_limit: int = Query(
        default=10, ge=1, le=50, description="Top patterns to include"
    ),
    slow_query_limit: int = Query(
        default=20, ge=1, le=100, description="Recent slow queries to include"
    ),
) -> PerformanceReport:
...
class AlertInfo(BaseModel):
    ...
    type: str = Field(
        ..., description="Alert type (e.g., 'slow_query', 'high_slow_query_rate')"
    )
    severity: str = Field(
        ..., description="Alert severity (info, warning, error, critical)"
    )
    message: str = Field(..., description="Alert message")
    pattern: Optional[str] = Field(None, description="Query pattern (if applicable)")
...
class AlertConfiguration(BaseModel):
    ...
    critical_query_threshold_ms: float = Field(
        ..., description="Threshold for critical query alerts (milliseconds)"
    )
    high_slow_query_rate_threshold: float = Field(
        ..., description="Threshold for high slow query rate (0-1)"
    )
    high_avg_time_threshold_ms: float = Field(
        ..., description="Threshold for high average query time (milliseconds)"
    )
    degradation_threshold_percent: float = Field(
        ..., description="Threshold for performance degradation (0-1)"
    )
    ...
    baseline_metrics: Optional[Dict[str, float]] = Field(
        None, description="Baseline metrics for degradation detection"
    )
    baseline_timestamp: Optional[str] = Field(
        None, description="ISO timestamp of baseline"
    )
...
async def get_alerts(
    limit: int = Query(
        default=50, ge=1, le=200, description="Maximum alerts to return"
    ),
...
async def update_alerts_config(
    slow_query_threshold_ms: Optional[float] = Query(
        default=None, ge=0.0, description="Slow query threshold (ms)"
    ),
    critical_query_threshold_ms: Optional[float] = Query(
        default=None, ge=0.0, description="Critical query threshold (ms)"
    ),
    high_slow_query_rate_threshold: Optional[float] = Query(
        default=None, ge=0.0, le=1.0, description="High slow query rate threshold (0-1)"
    ),
    high_avg_time_threshold_ms: Optional[float] = Query(
        default=None, ge=0.0, description="High average query time threshold (ms)"
    ),
    degradation_threshold_percent: Optional[float] = Query(

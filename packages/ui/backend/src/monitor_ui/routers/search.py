...
class SearchResultItem(BaseModel):
    ...
    payload: dict[str, Any] = Field(
        default_factory=dict, description="Stored metadata payload"
    )
    ...
    universe_id: Optional[str] = Field(default=None, description="Universe UUID string")
    story_id: Optional[str] = Field(default=None, description="Story UUID string")
    scene_id: Optional[str] = Field(default=None, description="Scene UUID string")
    node_type: Optional[str] = Field(
        default=None,
        description="Knowledge node type: axiom | fact | lore_event",
    )
    canon_level: Optional[str] = Field(
        default=None, description="Canon level: proposed | canon | retconned"
    )
...
class SemanticSearchResponse(BaseModel):
    ...
    collections_searched: list[str] = Field(
        description="Which collections were queried"
    )
...
def _build_result_item(
    scored_vector, collection: str, text: Optional[str] = None
) -> SearchResultItem:
...
def _embed_query_text(query_text: str) -> list[float]:
    """Embed query text using the configured embedding model (sync)."""
    from monitor_data.db.embeddings import embed_text
    from monitor_data.tools.qdrant_tools import run_sync

    return run_sync(embed_text(query_text))
...
async def semantic_search(
    ...
    universe_id: Optional[UUID] = Query(
        default=None, description="Scope search to a specific universe"
    ),
    entity_type: Optional[str] = Query(
        default=None,
        description="Filter entities by type: character | faction | location | object | concept | organization",
    ),
    canon_level: Optional[str] = Query(
        default=None,
    ...
    limit: int = Query(
        default=10, ge=1, le=100, description="Max results per collection"
    ),
    ...
    target_collections = [
        ...
        for c in (
            collections.split(",")
            if collections
            else ["entities", "scenes", "snippets", "knowledge"]
        )
    ...
    for collection in target_collections:
        try:
            ...
            log.warning(
                "search_collection_failed", collection=collection, error=str(exc)
            )
            continue
...
async def universe_search(
    ...
    try:
        ...
        raise HTTPException(
            status_code=503, detail=f"Failed to embed query: {exc}"
        ) from exc

...
def test_get_returns_session_or_404(client):
    ...
    with patch("monitor_ui.routers.play_sessions.mongodb_get_play_session", return_value=None):
...
def test_list_passes_filters(client):
    fake_list = PlaySessionListResponse(sessions=[_fake_session()], total=1, limit=50, offset=0)
    with patch(
...
def test_update_returns_404_when_missing(client):
    with patch(
        ...
        response = client.patch(f"/api/play-sessions/{uuid4()}", json={"status": "completed"})
    assert response.status_code == 404
...
def test_add_scene_is_idempotent_via_tool(client):
    ...
    with patch(
        ...
        scene_id = uuid4()
        response = client.post(f"/api/play-sessions/{session.session_id}/scenes/{scene_id}")
    assert response.status_code == 200
...
def test_play_home_returns_active_and_recent(client):
    ...
    with (
        patch(
            "monitor_ui.routers.play_sessions.mongodb_list_play_sessions",
            return_value=PlaySessionListResponse(sessions=[active], total=1, limit=10, offset=0),
        ),
        patch(

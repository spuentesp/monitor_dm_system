...
class TestUpdateCharacter:
    ...
    def test_update_character_name(self, mock_get, mock_update):
        ...
        resp = client.put(
            f"{BASE}/characters/{original['id']}", json={"name": "Updated Name"}
        )
...
class TestUpdateCharacterSheet:
    ...
    def test_update_sheet_success(self, mock_update):
        sheet_id = uuid4()
        mock_update.return_value = _make_sheet_payload(
            sheet_id, stats={"hp": {"current": 25, "max": 30}}
        )
...
class TestListCharacterSheets:
    ...
    def test_list_sheets_with_game_system_filter(self, mock_list):
        ...
        resp = client.get(
            f"{BASE}/character-sheets?game_system_id={game_system_id}&limit=10&offset=5"
        )

...
def test_create_entity_returns_201():
    ...
    with patch(f"{ENTITIES_TOOLS}.neo4j_create_entity") as mock_create:
        mock_create.return_value = _entity(universe_id=uni, name="Iron Brotherhood", entity_type="faction")
        resp = client.post(
...
def test_patch_entity_fields():
    eid = str(uuid4())
    with (
        patch(f"{ENTITIES_TOOLS}.neo4j_get_entity") as mock_get,
        patch(f"{ENTITIES_TOOLS}.neo4j_update_entity") as mock_update,
    ):
        mock_get.return_value = _entity(id=eid)
...
def test_patch_entity_tags_diffs_add_and_remove():
    ...
    with (
        patch(f"{ENTITIES_TOOLS}.neo4j_get_entity") as mock_get,
        patch(f"{ENTITIES_TOOLS}.neo4j_set_state_tags") as mock_tags,
    ):
...
def test_patch_entity_404():
    with patch(f"{ENTITIES_TOOLS}.neo4j_get_entity") as mock_get:
        mock_get.return_value = None
        resp = client.patch(f"/api/entities/entities/{uuid4()}", json={"name": "Nope"})
    assert resp.status_code == 404
...
def test_patch_entity_noop_when_tags_unchanged():
    ...
    with (
        ...
        resp = client.patch(f"/api/entities/entities/{eid}", json={"tags": ["hidden"]})
    assert resp.status_code == 200

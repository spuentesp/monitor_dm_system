  ...
  return (
    <div className="space-y-2">
      ...
      {sortedJobs.slice(0, 15).map((job) => {
        ...
        const errorText = job.error || job.errors?.[0] || job.last_error?.message || null;
        // Completed (or partially completed) jobs with proposals can be
        // reviewed on the by-ingest scope of /forge/review (F1-4).
        const hasReviewableProposals =
          !isLive && !isFailed && (job.proposals_generated ?? 0) > 0;
            ...
            <div className="p-3 flex items-center gap-3">
              ...
              <div className="flex items-center gap-1 self-start">
                ...
                {hasReviewableProposals && (
                  <button
                    onClick={() => router.push(`/forge/review?job=${job.id}`)}
                    className="flex items-center gap-1 px-2 py-1 rounded-lg text-[11px] text-emerald-300 border border-emerald-500/25 hover:bg-emerald-500/10 transition-colors"
                    title="Review this job's proposals"
                  >
                ...
                {job.pack_id && (
                  <button
                    onClick={() => router.push(`/forge/packs?pack=${job.pack_id}`)}

...
// ═══════════════════════════════════════════════════════════════
//  Create Random Table Modal (F3-2d)
// ═══════════════════════════════════════════════════════════════

const TABLE_TYPES = Object.keys(TABLE_TYPE_CONFIG);

function CreateRandomTableModal({
  universeId,
  onClose,
  onCreated,
}: {
  ...
  onClose: () => void;
  onCreated: (table: RandomTable) => void;
}) {
  const queryClient = useQueryClient();
  const [name, setName] = useState("");
  const [tableType, setTableType] = useState<string>("custom");
  ...
  const createMutation = useMutation({
    mutationFn: () =>
      randomTablesApi.create({
        name: name.trim(),
        table_type: tableType,
        dice_formula: diceFormula.trim() || "1d20",
        description: description.trim() || undefined,
        universe_id: universeId ?? undefined,
        // Backend requires >= 1 entry; seed one the user edits in TableEditor.
        entries: [{ min_roll: 1, max_roll: 20, value: "New entry" }],
      }),
    onSuccess: (table) => {
      queryClient.invalidateQueries({ queryKey: ["random-tables"] });
      onCreated(table);
    },
    onError: (err: Error) => setError(err.message || "Failed to create table"),
  });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <motion.div
        ...
        className="glass-dark rounded-2xl border border-white/10 w-full max-w-sm mx-4 overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/5">
          <div className="flex items-center gap-2">
            <Dice5 className="w-4 h-4 text-amber-400" />
            <h2 className="text-sm font-semibold text-slate-200">New Random Table</h2>
          </div>
          <button onClick={onClose} aria-label="Close" className="text-slate-500 hover:text-slate-300 transition-colors">
            <X className="w-4 h-4" />
          </button>
        </div>
        <div className="px-5 py-4 space-y-3">
          ...
          <div className="flex gap-3">
            <div className="space-y-1 flex-1">
              <label className="text-[10px] font-bold tracking-widest uppercase text-slate-500">Type</label>
              <select
                value={tableType}
              ...
              >
                {TABLE_TYPES.map((t) => (
                  <option key={t} value={t}>{TABLE_TYPE_CONFIG[t]?.label ?? t}</option>
                ))}
              </select>
            </div>
            <div className="space-y-1 w-28">
              <label className="text-[10px] font-bold tracking-widest uppercase text-slate-500">Dice</label>
              <input
                value={diceFormula}
          ...
          <div className="flex gap-2 pt-1">
            <button onClick={onClose} className="btn-ghost text-xs py-2 flex-1">Cancel</button>
            <button
              onClick={handleSubmit}
              disabled={createMutation.isPending}
              className="btn-cyber text-xs py-2 flex-1"
            >
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex items-center gap-2 px-4 py-2 border-b border-white/5 flex-shrink-0">
        ...
        <button
          ...
          title="Create a new random table"
          className="btn-ghost text-xs py-0.5 px-2"
        >
          <Plus className="w-3 h-3" /> New Table
        </button>
        <button onClick={() => refetch()} className="btn-ghost text-xs py-0.5 px-2" aria-label="Refresh tables">
      ...
      {/* Create modal — on success, drop straight into table-edit mode */}
      {showCreate && (
        <CreateRandomTableModal

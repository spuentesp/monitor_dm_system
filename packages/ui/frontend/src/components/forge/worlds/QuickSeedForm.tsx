...
const SEED_EXAMPLES = [
  ...
  "A frontier mining moon where the or whispers and the company owns your air.",
...
export function QuickSeedResultCard({ result }: { result: QuickWorldResult }) {
  return (
    <motion.div
    ...
    >
      ...
      <div className="flex flex-wrap gap-2 pt-1">
        ...
        <Link
          href={`/forge/worlds?universe=${result.universe_id}`}
...
/**
 * The "quick seed" world-creation form body (F1-3a). Shared between the
 * Ingest Studio QuickStartPanel (thin wrapper) and the /forge/worlds/new
 * wizard's Quick-seed step. The result card is the confirm-and-land UI:
 * it links into the Worlds tree at /forge/worlds?universe=<id>.
 */
export function QuickSeedForm({ showHeader = true }: { showHeader?: boolean }) {
  ...
  return (
    <div className={cn("space-y-5", showHeader && "max-w-3xl mx-auto")}>
      {showHeader && (
      ...
      <AnimatePresence>
        {mut.data && <QuickSeedResultCard result={mut.data} />}
      </AnimatePresence>
    </div>
  );
}

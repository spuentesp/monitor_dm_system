...
interface BubbleMetadata {
  ...
  thinking?: string;
  /** Set when the Narrator fell back to the GM's raw (unpolished) draft
   *  because its own LLM call hit a provider-level failure (rate limit,
   *  quota, auth). The turn still has real content — just less refined. */
  degraded?: { error_class?: string; message?: string } | null;
}
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Degraded-turn notice: the Narrator's own LLM call failed
            (rate limit / quota / auth) and it fell back to the GM's raw
            draft. The turn still has content, just unpolished — tell the
            player why instead of leaving it unexplained. */}
        {isGM && meta.degraded && (
          <div className="mt-2 rounded-lg border border-amber-500/20 bg-amber-500/5 px-2.5 py-1.5 text-[10px] text-amber-300/80">
            ⚠ Simplified response — {meta.degraded.message || "the GM's AI provider hit a usage limit"}.
          </div>
        )}

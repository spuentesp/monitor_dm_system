 ...
 * and "Wrap up session" (P1.3) canonizes the scene and produces a wrap-up
 * digest — recap, canon decisions, open threads, next-session teaser — while
 * persisting the recap onto the session (P1.4).
 */
export function SessionRecorder({
  ...
  // P1.3 — guided wrap-up digest modal; mounting it fires chatApi.wrapUp.
  const [showWrapUp, setShowWrapUp] = useState(false);
  // P1.1 — advisory live contradiction alerts, keyed to the logged entry text.
  const [entryAlerts, setEntryAlerts] = useState<
    Array<{ messageText: string; alert: Contradiction }>
  >([]);
  // P1.2 — per-entry capture insights, keyed to the logged entry text.
  const [entryInsights, setEntryInsights] = useState<
    Array<{ messageText: string; insight: CaptureInsight }>
  >([]);
  ...
  const logEntry = () => {
    ...
    chatApi
      ...
      .then(() => {
        ...
        // P1.1 — live contradiction check. Advisory, non-blocking (no
        // setWaiting), and failures are swallowed: a missed alert must never
        // break logging. Entries under 12 chars are too thin to check.
        if (text.length >= 12) {
          gmApi
            .checkEntryContradiction(universeId, text)
            .then((res) => {
              if (res.alert) {
                setEntryAlerts((prev) => [...prev, { messageText: text, alert: res.alert! }]);
              }
            })
            .catch(() => {
        ...
        // P1.2 — per-entry capture insights. Same advisory contract: a
        // failed analysis renders nothing and must never break logging.
        gmApi
          .captureInsights(universeId, text)
          .then((insight) => {
            setEntryInsights((prev) => [...prev, { messageText: text, insight }]);
          })
          .catch(() => {
            // advisory only — ignore
          });
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
        ...
        {active && (
          <>
            ...
            <button
              ...
              title="Canonize the scene and build the wrap-up digest — recap, canon decisions, open threads, next-session teaser (P1.3)"
            >
      ...
      {sessionsLoading ? (
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center px-8 space-y-3">
          ...
          <p className="text-xs text-slate-600 max-w-sm leading-relaxed">
            ...
            keeps the transcript, reflects insights back, and builds the wrap-up digest — canon
            decisions, recap, open threads — when you wrap up the session.
          </p>
        ...
        <>
          ...
          <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2.5">
            {messages.map((m) => {
              ...
              const liveAlert = entryAlerts.find((a) => a.messageText === m.content)?.alert;
              const insight = entryInsights.find((i) => i.messageText === m.content)?.insight;
              const hasInsight =
                insight &&
                (insight.participants.length > 0 ||
                  insight.locations.length > 0 ||
                  insight.candidate_facts.length > 0 ||
                  insight.advances_thread !== "");
              return (
                <div key={m.id} className="space-y-1.5">
                  ...
                  {liveAlert && (
                    <div
                      ...
                      className="rounded-xl border border-amber-500/25 bg-amber-500/5 px-3 py-2 text-xs text-amber-300 flex items-start gap-2"
                    >
                      <AlertTriangle className="w-3.5 h-3.5 flex-shrink-0 mt-0.5" />
                      <div>
                        <span className="font-semibold">Possible canon conflict.</span>{" "}
                        {liveAlert.explanation}
                        <span className="block text-[10px] text-amber-400/70 mt-0.5">
                          {liveAlert.suggestion}
                        </span>
                      </div>
                    </div>
                  )}
                  {hasInsight && insight && (
                    <div
                      data-testid="capture-insights"
                      className="rounded-xl border border-white/8 bg-white/2 px-3 py-2 space-y-1.5"
                    >
                      {(insight.participants.length > 0 || insight.locations.length > 0) && (
                        <div className="flex flex-wrap items-center gap-1.5">
                          {insight.participants.map((name) => (
                            <span
                              key={`p-${name}`}
                              className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full border border-emerald-500/20 bg-emerald-500/5 text-emerald-300"
                            >
                              <Users className="w-2.5 h-2.5" /> {name}
                            </span>
                          ))}
                          {insight.locations.map((name) => (
                            <span
                              key={`l-${name}`}
                              className="inline-flex items-center gap-1 text-[10px] px-2 py-0.5 rounded-full border border-sky-500/20 bg-sky-500/5 text-sky-300"
                            >
                      ...
                      {insight.advances_thread && (
                        <div className="flex items-center gap-1.5 text-[10px] text-purple-300">
                          <GitBranch className="w-3 h-3" /> Advances: {insight.advances_thread}
                        </div>
                      )}
                      {insight.candidate_facts.length > 0 && (
                        <details className="group">
                          <summary className="flex items-center gap-1.5 text-[10px] text-slate-500 uppercase tracking-wider cursor-pointer select-none">
                            <Lightbulb className="w-3 h-3" /> Candidate facts (
                            {insight.candidate_facts.length})
                          </summary>
                          <ul className="mt-1 space-y-0.5 pl-4 list-disc text-[11px] text-slate-400">
                            {insight.candidate_facts.map((fact, i) => (
                              <li key={i}>{fact}</li>
                            ))}
                          </ul>
                        </details>
      ...
      {showWrapUp && activeId && (
        <WrapUpModal sessionId={activeId} onClose={() => setShowWrapUp(false)} />
      )}
    </div>
  );
}

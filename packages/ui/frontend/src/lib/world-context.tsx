 ...
 * (e.g. in isolation tests). Returns null when no provider is present.
 */
export function useOptionalWorldContext(): WorldContextValue | null {
  return useContext(WorldContext) ?? null;
}

/**
 * Repair a persisted world selection after a deletion (F3-3 phase 7).
 * Deleting the selected universe — or its whole multiverse — must not leave
 * a dangling selection behind for Play/GM pages to trip over.
 */
export function repairWorldSelection(
  sel: WorldSelection,
  deleted: { universeId?: string; multiverseId?: string },
): WorldSelection {
  if (deleted.multiverseId && sel.multiverseId === deleted.multiverseId) {
    return { multiverseId: null, universeId: null, universeLabel: null };
  }
  if (deleted.universeId && sel.universeId === deleted.universeId) {
    return { ...sel, universeId: null, universeLabel: null };
  }
  return sel;
}

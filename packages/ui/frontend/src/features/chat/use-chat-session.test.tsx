...
describe("useChatSession — REST fallback when WS disconnected", () => {
  ...
  it("dice_request in the REST response sets pendingDiceRequest (WS-disconnected parity)", async () => {
    const apiModule = await import("@/lib/api");
    const sendMessageSpy = vi
      .spyOn(apiModule.chatApi, "sendMessage")
      .mockResolvedValue({
        ...
        session_id: SESSION,
        role: "gm",
        content: "Give me a Strength check.",
        timestamp: new Date().toISOString(),
        metadata: { dice_request: { spec: "1d20+3", reason: "Strength" } },
      } as never);
    ...
    await act(async () => {
      result.current.send("I push the door open", {});
    });

...
// Client-side input validation only — the roll itself is server-authoritative
// (POST /gm/roll); the client never generates die values.
function isValidDiceExpression(expr: string): boolean {
...
function DiceRoller() {
  ...
  const doRoll = (expression: string) => {
    if (!isValidDiceExpression(expression)) return;
    rollMutation.mutate(expression);
  };

  const exprValid = isValidDiceExpression(expr);
  ...
  return (
    <div className="flex flex-col h-full space-y-4 overflow-hidden">
      ...
      <div className="flex gap-2">
        ...
        <button
          onClick={() => doRoll(expr)}
          disabled={!exprValid || rollMutation.isPending}
          className={cn("btn-cyber px-3", (!exprValid || rollMutation.isPending) && "opacity-40 cursor-not-allowed")}
        >
          {rollMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Dices className="w-4 h-4" />}
        </button>
      </div>
...
function SessionNotebook({
  ...
  // P2.3 — server-backed scratchpad (replaces localStorage).
  // Hydrate from the API when the active universe changes; skip the query
  // when no universe is selected.
  const notesQuery = useQuery({
    queryKey: ["gm-notes", universeId],
    queryFn: () => gmApi.getNotes(universeId!),
    enabled: !!universeId,
    staleTime: 30_000,
  });

  useEffect(() => {
    if (notesQuery.data) setNotes(notesQuery.data.content);
  }, [notesQuery.data?.updated_at, notesQuery.data?.content]);

  // Debounced autosave: fire on every keystroke, but coalesce to one PUT
  // per 1.5s of idle. Skip when the field is empty (no row to write).
  const upsertNotes = useMutation({
    mutationFn: async (content: string) => gmApi.upsertNotes(universeId!, content),
    onSuccess: () => {
      setSaved(true);
  ...
  useEffect(() => {
    ...
    if (notes === notesQuery.data?.content) return; // no change
    const handle = setTimeout(() => {
      if (notes.trim().length > 0) {
        upsertNotes.mutate(notes);
      }
    }, 1500);
    return () => clearTimeout(handle);
  }, [notes, universeId, notesQuery.data?.content]);
    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex items-center gap-2 px-4 py-2 border-b border-white/5 flex-shrink-0">
        ...
        <AnimatePresence>
          ...
          {upsertNotes.isPending && (
            <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-xs text-slate-500 flex items-center gap-1">
...
function GMAssistantPageContent() {
  ...
  // The rules panel follows the selected universe's bound game system (P2.2)
  // unless the GM picked a system by hand — a manual choice always wins.
  const { data: selectedUniverse } = useQuery({
    queryKey: ["universe", universeId],
    queryFn: () => universesApi.getUniverse(universeId!),
    enabled: !!universeId,
  });
  useEffect(() => {
    if (systemOverridden) return;
    setSystemId(selectedUniverse?.default_game_system_id ?? null);
  }, [selectedUniverse, systemOverridden]);

  const handleSystemChange = (id: string | null) => {
    setSystemId(id);
    setSystemOverridden(true);
  };

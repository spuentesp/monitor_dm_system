 ...
 *
 * This page is slice-only. Merge lives in MergeModal (app/forge/packs/page.tsx)
 * and the PackLibrary batch bar; clone is handled inline via the clone mutation.
 * The page covers slice item selection and acts as a review surface for the
 * produced working copy before saving.
 */
...
function PackEditorPageContent() {
  ...
  const sliceMutation = useMutation({
    ...
    onSuccess: (saved) => {
      ...
      router.push(`/forge/packs?pack=${saved.id}`);
    },
  });
    ...
    return (
      <div className="flex items-center justify-center h-full px-6">
        <div className="max-w-lg rounded-2xl border border-amber-500/20 bg-amber-500/5 p-6 text-center space-y-3">
          ...
          <button className="btn-ghost mx-auto" onClick={() => router.push("/forge/ingest")}>Back to Ingest Studio</button>
        </div>
      </div>
    );
  }

...
import { CoveragePanel } from "@/components/forge/architect/CoveragePanel";
...
export default function ArchitectPage() {
  ...
  const chat = useChatSession({
    ...
    onTurnSettled: () => {
      ...
      // Turns may change the world — refresh the formal coverage report.
      qc.invalidateQueries({ queryKey: ARCHITECT_KEYS.coverageBase });
    },
  });
  ...
  // Gap-driven building (F2-1): clicking a coverage gap or a priority-gap
  // chip pre-fills the composer with the suggested prompt; the user edits
  // or sends. No autonomous behavior.
  const handleSuggestGap = (prompt: string) => setInput(prompt);
          ...
          {!activeSessionId ? (
            ...
            <>
              <ChatList
                ...
                renderBubble={(msg) => <ArchitectMessageBubble msg={msg} onSuggest={handleSuggestGap} />}
                onDiceResult={() => {
                  ...
                  chat.messages.length === 0 && !chat.isTyping && !chat.streamingMsg ? (
                    <div className="h-full flex flex-col items-center justify-center text-center space-y-3 px-8">
                      ...
                      <Link
                        ...
                        className="text-xs text-slate-600 hover:text-purple-300 transition-colors"
                      >
                        Or create a world step-by-step →
                      </Link>
                    </div>
                  ) : undefined
                }

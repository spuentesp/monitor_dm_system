...
def main() -> int:
    ...
    if existing:
        ...
        # Apply the demo content pack into the universe via the review flow
        # (apply → accept all → commit). The old direct /canonize route was
        # retired in F1-6; the apply endpoints are the single apply path.
        applied = requests.post(
            f"{api}/ingest/packs/{pack.id}/apply/{u_id}",
            json={"mode": "full"},
            timeout=180,
        )
        applied.raise_for_status()
        proposals = requests.get(
            f"{api}/ingest/packs/{pack.id}/proposals",
            params={"status": "pending", "per_page": 500},
            timeout=60,
        )
        proposals.raise_for_status()
        pending = proposals.json()["proposals"]
        if pending:
            reviewed = requests.post(
                f"{api}/ingest/proposals/batch",
                json={
                    "actions": [
                        {"proposal_id": p["proposal_id"], "action": "accept", "reason": "demo auto-accept"}
                        for p in pending
                    ]
                },
                timeout=120,
            ...
            committed = requests.post(f"{api}/ingest/packs/{pack.id}/commit", timeout=180)
            committed.raise_for_status()
        print(f"Committed demo pack: {len(ENTITIES)} entities, {len(LORE)} facts")

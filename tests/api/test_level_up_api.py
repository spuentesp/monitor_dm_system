...
class TestLevelUp:
    ...
    async def test_level_up_success_with_sufficient_xp(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "monitor_agents.services.entity_service.mongodb_get_working_state",
        ...
        mock_system.advancement.progression_table = [
            MagicMock(
                level=1, xp_required=0, features_gained=[], resource_increases={}
            ),
            MagicMock(
                level=2,
                xp_required=100,
                features_gained=["Extra Attack"],
                resource_increases={"HP": 10},
            ),
            MagicMock(
                level=3, xp_required=300, features_gained=[], resource_increases={}
            ),
        ]
        monkeypatch.setattr(
            "monitor_agents.services.entity_service.mongodb_get_game_system",
            lambda sid: mock_system,
        )

        # Mock working state update
        monkeypatch.setattr(
            "monitor_agents.services.entity_service.mongodb_update_working_state",
            lambda sid, upd: None,
        )

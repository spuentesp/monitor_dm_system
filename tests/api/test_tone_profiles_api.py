...
def patched_tone_client(monkeypatch) -> Generator[TestClient, None, None]:
    ...
    monkeypatch.setattr(tone_router, "mongodb_get_tone_profile", fake_get)
    ...
    monkeypatch.setattr(tone_router, "mongodb_create_tone_profile", fake_create)

    def fake_update(pid, body):
        p = _make_mock_profile(profile_id=pid, name="Updated")
        return p

    monkeypatch.setattr(tone_router, "mongodb_update_tone_profile", fake_update)

    def fake_delete(pid):
        return True

    monkeypatch.setattr(tone_router, "mongodb_delete_tone_profile", fake_delete)

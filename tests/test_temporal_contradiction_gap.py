...
class TestFactExpiration:
    ...
    def test_fact_expired(self):
        ...
        time_ref = datetime.now(timezone.utc) - timedelta(hours=2)
        duration = 3600  # 1 hour
        fact = {
            "id": uuid4(),
            "statement": "The castle is under siege",
    ...
    def test_fact_not_yet_started(self):
        ...
        time_ref = datetime.now(timezone.utc) + timedelta(hours=1)
        fact = {
            "id": uuid4(),
    ...
    def test_expiring_soon_warning(self):
        ...
        time_ref = datetime.now(timezone.utc) - timedelta(hours=23)
...
class TestTemporalValidation:
    ...
    def test_future_reference_detection(self):
        ...
        assert result.total_violations > 0
        future_refs = [v for v in result.violations 
    ...
    def test_future_event_reference(self):
        ...
        event_time = datetime(1000, 1, 2, tzinfo=timezone.utc)
        canon_events = [
            {
                "id": uuid4(),
                "title": "King's Death",
        ...
        event_refs = [v for v in result.violations 
                      if v.violation_type == TemporalViolationType.FUTURE_REFERENCE
    ...
    def test_expired_fact_reference(self):
        ...
        fact_time = datetime(1000, 1, 1, tzinfo=timezone.utc)
        canon_facts = [
            {
                "id": uuid4(),
                "statement": "The castle is under siege",
        ...
        request = TemporalValidationRequest(
            ...
            universe_id=uuid4(),
            story_id=uuid4(),
            scene_time_ref=datetime(1000, 1, 1, 2, tzinfo=timezone.utc),  # 2 hours later
        ...
        expired_refs = [v for v in result.violations 
    ...
    def test_temporal_paradox_detection(self):
        ...
        scene_time = datetime(1000, 1, 2, tzinfo=timezone.utc)
        canon_events = [
            {
                "id": uuid4(),
                "title": "King's Decision",
                "start_time": datetime(1000, 1, 1, tzinfo=timezone.utc),
...
class TestPlotThreadDetection:
    ...
    def test_detect_promise_thread(self):
        ...
        promises = [t for t in result.threads if t.category == PlotThreadCategory.PROMISE]
    ...
    def test_detect_threat_thread(self):
        ...
        assert threats[0].urgency in (PlotThreadUrgency.HIGH, PlotThreadUrgency.CRITICAL)
    ...
    def test_detect_mystery_thread(self):
        ...
        assert result.total_detected >= 1
        mysteries = [t for t in result.threads if t.category == PlotThreadCategory.MYSTERY]
    ...
    def test_detect_consequence_thread(self):
        ...
        assert result.total_detected >= 1
        consequences = [t for t in result.threads 
                        if t.category == PlotThreadCategory.CONSEQUENCE]
    ...
    def test_detect_relationship_thread(self):
        ...
        assert result.total_detected >= 1
        relationships = [t for t in result.threads 
                         if t.category == PlotThreadCategory.RELATIONSHIP]
    ...
    def test_detect_world_event_thread(self):
        ...
        assert result.total_detected >= 1
        events = [t for t in result.threads 
                  if t.category == PlotThreadCategory.WORLD_EVENT]
        assert len(events) > 0
    ...
    def test_classify_thread_content(self):
        ...
        assert classify_thread_content("I promise to help you") == PlotThreadCategory.PROMISE
        assert classify_thread_content("The enemy threatens to attack") == PlotThreadCategory.THREAT
        assert classify_thread_content("Who committed this crime?") == PlotThreadCategory.MYSTERY
        assert classify_thread_content("As a result, the war began") == PlotThreadCategory.CONSEQUENCE
...
class TestEdgeCases:
    ...
    def test_fact_with_no_id_raises_error(self):
        ...
        with pytest.raises(ValueError):
            check_fact_validity({"statement": "Test"}, datetime.now(timezone.utc))

    def test_concurrent_fact_expiration_checks(self):
        """Multiple facts can be checked concurrently."""
        now = datetime.now(timezone.utc)
        facts = [
            {"id": uuid4(), "statement": f"Fact {i}"} for i in range(100)
        ]

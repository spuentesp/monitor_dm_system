...
from monitor_data.schemas.resolutions import ActionType, SuccessLevel
...
class TestResolutionCreationProperties:
    ...
    @given(
        ...
        caller_auth=st.sampled_from([("Resolver", True), ("CanonKeeper", True), ("Narrator", False)]),
    )
    @settings(max_examples=50, deadline=1000)
    def test_resolution_preconditions(self, action, action_type, scene_active, caller_auth):
...
class TestEffectProperties:
    ...
    @given(
        ...
        damage_type=st.sampled_from(["fire", "cold", "lightning", "acid", "poison", None]),
        condition=st.sampled_from(["stunned", "prone", "charmed", "frightened", None]),
    )
    @settings(max_examples=50, deadline=1000)
    def test_effect_invariant_damage_type(self, effect_type, magnitude, damage_type, condition):
...
class TestResolutionMechanicsProperties:
    ...
    @given(
        ...
        target=st.integers(min_value=-10, max_value=100).filter(lambda x: x is not None and x <= 0),
    )
...
class TestEffectEdgeCaseProperties:
    ...
    @given(
        effect_type=st.sampled_from([EffectType.BUFF, EffectType.DEBUFF, EffectType.HEALING]),
        duration=st.just(0),  # Zero duration — valid for Pydantic, should fail invariant for BUFF/DEBUFF
    )

...
def valid_status_transition_strategy() -> st.SearchStrategy[tuple[SceneStatus, SceneStatus]]:
...
class TestSceneStatusTransitionProperties:
    ...
    def test_valid_transitions_accepted(self, transition: tuple[SceneStatus, SceneStatus]) -> None:
    ...
    def test_active_to_active_rejected(self, from_status: SceneStatus, to_status: SceneStatus) -> None:
    ...
    def test_append_turn_requires_active_scene(
        ...
        if scene.status == SceneStatus.ACTIVE:
            ...
            if speaker == Speaker.ENTITY and entity_id is None:
                ...
                result = ScenePreConditions.append_turn(scene, speaker, entity_id, "Narrator")
                assert result is True
        else:

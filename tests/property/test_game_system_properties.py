...
def track_def_ordered_strategy() -> st.SearchStrategy[TrackDefinition]:
    ...
    return st.builds(
        ...
        lambda t: (
            not isinstance(t.default_value, int)
...
class TestAttributeDefinitionProperties:
    ...
    def test_empty_name_accepted_by_pydantic(
...
class TestTrackDefinitionProperties:
    ...
    def test_accepts_negative_min_value(
        self, min_value: int, default_value: int
    ) -> None:
...
class TestTieredAbilitySystemProperties:
    ...
    def test_max_tier_at_least_number_of_tiers(
        self, system: TieredAbilitySystem
    ) -> None:
...
class TestThresholdEffectProperties:
    ...
    @settings(max_examples=100, deadline=1000)
    def test_effect_description_non_empty(
        self, value: int, direction: str, effect: str
    ) -> None:
...
class TestGameSystemCompositionProperties:
    ...
    @given(
        ...
        ability_systems=st.lists(
            tiered_ability_ordered_strategy(), min_size=0, max_size=5
        ),

...
class TestSYS1ApplicationStartup:
    ...
    @pytest.mark.asyncio
    async def test_neo4j_returns_health_dict(self):
    ...
    @pytest.mark.asyncio
    async def test_neo4j_unhealthy_when_connection_fails(self):
        """Test Neo4j returns unhealthy when connection fails."""
        with patch("monitor_data.health.get_neo4j_client") as mock_neo:
            mock_client = MagicMock()
            mock_client.verify_connectivity = AsyncMock(return_value=False)
            mock_neo.return_value = mock_client
    ...
    @pytest.mark.asyncio
    async def test_mongodb_returns_health_dict(self):
    ...
    @pytest.mark.asyncio
    async def test_mongodb_unhealthy_when_connection_fails(self):
...
class TestSYSAcceptanceCriteria:
    ...
    @pytest.mark.asyncio
    async def test_ac1_config_loading(self):
    ...
    @pytest.mark.asyncio
    async def test_ac2_db_connections(self):
    ...
    @pytest.mark.asyncio
    async def test_ac3_service_verification(self):
        """AC-3: Services are verified through health checks."""
        neo4j_result = await check_neo4j_connectivity()
        mongodb_result = await check_mongodb_connectivity()
        ...
        assert mongodb_result["status"] in [

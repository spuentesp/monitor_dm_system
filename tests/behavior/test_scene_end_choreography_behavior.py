...
class TestCompleteCurrentScene:
    ...
    async def test_advances_in_game_time(self):
        ...
        original = _make_story_state(
            in_game_time=datetime(2026, 1, 1, 12, 0, 0, tzinfo=UTC),
        )
        state = _make_state(
        ...
        expected = datetime(2026, 1, 1, 12, 45, 0, tzinfo=UTC)
    ...
    async def test_increments_scenes_completed(self):
        ...
        original = _make_story_state(
            in_game_time=datetime(2026, 1, 1, 8, 0, 0, tzinfo=UTC),
            scenes_completed=3,
        )
    ...
    async def test_no_memories_to_persist_is_safe(self):
        ...
        state = _make_state(
            story_state=_make_story_state(
                in_game_time=datetime(2026, 1, 1, tzinfo=UTC),
            ),
            memories_to_persist=[],
            actor_id=None,

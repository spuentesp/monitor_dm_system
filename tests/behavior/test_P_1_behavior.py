...
class TestP1StoryCreationHappyPath:
    ...
    def test_story_creation_with_all_fields(self):
        ...
        with patch("monitor_data.tools.neo4j_tools.stories.verify_node_exists"):
            with patch(
                "monitor_data.tools.neo4j_tools.stories.get_neo4j_client"
            ) as mock_get_client:
                client = MagicMock()
                mock_get_client.return_value = client
                client.execute_write = lambda q, p: _mock_execute_write_single_result(
                    q, {**p, "id": str(story_id), "universe_id": str(universe_id)}
                )
...
class TestP1StoryCreationFails:
    ...
    def test_story_creation_fails_without_universe(self):
        ...
        with patch(
            "monitor_data.tools.neo4j_tools.stories.verify_node_exists"
        ) as mock_verify:
...
class TestP1StoryOutlineCreation:
    ...
    def test_story_outline_created_with_empty_beats(self):
        ...
        with patch(
            "monitor_data.tools.mongodb_tools.stories.get_mongodb_client"
        ) as mock_get_client, patch(
            "monitor_data.tools.mongodb_tools.stories.get_neo4j_client"
        ) as mock_neo4j:
            client = MagicMock()
            mock_get_client.return_value = client
            ...
            params = StoryOutlineCreate(story_id=story_id, beats=[], pc_ids=[])
...
class TestP1CLICommand:
    ...
    async def test_create_story_interactive_returns_uuid(self):
        ...
        with patch("monitor_cli.commands.play.CanonKeeper") as mock_keeper:
            with patch("monitor_cli.commands.play.typer.prompt"):
                mock_instance = AsyncMock()
                mock_instance.create_story = AsyncMock(

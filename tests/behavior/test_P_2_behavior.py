...
from monitor_data.schemas.scenes import SceneCreate, TurnCreate
...
class TestP2SceneCreationHappyPath:
    ...
    def test_scene_creation_with_all_fields(self):
        ...
        with patch(
            "monitor_data.tools.mongodb_tools.scenes.get_mongodb_client"
        ) as mock_mongo, patch(
            "monitor_data.tools.mongodb_tools.scenes.get_neo4j_client"
        ) as mock_neo:
            ...
            def neo4j_read_side_effect(query, p):
                ...
                return [
                    {"story_id": str(story_id), "universe_id": str(universe_id)}
                ]
    ...
    def test_scene_node_properties_match_spec(self):
        ...
        with patch(
            ...
            params = SceneCreate(
                story_id=story_id, universe_id=universe_id, title="Test Scene"
            )
            result = mongodb_create_scene(params)
...
class TestP2SceneCreationFailsWithoutStory:
    ...
    def test_scene_creation_fails_without_story(self):
        ...
        with patch(
            ...
            params = SceneCreate(
                story_id=story_id, universe_id=universe_id, title="Orphaned Scene"
            )
...
class TestP2SceneTitleValidation:
    ...
    def test_empty_title_rejected(self):
        ...
        with pytest.raises(Exception, match="title"):
            params = SceneCreate(story_id=uuid4(), universe_id=uuid4(), title="")
            mongodb_create_scene(params)
...
class TestP2OpeningTurn:
    ...
    def test_append_opening_narration_turn(self):
        ...
        with patch(
            ...
            turn = mongodb_append_turn(
                scene_id,
                TurnCreate(speaker=Speaker.GM, text="The tavern is crowded tonight..."),
            )
...
class TestP2AcceptanceCriteria:
    ...
    def test_ac1_story_must_exist(self):
        ...
        with patch(
            ...
            with pytest.raises(ValueError, match="Story"):
                mongodb_create_scene(
                    SceneCreate(story_id=uuid4(), universe_id=uuid4(), title="Test")
                )
    ...
    def test_ac5_title_required(self):
        ...
        with pytest.raises(Exception, match="title"):
            mongodb_create_scene(
                SceneCreate(story_id=uuid4(), universe_id=uuid4(), title="")
            )
    ...
    def test_ac6_status_defaults_to_active(self):
        ...
        with patch(
            ...
            neo4j_client.execute_read = lambda q, p: [
                {"story_id": str(uuid4()), "universe_id": str(uuid4())}
            ]
            mongo_client.get_collection.return_value.insert_one = MagicMock(
                return_value=MagicMock(inserted_id=str(uuid4()))
            )

            result = mongodb_create_scene(
                SceneCreate(story_id=uuid4(), universe_id=uuid4(), title="Test")
            )
            assert result.status == SceneStatus.ACTIVE

...
class TestPlotHookSchema:
    ...
    def test_minimal_plot_hook(self):
        h = PlotHook(title="Mystery at the docks", description="A ship arrived at midnight.")
    ...
    def test_plot_hook_response_wraps_list(self):
        resp = PlotHookListResponse(hooks=[
...
class TestSuggestHooksHeuristic:
    ...
    async def test_empty_universe_yields_no_hooks(self):
        ...
        with patch.object(agent, "call_llm_structured", new=AsyncMock(side_effect=RuntimeError("no LLM"))):
    ...
    async def test_threads_produce_hooks(self):
        ...
        async def fake_call_tool(name, args):
            if name == "neo4j_list_plot_threads":
                return {
                    "threads": [
                        {"title": "The Lost Crown", "description": "A relic vanished.", "status": "active"},
                        {"title": "Smugglers in the Bay", "description": "Activity at the docks.", "status": "active"},
                    ]
                }
        ...
        with patch.object(agent, "call_llm_structured", new=AsyncMock(side_effect=RuntimeError("no LLM"))):
            with patch.object(agent, "call_tool", new=AsyncMock(side_effect=fake_call_tool)):
    ...
    async def test_max_hooks_is_respected(self):
        ...
        async def fake_call_tool(name, args):
            if name == "neo4j_list_plot_threads":
                return {
                    "threads": [
                        {"title": f"Thread {i}", "description": f"desc {i}", "status": "active"}
                        for i in range(20)
                    ]
                }
    ...
    async def test_connected_entities_populated_from_threads(self):
        ...
        assert any(h.title == "The Baron's Secret" or "Baron" in (h.title + h.description) for h in hooks)
...
class TestSuggestHooksLLMPath:
    ...
    async def test_llm_response_hooks_are_returned(self):
        ...
        async def fake_llm(model_class, messages, **kwargs):
            return PlotHookListResponse(hooks=[
                PlotHook(
                    title="LLM Hook 1",
        ...
        with patch.object(agent, "call_tool", new=AsyncMock(return_value={})):
            with patch.object(agent, "call_llm_structured", new=AsyncMock(side_effect=fake_llm)):
                hooks = await agent.suggest_hooks(universe_id=uuid4(), max_hooks=5)
        assert len(hooks) == 1
...
class TestContradictionSchema:
    ...
    def test_contradiction_response_wraps_list(self):
        resp = ContradictionListResponse(contradictions=[
            Contradiction(
                fact_a="X", fact_b="not X", severity="high",
                explanation="opposites", suggestion="pick one",
            ),
        ])
...
class TestDetectContradictionsLLMPath:
    ...
    async def test_llm_response_returned(self):
        ...
        async def fake_llm(model_class, messages, **kwargs):
            return ContradictionListResponse(contradictions=[
                Contradiction(
        ...
        with patch.object(agent, "call_tool", new=AsyncMock(return_value={"facts": []})):
            with patch.object(agent, "call_llm_structured", new=AsyncMock(side_effect=fake_llm)):
                contradictions = await agent.detect_contradictions(universe_id=uuid4())
...
class TestGenerateSessionPrep:
    ...
    async def test_recap_uses_recent_scene_summaries(self):
        ...
        async def fake_call_tool(name, args):
            if name == "mongodb_list_scenes":
                return {
                    "scenes": [
                        {"title": "The Ambush", "summary": "Bandits attacked the caravan."},
                        {"title": "Into the Forest", "summary": "The party tracked the bandits."},
                    ]
                }
            return {}
    ...
    async def test_open_threads_populated(self):
        ...
        assert any("Crown" in t for t in prep.open_threads) or len(prep.open_threads) == 0
    ...
    async def test_world_state_changes_filters_canon_facts(self):
            ...
            if name == "neo4j_list_facts":
                return {
                    "facts": [
                        ...
                        {"content": "The kingdom fell to ruin.", "canon_level": "established"},
                    ]
                }
            return {}
        ...
        for change in prep.world_state_changes:
            assert "baker" not in change.lower(), "rumor-level fact should not be world state change"

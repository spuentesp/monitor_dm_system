...
class TestArcLabelToPurpose:
    def test_rising_action(self):
        from monitor_agents.loops.story_loop import _arc_label_to_purpose
        assert _arc_label_to_purpose("rising_action") == "Tension Building"

    def test_climax(self):
        from monitor_agents.loops.story_loop import _arc_label_to_purpose
        assert _arc_label_to_purpose("climax") == "Climactic Confrontation"

    def test_falling_action(self):
        from monitor_agents.loops.story_loop import _arc_label_to_purpose
        assert _arc_label_to_purpose("falling_action") == "Aftermath"

    def test_resolution(self):
        from monitor_agents.loops.story_loop import _arc_label_to_purpose
        assert _arc_label_to_purpose("resolution") == "Resolution"

    def test_new_thread(self):
        from monitor_agents.loops.story_loop import _arc_label_to_purpose
        assert _arc_label_to_purpose("new_thread") == "New Development"

    def test_unknown_label_falls_back(self):
        from monitor_agents.loops.story_loop import _arc_label_to_purpose
        assert _arc_label_to_purpose("totally_made_up") == "Story Continues"

    def test_empty_string_falls_back(self):
        from monitor_agents.loops.story_loop import _arc_label_to_purpose
        assert _arc_label_to_purpose("") == "Story Continues"
...
class TestLoadStoryOutline:
    def test_returns_none_when_missing(self, monkeypatch):
        ...
        monkeypatch.setattr(story_loop, "run_sync_read", fake_run_sync_read)
        monkeypatch.setattr(story_loop, "mongodb_get_story_outline", lambda *a, **k: None)

...
class SystemAgent:
    ...
    async def list_scenes(
        self, story_id: str, order: str = "desc", limit: int | None = None
    ) -> list[Scene]:
    ...
    async def get_proposed_changes(
        self, scene_id: str, status: str = "accepted"
    ) -> list[ProposedChange]:
...
class NarratorAgent:
    ...
    async def generate_recap(
        self, scene_history: dict, target_audience: str, detail_level: str
    ) -> str:
...
@pytest.mark.unit
async def test_scenario1_single_scene_recap(
    system_agent,
    narrator_agent,
    neo4j_agent,
    fake_mongodb,
    ...
    system_agent.get_proposed_changes = AsyncMock(
        return_value=[
            ProposedChange(
                id="prop-001",
                scene_id=sample_scene.id,
    ...
    neo4j_agent.list_entities = AsyncMock(
        return_value=[
    ...
    neo4j_agent.list_plot_threads = AsyncMock(
        side_effect=[
    ...
    narrator_agent.generate_recap = AsyncMock(
        return_value="The party arrived at the Rusty Anchor tavern. They investigated the basement and decided to pursue dragon rumors. A mysterious thread was opened."
    )

    narrator_agent.extract_decisions = AsyncMock(
        return_value=[
            Decision(
    ...
    narrator_agent.extract_loot = AsyncMock(
        return_value=[
            Loot(
    ...
    closed_threads = await neo4j_agent.list_plot_threads(
        sample_scene.story_id, "resolved"
    )
    ...
    recap = Recap(
        ...
        summary=summary,
        key_events=[
            "Arrived at Rusty Anchor",
            "Investigated basement",
            "Decided to pursue dragons",
        ],
...
@pytest.mark.unit
async def test_scenario2_full_story_recap(
    system_agent,
    narrator_agent,
    neo4j_agent,
    fake_mongodb,
    ...
    narrator_agent.generate_recap = AsyncMock(
        return_value="The party embarked on a grand adventure across five scenes. They faced challenges, made important decisions, and ultimately emerged victorious. The story arc progressed from investigation to confrontation to resolution."
    )
    ...
    turn_stats = TurnStats(
        total=100, by_type={TurnType.ACTION: 100}, duration=timedelta(hours=10)
    )
...
@pytest.mark.unit
async def test_scenario3_most_recent_scene_recap(
    system_agent,
    narrator_agent,
    neo4j_agent,
    fake_mongodb,
    ...
    system_agent.get_turns = AsyncMock(
        return_value=[
            Turn(
                "turn-3-1", "scene-3", TurnType.ACTION, "Recent action", datetime.now()
            ),
            Turn(
                "turn-3-2",
                "scene-3",
    ...
    narrator_agent.generate_recap = AsyncMock(
        return_value="In the most recent session, the party took action and had important dialogue. This scene focused on immediate goals and short-term objectives."
    )
    ...
    neo4j_agent.list_entities = AsyncMock(
        return_value=[{"name": "Recent NPC", "type": "character"}]
    )
    neo4j_agent.list_plot_threads = AsyncMock(return_value=[])

    # Execute: Get most recent scene
    recent_scenes = await system_agent.list_scenes(
        sample_story.id, order="desc", limit=1
    )
...
@pytest.mark.unit
async def test_scenario4_custom_detail_level(
    system_agent, narrator_agent, fake_llm, sample_scene, sample_turns
):
    """
    ...
    brief_summary = await narrator_agent.generate_recap(
        scene_history, "gm_only", "brief"
    )
    ...
    detailed_summary = await narrator_agent.generate_recap(
        scene_history, "gm_only", "detailed"
    )
    ...
    detailed_recap = Recap(
        ...
        key_events=[
            "Arrived at Rusty Anchor",
            "Spoke with innkeeper",
            "Investigated basement",
            "Found dragon evidence",
            "Decided to pursue dragon",
        ],
        decisions_made=[],
        npcs_encountered=[],
        threads_opened=[],
...
@pytest.mark.unit
async def test_scenario5_player_audience_recap(
    system_agent, narrator_agent, fake_llm, sample_scene, sample_turns
):
    """
    ...
    narrator_agent.generate_recap = AsyncMock(
        return_value="The party arrived at the Rusty Anchor. They investigated the basement and found something interesting. They decided to pursue this new lead together."
    )
    ...
    assert (
        "dragon" not in summary.lower() or "rumors" in summary.lower()
    )  # If mentioned, should be vague
    ...
    recap = Recap(
        ...
        key_events=[
            "Arrived at Rusty Anchor",
            "Investigated basement",
            "Found interesting clue",
            "Decided to pursue lead",
        ],
        decisions_made=[],
        npcs_encountered=[],
        threads_opened=[],
...
async def test_error_scene_not_found(system_agent, fake_mongodb):
    ...
    system_agent.get_scene = AsyncMock(
        side_effect=Exception("Scene not found: scene-999")
    )
...
@pytest.mark.unit
async def test_success_criterion_summary_quality(
    narrator_agent, fake_llm, sample_turns
):
    """
    ...
    narrator_agent.generate_recap = AsyncMock(
        return_value="The party experienced many events. They began by arriving, then explored, then fought, then rested, and finally departed. Each phase brought new challenges and discoveries."
    )
...
async def test_success_criterion_tone_consistency(narrator_agent, fake_llm):
    ...
    narrator_agent.generate_recap = AsyncMock(
        return_value="The brave knights ventured forth, their armor gleaming in the sunlight. Dragons soared above, casting shadows on the ancient castle. Magic pulsed through the air as the heroes prepared for their legendary quest."
    )
...
@pytest.mark.unit
async def test_postcondition_recap_generated(
    system_agent, narrator_agent, fake_llm, sample_scene, sample_turns
):
    """

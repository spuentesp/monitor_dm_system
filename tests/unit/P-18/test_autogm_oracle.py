...
class ContextAssembly:
    ...
    async def detect_oracle_query(self, input: str) -> bool:
        ...
        question_words = ["Is", "Are", "Do", "Does", "Has", "Have", "Will", "Can", "Could"]
...
class OracleAgent:
    ...
    def get_outcome_from_roll(self, roll: int, likelihood: LikelihoodLevel) -> OracleOutcome:
        ...
        elif likelihood.value == "fifty_fifty":
            if roll <= 49:
                return OracleOutcome(value="no")
            elif roll <= 95:
                return OracleOutcome(value="yes")
        ...
        elif likelihood.value == "unlikely":
            if roll <= 49:
                return OracleOutcome(value="no")
            elif roll <= 95:
                return OracleOutcome(value="yes")
...
class CanonKeeper:
    ...
    async def canonize_oracle_result(self, query: str, outcome: OracleOutcome, roll: int, tension_score: int) -> Fact:
...
class SceneLoop:
    ...
    def __init__(self, mcp_client, context_assembly, oracle_agent, canon_keeper, narrator):
        self.mcp_client = mcp_client
...
def scene_loop(
    ...
    return SceneLoop(fake_mcp_client, context_assembly, oracle_agent, canon_keeper, narrator)
...
async def test_p18_scenario_1_simple_yes_no(
    ...
    assert likelihood.value == "fifty_fifty", f"Expected fifty_fifty, got {likelihood.value}"
    ...
    fact = await canon_keeper.canonize_oracle_result(query, outcome, roll, tension_score)
    assert fact.content == f"{query}: yes", f"Expected 'Is the door locked?: yes', got {fact.content}"
    assert fact.canon_level == "cards", f"Expected canon_level 'cards', got {fact.canon_level}"
    assert f"oracle:{roll}:{tension_score}" in fact.evidence_refs, "Evidence refs should contain oracle reference"
    ...
    user_turn = turns[0]
    assert user_turn.turn_type == "question", f"Expected turn_type 'question', got {user_turn.turn_type}"
    assert user_turn.metadata.get("oracle_query") is True, "User turn should have oracle_query metadata"
    
    gm_turn = turns[1]
    assert gm_turn.turn_type == "oracle_response", f"Expected turn_type 'oracle_response', got {gm_turn.turn_type}"
    assert gm_turn.metadata.get("oracle_result") == "yes", f"Expected oracle_result 'yes', got {gm_turn.metadata.get('oracle_result')}"
    assert gm_turn.metadata.get("roll") == 60, f"Expected roll 60, got {gm_turn.metadata.get('roll')}"
    assert gm_turn.metadata.get("likelihood") == "fifty_fifty", f"Expected likelihood 'fifty_fifty', got {gm_turn.metadata.get('likelihood')}"
    assert str(fact.fact_id) in gm_turn.metadata.get("fact_canonized", ""), "GM turn should reference canonized fact"
...
async def test_p18_scenario_2_extreme_roll(
    ...
    likelihood = await oracle_agent.determine_likelihood(tension_score)
    assert likelihood.value == "impossible", f"Expected impossible, got {likelihood.value}"
    ...
    # Action 4: Canonize Oracle Result
    fact = await canon_keeper.canonize_oracle_result(query, outcome, roll, tension_score)
    assert fact.content == f"{query}: no_and", f"Expected 'Can I pick the lock?: no_and', got {fact.content}"
    assert f"oracle:{roll}:{tension_score}" in fact.evidence_refs, "Evidence refs should contain oracle reference"
    ...
    narrative = await narrator.describe_oracle_outcome(query, outcome, context)
    assert narrative is not None, "Narrator should return narrative"
    assert "no_and" in narrative.lower() or "complication" in narrative.lower() or "not only" in narrative.lower(), \
    ...
    gm_turn = turns[1]
    assert gm_turn.metadata.get("oracle_result") == "no_and", f"Expected oracle_result 'no_and', got {gm_turn.metadata.get('oracle_result')}"
    assert gm_turn.metadata.get("roll") == 98, f"Expected roll 98, got {gm_turn.metadata.get('roll')}"
    assert gm_turn.metadata.get("likelihood") == "impossible", f"Expected likelihood 'impossible', got {gm_turn.metadata.get('likelihood')}"
...
@pytest.mark.unit
async def test_p18_error_case_invalid_input(
    scene_loop,
    context_assembly
):
    """
...
@pytest.mark.unit
async def test_p18_success_criteria_question_detection(
    context_assembly
):
    """
    ...
    assert accuracy >= 95, f"Question detection accuracy {accuracy}% is below 95% threshold"


@pytest.mark.unit
async def test_p18_success_criteria_tension_retrieval(
    scene_loop,
    scene_with_tension
):
    """
    ...
    assert scene.tension_score == 5, f"Expected tension_score 5, got {scene.tension_score}"
...
async def test_p18_success_criteria_canonization(
    ...
    assert fact.content == f"{query}: yes", "Fact content should match query and outcome"
    ...
    assert f"oracle:{roll}:{tension_score}" in fact.evidence_refs[0], "Evidence ref should reference oracle"


@pytest.mark.unit
async def test_p18_success_criteria_evidence_tracking(
    canon_keeper,
    oracle_agent
):
    """
    ...
    for query, outcome, roll in queries:
        fact = await canon_keeper.canonize_oracle_result(query, outcome, roll, tension_score)
        assert len(fact.evidence_refs) > 0, f"Fact for '{query}' should have evidence refs"
    ...
    for fact in canon_keeper.canonized_facts:
        assert len(fact.evidence_refs) > 0, f"Fact {fact.fact_id} is missing evidence refs"


@pytest.mark.unit
async def test_p18_success_criteria_narrator_respects_outcome(
    narrator,
    oracle_agent
):
    """
    ...
    for outcome_value, expected_phrase in outcomes:
        ...
        assert narrative is not None, f"Narrator should return narrative for outcome '{outcome_value}'"
...
async def test_p18_turn_metadata_completeness(
    ...
    try:
        ...
        outcome = oracle_agent.get_outcome_from_roll(roll, likelihood)
        fact = await canon_keeper.canonize_oracle_result(query, outcome, roll, tension_score)
        narrative = await narrator.describe_oracle_outcome(query, outcome, {"location": "tavern"})
        turns = await scene_loop.append_turns(scene_id, query, narrative, outcome, roll, likelihood, fact.fact_id)
    finally:
        random.randint = original_random
    
    # Verify user turn metadata
    user_turn = turns[0]
    assert "oracle_query" in user_turn.metadata, "User turn should have oracle_query metadata"
    ...
    for field in required_metadata:
        assert field in gm_turn.metadata, f"GM turn should have metadata field '{field}'"
    ...
    assert gm_turn.metadata["likelihood"] == "fifty_fifty", "likelihood should be correct"
    assert fact.fact_id in gm_turn.metadata["fact_canonized"], "fact_canonized should reference correct fact"
...
async def test_p18_postconditions(
    ...
    gm_turn = turns[1]
    assert gm_turn.turn_type == "oracle_response", "GM turn should be ORACLE_RESPONSE type"
    assert gm_turn.metadata.get("oracle_result") == "yes", "GM turn should have oracle_result"
    assert gm_turn.metadata.get("roll") == 60, "GM turn should have roll"
    assert gm_turn.metadata.get("likelihood") == "fifty_fifty", "GM turn should have likelihood"
    ...
    assert updated_scene.updated_at >= original_updated_at, "Scene updated_at should be updated"

...
class CanonKeeperAgent:
    ...
    async def analyze_open_questions(self, dialogue: List[Dialogue], facts: List[Any], turns: List[Turn]) -> List[OpenQuestion]:
        """Analyze dialogue to identify unanswered questions."""
        pass
    
    async def analyze_dangling_hooks(self, turns: List[Turn], facts: List[Any]) -> List[DanglingHook]:
        """Analyze turns to identify hooks that weren't followed up."""
        pass
    
    async def analyze_missing_payoffs(self, turns: List[Turn], facts: List[Any]) -> List[MissingPayoff]:
...
@pytest.mark.unit
async def test_scenario1_open_questions_detection(system_agent, canonkeeper_agent, context_assembly_agent, fake_mongodb, fake_neo4j, fake_llm, sample_story, sample_scenes):
    """
    Scenario 1: Open questions detection.
...
@pytest.mark.unit
async def test_scenario2_unfulfilled_promise_detection(system_agent, canonkeeper_agent, fake_mongodb, fake_neo4j, fake_llm, sample_story, sample_scenes):
    """
    Scenario 2: Unfulfilled promise detection.
...
@pytest.mark.unit
async def test_scenario3_dangling_hook_detection(system_agent, canonkeeper_agent, fake_mongodb, fake_neo4j, fake_llm, sample_story, sample_scenes):
    """
    Scenario 3: Dangling hook detection.
...
@pytest.mark.unit
async def test_scenario4_incomplete_quest_detection(system_agent, fake_neo4j, sample_story, sample_scenes):
    """
    Scenario 4: Incomplete quest detection.
    ...
    remaining_objectives = [
        obj["name"] for obj in objectives if not obj["completed"]
    ]
...
async def test_scenario5_filtered_thread_list(canonkeeper_agent, fake_llm):
    ...
    by_importance_filtered = {
        Importance.CRITICAL: len([t for t in filtered_threads if t.importance == Importance.CRITICAL]),
        Importance.MAJOR: len([t for t in filtered_threads if t.importance == Importance.MAJOR]),
        Importance.MINOR: len([t for t in filtered_threads if t.importance == Importance.MINOR])
    }
...
async def test_error_story_not_found(system_agent, fake_mongodb):
    ...
    system_agent.get_story = AsyncMock(side_effect=Exception("Story not found: story-999"))
...
async def test_error_mongodb_connection_failure(system_agent, fake_mongodb):
    ...
    system_agent.get_story = AsyncMock(side_effect=Exception("MongoDB connection failed"))
...
async def test_error_llm_failure(canonkeeper_agent, fake_llm):
    ...
    """
    # Setup: LLM fails
    canonkeeper_agent.analyze_open_questions = AsyncMock(side_effect=Exception("LLM timeout"))
...
@pytest.mark.unit
async def test_postcondition_threads_identified(system_agent, canonkeeper_agent, fake_mongodb, fake_neo4j, fake_llm):
    """
    ...
    canonkeeper_agent.analyze_open_questions = AsyncMock(return_value=[
        OpenQuestion("Question?", "Player", "Context", Importance.MAJOR, "scene-001", "turn-001")
    ])
    canonkeeper_agent.analyze_dangling_hooks = AsyncMock(return_value=[
        DanglingHook("Hook", "GM", "Context", Importance.MINOR, "scene-001", "turn-002")
    ])
    canonkeeper_agent.analyze_missing_payoffs = AsyncMock(return_value=[
        MissingPayoff("Setup", "NPC", "Context", Importance.MAJOR, "reveal", "scene-001", "turn-003")
    ])
...
async def test_postcondition_threads_ranked():
    ...
    calculated_priority = (importance_score * 3.0) + (urgency_score * 2.0) + recency_score

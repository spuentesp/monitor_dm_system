...
class SceneSummary:
    def __init__(self, id: str, story_id: str, title: str, summary: str, entity_refs: List[str], turn_count: int):
        self.id = id
...
class ProposedChange:
    def __init__(self, id: str, scene_id: str, change_type: str, description: str, confidence: float):
        self.id = id
        self.scene_id = scene_id
...
class RecordingReview:
    def __init__(self, id: str, session_id: str, draft_scenes: List[SceneSummary], total_turns: int, proposals: List[ProposedChange]):
        self.id = id
...
@pytest.mark.unit
async def test_scenario1_live_notes_recording(orchestrator, narrator, indexer, fake_mcp, sample_story):
    """
    ...
    narrator.parse_gm_input = AsyncMock(side_effect=[
        {
    ...
    indexer.extract_entities_realtime = AsyncMock(side_effect=[
        {"entities": [{"name": "The Rusty Anchor", "type": "location"}]},
        {"entities": [{"name": "Fighter", "type": "character"}]},
        {"entities": []}
    ])
    
    fake_mcp.call_tool = AsyncMock(side_effect=[
        {"turn_id": "turn-001"},
        {"turn_id": "turn-002"},
        {"turn_id": "turn-003"}
    ])
    ...
    result_session_id = await orchestrator.start_recording_session(sample_story.id, RecordingMode.LIVE_NOTES)
    ...
    for note in notes:
        ...
        entities = await indexer.extract_entities_realtime(note)
        await fake_mcp.call_tool("append_turn", {
            "scene_id": scene_id,
    ...
    orchestrator.finalize_recording = AsyncMock(return_value=RecordingReview(
        id="review-001",
        session_id=session_id,
        draft_scenes=[SceneSummary(
            id=scene_id,
...
@pytest.mark.unit
async def test_scenario2_transcript_upload(orchestrator, narrator, fake_mcp, sample_story):
    """
    ...
    narrator.parse_gm_input = AsyncMock(return_value={
        "turn_type": TurnType.ACTION,
        "content": "The dragon attacks the village",
        "metadata": {"source": "transcript"}
    })
    
    narrator.segment_session_into_scenes = AsyncMock(return_value=[
        SceneSummary(
            id="scene-002a",
            story_id=sample_story.id,
    ...
    # Execute: Start recording
    result = await orchestrator.start_recording_session(sample_story.id, RecordingMode.TRANSCRIPT)
...
@pytest.mark.unit
async def test_scenario3_audio_capture_with_annotations(orchestrator, narrator, indexer, fake_mcp, sample_story):
    """
    ...
    indexer.extract_entities_realtime = AsyncMock(return_value={
        "entities": [
    ...
    # Execute: Start recording
    result = await orchestrator.start_recording_session(sample_story.id, RecordingMode.AUDIO)
    assert result == session_id
...
@pytest.mark.unit
async def test_scenario4_gm_annotations_exclusion(orchestrator, narrator, fake_mcp, sample_story):
    """
    ...
    # Execute: Start recording
    result = await orchestrator.start_recording_session(sample_story.id, RecordingMode.LIVE_NOTES)
    assert result == session_id
...
@pytest.mark.unit
async def test_scenario5_multiscene_entity_tracking(orchestrator, narrator, indexer, fake_mcp, sample_story):
    """
    ...
    indexer.extract_entities_realtime = AsyncMock(side_effect=[
        ...
        {"entities": [{"name": "Sir Lancelot", "type": "character"}, {"name": "Emerald Dragon", "type": "creature"}]}
    ])
    ...
    parsed1 = await narrator.parse_gm_input("King Arthur asks for help")
    ...
    parsed2 = await narrator.parse_gm_input("I shall hunt the dragon")
    ...
    relationship_props = [p for p in review.proposals if p.change_type == "new_relationship"]
...
async def test_error_story_not_found(orchestrator, fake_mcp):
    ...
    with pytest.raises(Exception) as exc_info:
        await orchestrator.start_recording_session("story-999", RecordingMode.LIVE_NOTES)
...
async def test_error_scene_segmentation_fails(narrator):
    ...
    narrator.segment_session_into_scenes = AsyncMock(side_effect=Exception("LLM timeout"))
...
async def test_success_criterion_entity_detection(indexer):
    ...
    assert any(e["name"] == "Sir Galahad" and e["type"] == "character" for e in result["entities"])
    assert any(e["name"] == "Round Table" and e["type"] == "organization" for e in result["entities"])
    assert any(e["name"] == "ancient cathedral of Camelot" and e["type"] == "location" for e in result["entities"])
    assert any(e["name"] == "Archbishop Thomas" and e["type"] == "character" for e in result["entities"])
...
@pytest.mark.unit
async def test_success_criterion_gm_annotations_respected(orchestrator, narrator, fake_mcp):
    """

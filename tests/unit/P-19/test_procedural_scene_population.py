...
class SceneLoop:
    ...
    async def detect_scene_transition(
        self, input: str, scene_id: uuid.UUID
    ) -> TransitionEvent:
    ...
    async def retrieve_random_tables(
        self, location: str, world_id: str
    ) -> list[RandomTable]:
    ...
    async def roll_on_table(self, table: RandomTable) -> list[TableEntry]:
        ...
        for entry_data in table.entries:
            ...
            if len(roll_range_parts) == 2:
                ...
                random.randint(min_roll, max_roll)
            else:
                random.randint(1, 6)
            ...
            if (
                selected_entry.subtable_ref
                and selected_entry.subtable_ref in self.tables
            ):
    ...
    async def generate_entities(
        self, table_entries: list[TableEntry], world_id: str
    ) -> list[Entity]:
    ...
    async def stage_entities(
        self, scene_id: uuid.UUID, entities: list[Entity], is_permanent: list[bool]
    ) -> None:
        ...
        for entity, permanent in zip(entities, is_permanent):
            if permanent:
                ...
                scene.staged_entities.append(
                    {"entity_id": entity.entity_id, "temporary": True}
                )
    ...
    async def append_procedural_turns(
        ...
        user_turn = Turn(
            ...
            metadata={
                ...
                "target_location": scene.current_location,
            },
        )
...
class ContextAssembly:
    ...
    async def generate_scene_opening_prompt(
        self, scene: Scene, entities: list[Entity]
    ) -> str:
...
async def test_p19_scenario_1_cave_exploration(
    scene_loop, scene, context_assembly, narrator, cave_table, user_id, world_id
):
    """
    Scenario 1: Cave Exploration
    ...
    assert transition.source_location == "tavern", (
        f"Expected source 'tavern', got {transition.source_location}"
    )
    ...
    assert transition.target_location in ["cave", "the"], (
        f"Expected target 'cave' or 'the', got {transition.target_location}"
    )
    ...
    assert tables[0].table_type == "encounters", (
        f"Expected encounters table, got {tables[0].table_type}"
    )

    # Action 4: Roll on Each Table
    table_entries = await scene_loop.roll_on_table(tables[0])
    assert len(table_entries) >= 1, (
        f"Expected at least 1 entry, got {len(table_entries)}"
    )
    ...
    assert entities[0].entity_type == table_entries[0].entity_type, (
    ...
    assert len(updated_scene.entity_registry) >= 1, (
        f"Expected at least 1 entity in registry, got {len(updated_scene.entity_registry)}"
    )

    # Action 7: Generate Prompt for Narrator
    updated_scene.current_location = "cave"
    scene_loop.scenes[scene_id] = updated_scene
    prompt = await context_assembly.generate_scene_opening_prompt(
        updated_scene, entities
    )
    ...
    user_turn = turns[0]
    assert user_turn.turn_type.value == "MOVEMENT", (
        f"Expected MOVEMENT, got {user_turn.turn_type.value}"
    )
    assert user_turn.metadata.get("scene_transition") is True, (
        "User turn should have scene_transition metadata"
    )

    gm_turn = turns[1]
    assert gm_turn.turn_type.value == "SCENE_START", (
        f"Expected SCENE_START, got {gm_turn.turn_type.value}"
    )
    assert gm_turn.metadata.get("procedural_generation") is True, (
        "GM turn should have procedural_generation metadata"
    )
    assert len(gm_turn.metadata.get("tables_used", [])) >= 1, (
        "GM turn should list tables used"
    )
    assert len(gm_turn.metadata.get("entities_generated", [])) >= 1, (
        "GM turn should list entities generated"
    )
...
async def test_p19_scenario_2_forest_exploration(
    scene_loop, scene, context_assembly, narrator, forest_table, user_id, world_id
):
    """
    Scenario 2: Forest Exploration
    ...
    assert transition.target_location in ["forest", "into"], (
        f"Expected target 'forest' or 'into', got {transition.target_location}"
    )
    ...
    assert entities[0].name in updated_scene.entity_registry, (
        "Feature should be in entity registry"
    )
    ...
    turns[1]
    assert (
        "forest" in description.lower()
        or entities[0].name.lower() in description.lower()
...
async def test_p19_error_case_input_parsing_fails(scene_loop, context_assembly):
    ...
    try:
        ...
        assert transition.target_location == "unknown", (
            "Non-movement input should result in unknown target"
        )
...
async def test_p19_error_case_no_tables_found(scene_loop, scene, user_id, world_id):
    ...
    assert transition.target_location in ["unknown_location", "enter"], (
        "Should handle movement without tables"
    )
...
async def test_p19_success_criteria_transition_detection(scene_loop, scene):
    ...
    assert accuracy >= 95, (
        f"Transition detection accuracy {accuracy}% is below 95% threshold"
    )
...
@pytest.mark.unit
async def test_p19_success_criteria_table_retrieval(
    scene_loop, cave_table, forest_table, world_id
):
    """
    ...
    assert cave_tables[0].location_type == "dungeon", (
        "Cave should map to dungeon location type"
    )
    ...
    assert forest_tables[0].location_type == "wilderness", (
        "Forest should map to wilderness location type"
    )
...
@pytest.mark.unit
async def test_p19_success_criteria_canonization_rules(
    scene_loop, scene, cave_table, world_id
):
    """
    ...
    assert entities[0].name in updated_scene.entity_registry, (
        "Permanent entity should be in registry"
    )
    ...
    assert entities[1].entity_id in staged_entity_ids, (
        "Temporary entity should be staged"
    )
...
async def test_p19_success_criteria_metadata_completeness(
    ...
    # Execute full flow
    await scene_loop.detect_scene_transition(movement, scene_id)
    tables = await scene_loop.retrieve_random_tables("cave", world_id)
    table_entries = await scene_loop.roll_on_table(tables[0])
    entities = await scene_loop.generate_entities(table_entries, world_id)
    ...
    assert metadata.get("procedural_generation") is True, (
        "Should have procedural_generation flag"
    )
    assert len(metadata.get("tables_used", [])) >= 1, "Should have tables_used list"
    assert len(metadata.get("entities_generated", [])) >= 1, (
        "Should have entities_generated list"
    )
    assert len(metadata.get("entities_canonized", [])) >= 1, (
        "Should have entities_canonized list"
    )
...
async def test_p19_postconditions(
    ...
    assert len(updated_scene.entity_registry) >= 1, (
        "Permanent entities should be in registry"
    )
    assert entities[0].name in updated_scene.entity_registry, (
        "Entity should be in registry"
    )
    ...
    gm_turn = turns[1]
    assert gm_turn.turn_type.value == "SCENE_START", (
        "GM turn should be SCENE_START type"
    )
    assert gm_turn.metadata.get("procedural_generation") is True, (
        "GM turn should have procedural_generation flag"
    )

    # Postcondition 6: Scene current_location updated
    assert updated_scene.current_location == "cave", (
        f"Scene current_location should be 'cave', got {updated_scene.current_location}"
    )
...
async def test_p19_subtable_recursion(scene_loop, world_id):
    ...
    assert len(table_entries) >= 2, (
        f"Expected at least 2 entries (main + subtable), got {len(table_entries)}"
    )
...
@pytest.mark.unit
async def test_p19_multiple_tables_per_location(
    scene_loop, scene, cave_table, forest_table, user_id, world_id
):
    """
    ...
    assert len(all_entities) >= 2, (
        f"Expected at least 2 entities from all tables, got {len(all_entities)}"
    )
    ...
    updated_scene = scene_loop.scenes[scene_id]
    assert len(updated_scene.entity_registry) >= 2, (
        "All permanent entities should be in registry"
    )
...
async def test_p19_entity_properties_verification(scene_loop, cave_table, world_id):
    ...
    assert "generated_from" in entity.properties, (
        "Entity properties should include generated_from"
    )

    # Verify entity_type matches
    matching_entry = next((e for e in table_entries if e.entity_type), None)
    if matching_entry:
        assert entity.entity_type == matching_entry.entity_type, (
            "Entity type should match entry"
        )
...
@pytest.mark.unit
async def test_p19_multiple_entities_mixed_permanence(
    scene_loop, scene, cave_table, user_id, world_id
):
    """
    ...
    staged_entity = next(
        e
        for e in updated_scene.staged_entities
        if e["entity_id"] == entities[1].entity_id
    )
    assert staged_entity.get("temporary") is True, (
        "Staged entity should have temporary=True flag"
    )

...
async def test_clone_pack_rejects_use_while_backing_job_is_running(monkeypatch):
    ...
    with pytest.raises(HTTPException) as exc:
        await pack_library.clone_pack(
            pack_id, pack_library.ClonePackRequest(name="Clone")
        )
...
async def test_get_pack_returns_pack_data(monkeypatch):
    ...
    monkeypatch.setattr(
        pack_library, "mongodb_get_knowledge_pack", lambda _uid: pack_data
    )
    monkeypatch.setattr(
        pack_library,
        "_pack_to_dict",
        lambda pack: {"name": pack.name, "status": str(pack.status.value)},
    )
...
async def test_list_packs_filters_by_status(monkeypatch):
    ...
    pack1 = SimpleNamespace(
        pack_id=uuid4(), name="Pack 1", status=KnowledgePackStatus.READY, tags=[]
    )
    pack2 = SimpleNamespace(
        pack_id=uuid4(), name="Pack 2", status=KnowledgePackStatus.PENDING, tags=[]
    )

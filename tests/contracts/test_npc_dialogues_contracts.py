...
class TestNPCDialogueCreate:
    def test_minimal(self):
        d = NPCDialogueCreate(
            entity_id=uuid4(), universe_id=uuid4()
        )
...
class TestAppendDialogueMessage:
    def test_minimal(self):
        msg = AppendDialogueMessage(
            dialogue_id=uuid4(), role="player", text="Hi"
        )
        assert msg.role == "player"
        assert msg.text == "Hi"
        assert msg.memory_refs_read == []
    ...
    def test_text_empty_rejected(self):
        with pytest.raises(ValidationError):
            AppendDialogueMessage(
                dialogue_id=uuid4(), role="player", text=""
            )

    def test_text_too_long(self):
        with pytest.raises(ValidationError):
            AppendDialogueMessage(
                dialogue_id=uuid4(), role="player", text="x" * 10001
            )

    def test_role_too_long(self):
        with pytest.raises(ValidationError):
            AppendDialogueMessage(
                dialogue_id=uuid4(), role="x" * 21, text="x"
            )

...
class TestCondition:
    def test_minimal(self):
        c = Condition(name="Stunned", source="Monk's Stunning Strike", duration_type="rounds")
...
class TestAddCombatParticipant:
    def test_minimal(self):
        eid = uuid4()
        r = AddCombatParticipant(encounter_id=uuid4(), entity_id=eid, name="Orc", side=CombatSide.ENEMY)
        assert r.name == "Orc"
...
class TestCombatEnvironment:
    ...
    def test_custom(self):
        env = CombatEnvironment(terrain="swamp", lighting="dim", hazards=[{"type": "quicksand"}])
...
class TestCombatLogEntry:
    def test_valid(self):
        ...
        entry = CombatLogEntry(
            round=1, turn=1, actor_id=uuid4(), action="Attack", summary="Goblin attacks", timestamp=now
        )
    ...
    def test_round_must_be_ge_1(self):
        with pytest.raises(ValidationError):
            CombatLogEntry(round=0, turn=1, actor_id=uuid4(), action="X", summary="Y", timestamp=datetime.now(timezone.utc))


class TestAddCombatLogEntry:
    def test_valid(self):
        r = AddCombatLogEntry(encounter_id=uuid4(), round=2, turn=3, actor_id=uuid4(), action="Cast Fireball", summary="BOOM")
        assert r.round == 2

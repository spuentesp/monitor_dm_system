...
class TestNeo4jListMultiversesContract:
    ...
    async def test_returns_list_of_multiverse_summaries(self, fake_mcp_client):
        ...
        # Setup
        fake_mcp_client.add_response("neo4j_list_multiverses", {"omniverse_id": "omni123"}, [
            {
                "id": "multi123",
                "name": "D&D Worlds",
                "system_name": "D&D 5e",
        ...
        # Execute
        result = await fake_mcp_client.call_tool("neo4j_list_multiverses", {"omniverse_id": "omni123"})
    ...
    async def test_invalid_omniverse_id_raises_error(self, fake_mcp_client):
        ...
        fake_mcp_client.add_error("neo4j_list_multiverses", {}, 
                                 FileNotFoundError("Omniverse not found: invalid123"))
        
        # Execute & Verify
        with pytest.raises(FileNotFoundError, match="Omniverse not found"):
            await fake_mcp_client.call_tool("neo4j_list_multiverses", {"omniverse_id": "invalid123"})
...
class TestNeo4jCreateUniverseContract:
    ...
    async def test_returns_universe_id_uuid_string(self, fake_mcp_client):
        ...
        # Setup
        fake_mcp_client.add_response("neo4j_create_universe", {
            ...
            "branch_from": None
        }, "universe12345678901234567890123456789012")  # 36-char UUID
        
        # Execute
        result = await fake_mcp_client.call_tool("neo4j_create_universe", {
            "multiverse_id": "multi123",
            "params": {
                "name": "Forgotten Realms",
                "genre": "Fantasy",
    ...
    async def test_invalid_multiverse_id_raises_error(self, fake_mcp_client):
        ...
        fake_mcp_client.add_error("neo4j_create_universe", {}, 
                                 FileNotFoundError("Multiverse not found: invalid123"))
        
        # Execute & Verify
        with pytest.raises(FileNotFoundError, match="Multiverse not found"):
            await fake_mcp_client.call_tool("neo4j_create_universe", {
                "multiverse_id": "invalid123",
                "params": {"name": "Test", "genre": "Fantasy", "tone": "Heroic", "tech_level": "Pre-Industrial"},
                "branch_from": None
            })
    ...
    async def test_invalid_name_raises_validation_error(self, fake_mcp_client):
        ...
        fake_mcp_client.add_error("neo4j_create_universe", {}, 
                                 ValueError("Name must be 1-200 characters"))
    ...
    async def test_invalid_genre_raises_validation_error(self, fake_mcp_client):
        ...
        fake_mcp_client.add_error("neo4j_create_universe", {}, 
                                 ValueError("Invalid genre. Must be one of: Fantasy, Sci-Fi, Cyberpunk, etc."))
    ...
    async def test_invalid_tech_level_raises_validation_error(self, fake_mcp_client):
        ...
        fake_mcp_client.add_error("neo4j_create_universe", {}, 
                                 ValueError("Invalid tech level. Must be one of: Pre-Industrial, Modern, Future, etc."))
    ...
    async def test_connection_error_raises_exception(self, fake_mcp_client):
        ...
        fake_mcp_client.add_error("neo4j_create_universe", {}, 
                                 ConnectionError("Failed to create universe"))
...
class TestNeo4jListUniversesContract:
    ...
    async def test_returns_list_of_universe_summaries(self, fake_mcp_client):
        ...
        # Setup
        fake_mcp_client.add_response("neo4j_list_universes", {"multiverse_id": "multi123"}, [
            {
                "id": "universe123",
                "name": "Forgotten Realms",
                "genre": "Fantasy",
        ...
        # Execute
        result = await fake_mcp_client.call_tool("neo4j_list_universes", {"multiverse_id": "multi123"})
    ...
    async def test_invalid_multiverse_id_raises_error(self, fake_mcp_client):
        """Contract: Invalid multiverse_id raises NotFoundError."""
        # Setup - simulate multiverse not found
        fake_mcp_client.add_error("neo4j_list_universes", {}, 
                                 FileNotFoundError("Multiverse not found: invalid123"))
        
        # Execute & Verify
        with pytest.raises(FileNotFoundError, match="Multiverse not found"):
            await fake_mcp_client.call_tool("neo4j_list_universes", {"multiverse_id": "invalid123"})
...
class TestUniverseCreatorContract:
    ...
    async def test_create_universe_validates_genre_enum(self):
        ...
        valid_genres = ["Fantasy", "Sci-Fi", "Cyberpunk", "Horror", "Western", "Modern",
                       "Historical", "Post-Apocalyptic", "Steampunk", "Urban Fantasy",
                       "Space Opera", "Superhero"]
    ...
    async def test_create_universe_validates_tone_enum(self):
        ...
        valid_tones = ["Dark", "Heroic", "Whimsical", "Grimdark", "Optimistic",
                      "Mysterious", "Campy", "Serious", "Light-Hearted"]
    ...
    async def test_create_universe_validates_tech_level_enum(self):
        ...
        valid_tech_levels = ["Pre-Industrial", "Steam", "Early Modern", "Modern",
                            "Near Future", "Far Future", "Post-Apocalyptic", "Ancient", "Medieval"]
...
class TestUniverseCreationIntegration:
    ...
    async def test_full_universe_creation_workflow_fresh_start(self, fake_mcp_client):
        ...
        # Execute workflow
        multiverses = await fake_mcp_client.call_tool("neo4j_list_multiverses", {"omniverse_id": "omni123"})
        assert len(multiverses) >= 1
        multiverse_id = multiverses[0]["id"]
        
        universe_id = await fake_mcp_client.call_tool("neo4j_create_universe", {
    ...
    @pytest.mark.asyncio
    async def test_full_universe_creation_workflow_with_branching(self, fake_mcp_client):
        ...
        universes = await fake_mcp_client.call_tool("neo4j_list_universes", {"multiverse_id": "multi123"})

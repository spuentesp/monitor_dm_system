...
class TestBehavioralTrigger:
    ...
    def test_topic_too_long(self):
        with pytest.raises(ValidationError):
            SceneBehavioralTrigger(trigger_topic="x" * 201, reaction="y")

    def test_reaction_too_long(self):
        with pytest.raises(ValidationError):
            SceneBehavioralTrigger(trigger_topic="x", reaction="y" * 501)

    def test_gm_note_too_long(self):
        with pytest.raises(ValidationError):
            SceneBehavioralTrigger(trigger_topic="x", reaction="y", gm_note="z" * 301)
...
class TestScenePrompt:
    def test_minimal(self):
        s = ScenePrompt(
            title="The Midnight Arrival", setup="A traveler arrives at midnight."
        )
...
class TestNPCSceneGeneratorOutput:
    ...
    def test_with_content(self):
        o = NPCSceneGeneratorOutput(
            ...
            scenes=[ScenePrompt(title="A Storm", setup="Dark clouds gather.")],

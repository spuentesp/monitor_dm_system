...
class TestProviderBaseUrls:
    ...
    def test_openai_in_default_urls(self):
        assert PROVIDER_BASE_URLS[LLMProviderType.OPENAI] == "https://api.openai.com/v1"
...
class TestLLMProviderConfig:
    ...
    def test_id_required(self):
        with pytest.raises(ValidationError):
            LLMProviderConfig(name="x", provider=LLMProviderType.OPENAI, model="gpt-4o")  # type: ignore[call-arg]

    def test_name_required(self):
        with pytest.raises(ValidationError):
            LLMProviderConfig(id="x", provider=LLMProviderType.OPENAI, model="gpt-4o")  # type: ignore[call-arg]

    def test_model_required(self):
        with pytest.raises(ValidationError):
            LLMProviderConfig(id="x", name="y", provider=LLMProviderType.OPENAI)  # type: ignore[call-arg]

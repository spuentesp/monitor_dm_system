...
class TestVectorCollection:
    ...
    def test_valid_collections_set(self):
        assert VALID_COLLECTIONS == {"scenes", "memories", "snippets", "entities", "knowledge"}
...
class TestVectorSearchRequest:
    ...
    def test_with_query_vector(self):
        r = VectorSearchRequest(
            collection="scenes", query_vector=[0.1, 0.2], top_k=20
        )
        assert r.top_k == 20

    def test_with_query_text(self):
        r = VectorSearchRequest(
            collection="scenes", query_text="find me a tavern"
        )
    ...
    def test_score_threshold_bounds(self):
        with pytest.raises(ValidationError):
            VectorSearchRequest(
                collection="scenes", score_threshold=-0.01
            )
        with pytest.raises(ValidationError):
            VectorSearchRequest(
                collection="scenes", score_threshold=1.01
            )
...
class TestScoredVector:
    ...
    def test_with_payload(self):
        s = ScoredVector(
            id=uuid4(), score=0.5, payload={"text": "x"}
        )
...
class TestVectorSearchResponse:
    def test_empty(self):
        r = VectorSearchResponse(
            collection="scenes", results=[], count=0
        )
        assert r.results == []
        assert r.count == 0
    ...
    def test_with_results(self):
        ...
        r = VectorSearchResponse(
            collection="scenes", results=[scored], count=1
        )
...
class TestVectorDeleteResponse:
    def test_minimal(self):
        r = VectorDeleteResponse(
            success=True, collection="scenes", deleted_count=5
        )
...
class TestCollectionInfo:
    ...
    def test_required_vector_size(self):
        with pytest.raises(ValidationError):
            CollectionInfo(
                name="x", points_count=0, distance="Cosine", status="green"
            )

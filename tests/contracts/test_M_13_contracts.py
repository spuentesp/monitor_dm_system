...
class TestM13DataLayerContracts:
    ...
    async def test_neo4j_list_archetypes(self, fake_mcp_client: FakeMCPClient):
        ...
        result = await fake_mcp_client.call(
            "neo4j_list_archetypes", {"universe_id": "universe123"}
        )
        ...
        assert result[0]["entity_type"] == "archetype", (
            "Entity type must be 'archetype'"
        )

    async def test_neo4j_list_archetypes_universe_not_found(
        self, fake_mcp_client: FakeMCPClient
    ):
        """Test neo4j_list_archetypes raises NotFoundError for invalid universe_id."""
        fake_mcp_client.add_error(
            "neo4j_list_archetypes",
            {},
            Exception("Universe not found: invalid_universe"),
        )
        with pytest.raises(Exception, match="Universe not found"):
            await fake_mcp_client.call(
                "neo4j_list_archetypes", {"universe_id": "invalid_universe"}
            )
    ...
    async def test_neo4j_create_entity_character_universe_not_found(
        self, fake_mcp_client: FakeMCPClient
    ):
        """Test neo4j_create_entity raises NotFoundError for invalid universe_id."""
        fake_mcp_client.add_error(
            "neo4j_create_entity", {}, Exception("Universe not found: invalid_universe")
        )
        with pytest.raises(Exception, match="Universe not found"):
            await fake_mcp_client.call(
                "neo4j_create_entity",
                {
                    ...
                    "entity_type": "character",
                    "params": {"name": "Test Character", "role": "NPC"},
                },
            )

    async def test_neo4j_create_entity_character_invalid_name(
        self, fake_mcp_client: FakeMCPClient
    ):
        """Test neo4j_create_entity raises ValidationError for invalid name."""
        # Name too short
        fake_mcp_client.add_error(
            "neo4j_create_entity", {}, Exception("Name must be 1-200 characters")
        )
    ...
    async def test_neo4j_create_relationship_derives_from(
        self, fake_mcp_client: FakeMCPClient
    ):
    ...
    async def test_neo4j_create_relationship_has_entity(
        self, fake_mcp_client: FakeMCPClient
    ):
    ...
    async def test_neo4j_create_relationship_invalid_from_entity(
        self, fake_mcp_client: FakeMCPClient
    ):
        ...
        fake_mcp_client.add_error(
            "neo4j_create_relationship",
            {},
            Exception("Source entity not found: invalid_entity"),
        )
    ...
    async def test_neo4j_create_relationship_invalid_to_entity(
        self, fake_mcp_client: FakeMCPClient
    ):
        ...
        fake_mcp_client.add_error(
            "neo4j_create_relationship",
            {},
            Exception("Target entity not found: invalid_archetype"),
        )
    ...
    async def test_neo4j_create_relationship_invalid_relationship_type(
        self, fake_mcp_client: FakeMCPClient
    ):
        ...
        fake_mcp_client.add_error(
            "neo4j_create_relationship",
            {},
            Exception("Invalid relationship type: INVALID_TYPE"),
        )
    ...
    async def test_mongodb_create_character_sheet_invalid_entity_id(
        self, fake_mcp_client: FakeMCPClient
    ):
        ...
        fake_mcp_client.add_error(
            "mongodb_create_character_sheet",
            {},
            Exception("Character entity not found: invalid_entity"),
        )
        with pytest.raises(Exception, match="Character entity not found"):
            await fake_mcp_client.call(
                "mongodb_create_character_sheet",
                {
                    ...
                    "resources": {"hp_max": 14, "hp_current": 14},
                },
            )

    async def test_mongodb_create_character_sheet_invalid_stats(
        self, fake_mcp_client: FakeMCPClient
    ):
        ...
        fake_mcp_client.add_error(
            "mongodb_create_character_sheet",
            {},
            Exception("Invalid stats: missing required keys"),
        )
...
class TestM13AgentContracts:
    ...
    def character_creator(self):
        ...
        from monitor_agents.character_creator.agent import CharacterCreator
    ...
    def test_validate_character_params_valid(self, character_creator):
        ...
        result = character_creator.validate_character_params(
            ...
            role="ally",
            description="A wise and powerful wizard",
            stats={"STR": 16, "DEX": 14, "CON": 14, "INT": 18, "WIS": 16, "CHA": 16},
        )
    ...
    def test_validate_character_params_invalid_name_too_long(self, character_creator):
        ...
        with pytest.raises(ValidationError, match="Name must be 1-200 characters"):
            character_creator.validate_character_params(
                name="x" * 201, role="PC", description="Test"
            )
    ...
    def test_validate_character_params_invalid_stats_not_dict(self, character_creator):
        ...
        with pytest.raises(ValidationError, match="Stats must be a dictionary"):
            character_creator.validate_character_params(
                name="Test", role="PC", description="Test", stats="not_a_dict"
            )

    def test_validate_character_params_invalid_stats_value_too_low(
        ...
        with pytest.raises(ValidationError, match="must be between 1 and 30"):
            character_creator.validate_character_params(
                ...
                role="PC",
                description="Test",
                stats={"STR": 0, "DEX": 14, "CON": 14, "INT": 18, "WIS": 16, "CHA": 16},
            )

    def test_validate_character_params_invalid_stats_value_too_high(
        self, character_creator
    ):
...
class TestM13CharacterCreationFlow:
    ...
    async def test_create_character_pc_with_archetype(
        self, fake_mcp_client: FakeMCPClient, llm_client: FakeLLMClient
    ):
        """Test creating a PC with archetype and stats."""
        # Create character entity
        entity_id = await fake_mcp_client.call(
            "neo4j_create_entity",
            {
                "universe_id": "universe123",
                "entity_type": "character",
    ...
    async def test_create_character_npc_without_archetype(
        self, fake_mcp_client: FakeMCPClient, llm_client: FakeLLMClient
    ):
    ...
    async def test_create_character_antagonist_with_stats(
        self, fake_mcp_client: FakeMCPClient, llm_client: FakeLLMClient
    ):
...
class TestM13ErrorHandling:
    ...
    async def test_character_creation_fails(self, fake_mcp_client: FakeMCPClient):
        ...
        fake_mcp_client.add_error(
            "neo4j_create_entity",
            {},
            Exception("Failed to create character: ConnectionError"),
        )
    ...
    async def test_character_sheet_creation_fails(self, fake_mcp_client: FakeMCPClient):
        ...
        fake_mcp_client.add_error(
            "mongodb_create_character_sheet",
            {},
            Exception("Failed to create character sheet: ConnectionError"),
        )

...
class TestCharacterPreference:
    ...
    def test_with_reason(self):
        p = CharacterPreference(
            category="people", item="soldiers", valence=-0.5, reason="lost brother in war"
        )
    ...
    def test_reason_too_long(self):
        with pytest.raises(ValidationError):
            CharacterPreference(
                category="x", item="y", valence=0.0, reason="z" * 501
            )
...
class TestNPCProfileCreate:
    ...
    def test_full(self):
        ...
        p = NPCProfileCreate(
            ...
            triggers=[
                BehavioralTrigger(
                    condition="sees a sword", reaction="freezes"
                )
            ],
            ...
            relationship_states={
                str(uuid4()): {"trust": 0.2, "hostility": 0.5}
            },
        )
    ...
    def test_current_emotion_too_long(self):
        with pytest.raises(ValidationError):
            NPCProfileCreate(
                entity_id=uuid4(), current_emotional_state="x" * 201
            )

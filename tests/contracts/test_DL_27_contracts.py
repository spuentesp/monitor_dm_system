...
class TestStatModification:
    ...
    def test_source_id_optional(self):
        now = datetime.now(timezone.utc)
        m = StatModification(mod_id=uuid4(), stat_or_resource="STR", change=2, source="Potion", timestamp=now)
...
class TestInventoryChange:
    def test_valid(self):
        now = datetime.now(timezone.utc)
        c = InventoryChange(change_type=InventoryChangeType.ADD, item="Potion of Healing", quantity=1, timestamp=now)
...
class TestAddStatModification:
    def test_valid(self):
        a = AddStatModification(state_id=uuid4(), stat_or_resource="HP", change=-5, source="Sword")
...
class TestWorkingStateResponse:
    def test_valid(self):
        ...
        sid = uuid4()
        state = CharacterWorkingState(
            id=sid, state_id=sid, entity_id=uuid4(), scene_id=uuid4(), story_id=uuid4(),

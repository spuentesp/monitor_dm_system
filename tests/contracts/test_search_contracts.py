...
class TestSemanticSearchResponseContract:
    ...
    def test_response_basic(self):
        from monitor_ui.routers.search import SearchResultItem, SemanticSearchResponse
    ...
    def test_response_collections_tracked(self):
        ...
        results = [
            SearchResultItem(
                id=str(uuid4()), collection="entities", score=0.9, payload={}
            ),
            SearchResultItem(
                id=str(uuid4()), collection="scenes", score=0.85, payload={}
            ),
            SearchResultItem(
                id=str(uuid4()), collection="snippets", score=0.8, payload={}
            ),
            SearchResultItem(
                id=str(uuid4()), collection="knowledge", score=0.75, payload={}
            ),
        ]
        response = SemanticSearchResponse(
...
class TestSearchRouterContract:
    ...
    def test_search_router_has_semantic_search_route(self):
        ...
        assert "GET" in routes["/search"], "Expected GET method on /search"
    ...
    def test_search_router_has_universe_search_route(self):
        ...
        assert len(universe_routes) >= 1, (
            f"Expected universe search route, got {routes}"
        )
...
class TestSearchGracefulDegradation:
    ...
    def test_qdrant_failure_partial_results(self):
        ...
        from fastapi.testclient import TestClient
        from monitor_data.schemas.vectors import ScoredVector, VectorSearchResponse
        from monitor_ui.main import create_app
        ...
        with patch("monitor_ui.routers.search.qdrant_search", mock_qdrant_search):
            with patch(
                "monitor_ui.routers.search._embed_query_text", return_value=[0.1] * 1536
            ):
                response = client.get("/api/search/search?q=wizard")
                # Should still return 200 with partial results
                assert response.status_code == 200, (
                    f"Expected 200 with partial results: {response.text}"
                )
                data = response.json()

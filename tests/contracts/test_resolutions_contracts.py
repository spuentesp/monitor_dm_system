...
class TestResolutionType:
    def test_values(self):
        expected = {
            "dice", "card", "narrative", "deterministic", "contested",
            "forced_narrative",
        }
...
class TestEffectType:
    def test_values(self):
        expected = {
            "damage", "healing", "condition", "buff", "debuff",
            "resource_change", "stat_change", "position_change", "other",
        }
...
class TestCardDraw:
    ...
    def test_special_too_long(self):
        with pytest.raises(ValidationError):
            CardDraw(
                cards_drawn=["x"], total_value=0, special="y" * 201
            )

...
from monitor_data.schemas.universe import UniverseCreate, UniverseUpdate, UniverseResponse
from monitor_data.schemas.base import (
    AltWorldType, CanonLevel, Authority, KnowledgeTreeType,
)
from monitor_data.schemas.world_snapshots import (
    WorldSnapshotCreate, WorldSnapshot, WorldSnapshotResponse,
    WorldSnapshotFilter, WorldSnapshotListResponse,
)


class TestUniverseCreateForPhase3:
    def test_minimal(self):
        u = UniverseCreate(multiverse_id=uuid4(), name="Test World", description="A test")
    ...
    def test_confidence_bounds(self):
        with pytest.raises(ValidationError):
            UniverseCreate(multiverse_id=uuid4(), name="Test", description="x", confidence=-0.1)
        with pytest.raises(ValidationError):
            UniverseCreate(multiverse_id=uuid4(), name="Test", description="x", confidence=1.1)
...
class TestWorldSnapshotForPhase3:
    ...
    def test_with_description(self):
        s = WorldSnapshotCreate(universe_id=uuid4(), name="Pre-Battle", description="Before the fight")

...
class TestResolverResolveTurnContract:
    ...
    async def test_resolve_turn_returns_dict(self):
        """Contract: resolve_turn() returns (Dict[str, Any], Optional[GMVerdict]).

        Updated for the GM-authority architecture: resolve_turn now
        returns the resolution dict alongside the GMVerdict that
        produced it (carries narrative_draft for the Narrator's
        downstream refinement pass) — see docs/architecture/GM_AS_AUTHORITY.md.
        """
        ...
        with patch.object(resolver, "call_tool", new_callable=AsyncMock) as mock_tool:
            ...
            with patch.object(
                resolver, "call_llm_structured", new_callable=AsyncMock
            ) as mock_llm:
                mock_llm.return_value = MagicMock(
                    action_type="action", outcome="success", narrative="You hit!"
                )
                result, gm_verdict = await resolver.resolve_turn(
                    scene_id="scene-123",
...
class TestNarratorDescribeContract:
    ...
    async def test_narrate_turn_with_resolution(self):
        ...
        with (
            patch.object(
                narrator, "_generate_narrative_and_proposals", new_callable=AsyncMock
            ) as mock_gen,
            patch.object(
                narrator, "_persist_turn", new_callable=AsyncMock
            ) as mock_persist,
        ):
            mock_gen.return_value = ("The goblin falls!", [], 5, [], None)
...
class TestNeo4jGetEntityContract:
    ...
    async def test_returns_entity_with_all_required_fields(self, fake_mcp_client):
        ...
        fake_mcp_client.call_tool.return_value = {
            ...
            "relationships": [],
            "canon": {"role": "enemy", "location": "forest"},
        }

        # Call tool (simulated)
        result = await fake_mcp_client.call_tool(
            "neo4j_get_entity", {"entity_id": "goblin-001"}
        )
    ...
    async def test_raises_entity_not_found_error(self, fake_mcp_client):
        ...
        with pytest.raises(Exception) as exc_info:
            await fake_mcp_client.call_tool(
                "neo4j_get_entity", {"entity_id": "entity-999"}
            )
...
class TestDiceRollContract:
    ...
    async def test_returns_dice_roll_with_all_required_fields(self, fake_mcp_client):
        ...
        # Call tool (simulated)
        result = await fake_mcp_client.call_tool("dice_roll", {"formula": "1d20+5"})
    ...
    async def test_parses_standard_dice_notation(self, fake_mcp_client):
        ...
        result = await fake_mcp_client.call_tool("dice_roll", {"formula": "2d6"})
    ...
    async def test_parses_keep_highest_notation(self, fake_mcp_client):
        ...
        result = await fake_mcp_client.call_tool("dice_roll", {"formula": "4d6kh3"})
    ...
    async def test_raises_invalid_dice_formula_error(self, fake_mcp_client):
        ...
        # Verify contract: raises error
        with pytest.raises(Exception) as exc_info:
            await fake_mcp_client.call_tool("dice_roll", {"formula": "invalid"})
...
class TestMongodbCreateResolutionContract:
    ...
    @pytest.mark.unit
    async def test_accepts_dice_parameters_when_resolution_type_is_dice(

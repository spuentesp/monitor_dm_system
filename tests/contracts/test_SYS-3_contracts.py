...
class TestApplicationExitHandlerCheckActiveSceneContract:
    ...
    @pytest.mark.unit
    async def test_returns_scene_when_active_scene_exists(self, fake_mcp_client, fake_llm_client):
        ...
        handler = ApplicationExitHandler(mcp_client=fake_mcp_client, llm_client=fake_llm_client)
    ...
    @pytest.mark.unit
    async def test_returns_none_when_no_active_scene(self, fake_mcp_client, fake_llm_client):
...
class TestApplicationExitHandlerPromptSaveSceneContract:
    ...
    async def test_returns_true_when_user_confirms_save(self, mock_input, fake_mcp_client, fake_llm_client):
    ...
    @pytest.mark.unit
    @patch('builtins.input')
    async def test_returns_false_when_user_declines_save(self, mock_input, fake_mcp_client, fake_llm_client):
...
class TestApplicationExitHandlerCloseAllConnectionsContract:
    ...
    @pytest.mark.unit
    async def test_closes_all_database_connections(self, fake_mcp_client, fake_llm_client):
    ...
    @pytest.mark.unit
    async def test_continues_when_one_connection_fails(self, fake_mcp_client, fake_llm_client):
...
class TestSYS3Integration:
    ...
    async def test_full_exit_workflow_with_save_prompt_confirmed(
        self, mock_exit, mock_input, mock_get_auto_save, fake_mcp_client, fake_llm_client
    ):

...
def _speed_up_chat_router_timers(monkeypatch):
    ...
    monkeypatch.setattr(chat, "_db_save_session", lambda _session: None)
...
async def test_build_gm_opening_uses_non_persisting_narrator_path(monkeypatch):
    ...
    monkeypatch.setattr("monitor_agents.narrator.Narrator", _FakeNarrator)
...
async def test_active_play_ooc_question_uses_ooc_answer_path(monkeypatch):
    ...
    monkeypatch.setattr(chat_loops, "answer_ooc_question", fake_answer)
...
@pytest.mark.asyncio
async def test_send_message_persists_working_state_from_scene_turn_metadata(monkeypatch):
    ...
    assert chat._SESSIONS[session_id]["latest_working_state"]["resources"]["HP"]["current"] == 7
    assert chat._SESSIONS[session_id]["latest_working_state"]["conditions"] == ["pressured"]
    assert chat._SESSIONS[session_id]["latest_scene_checkpoint"]["success_level"] == "success"
...
async def test_send_message_schedules_websocket_fanout(monkeypatch):
    ...
    async def fake_fanout(_session_id: str, gm_msg: dict[str, object], *, exclude=None):  # noqa: ANN001
...
async def test_redis_ws_listener_rebroadcasts_remote_payload(monkeypatch):
    ...
    class _FakeRedisClient:
        ...
        def create_pubsub(self, *channels: str):  # noqa: ARG002
            return object()

        def get_pubsub_message_json(self, pubsub, *, timeout: float = 1.0):  # noqa: ARG002
            event = self._events.pop(0) if self._events else None
            if event is None:
                chat_ws._WS_SUBSCRIBERS.pop(session_id, None)
            return event

        def close_pubsub(self, pubsub) -> None:  # noqa: ARG002
            return None

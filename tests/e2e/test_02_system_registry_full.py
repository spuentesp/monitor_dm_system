...
class TestSystemSpecificBehavior:
    ...
    def test_vtm_hunger_mechanic(self, registry):
        """VtM 5e character has Hunger dice that can be set."""
        from monitor_data.schemas.rpg_ontology import make_entity_model

        model_cls = make_entity_model(
            registry.get("vampire_the_masquerade_5th_edition"), "character"
        )
        # VtM sheets should accept hunger pool
        field_names = set(model_cls.model_fields)
        hunger_fields = [n for n in field_names if "hunger" in n]
        assert len(hunger_fields) >= 1, f"No hunger field found; fields: {field_names}"
    ...
    def test_narrative_weighted_has_approach(self, registry):
        ...
        assert len(approach_fields) >= 4, f"Narrative Weighted should have 4 approaches (force/finesse/insight/presence), got: {field_names}"
        ...
        pack = KnowledgePackCreate(
            ...
            entity_archetypes=[
                ...
                ExtractedEntityArchetype(
                    ...
                    properties={"hit_die": "d6", "spellcasting": "arcane", "spellbook": True},

...
def seeded_world(e2e_databases):
    ...
    event_shadow_war = neo4j_create_event(
        EventCreate(
            ...
            start_time=datetime(1, 1, 1, tzinfo=_tz.utc),  # Year 0 fiction
            end_time=datetime(12, 1, 1, tzinfo=_tz.utc),  # Year 12 fiction
    ...
    event_sundering = neo4j_create_event(
        EventCreate(
            ...
            start_time=datetime(312, 1, 1, tzinfo=_tz.utc),
            authority=Authority.SOURCE,
            canon_level=CanonLevel.CANON,
        )
    )
...
def mock_dspy_narrator():
    ...
    with patch(
        "monitor_agents.prompts.narrator.NarratorModule.__call__",

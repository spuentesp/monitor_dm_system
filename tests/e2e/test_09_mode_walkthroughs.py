...
def test_world_architect_walkthrough(
    live_backend: str, walkthrough_universe: dict[str, Any]
) -> None:
    ...
    # 2b. Direct path: commit a second pack straight into the universe via the
    # review flow (apply → accept all → commit). The old /canonize route was
    # retired in F1-6; the apply endpoints are the single apply path.
    pack2 = mongodb_create_knowledge_pack(
        KnowledgePackCreate(
            name=f"Millhaven Canon {uuid4().hex[:6]}",
            description="Walkthrough direct-commit pack",
            pack_type=KnowledgePackType.SETTING_SUPPLEMENT,
            status=KnowledgePackStatus.READY,
            entity_archetypes=[
                ExtractedEntityArchetype(
    ...
    applied2 = requests.post(
        f"{live_backend}/ingest/packs/{pack2.id}/apply/{universe_id}",
        json={"mode": "full"},
        timeout=180,
    )
    assert applied2.status_code == 200, applied2.text
    proposals2 = requests.get(
        f"{live_backend}/ingest/packs/{pack2.id}/proposals",
        params={"status": "pending", "per_page": 500},
        timeout=60,
    )
    assert proposals2.status_code == 200, proposals2.text
    pending2 = proposals2.json()["proposals"]
    assert pending2, "expected pending proposals for the second pack"
    reviewed2 = requests.post(
        f"{live_backend}/ingest/proposals/batch",
        json={
            "actions": [
                {
                    "proposal_id": p["proposal_id"],
                    "action": "accept",
                    "reason": "walkthrough auto-accept",
                }
                for p in pending2
            ]
        },
    ...
    committed2 = requests.post(
        f"{live_backend}/ingest/packs/{pack2.id}/commit", timeout=180
    )
    assert committed2.status_code == 200, committed2.text
...
def test_gm_copilot_walkthrough(
    live_backend: str, walkthrough_universe: dict[str, Any]
) -> None:
    ...
    queue = requests.get(f"{live_backend}/stories/{story_id}/canon-queue", timeout=60)
...
@pytest.mark.asyncio
async def test_autonomous_gm_extras_walkthrough(
    walkthrough_universe: dict[str, Any],
) -> None:
